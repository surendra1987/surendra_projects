...
<template>
  <div ref="comments" class="tui-commentThread">
    ...
    <div class="tui-commentThread__comments">
      ...
      <template v-for="(comment, index) in thread.comments" :key="comment.id">
        <Comment
          :data-counter="index"
          ...
          :user-profile-url="comment.user.profileurl"
...
export default {
  ...
  apollo: {
    thread: {
      ...
      result({ data: { comments, cursor } }) {
        ...
        this.$emit('update-total-comments', cursor.total_with_replies);
      },
    },
  },
  ...
  methods: {
    ...
    addReplyToComment(commentId) {
      // TODO: TL-33188 cache updates superfluous
      const variables = {
        instanceid: this.instanceId,
        component: this.component,
        area: this.area,
    ...
    updateReactStatus({ id, status }) {
      ...
      this.$emit('update-react-status', { id, status });
    },
  },
};
</script>
