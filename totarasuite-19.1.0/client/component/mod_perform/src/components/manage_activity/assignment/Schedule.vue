...
<template>
  <div class="tui-performAssignmentSchedule">
    <h2 class="tui-performAssignmentSchedule__heading">
      {{ $str('activity_instance_creation_heading', 'mod_perform') }}
    </h2>

    <Toggles v-model:value="toggleValues" :activity="activity" />
...
export default {
  ...
  methods: {
    ...
    /**
     * Stop due date being used for closure condition
     * as due date is no longer in use
     *
     */
    async disableDueDateClosure() {
    ...
    async trySave() {
      ...
      this.isSaving = true;
      try {
        // The due data is currently used for closure but we have just disabled it
        if (
          this.activity.settings.close_on_due_date &&
          !this.formValuesToSave.dueDate.dueDateEnabled
        ) {
          await this.disableDueDateClosure();
        }
