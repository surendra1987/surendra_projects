...
export default {
  props: {
    ...
    /**
     * Transform the name of each uploaded file to be unique.
     */
    uniqueName: {
      type: Boolean,
      default: true,
  ...
  emits: ['progress', 'error', 'load', 'upload-finished', 'upload-started'],
  ...
  methods: {
    async upload(file) {
      ...
      data.append('title', this.getFileName(file));
    ...
    $_getFileInput() {
      if (!this.inputRef.value || this.inputRef.value.type != 'file') {
        throw new Error(
          "Upload element must have input type=file HTML element with :ref='inputRef', where inputRef comes from the slot props."
        );
      }
    ...
    getFileName(file) {
      ...
      if (extPos === -1) {
        return file.name + '-' + randomId;
      }

      return (
        file.name.slice(0, extPos) + '-' + randomId + file.name.slice(extPos)
      );
    },
  },
