...
export default {
  ...
  props: ['class', 'disabled', 'readonly', 'value', 'charLength'],
  ...
  methods: {
    ...
    handleTextChange(e) {
      ...
      this.$emit('update:value', this.lastValidHexValue);
...
.tui-inputColor {
  ...
  &__colorBlock {
    position: absolute;
    // prettier-ignore
    top: calc(#{$color-block-padding} + var(--form-input-border-size));
    left: calc(#{$color-block-padding} + var(--form-input-border-size));
    width: var(--gap-8);
    // prettier-ignore
    height: calc(100% - #{$color-block-padding * 2} - var(--form-input-border-size) * 2);
  ...
  & &__input[type='text'] {
    padding-left: calc(
      var(--gap-8) + #{$color-block-padding * 2} + var(
