...
<template>
  <div class="tui-responsive">
    <slot :current-boundary-name="currentBoundaryName" />
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (this.$el instanceof Element && this.breakpoints.length) {
      ...
      const resize = throttle(entries => {
        ...
        let boundaryName = this.getBoundaryName(entries);
  ...
  methods: {
     ...
     *
     * @param {Array} entries resize event objects
     * @returns {String} the name of the current boundary
     **/
    getBoundaryName(entries) {
      let boundaryName = this.largestBreakpoint.name;
