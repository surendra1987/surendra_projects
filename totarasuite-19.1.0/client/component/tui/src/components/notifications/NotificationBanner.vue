...
<template>
  ...
  >
    ...
    <div v-if="$slots['body']" class="tui-notificationBanner__body">
      <slot name="body" />
    </div>
    ...
    <div v-if="isDismissable" class="tui-notificationBanner__dismiss">
      <CloseButton
        class="tui-notificationBanner__dismiss_button"
        :size="300"
        :aria-label="labelForCloseButton"
        @click="dismiss"
      />
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    labelForIconType() {
      switch (this.type) {
        case 'info':
          return this.$str('info', 'core');
        case 'success':
          return this.$str('success', 'core');
        case 'warning':
          return this.$str('warning', 'core');
        case 'error':
          return this.$str('error', 'core');
        default:
          return null;
      }
    },
    /**
     * Label for the notification's close button.
     *
     * @returns {string}
     */
    labelForCloseButton() {
      switch (this.type) {
        case 'info':
          return this.$str('closebuttoninfobanner', 'core');
        case 'success':
          return this.$str('closebuttonsuccessbanner', 'core');
        case 'warning':
          return this.$str('closebuttonwarningbanner', 'core');
        case 'error':
          return this.$str('closebuttonerrorbanner', 'core');
        default:
          return null;
      }
    },
