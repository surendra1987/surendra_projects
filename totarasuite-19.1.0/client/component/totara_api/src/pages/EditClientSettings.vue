  ...
  >
    ...
    <template v-slot:content>
      ...
      >
        ...
        <FormRow
          :label="$str('setting:allowed_ip_list', 'totara_api')"
          :aria-describedby="
            `${$id('allowed-ip-list-default')} ` +
              `${$id('allowed-ip-list-desc')} ` +
              $id('allowed-ip-list-info')
          "
        >
          <template v-slot:help-message>
            <div
              :id="$id('allowed-ip-list-info')"
              v-html="$str('setting:allowed_ip_list_info', 'totara_api')"
            />
          </template>
          <FormTextarea name="allowedIpList" :validate="validateIps" />

          <FormRowDefaults :id="$id('allowed-ip-list-default')">
            {{ $str('setting:allowed_ip_list_default', 'totara_api') }}
          </FormRowDefaults>
          <FormRowDetails :id="$id('allowed-ip-list-desc')">
            {{ $str('setting:allowed_ip_list_desc', 'totara_api') }}
          </FormRowDetails>
        </FormRow>
...
export default {
    ...
    settings: {
      ...
      result({ data }) {
        this.initialValues = {
          ...
          allowedIpList: data.settings.client_settings.allowed_ip_list,
        };
      },
    },
  },
  ...
  methods: {
          ...
          variables: {
            input: {
              ...
              enable_introspection: formValue.enableIntrospection
                ? true
                : false,
              allowed_ip_list: formValue.allowedIpList,
            },
          },
        });
     ...
     */
    validateIps(allowedIpList) {
      if (!allowedIpList) {
        return; // Nothing to validate
      }

      const individualIpRegex = new RegExp('^(\\d{1,3})(\\.\\d{1,3}){0,3}$');
      const cidrIpRegex = new RegExp(
        '^(\\d{1,3})(\\.\\d{1,3}){0,3}(/\\d{1,2})$'
      );
      const ipRangeRegex = new RegExp(
        '^(\\d{1,3})(\\.\\d{1,3}){3}(-\\d{1,3})$'
      );

      const invalidIpList = allowedIpList.split('\n').filter(ip => {
        if (ip.length === 0) {
          return;
        }
        return (
          !individualIpRegex.test(ip) &&
          !cidrIpRegex.test(ip) &&
          !ipRangeRegex.test(ip)
        );
      });

      if (invalidIpList.length > 0) {
        return this.$str(
          'setting:allowed_ip_list_error',
          'totara_api',
          invalidIpList[0]
        );
      }
