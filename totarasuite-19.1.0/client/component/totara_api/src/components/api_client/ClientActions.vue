...
<template>
  <div class="tui-totara_api-clientActions">
    <Dropdown>
      ...
      <DropdownItem
        :title="$str('rotate_client_secret_name', 'totara_api', clientName)"
        :aria-label="
          $str('rotate_client_secret_name', 'totara_api', clientName)
        "
        @click="$emit('rotate-client-secret')"
      >
        {{ $str('rotate_client_secret', 'totara_api') }}
      </DropdownItem>
      <DropdownItem
