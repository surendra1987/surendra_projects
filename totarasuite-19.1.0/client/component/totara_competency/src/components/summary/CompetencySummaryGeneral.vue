...
<template>
  <div v-if="!$apollo.loading" class="tui-competencySummaryGeneral">
    <SectionHeader :edit-url="editUrl" :title="$str('general', 'core')" />
...
export default {
  ...
  computed: {
    ...
    aggregationMethodName() {
      ...
      if (this.competency.aggregation_method === 1) {
        return this.$str('all', 'totara_competency');
      } else if (this.competency.aggregation_method === 2) {
        return this.$str('any', 'totara_competency');
      } else {
        return this.$str('off', 'totara_competency');
      }
    },
  },
