...
// Totara: Added ability to let plugin redirect away from this page.
$hook = new \totara_core\hook\group_index($course);
$hook->execute();
...
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(format_string($course->fullname));
...
echo '<select name="groups[]" multiple="multiple" id="groups" size="15" class="form-control mb-1" onchange="'.$onchange.'">'."\n";
...
echo '<p><input type="submit" name="act_updatemembers" id="updatemembers" value="'
        . get_string('showmembersforgroup', 'group') . '"  class="btn btn-secondary" /></p>'."\n";
...
echo '<div class="userselector addselect_wrapper">';
echo '<select name="user" id="members" size="15" class="form-control mb-1">'."\n";
