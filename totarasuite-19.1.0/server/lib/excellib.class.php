...
class MoodleExcelWorkbook {
    ...
    */
     ...
     */
    public function __construct($filename, $type = 'Xlsx', $save = false) {
...
class MoodleExcelWorksheet {
    ...
    public function write_string($row, $col, $str, $format = null) {
        ...
        // Totara: Don't pass a null as string to avoid PHP 8.1 deprecation notices
        $str ??= '';

        $this->worksheet->getStyle(self::getCoords($col, $row + 1))->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
        $this->worksheet->setCellValueExplicit(self::getCoords($col, $row + 1), $str, DataType::TYPE_STRING);
    ...
    public function write_number($row, $col, $num, $format = null) {
        ...
        $this->worksheet->getStyle(self::getCoords($col, $row + 1))->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_GENERAL);
        $this->worksheet->setCellValueExplicit(self::getCoords($col, $row + 1), $num, DataType::TYPE_NUMERIC);
        $this->apply_format($row, $col, $format);
    }
    ...
    public function write_url($row, $col, $url, $format = null) {
        ...
        $this->worksheet->setCellValue(self::getCoords($col, $row + 1), $url);
        $this->worksheet->getCell(self::getCoords($col, $row + 1))->getHyperlink()->setUrl($url);
        $this->apply_format($row, $col, $format);
    }
    ...
    public function write_date($row, $col, $date, $format = null) {
        ...
        $this->worksheet->setCellValue(self::getCoords($col, $row + 1), $exceldate);
        $style = $this->worksheet->getStyle(self::getCoords($col, $row + 1));
    ...
    public function write_formula($row, $col, $formula, $format = null) {
        ...
        $this->worksheet->setCellValueExplicit(self::getCoords($col, $row + 1), $formula, DataType::TYPE_FORMULA);
        $this->apply_format($row, $col, $format);
    }
    ...
    public function write_blank($row, $col, $format = null) {
        ...
        $this->worksheet->setCellValue(self::getCoords($col, $row + 1), '');
        $this->apply_format($row, $col, $format);
    }
    ...
    public function write($row, $col, $token, $format = null) {
        ...
        if (preg_match("/^([+-]?)(?=\d|\.\d)\d*(\.\d*)?([Ee]([+-]?\d+))?$/", $token)) {
            // Match number
            // Totara: The pattern can end in a linebreak, which is removed.
            if (is_string($token)) {
                $token = rtrim($token);
            }
    ...
    */
    public function insert_bitmap($row, $col, $bitmap, $x = 0, $y = 0, $scalex = 1, $scaley = 1) {
        ...
        if ($scalex != 1) {
            ...
            $objdrawing->setWidth($objdrawing->getWidth() * $scalex);
        }
        if ($scaley != 1) {
            $objdrawing->setResizeProportional(false);
    ...
    */
    public function merge_cells($firstrow, $firstcol, $lastrow, $lastcol) {
        // For PhpSpreadsheet library, the column indexes start on 1 (instead of 0 as before).
        $this->worksheet->mergeCells([$firstcol + 1, $firstrow + 1, $lastcol + 1, $lastrow + 1]);
    }
    ...
    protected function apply_format($row, $col, $format = null) {
        ...
        $this->worksheet->getStyle(self::getCoords($col, $row + 1))->applyFromArray($format->get_format_array());
    }
    ...
    /**
     * Helper method to calculate the cell coordinates
     *
     * @param int $col
     * @param int $row
     * @return string
     */
    protected static function getCoords(int $col, int $row): string {
