...
class page_requirements_manager {
     ...
     *
     * Sources:
     * * requirements_tracker::DEFAULT_ORIGIN -- requirements specific to the page content
     * * shell -- universal requirements -- on every page / and/or used by the app shell (navigation, theme, etc)
     *
     * @var requirements_tracker
     */
    protected requirements_tracker $tracker;

    /** List of string available from JS */
    protected const STRINGS_FOR_JS = 'strings_for_js';

    /** Included JS scripts (header) */
    protected const JS_INCLUDES_HEADER = 'js_includes_head';

    /** Included JS scripts (footer) */
    protected const JS_INCLUDES_FOOTER = 'js_includes_footer';

    /** Inline scripts using RequireJS module loading. */
    protected const AMD_JS_CODE = 'amd_js_code';

    /** List of JS variables to be initialised (header) */
    protected const JS_INIT_VARIABLES_HEADER = 'js_init_variables_head';

    /** List of JS variables to be initialised (footer) */
    protected const JS_INIT_VARIABLES_FOOTER = 'js_init_variables_footer';

    /** List of needed function calls (normal) */
    protected const JS_CALLS_NORMAL = 'js_calls_normal';

    /** List of needed function calls (on DOM ready) */
    protected const JS_CALLS_ON_DOM_READY = 'js_calls_ondomready';

    /** Javascript code used for initialisation of page, it should be relatively small */
    protected const JS_INIT_CODE = 'js_init_code';

    /** Extra YUI modules */
    protected const EXTRA_YUI_MODULES = 'extra_yui_modules';

    /** Requested event handlers */
    protected const YUI_EVENT_HANDLERS = 'yui_event_handlers';

    /** Theme sheets, initialised only from core_renderer */
    protected const CSS_THEME_URLS = 'css_theme_urls';
     ...
     */
    protected const CSS_URLS = 'css_urls';

    /** URLs to preload */
    protected const PRELOAD = 'preload';

    /**
     * Origin value to use when reading requirements from tracker. (null = all)
     *
     * @var string|null
     */
    protected $read_origin = null;
    ...
    /**
     * Should we output JS requirements from the content while rendering the page?
     *
     * @var bool
     */
    public bool $output_content_js = true;
    ...
    public function __construct() {
        ...
        $this->tracker = new requirements_tracker();
...
require(['core/autoinitialise'], function(ai) {
    ai.scan().then(function() {
        ...
        $this->with_origin('shell', function () use ($autoinitialisejs) {
            $this->tracker->add(static::AMD_JS_CODE, $autoinitialisejs, 'core/autoinitialise');
        });
        $this->tracker->add(static::AMD_JS_CODE, $autoinitialisejs, 'core/autoinitialise');
        ...
        $this->with_origin('shell', function () {
     ...
     *
     * @param string $origin E.g. 'shell' or requirements_tracker::DEFAULT_ORIGIN
     * @param callable $fn
     * @return void
     */
    public function with_origin(string $origin, callable $fn): void {
        $this->tracker->with_origin($origin, $fn);
    }

    /**
     * Get all requirements of the specified type, filtered by the current read_origin.
     *
     * @param string $type
     * @return array
     */
    protected function get_entries(string $group): array {
        return $this->tracker->get_entries($group, $this->read_origin);
    }
    ...
    protected function init_requirements_data(moodle_page $page, core_renderer $renderer) {
        ...
        // Accessibility stuff.
        $this->skip_link_to($page->main_skip_target_id, get_string('tocontent', 'access'));
        if ($page->blocks->is_loaded()) {
            if ($page->blocks->region_has_content('side-pre', $renderer)) {
                $this->skip_link_to('block-region-side-pre', get_string('tosidebar', 'access'));
            } else if ($page->blocks->region_has_content('side-post', $renderer)) {
                $this->skip_link_to('block-region-side-post', get_string('tosidebar', 'access'));
            }
        }
    ...
    public function js($url, $inhead = false) {
        ...
        $type = $inhead ? static::JS_INCLUDES_HEADER : static::JS_INCLUDES_FOOTER;
        $this->tracker->add($type, $url, $url->out());
    }
    ...
    public function js_module($module) {
        ...
        if ($this->headdone) {
            $this->tracker->add(static::EXTRA_YUI_MODULES, $module, $module['name']);
        } else {
    ...
    protected function js_module_loaded($module) {
        ...
        return array_key_exists($modulename, $this->YUI_config->modules) ||
            $this->tracker->get_by_key(static::EXTRA_YUI_MODULES, $modulename, $this->read_origin);
    }
    ...
    public function css($stylesheet) {
        ...
        $this->tracker->add(static::CSS_URLS, $stylesheet, $stylesheet->out());
    }
    ...
    public function css_theme(moodle_url $stylesheet) {
        $this->tracker->add(static::CSS_THEME_URLS, $stylesheet);
    }
    ...
    public function js_function_call($function, array $arguments = null, $ondomready = false, $delay = 0) {
        $this->tracker->add(
            $ondomready ? static::JS_CALLS_ON_DOM_READY : static::JS_CALLS_NORMAL,
            [$function, $arguments, $delay]
        );
    }
    ...
    public function js_amd_inline($code) {
        $this->tracker->add(static::AMD_JS_CODE, $code);
    }
    ...
    public function get_raw_amd_js_code() {
        return $this->get_entries(static::AMD_JS_CODE);
    }
    ...
    public function yui_module($modules, $function, array $arguments = null, $galleryversion = null, $ondomready = false) {
        ...
        $this->tracker->add(static::JS_INIT_CODE, $jscode);
    }
    ...
    public function string_for_js($identifier, $component, $a = null) {
        ...
        $added = $this->tracker->get_by_key(static::STRINGS_FOR_JS, $key);
        if ($added) {
            if ($added['a'] !== $a) {
            ...
            $this->tracker->add(
                static::STRINGS_FOR_JS,
                ['lang_string' => new lang_string($identifier, $component, $a), 'a' => $a],
                $key
            );
        }
    }
    ...
    public function data_for_js($variable, $data, $inhead=false) {
        $this->tracker->add(
            $inhead ? static::JS_INIT_VARIABLES_HEADER : static::JS_INIT_VARIABLES_FOOTER,
            [$variable, $data]
        );
    }
    ...
    public function event_handler($selector, $event, $function, array $arguments = null) {
        $this->tracker->add(
            static::YUI_EVENT_HANDLERS,
            ['selector' => $selector, 'event' => $event, 'function' => $function, 'arguments' => $arguments]
        );
    }

    /**
     * Add an entry to be preloaded with <link rel="preload">.
     *
     * https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/rel/preload
     *
     * @param string $as Preload type, e.g. "image". See link above for full list.
     * @param string|moodle_url $href URL to preload
     * @return void
     */
    public function preload(string $as, string|moodle_url $href): void {
        $this->tracker->add(static::PRELOAD, ['href' => $href, 'as' => $as]);
    }
    ...
    protected function get_event_handler_code() {
        $output = '';
        foreach ($this->get_entries(static::YUI_EVENT_HANDLERS) as $h) {
    ...
    protected function get_javascript_code($ondomready) {
        $output = '';
        $jscalls = $this->get_entries($ondomready ? static::JS_CALLS_ON_DOM_READY : static::JS_CALLS_NORMAL);
        if ($jscalls) {
            foreach ($jscalls as $data) {
    ...
    protected function get_javascript_init_code() {
        $jsinitcode = $this->get_entries(static::JS_INIT_CODE);
        if (count($jsinitcode)) {
            return implode("\n", $jsinitcode) . "\n";
        }
        return '';
    ...
    protected function get_amd_footercode($initialiseamd = true) {
        ...
        $amdjscode = $this->get_entries(static::AMD_JS_CODE);
    ...
    protected function get_css_code() {
        ...
        $urls = array_merge(
            $this->get_entries(static::CSS_THEME_URLS),
            $this->get_entries(static::CSS_URLS),
        );
        foreach ($this->frameworks as $manager) {
    ...
    protected function get_extra_modules_code() {
        $extra_modules = $this->get_entries(static::EXTRA_YUI_MODULES);
        if (empty($extra_modules)) {
            return '';
        }
        $extra_modules = array_combine(array_column($extra_modules, 'name'), $extra_modules); // key by name
        return html_writer::script(js_writer::function_call('M.yui.add_module', [$extra_modules]));
    }
    ...
    public function get_head_code(moodle_page $page, core_renderer $renderer) {
        ...
        $preload = $this->get_entries(static::PRELOAD);
        foreach ($preload as $entry) {
            $output .= html_writer::tag('link', '', ['rel' => 'preload', 'href' => $entry['href'], 'as' => $entry['as']]) . "\n";
        }
        ...
        $js .= "window.addEventListener('DOMContentLoaded',function(){function s(){s=function(){},alert(" .
            ...
            "if (window.loadErrors && loadErrors.length) s(); if (!window.loadErrors || loadErrors.length) loadErrors = { push: s }; });\n";
        $js = $this->YUI_config->update_header_js($js);

        $output .= html_writer::script($js);

        // Add variables.
        $js_init_variables_header = $this->get_entries(static::JS_INIT_VARIABLES_HEADER);
        if ($js_init_variables_header) {
            $js = '';
            foreach ($js_init_variables_header as $data) {
                list($var, $value) = $data;
    ...
    public function get_js_page_config(moodle_page $page) {
        ...
        $totara_lang_id = current_language();
        if (!get_string_manager()->translation_exists($totara_lang_id)) {
            $totara_lang_id = 'en';
        }

        $language = get_string('iso6391', 'langconfig');

        // convert locale to IETF BCP 47 language tag
        $locale = get_string('locale', 'langconfig');
        $dot_pos = strpos($locale, '.');
        $locale = $dot_pos !== false ? substr($locale, 0, $dot_pos) : $locale; // remove .UTF-8 suffix
        $locale = str_replace('_', '-', $locale); // convert en_NZ to en-NZ
    ...
    public function get_top_of_body_code(core_renderer $renderer) {
        ...
        foreach ($this->get_entries(static::JS_INCLUDES_HEADER) as $url) {
            $output .= html_writer::script('', $url);
        }
    ...
    public function get_end_code($initialiseamd = true) {
        ...
        $this->read_origin = $this->output_content_js ? null : 'shell';
        ...
        if ($initialiseamd) {
            ...
            $this->with_origin('shell', function () use ($logconfig, $USER) {
                $this->js_call_amd('core/log', 'setConfig', [$logconfig]);
                $this->js_call_amd('core/storage_validation', 'init', [
                    !empty($USER->currentlogin) ? (int) $USER->currentlogin : time()
                ]);
            });
        }
        ...
        $this->with_origin('shell', function () use ($logconfig) {
        ...
        $this->read_origin = null;

        return $output;
    }

    /**
     * Generate HTML that needs to go after page contents.
     *
     * This is mainly used when rendering of contents is delayed (e.g. embedded in a Tui component).
     *
     * @return string
     */
    public function get_content_end_code(): string {
        $this->read_origin = requirements_tracker::DEFAULT_ORIGIN;

        $output = '';

        // Call amd init functions.
        $output .= $this->get_amd_footercode(false);
