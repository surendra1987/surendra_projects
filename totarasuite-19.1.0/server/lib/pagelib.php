 ...
 * @property-read string $privacy_aware_title An optional version of the title that excludes any personally-identifiable
 *                                            information (such as user's names), making it suitable for sharing with
 *                                            external systems.
 ...
 * @property-read string $main_skip_target_id The ID of the target for the main skip content section. Totara property.
 */
class moodle_page {
    ...
    /**
     * @var string An optional version of the title that excludes any personally-identifiable
     *             information (such as user's names), making it suitable for sharing with
     *             external systems.
     */
    protected $_privacy_aware_title = '';
    ...
    /**
     * @var string The ID of the target for the main skip link on the page.
     */
    protected $_main_skip_target_id = 'maincontent';
    ...
    /**
     * Please do not call this method directly, use the ->privacy_aware_title syntax. {@link moodle_page::__get()}.
     * @return string An optional version of the title that excludes any personally-identifiable
     *                information (such as user's names), making it suitable for sharing with
     *                external systems.
     */
    protected function magic_get_privacy_aware_title() {
        return $this->_privacy_aware_title;
    }
    ...
    protected function magic_get_requestorigin() {
        ...
        if (PHPUNIT_TEST && \core_phpunit\internal_util::is_web()) {
            return 'web';
        }

        if (EXTERNAL_API || PHPUNIT_TEST && \core_phpunit\internal_util::is_external_api()) {
            return 'api';
        }

        if (TOTARA_MOBILE_ACCESS || PHPUNIT_TEST && \core_phpunit\internal_util::is_mobile_login()) {
            return 'mobile';
        }
    ...
    /**
     * Returns the id of the main target skip link.
     * @return string
     */
    protected function magic_get_main_skip_target_id(): string {
        return $this->_main_skip_target_id;
    }
    ...
    public function set_pagelayout($pagelayout) {
        ...
        if (!empty($SESSION->forcepagelayout)) {
            ...
            $hook = new \core\hook\set_page_layout($pagelayout);
            $hook->execute();
            $this->_pagelayout = $hook->get_page_layout();
        }
    }
    ...
    /**
     * Sets the privacy aware title for the page.
     *
     * This is an optional version of the title that excludes any personally-identifiable
     * information (such as user's names), making it suitable for sharing with external systems.
     *
     * @param string $title The privacy aware version of the page title.
     */
    public function set_privacy_aware_title($title) {
        $title = format_string($title);
        $title = strip_tags($title);
        $title = str_replace('"', '&quot;', $title);
        $this->_privacy_aware_title = $title;
    }
    ...
    /**
     * Sets the target of the 'Skip to main content' skip link at the top of the site.
     *
     * @param $main_skip_target_id
     * @return void
     */
    public function set_main_skip_target_id($main_skip_target_id): void {
        $this->_main_skip_target_id = $main_skip_target_id;
    }
    ...
    public function initialise_theme_and_output() {
        ...
        $this->requires->with_origin('shell', function () {
            ...
            $hook = new \core\hook\theme_initialised_for_page_layout($this->pagelayout);
            $hook->execute();
            $this->set_pagelayout($hook->get_page_layout());

            $this->_theme->setup_blocks($this->pagelayout, $this->blocks);
            if (!during_initial_install()) {
                $this->requires->js_call_amd('core/blocks', 'pageInit');
            }
            if ($this->_theme->enable_dock && !empty($CFG->allowblockstodock)) {
                $this->requires->strings_for_js(
                    ['addtodock', 'undockitem', 'dockblock', 'undockblock', 'undockall', 'hidedockpanel', 'hidepanel'],
                    'block'
                );
    ...
    public function reset_theme_and_output() {
        ...
        $hook = new core\hook\reset_theme_and_output();
        $hook->execute();
    }
