...
function s($var) {
    ...
    // Totara: A null would always return an empty string.
    if ($var === null) {
        return '';
    }
...
function strip_querystring($url) {
    // Totara: Avoid passing null into a string function
    if ($url !== null && $commapos = strpos($url, '?')) {
...
class moodle_url {
    ...
    public function __construct($url, array $params = null, $anchor = null) {
        ...
        if ($url instanceof moodle_url) {
            ...
            // Totara: $url is expected to be a string at this point, transform any nulls
            $url = $url ?? '';
    ...
    /**
     * Factory method for creation of url pointing to plugin file.
     * This method is the same that make_pluginfile_url but pointing to the external API pluginfile.php script.
     * It should be used only in external functions.
     *
     * @param int $contextid
     * @param string $component
     ...
     * @return moodle_url
     */
    public static function make_external_api_pluginfile_url($contextid, $component, $area, $itemid, $pathname, $filename,
                                                          $forcedownload = false) {
        global $CFG;
        $urlbase = "$CFG->wwwroot/api/pluginfile.php";
        if ($itemid === null) {
            return self::make_file_url($urlbase, "/$contextid/$component/$area".$pathname.$filename, $forcedownload);
        } else {
            return self::make_file_url($urlbase, "/$contextid/$component/$area/$itemid".$pathname.$filename, $forcedownload);
...
function validate_email($address) {
    // Totara: We expect a string, a null is immediately invalid.
    if ($address === null) {
        return false;
    }
 ...
 *      class-wrapper : Wraps the formatted text with an outer div to mark it as user-generated content. This is enabled
 *                      by default with overflowdiv is enabled. Option can be bool true or false, or a custom class name
 *                      to use instead of "generated-content--user".
...
function format_text($text, $format = FORMAT_MOODLE, $options = null, $courseiddonotuse = null) {
    ...
    // The class-wrapper is meant to be a string with a class name, but we have to handle booleans correctly too.
    if (!isset($options['class-wrapper'])) {
        // We generally want the user content wrapper when we want the overflow div.
        if ($options['overflowdiv']) {
            $options['class-wrapper'] = 'generated-content--user legacy-rendered';
        } else {
            $options['class-wrapper'] = null;
        }
    } elseif (empty($options['class-wrapper'])) {
        $options['class-wrapper'] = null;
    } elseif ($options['class-wrapper'] === true) {
        $options['class-wrapper'] = 'generated-content--user legacy-rendered';
    }
    ...
    if (!empty($options['overflowdiv'])) {
        $tag_to_check = '<div class="no-overflow">';
        if (strpos($text, $tag_to_check) === false) {
            $text = html_writer::tag('div', $text, array('class' => 'no-overflow'));
        }
        $options['overflowdiv'] = false; // Only do this once.
    }

    if (!is_null($options['class-wrapper'])) {
        $tag_to_check = '<div class="' . $options['class-wrapper'] . '">';
        if (strpos($text, $tag_to_check) === false) {
            $text = html_writer::tag('div', $text, array('class' => $options['class-wrapper']));
        }
        $options['class-wrapper'] = null;
    }
...
function purify_html($text, $options = array()) {
    ...
    if (empty($purifiers[$type])) {
        ...
        // Use our cache handler
        HTMLPurifier_DefinitionCacheFactory::instance()->register('local', 'HTMLPurifier_LocalSerializer');
        $config->set('Cache.DefinitionImpl', 'local');
        ...
        if ($def = $config->maybeGetRawHTMLDefinition()) {
            // ARIA attributes
            $def->manager->addModule(new \core\sanitisation\html_purifier\aria_module());
...
function send_headers($contenttype, $cacheable = true) {
    ...
    // Only allow http caching if the setting is on
    $allow_http_cache = !empty($CFG->allow_http_cache);

    if ($cacheable && $allow_http_cache) {
...
function redirect($url, $message='', $delay=null, $messagetype = \core\output\notification::NOTIFY_INFO) {
    ...
    if (!empty($message)) {
        if (!$debugdisableredirect && !headers_sent() && core\session\manager::is_session_active()) {
            // Totara: Normalise the message type, this is used only if session is active.
...
function debugging($message = '', $level = DEBUG_NORMAL, $backtrace = null) {
    ...
    if ($message) {
        ...
        if (PHPUNIT_TEST) {
            if (\core_phpunit\internal_util::debugging_triggered($message, $level, $from)) {
...
function get_formatted_help_string($identifier, $component, $ajax = false, $a = null) {
    ...
    if ($sm->string_exists($identifier . '_help', $component)) {
        ...
        if ($sm->string_exists($helplink, $component) && !$sm->string_deprecated($helplink, $component)) {  // Link to further info in Totara docs.
            $link = get_string($helplink, $component);
            $doc_url = get_docs_url($link);

            $linktext = get_string('morehelp');

            if ($doc_url) {
                $data->doclink = new stdClass();
                $url = new moodle_url($doc_url);
