...
function has_capability_in_any_context(string $capability, array $levels = null, $user = null, $doanything = true) {
    ...
    $mariadb_force_search_depth = $CFG->mariadb_search_depth_has_capability_in_context ?? null;
    if ($mariadb_force_search_depth !== null) {
        $sql = $DB->get_optimizer_hint('mariadb_force_search_depth', $mariadb_force_search_depth) . $sql;
    }
 ...
 * @param bool $check_cap_allow_only Check CAP_ALLOW only. Default to false. If false, evaluates CAP_ALLOW and CAP_PREVENT.
 * @return bool
 * @throws \coding_exception
 * @throws \dml_exception
 */
function has_role_with_capability(string $capability, ?array $levels = [], ?int $user_id = null, bool $do_anything = true, bool $check_cap_allow_only = false) {
    global $USER, $DB, $CFG;
    ...
    if (isguestuser($user_id) || $user_id == 0) {
        // Make sure the guest account and not-logged-in users never get any risky caps no matter what the actual settings are.
        $risky_caps = ($cap_info->captype === 'write') || ($cap_info->riskbitmask & (RISK_XSS | RISK_CONFIG | RISK_DATALOSS));

        // Make sure forcelogin cuts off not-logged-in users if enabled.
        $not_logged_in_users = !empty($CFG->forcelogin) && $user_id == 0;

        if ($risky_caps || $not_logged_in_users) {
            return false;
        }
    ...
    $permission_check = "(rc.permission = 1 OR rc.permission = -1)";
    if ($check_cap_allow_only) {
        $permission_check = "(rc.permission = 1)";
    }
    ...
    $sql = "
            ...
            JOIN {role_capabilities} rc ON ra.roleid = rc.roleid
                AND rc.capability = :capability AND {$permission_check}
...
function get_role_archetypes() {
    return array(
        ...
        'apiuser' => 'apiuser',
        'approvalworkflowmanager' => 'approvalworkflowmanager',
...
/**
 * Mark a batch of users as dirty (with timestamp) to force reloading of the user session.
 *
 * @param array $user_ids
 * @return void
 */
function mark_user_dirty_bulk(array $user_ids) {
    global $CFG, $ACCESSLIB_PRIVATE;
    ...
    foreach ($user_ids as $user_id) {
        if (empty($user_id)) {
            throw new coding_exception('Invalid user parameter supplied for mark_user_dirty_bulk() function.');
        }
    }

    // Set dirty flag in database, set dirty field locally, and clear local accessdata cache.
    set_cache_flag_bulk('accesslib/dirtyusers', $user_ids, 1, time() + $CFG->sessiontimeout);

    foreach ($user_ids as $user_id) {
        $ACCESSLIB_PRIVATE->dirtyusers[$user_id] = 1;
        unset($ACCESSLIB_PRIVATE->accessdatabyuser[$user_id]);
    }

    // Clear the access controller instance cache. If any user had an entry in there it may now be stale.
    \core_user\access_controller::clear_instance_cache();
}
...
function role_unassign_all_bulk(array $params, $subcontexts = false, $includemanual = false) {
    ...
    if (!empty($ras)) {
        ...
        mark_user_dirty_bulk(array_keys($resetusers));
...
function get_default_role_archetype_allows($type, $archetype) {
    ...
    $defaults = array(
        'assign' => array(
            ...
            'workspaceowner' => array(),
            'workspacecreator' => array(),
            'apiuser' => array(),
            'approvalworkflowmanager' => array(),
            'approvalworkflowapprover' => array(),
        ),
        'override' => array(
...
function get_default_contextlevels($rolearchetype) {
    static $defaults = array(
        ...
        'workspaceowner' => array(CONTEXT_COURSE),
        'workspacecreator' => array(CONTEXT_SYSTEM, CONTEXT_COURSECAT),
        'apiuser' => array(CONTEXT_SYSTEM, CONTEXT_COURSECAT, CONTEXT_TENANT),
        'approvalworkflowmanager' => array(CONTEXT_COURSE, CONTEXT_MODULE),
        'approvalworkflowapprover' => array(CONTEXT_MODULE),
    );
...
function role_get_name(stdClass $role, $context = null, $rolenamedisplay = ROLENAME_ALIAS) {
    ...
    if (trim($role->name) !== '') {
        ...
        switch ($role->shortname) {
            ...
            // Workspace roles
            case 'workspaceowner': $original = get_string('workspaceowner'); break;
            case 'workspacecreator': $original = get_string('workspacecreator'); break;
            // API users
            case 'apiuser': $original = get_string('apiuser'); break;
            // Approval workflow roles
            case 'approvalworkflowmanager': $original = get_string('approvalworkflowmanager'); break;
            case 'approvalworkflowapprover': $original = get_string('approvalworkflowapprover'); break;
            case 'intelliboardapiuser': $original = get_string('intelliboardapiuser'); break;
...
function role_get_description(stdClass $role) {
    ...
    switch ($role->shortname) {
        ...
        // Workspace roles
        case 'workspacecreator': return get_string('workspacecreatordescription');
        case 'workspaceowner': return get_string('workspaceownerdescription');
        // API user
        case 'apiuser': return get_string('apiuserdescription');
        // Approval workflow roles
        case 'approvalworkflowmanager': return get_string('approvalworkflowmanagerdescription');
        case 'approvalworkflowapprover': return get_string('approvalworkflowapproverdescription');
        case 'intelliboardapiuser': return get_string('intelliboardapiuserdescription');
        default:                return '';
    }
}
...
function role_fix_names($roleoptions, context $context = null, $rolenamedisplay = ROLENAME_ALIAS, $returnmenu = null) {
    ...
    // Add localname property to each role and handle missing or invalid roles.
    foreach ($roleoptions as $rid => $role) {
        // Ensure $role is an object. Convert strings to objects if necessary.
        if (is_string($role)) {
            $roleoptions[$rid] = (object)[
                'id' => $rid,
                'name' => $role,
            ];
            $role = $roleoptions[$rid];
        }

        // Assign the localname property using role_get_name or provide a fallback for missing roles.
        if ($role !== null && is_object($role)) {
            ...
            $roleoptions[$rid] = (object)[
                'id' => $rid,
                'coursealias' => $missing_role_name,
                'localname' => $missing_role_name,
            ];
        }
    }
    ...
    // Construct the $menu array with safe handling for each role.
    $menu = [];
    foreach ($roleoptions as $rid => $role) {
        $menu[$rid] = (is_object($role) && property_exists($role, 'localname'))
            ? $role->localname
            : get_string('roleunknown', 'core_role');
    }
    return $menu;
}
...
class context_helper extends context {
    ...
    private static function init_levels() {
        ...
        if (!is_array($levels)) {
            // TOTARA: Set allowed_classes to false as this should be a simple data array
            $levels = @unserialize($levels, ['allowed_classes' => false]);
        }
        if (!is_array($levels)) {
