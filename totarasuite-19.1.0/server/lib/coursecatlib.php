...
class coursecat implements renderable, cacheable_object, IteratorAggregate {
    ...
    /**
     * Totara: A flag to see whether the category belongs to a container or not.
     * @var bool
     */
    ...
    public static function create($data, $editoroptions = null) {
        ...
        $newcategory->sortorder = MAX_COURSES_IN_CATEGORY*MAX_COURSE_CATEGORIES;
        ...
        // Totara: Add the ability to create container categories
        $newcategory->iscontainer = 0;
        if (isset($data->iscontainer)) {
            $newcategory->iscontainer = $data->iscontainer;
        }
        ...
        course_category::fix_category_sortorder();
    ...
    protected function get_not_visible_children_ids() {
        ...
        if (($invisibleids = $coursecatcache->get($cache_key)) === false) {
            ...
            $containers = $DB->get_fieldset_select('course_categories', 'id', "iscontainer=:container", ['container' => 1]);
    ...
    public function get_children($options = array()) {
        ...
        $where_system = 'AND cc.issystem = :is_system';
        $where_system_param = [
            'is_system' => 0
        ];
        ...
        // Totara: Users should not see container categories, however we return them if system are returned
        $where_container = '';
        $where_container_param = [];
        if (empty($where_system_param['is_system'])) {
            $where_container = ' AND cc.iscontainer = :is_container';
            $where_container_param['is_container'] = 0;
        }
        ...
        if ($sortedids === false) {
            ...
            if ($sortfieldskeys[0] === 'sortorder') {
                ...
                if (count($invisibleids)) {
                    ...
                    $records = self::get_records('cc.parent = :parent AND cc.id '. $sql . " " . $where_system . $where_container,
                            array('parent' => $this->id) + $params + $where_system_param + $where_container_param);
                } else {
                    // Totara: Added support for course categories type.
                    $records = self::get_records('cc.parent = :parent ' . $where_system . $where_container, array('parent' => $this->id) + $where_system_param + $where_container_param);
                }
        ...
        if (isset($records)) {
            ...
            if (!empty($citems)) {
                // Restore the sort order from sortedids and category must be visible.
                foreach ($sortedids as $id) {
                    // Mimic the issystem checks in the sql.
                    if (empty($options['is_system']) && !empty($citems[$id]->iscontainer)) {
                        continue;
                    }

                    if (isset($citems[$id]) && $this->visible) {
                        $rv[$id] = $citems[$id];
                    }
                }
            }
            ...
            if (!empty($sortedids)) {
                ...
                $records = self::get_records('cc.id '. $sql . " " . $where_system . $where_container, array('parent' => $this->id) + $params + $where_system_param + $where_container_param);
            }
        }
     ...
     *     - raw_data - Returns the course records without course_in_list objects
     *     - completion_tracked - If handed through, limits the results to matching enablecompletion true/false
    ...
    public static function search_courses($search, $options = array(), $requiredcapabilities = array()) {
        ...
        $completion_tracked = isset($options['completion_tracked']) ?? null;

        $comp_cache_key = '';
        $completion_tracked = null;
        if (isset($options['completion_tracked'])) {
            if ($options['completion_tracked']) {
                $completion_tracked = true;
                $comp_cache_key = 'ce-'; // Completion enabled.
            } else {
                $completion_tracked = false;
                $comp_cache_key = 'cd-'; // Completion disabled.
            }
        }

        $coursecatcache = cache::make('core', 'coursecat');
        $cachekey = 's-' . $comp_cache_key . serialize(
            $search + array('sort' => $sortfields) + array('requiredcapabilities' => $requiredcapabilities)
        );

        $cntcachekey = 'scnt-' . $comp_cache_key .  serialize($search);
        ...
        if ($ids !== false) {
            ...
            if (!empty($ids)) {
                ...
                // If option 'raw_data' is specified, no further action is needed just return the records.
                if (!empty($options['raw_data'])) {
                    return $records;
                }
        ...
        if (empty($search['blocklist']) && empty($search['modulelist']) && empty($search['tagid'])) {
            ...
            $courselist = get_courses_search($searchterms, 'c.sortorder ASC', 0, 9999999, $totalcount, $requiredcapabilities, $completion_tracked);
     ...
     *    - raw_data - Returns the course records without course_in_list objects
     *    - completion_tracked - Limits courses returned to courses with completion enabled
     *    - check_visibility - If false then visibility checks are skipped, defaults to true
     * @return course_in_list[]
     */
    public function get_courses($options = array()) {
        ...
        $coursecatcache = cache::make('core', 'coursecat');
        $cachekey = 'l-'. $this->id. '-' .
            (!empty($options['recursive']) ? 'r-' : '') .
            $comp_cache_key .
            serialize($sortfields);

        $cntcachekey = 'lcnt-'. $this->id. '-' .
            (!empty($options['recursive']) ? 'r' : '') .
            $comp_cache_key;
        ...
        if ($ids !== false) {
            ...
            if (!empty($ids)) {
                ...
                // Include the completion filter.
                if (isset($options['completion_tracked'])) {
                    $sql .= ' AND c.enablecompletion = :encomp';
                    $params['encomp'] = $options['completion_tracked'] ? COMPLETION_ENABLED : COMPLETION_DISABLED;
                }
        ...
        // Include the completion filter.
        if (isset($options['completion_tracked'])) {
            $where .= ' AND c.enablecompletion = :encomp';
            $params['encomp'] = $options['completion_tracked'] ? COMPLETION_ENABLED : COMPLETION_DISABLED;
        }

        // Get list of courses without preloaded coursecontacts because we don't need them for every course.
        $check_visibility = $options['check_visibility'] ?? true;
        $list = $this->get_course_records($where, $params, array_diff_key($options, array('coursecontacts' => 1)), $check_visibility);
    ...
    public static function prime_catalog_categories_caches() {
        // This is a session cache and will be primed once per session.
        $catscache = cache::make('core', 'coursecat');
        // This is a request cache and will be primed once per request.
        $catsrecordcache = cache::make('core', 'coursecatrecords');

        $primed_catscache = (bool) $catscache->get('isprimed');
        $primed_catsrecordcache = (bool) $catsrecordcache->get('isprimed');

        if ($primed_catscache && $primed_catsrecordcache) {
            // Nothing to do. Both are primed.
            return;
        }

        // Check if either cache needs to be primed, if so then load the categories from the database in preparation.
        if (!$primed_catscache || !$primed_catsrecordcache) {
            // Load all categories.
            $cats = self::get_records('issystem = :sys AND iscontainer = :container', ['sys' => 0, 'container' => 0]);
        }

        if (!$primed_catscache) {
            // Check the session cache first. If we have to prime the session cache, we can freely prime both caches.
            $sortfields = array('sortorder' => 1);
            $children = [];
            foreach ($cats as $cat) {
            ...
            $catsrecordcache->set('isprimed', true);
        } else if (!$primed_catsrecordcache) {
            // In this case just the records cache needs to be primed.
            $cats = array_map(fn($cat) => new coursecat($cat), $cats);
            $cats['isprimed'] = true;
            $catsrecordcache->set_many($cats);
        }
    }
    ...
    public function delete_full($showfeedback = true) {
        ...
        // Update cohort role category context id
        update_cohort_role_category_context($this);
        ...
        if ($this->id == $CFG->defaultrequestcategory) {
            set_config('defaultrequestcategory', $DB->get_field('course_categories', 'MIN(id)', array('parent' => 0, 'issystem' => 0, 'iscontainer' => 0)));
        }
    ...
    public function delete_move($newparentid, $showfeedback = false) {
        ...
        if ($children) {
            // Change all child categories to new parent
            foreach ($children as $childcat) {
                $childcat->change_parent_raw($newparentcat);
            }
            // Rebuild all sort orders and contexts
            course_category::fix_category_sortorder();
            // Emit the events
            foreach ($children as $childcat) {
                // Log action.
        ...
        // Fix sort order of categories after delete category
        course_category::fix_category_sortorder();
    ...
    protected function change_parent_raw(coursecat $newparentcat) {
        ...
        $DB->set_field('course_categories', 'sortorder', MAX_COURSES_IN_CATEGORY*MAX_COURSE_CATEGORIES+$this->sortorder, array('id' => $this->id));
    ...
    public function change_parent($newparentcat) {
        ...
        if ($newparentcat->id != $this->parent) {
            ...
            // Fix category sort order
            \core\model\course_category::fix_category_sortorder();
    ...
    public static function make_categories_list($requiredcapability = '', $excludeid = 0, $separator = ' / ') {
        ...
        if ($baselist === false) {
            ...
            $sql = "SELECT cc.id, cc.sortorder, cc.name, cc.visible, cc.parent, cc.path, $ctxselect
                    FROM {course_categories} cc
                    JOIN {context} ctx ON cc.id = ctx.instanceid AND ctx.contextlevel = :contextcoursecat
                    WHERE cc.issystem = 0 AND cc.iscontainer = 0
                    ORDER BY cc.sortorder";
    ...
    /**
     * Recursively create the path of the given category as a string
     * e.g. "Grandparent1 / Parent2 / Category3"
     *
     * @param int $category_id - The id of the category to build the string for
     * @return string
     */
    public static function make_category_path_string(int $category_id, $seperator = ' / '): string {
        $category = self::get($category_id, MUST_EXIST);

        $context = context_coursecat::instance($category_id);
        $cat_name = format_string($category->name, true, array('context' => $context));
        if (empty($category->parent)) {
            return $cat_name;
        } else {
            return self::make_category_path_string($category->parent) . $seperator . $cat_name;
        }
    }
    ...
    public function can_change_visibility() {
        ...
        if (!empty($CFG->tenantsenabled)) { // Perf hack - tenants have to be deleted before disabling them.
            ...
            if (in_array($this->id, \totara_tenant\util::get_tenant_category_ids())) {
                return false;
            }
        }
        return $this->parent_has_manage_capability();
    ...
    public function resort_courses($field, $cleanup = true) {
        ...
        if (count($courses) > 0) {
            ...
            if ($cleanup) {
                // This should not be needed but we do it just to be safe.
                $category_model = \core\model\course_category::load_by_id($this->id);
    ...
    /**
     *
     * @param string $viewtype - Whether the page is course/program/certification
     * @return moodle_url      - The associated management page
     */
    public static function build_manage_url(string $viewtype = ''): \moodle_url {
        global $CFG;

        if (empty($viewtype) || $viewtype == 'course') {
            if (empty($CFG->simplified_category_management)) {
                return new moodle_url('/course/management.php');
            } else {
                return new moodle_url('/course/categories.php');
            }
        } else {
            if (empty($CFG->simplified_category_management)) {
                $url = new moodle_url('/totara/program/manage.php');
            } else {
                $url = new moodle_url('/totara/program/categories.php');
            }

            $url->param('viewtype', $viewtype);
            return $url;
        }
    }
}
