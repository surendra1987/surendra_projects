 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_form_duration_test extends \core_phpunit\testcase {
    ...
    public function test_constructor_limited_units() {
        ...
        $this->assertStringContainsString('<option title="minutes" value="60" selected="selected">minutes</option>', $html);
        $this->assertStringContainsString('<option title="seconds" value="1">seconds</option>', $html);
    ...
    public function test_exportValue() {
        ...
        $this->assertEquals(20, $el->exportValue($values)['testel']);
        ...
        $this->assertEquals(10, $el2->exportValue($values)['testel']);
        $values = ['testel' => ['number' => 10, 'timeunit' => 1, 'enabled' => 0]];
        $this->assertEquals(['testel' => 0], $el2->exportValue($values, true));
        $this->assertEquals(null, $el2->exportValue($values)['testel']);
    }
}
