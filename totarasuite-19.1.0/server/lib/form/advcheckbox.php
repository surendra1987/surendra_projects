...
class MoodleQuickForm_advcheckbox extends HTML_QuickForm_advcheckbox implements templatable {
    ...
    {
        $this->_generateId();
        if ($this->_text === null || 0 == strlen($this->_text)) {
            $label = '';
        } else {
            $label = '<label for="' . $this->getAttribute('id') . '">' . $this->_text . '</label>';
        }

        $hidden_input = '<input' . $this->_getAttrString([
            'type'  => 'hidden',
            'name'  => $this->getName(),
            'value' => $this->_values[0]
        ]) . ' />';

        $html = $hidden_input . HTML_QuickForm_input::toHtml() . $label;

        return '<span class="fadvcheckbox-content">' . $html . '</span>';
    }
