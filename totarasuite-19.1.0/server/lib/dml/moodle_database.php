...
abstract class moodle_database {
     ...
     * @throws dml_exception A DML specific exception is thrown for any errors.
     */
    public function get_counted_records_sql($sql, ?array $params, $limitfrom, $limitnum, &$count) {
    ...
    public function insert_records_via_batch($table, $iterator, $processor=null, $pextraparams=array(), $validator=null, $vextraparams=array()) {
        ...
        try {
            ...
            // Pass the exception to rollback() so that it will re-throw it and we don't need to do it ourselves.
    ...
    public function sql_ceil($fieldname) {
        ...
        return ' CEIL(' . sprintf('%F', $fieldname) . ')';
    }
     ...
     * @param bool $allow_calculated_field_clause Allow a non-literal value for a calculated field, e.g. 'AVG(my_field_name)'.
     * @return string The piece of SQL code to be used in your statement.
     */
    public function sql_round($fieldname, $places = 0, $allow_calculated_field_clause = false) {
        if ($allow_calculated_field_clause) {
            return "ROUND(" . sprintf('%s', $fieldname) . ", {$places})";
        } else {
            // We use sprintf to convert the value to a locale-independent float value
            return "ROUND(" . sprintf('%F', $fieldname) . ", {$places})";
        }
    }
    ...
    public function sql_like_escape($text, $escapechar = '\\') {
        // Totara: Don't perform string transformations on a null
        if ($text === null) {
            return '';
        }
    ...
    public function unformat_fts_content(?string $content, int $format) {
        ...
        if ($format == FORMAT_JSON_EDITOR) {
            // Prevent the string formatter from trying to parse any linked files. Note that linked file names will
            // be included in the content but alt text is not.
     ...
     * @param bool $orfallback     should an "OR field_name LIKE '%term%'" be added to the query
     * @return array [sql, params[]]
     */
    protected function build_fts_subquery(string $table, array $searchfields, string $searchtext, bool $orfallback = false): array {
     ...
     *
     * Sometimes, for partial word search, FTS doesn't find results, in that scenario a
     * standard SQL: "OR field_name LIKE '%term%'" can be included in the query using $orfallback
     *
     *
     ...
     * @param array  $searchfields ['field_name'=>weight, ...] eg: ['high'=>3, 'medium'=>2, 'low'=>1]
     * @param string $searchtext   natural language search text
     * @param bool $orfallback     should the FTS subquery include fallback SQL like '%term%'
     *
     * @return array [sql, params[]]
     */
    final public function get_fts_subquery(string $table, array $searchfields, string $searchtext, bool $orfallback = false): array {
        ...
        return $this->build_fts_subquery($table, $searchfields, $searchtext, $orfallback);
    }
    ...
    protected function add_row_to_result($row, array &$result, bool $unique_id = true) {
        ...
        // Totara: Cannot call reset or key on an object
        $row = (array) $row;
