...
class sqlsrv_native_moodle_database extends moodle_database {
    ...
    public function sql_round($fieldname, $places = 0, $allow_calculated_field_clause = false) {
        $format_string = ($allow_calculated_field_clause ? '%s' : '%F');
        // We use sprintf to convert the value to a locale-independent float value
        if ($places >= 0) {
            return "CAST(ROUND(".sprintf($format_string, $fieldname).", {$places}) AS DECIMAL(20, {$places}))";
        } else {
            return "ROUND(CAST(".sprintf($format_string, $fieldname)." AS DECIMAL(20, 0)), {$places})";
        }
    }
    ...
    protected function build_fts_subquery(string $table, array $searchfields, string $searchtext, bool $orfallback = false): array {
        ...
        if ($orfallback) {
            // Find the field with the heighest weight and perform LIKE on it
            $fieldname = array_search(max($searchfields), $searchfields);
            // Make the LIKE case/accent insensitive
            $collation = $this->get_collation();
            $collation = str_replace('_CS', '_CI', $collation);
            $collation = str_replace('_AS', '_AI', $collation);
            $paramname = $this->get_unique_param('ftslike');
            $params[$paramname] = $searchtext;
            $sql .= " OR basesearch.{$fieldname} COLLATE $collation LIKE CONCAT('%', REPLACE(:$paramname, '*', '%'), '%') ESCAPE '\\'";
        }
     ...
     *
     * Note: this value is cached for this PHP request; it is highly unlikely the
     * database suddenly changes the accent setting over the course of a run. In
     * any case, since PHP recreates the entire multiverse every request, any
     * change will be picked up in the next run.
     *
     * @since Totara 12
     * @return bool
     */
    public function is_fts_accent_sensitive(): bool {
        ...
        if (is_null($is_active)) {
            ...
            $is_active = !empty($this->get_field_sql($sql, $params));
        }
