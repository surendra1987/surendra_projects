...
/**
 * AI Autocomplete wrapper for select2 library.
 *
 * @module     core/form_tags_ai_autocomplete
 * @class      tags_ai_autocomplete
 * @package    core
 * @copyright  2015 Damyon Wiese <damyon@moodle.com>
 * @copyright 2023 Totara Learning Solutions Ltd {@link http://www.totara.com/}
 * @Author 2015 Damyon Wiese <damyon@moodle.com>
 * @Author 2023 Arshad Anwer <arshad.anwer@totara.com>
 * @since      3.0
 */


/* globals require: false */
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification', 'core/webapi'], function($, log, str, templates, notification, WebAPI) {
    ...
    /** @var {Object} latest The last search term that triggered the autocomplete */
    var latest = {};
    ...
    var updateSuggestions = function(options, state, query, originalSelect) {
        ...
        // Implement locking for race conditions (best triggered by behat)
        inputElement.data('autocomplete-query', query);
        templates.render(
            'core/form_autocomplete_suggestions',
            context
        ).done(function(newHTML) {
            if (inputElement.data('autocomplete-query') !== query) {
                // this is old data
                return;
            }
            ...
            if (matchingElements) {
                ...
                str.get_string('nosuggestions', 'form').done(function(nosuggestionsstr) {
                    if (inputElement.data('autocomplete-query') !== query) {
                        // This data has been superceeded.
                        return;
                    }
    ...
    var updateAjax = function(e, options, state, originalSelect, ajaxHandler) {
        ...
        // Store latest query to prevent resultset 1 overwriting resultset 2 when resultset 1 takes time.
        latest[options.selector] = query;
        ...
        ajaxHandler.transport(options.selector, query, function(results) {
            if (latest[options.selector] !== query) {
                return;
            }
    ...
    /**
     * Toggle AI suggestion container.
     *
     * @method toggleAiContainer
     * @param {Object} context Context related to AI tag container.
     * @param {boolean} isCollapseContainer Whether to collapse AI suggestions container.
     */
    var toggleAiContainer = function(context, isCollapseContainer) {
        const containerActiveClass = 'form-aitags__container--active';
        var tagsContainerEl = $(document.getElementById(context.ai_tags_container_id));
        var suggestionContainerEl = $(document.getElementById(context.ai_suggestion_container_id));

        if(isCollapseContainer) {
            var pendingKey = "form-toggle-ai-container";
            M.util.js_pending(pendingKey);

            var btnEl = $(document.getElementById(context.trigger_id));
            var suggestionBodyEl = $(document.getElementById(context.ai_suggestions_body_id));

            $(tagsContainerEl).removeClass(containerActiveClass);
            $(suggestionContainerEl).animate({opacity: 0}, 200, function () {
                $(suggestionContainerEl).hide();
            });

            var renderHeader = templates.render('core/form_tags_ai_header', context);
            var renderSuggestions = templates.render('core/form_tags_ai_suggestions_selector', context);

            $.when(renderHeader, renderSuggestions).done(function(actionHtml, suggestionHtml) {
                btnEl.html(context.generate_tags_string);
                suggestionBodyEl.empty().append(suggestionHtml);

                M.util.js_complete(pendingKey);
            }).fail(notification.exception);
        } else {
            $(tagsContainerEl).addClass(containerActiveClass);
            $(suggestionContainerEl).show().animate({opacity:1}, 200);
        }
    };

    /**
     * Update suggestions container on graphql response.
     *
     * @method updateAiSuggestionBody
     * @param {Object} context Context related to AI tag container.
     * @param {Array} suggestions list of suggestions from AI.
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     */
    var updateAiSuggestionBody = function(context, suggestions, originalSelect) {
        var suggestionBodyEl = $(document.getElementById(context.ai_suggestions_body_id));
        var itemList = [];
        var dataContext = $.extend({}, context, {ai_suggestions: itemList});

        if(suggestions.length <= 0) {
            dataContext = $.extend({}, context, {ai_suggestions: itemList});
        }
        suggestions.forEach(function(item, index) {
            var timer = index === 0 ? 0 : 100 * index;
            window.setTimeout(function() {
                itemList.push((item));
                dataContext = $.extend({}, context, {ai_suggestions: itemList});
                templates.render('core/form_tags_ai_suggestions_selector', dataContext).done(function(newHtml) {
                    suggestionBodyEl.empty().append(newHtml);

                    if(index >= suggestions.length - 1) {
                        // Bind to click event if it's the last element.
                        bindAiSuggestionSelectionEvent(dataContext, originalSelect);
                    }
                }).fail(notification.exception);
            }, timer);
        });
    ...
    /**
     * Switch tags header template to Re-generate state.
     *
     * @method updateTagsHeaderState
     * @param {Object} context Context related to AI tag container.
     * @param dataContext
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     */
    function updateTagsHeaderState(context, originalSelect) {
        var actionTriggerWrapperEl = $(document.getElementById(context.action_trigger_wrapper_id));

        templates.render('core/form_tags_ai_header', context).done(function(actionHtml) {
            actionTriggerWrapperEl.empty().append(actionHtml);
            var btnEl = $(document.getElementById(context.trigger_id));

            btnEl.html(context.re_generate_tags_string);
            bindAiGenerateClickEvent(context, originalSelect);
        });
    }
     ...
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     *
     * @returns {Promise} Resolves once the tags are rendered
     */
    var getAiSuggestions = function(context, originalSelect) {
        var pendingKey = 'form-get-ai-suggestions';
        var suggestionBodyEl = $(document.getElementById(context.ai_suggestions_body_id));

        var dataContext = $.extend({}, context, {
            is_loading: false,
            ai_suggestions: []
        });
        ...
        return WebAPI.call({
            operationName: "core_ai_course_suggest_tags",
            variables: {
                input: {
                    course: {
                        fullname: document.getElementById('id_fullname').value,
                        shortname: document.getElementById('id_shortname').value,
                        description: document.getElementById('id_summary_editor').value,
            ...
            var selectionElement = $(document.getElementById(context.selectionId));
            var selectedValueElements = selectionElement.children('[aria-selected=true]');

            var formattedResult = [];
            data.forEach(function(item) {
                formattedResult.push(item.rawname);
            });

            // Filter out values that are already selected
            selectedValueElements.each(function(index, ele) {
                var valIndex = formattedResult.indexOf($(ele).attr('data-value'));

                if (valIndex >= 0) {
                    formattedResult.splice(valIndex, 1);
                }
            });

            templates.render('core/form_tags_ai_suggestions_selector', dataContext).done(function(newHtml) {
                suggestionBodyEl.empty().append(newHtml);
                updateAiSuggestionBody(context, formattedResult, originalSelect);
                updateTagsHeaderState(dataContext, originalSelect);
    ...
    /**
     * Bind click event to AI suggestions.
     *
     * @method bindAiSuggestionsEvent
     * @param {Object} context Context related to AI tag container.
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     */
    function bindAiSuggestionSelectionEvent(context, originalSelect) {
        var selectionWrapperEl = $(document.getElementById(context.ai_selection_wrapper_id));

        selectionWrapperEl.on('click', function(e) {
            var items = [];
            var newSelection = $(document.getElementById(context.selectionId));
            var activeId = newSelection.attr('aria-activedescendant');
            var activeValue = false;
            var currentSelected = null;
            currentSelected = e.target.innerText;

            var pendingKey = 'autocomplete-ai-suggestions';
            M.util.js_pending(pendingKey);
            ...
            originalSelect.children('option').each(function(index, ele) {
                if ($(ele).attr('value') == currentSelected) {
                    $(ele).prop('selected', true);
                }
            });
            ...
            originalSelect.children('option').each(function(index, ele) {
                if ($(ele).prop('selected')) {
                    ...
                    // Remove items from AI suggestion container that are already selected
                    context.ai_suggestions = context.ai_suggestions.filter(function(item) {
                        if ($(ele).html() == item) {
                            return false;
                        }
                        return true;
            ...
            var updatedContext = $.extend({items: items}, context);

            templates.render('core/form_tags_ai_suggestions_selector', updatedContext).done(function(newHtml) {
                selectionWrapperEl.empty().append($(newHtml).html());
                M.util.js_complete(pendingKey);
            }).fail(notification.exception);

            templates.render('core/form_autocomplete_selection', updatedContext).done(function(newHTML) {
                // Add it to the page.
                newSelection.empty().append($(newHTML).html());
                ...
                if (activeValue !== false) {
                    ...
                    newSelection.children('[aria-selected=true]').each(function(index, ele) {
                        if ($(ele).attr('data-value') === activeValue) {
                            activateSelection(index, context);
                        }
                    });
                }
            }).fail(notification.exception);
    ...
    /**
     * Bind event to expand AI tag container.
     *
     * @method bindAiGenerateClickEvent
     * @param {Object} context Context related to AI tag container.
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     */
    function bindAiGenerateClickEvent(context, originalSelect) {
        var btnEl = $(document.getElementById(context.trigger_id));
        var suggestionBodyEl = $(document.getElementById(context.ai_suggestions_body_id));
        ...
        btnEl.on('click', function() {
            var pendingKey = "form-aitags-show-suggestions";
            M.util.js_pending(pendingKey);

            const updatedContext = $.extend({}, context, { is_loading: true, loading_pill_counter: new Array(4)});

            var renderHeader = templates.render('core/form_tags_ai_header', updatedContext);
            var renderBodyLoadingState = templates.render('core/form_tags_ai_selection_loading', updatedContext);

            // Switch ui to loading state when fetching data.
            $.when(renderHeader, renderBodyLoadingState).done(function(actionHtml, bodyLoading) {
                suggestionBodyEl.empty().append(bodyLoading);
                actionTriggerWrapperEl.empty().append(actionHtml);
                toggleAiContainer(context);
                M.util.js_complete(pendingKey);
            }).fail(notification.exception);

            // Get data from ai api and merge with existing context
            getAiSuggestions(context, originalSelect);
        });
    }
     ...
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     */
    var addNavigation = function(options, state, aiOptions, originalSelect) {
        ...
        inputElement.on('keypress', function(e) {
            ...
            // Close AI container on input keypress
            var aiContext = $.extend({}, options, state, aiOptions, {ai_suggestions: []});
            toggleAiContainer(aiContext, true);
    ...
    return /** @alias module:core/form_tags_ai_autocomplete */ {
        // Public variables and functions.
        /**
         * Integrate AI tag suggestions with auto-complete select box.
         *
         * @method enhance
         ...
         * @param {object} aiOptions - Properties relevant to AI
         */
        enhance: function(selector, tags, ajax, placeholder, caseSensitive, showSuggestions, aiOptions) {
            ...
            var options = {
                ...
                manageLink: aiOptions.manage_link
            };
            ...
            var state = {
                ...
                inputContainerId: 'form_input_container-' + uniqueId,
                inputWrapperId: 'form_input_wrapper-' + uniqueId,
                ...
                // relevant container ids specific for AI ui
                ai_tags_container_id: 'form_aitags_container-' + uniqueId,
                action_trigger_wrapper_id: 'form_aitags_trigger_wrapper-' + uniqueId,
                trigger_id: 'form_aitags_trigger-' + uniqueId,
                aiTagsTriggerWrapperId: 'form_aitags_trigger_wrapper-' + uniqueId,
                ai_suggestion_container_id: 'form_aitags_suggestions_container-' + uniqueId,
                ai_suggestions_body_id: 'form_aitags_suggestions_body-' + uniqueId,
                ai_selection_wrapper_id: 'form_aitags_selection_wrapper-' + uniqueId
            };

            var editorFormat = document.getElementById('menusummary_editorformat');
            if (editorFormat) {
                switch (editorFormat.value) {
                    case 0:
                        format = 'RAW';
                        break;
                    case 1:
                ...
                // weka doesn't render the #menusummary_editorformat element
                format = 'MOBILE';
            }
            ...
            var renderInput = templates.render('core/form_tags_ai_input', context);
            ...
            var aiContext = $.extend({}, options, aiOptions, state);

            var renderAiHandler = templates.render('core/form_tags_ai_container', aiContext);

            $.when(renderInput, renderDatalist, renderSelection, renderAiHandler).done(function(input, suggestions, selection, aiSuggestionContainer) {
                ...
                var inputContainerEl = $(document.getElementById(state.inputContainerId));
                var inputWrapperEl = $(document.getElementById(state.inputWrapperId));
                // Append tags suggestion and manage link.
                inputWrapperEl.prepend(selection);
                inputWrapperEl.append(suggestions);
                inputWrapperEl.append(options.manageLink);
                // Append the AI ui suggestion block.
                inputContainerEl.append(aiSuggestionContainer);

                // Add the event handlers.
                addNavigation(options, state, aiOptions, originalSelect);

                // Append AI tag container header
                var tagsHeaderEl = $(document.getElementById(state.action_trigger_wrapper_id));
                templates.render('core/form_tags_ai_header', aiContext).done(function(headerHtml) {
                    tagsHeaderEl.append(headerHtml);
                    var btnEl = $(document.getElementById(context.trigger_id));
                    btnEl.html(aiContext.generate_tags_string);

                    // Add relevant event handlers
                    bindAiGenerateClickEvent(aiContext, originalSelect);
                }).fail(function(error) {
                    M.util.js_complete(pendingKey);
                    notification.exception(error);
                });
