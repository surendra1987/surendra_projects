 ...
 * @since      3.1
 */
define(['jquery', 'core/ajax', 'core/templates', 'core/notification', 'core/str', 'core/config', 'core/flex_icon'],
        function($, ajax, templates, notification, str, cfg, flexicons) {
    ...
    $('body').on('click keypress', '[data-inplaceeditable] [data-inplaceeditablelink]', function(e) {
        ...
        var addSpinner = function(element) {
            ...
            var spinner = element.find('.icon-spinner');
            if (spinner.length) {
                spinner.show();
            } else {

                spinner = $('<span class="icon-spinner"/>');
                str.get_string('loading', 'admin').then(function(str) {
                    return templates.renderIcon('loading', {alt: str});
                }).then(function(html) {
                    if (spinner) {
                        spinner.prepend(html);
                    }
                });
        ...
        var removeSpinner = function(element) {
            ...
            element.find('.icon-spinner').hide();
        };
            ...
            ajax
                .call([{
                    ...
                    args: {
                        ...
                        flexicons.getIconData('t/editstring', 'core', {alt: data.edithint}).then(function(icondata) {
                            data.icon = icondata;
        ...
        var turnEditingOnText = function(el) {
            str.get_string('edittitleinstructions').done(function(s) {
                var instr = $('<span class="editinstructions">' + s + '</span>').
                    ...
                    lbl = $('<label class="accesshide"/>')
                        .attr('for', inputelement.attr('id'))
                        .text(mainelement.attr('data-editlabel'));
