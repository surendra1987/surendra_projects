...
class pdf extends TCPDF {
    ...
    /**
     * Generate a QR code in base64 format with the provided payload.
     * Returns a base64 encoded image.
     *
     * @param string $payload The string data to include in the QR code.
     * @param int $size Size of the squares in the QR code.
     * @return string
     */
    public static function qr(string $payload, int $size = 10): string {
        if (empty($payload)) {
            throw new \coding_exception('payload not provided');
        }

        $barcode = new \TCPDF2DBarcode($payload, 'QRCODE,L');
        $qr_code = $barcode->getBarcodePngData($size, $size);
        if (!$qr_code) {
            throw new \coding_exception('Unable to generate a QR code with tcpdf');
        }

        return base64_encode((string) $qr_code);
    }
}
