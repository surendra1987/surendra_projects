...
Y.extend(DRAGDROP, Y.Base, {
    ...
    get_drag_handle: function(title, classname, iconclass, large) {
        var titleid = Y.stamp({title: title});
        var dragelement = Y.Node.create('<span></span>')
            ...
            .setAttribute('data-draghandle-needs-icon', titleid)
            .setAttribute('role', 'button');

        require(['core/templates'], function (templates) {
            templates.renderIcon('arrows', title, iconclass).done(function (icon) {
                dragelement.append(icon).removeAttribute('data-draghandle-needs-icon');
                // This is double handling a bit, but we use promises within non-promise code :(
                Y.all('span[data-draghandle-needs-icon="'+titleid+'"]').append(icon).removeAttribute('data-draghandle-needs-icon');
            });
        });
