...
DOCK.prototype = {
    ...
    _initialiseDockControls: function() {
        ...
        var buttonsnode = Y.Node.create('<div class="'+CSS.controls+'"></div>');
        var that = this;
        this.get('buttonsNode').append(buttonsnode);

        require(['core/templates'], function (templates) {
            templates.renderIcon('block-undock', M.util.get_string('undockall', 'block')).done(function (html) {
                var removeall = Y.Node.create('<a src="#">' + html + '</a>');

                removeall.on('removeall|click', that.removeAll, that);
                removeall.on('dock:actionkey', that.removeAll, that, {actions:{enter:true}});
                buttonsnode.append(removeall);
            });
        });
    },
    /**
    ...
    resizeBlockSpace: function() {
        ...
        // Update page skip link
        const firstPopulatedSide = populatedBlockRegions.includes('side-pre')
            ? 'side-pre'
            : (populatedBlockRegions.includes('side-post') ? 'side-post' : null);
        document.querySelectorAll('body > .skiplinks a').forEach(function(link) {
            const match = /^#block-region-(.*)/.exec(link.getAttribute('href'));
            if (match && match[1].startsWith('side-')) {
                if (firstPopulatedSide === null) {
                    link.style.display = 'none';
                } else {
                    link.style.display = '';
                    link.setAttribute('href', '#block-region-' + firstPopulatedSide);
                }
            }
        });
    },
...
BLOCK.prototype = {
    ...
    moveToDock: function(e) {
        ...
        blocktitle = Y.Node.create('<h2>' + this.cachedcontentnode.one('.title').getData('dock-title') + '</h2>');
        ...
        require(['core/templates'], function (templates) {
            templates.renderIcon('block-undock', Y.Escape.html(M.util.get_string('undockitem', 'block'))).done(function (html) {
                moveto.append(html);
            });
        });
...
DOCKEDITEM.prototype = {
    ...
    draw: function() {
        ...
        closeicon = create('<a href="#" class="hidepanelicon"></a>');
        require(['core/templates'], function (templates) {
            templates.renderIcon('times-circle-o', M.util.get_string('hidepanel', 'block')).done(function (html) {
                closeicon.append(create(html));
            });
        });
