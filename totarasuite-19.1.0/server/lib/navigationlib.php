...
class navigation_node implements renderable {
    ...
    /**
     * Resets the flag to load the admin tree
     */
    public static function reset_admin_tree_required_flag() {
    ...
    public function action() {
        if ($this->action instanceof moodle_url) {
            return $this->action;
        } elseif ($this->action instanceof action_link) {
...
class global_navigation extends navigation_node {
    ...
    public function __construct(moodle_page $page) {
        ...
        } elseif (totara_catalog_admin_setting_catalogtype::is_totara() &&
    ...
    public function initialise() {
        ...
        } elseif (get_home_page() == HOMEPAGE_SITE) {
    ...
    private function legacy_initialise() {
        ...
        switch ($this->page->context->contextlevel) {
            ...
            case CONTEXT_COURSE :
                ...
                } elseif ($coursenode->isexpandable === false) {
                    // Obviously the situation has changed, update the cache and adjust the node.
    ...
    protected function current_user_is_parent_role() {
        ...
        if ($this->useridtouseforparentchecks && $this->useridtouseforparentchecks != $USER->id) {
            ...
            // Totara: do not try any dodgy Moodle hack with parent role assignments here!
            return has_capability('moodle/user:viewalldetails', $usercontext);
        }
        return false;
    }
    ...
    protected function show_categories($ismycourse = false) {
        ...
        if ($this->showcategories === null) {
            ...
            if ($this->page->context->contextlevel == CONTEXT_COURSECAT) {
                $show = true;
            } elseif (!empty($CFG->navshowcategories) && $DB->count_records('course_categories', ['issystem' => 0]) > 1) {
                $show = true;
            }
    ...
    protected function load_all_courses($categoryids = null) {
        ...
        if ($this->show_categories()) {
            ...
            } elseif ($toload == self::LOAD_ROOT_CATEGORIES) {
    ...
    protected function can_add_more_courses_to_category($category) {
        ...
        } elseif ($category instanceof navigation_node) {
        ...
        } elseif (is_object($category) && property_exists($category,'id')) {
    ...
    protected function load_all_categories($categoryid = self::LOAD_ROOT_CATEGORIES, $showbasecategories = false) {
        ...
        } elseif ($categoryid == self::LOAD_ROOT_CATEGORIES) { // can be 0
            // We are going to load all of the first level categories (categories without parents)
            $sqlwhere .= " AND cc.parent = 0";
        } elseif (array_key_exists($categoryid, $this->addedcategories)) {
        ...
        foreach ($categoriesrs as $category) {
            ...
            } elseif (array_key_exists($category->parent, $this->addedcategories)) {
            ...
            if (array_key_exists($category->id, $this->addedcategories)) {
                ...
                foreach ($path as $catid) {
                    if (!array_key_exists($catid, $this->addedcategories)) {
                        ...
                        } elseif (array_key_exists($subcategory->parent, $this->addedcategories)) {
    ...
    protected function add_category(stdClass $category, navigation_node $parent, $nodetype = self::TYPE_CATEGORY) {
        ...
        $cat = coursecat::get($category->id, IGNORE_MISSING);
    ...
    protected function load_course(stdClass $course) {
        ...
        } elseif (array_key_exists($course->id, $this->addedcourses)) {
    ...
    protected function load_stealth_activity(navigation_node $coursenode, $modinfo) {
        ...
        if (!$cm->uservisible) {
            ...
            $activitynode->display = false;
        } elseif (self::module_extends_navigation($cm->modname)) {
    ...
    protected function load_activity($cm, stdClass $course, navigation_node $activity) {
        ...
        if (file_exists($file)) {
            ...
            if (function_exists($function)) {
                ...
                $function($activity, $course, $activityrecord, $cm);
            }
        }
    ...
    public function load_for_user($user=null, $forceforcontext=false) {
        ...
        // Add a branch for user profile.
        if ($issitecourse) {
            $useraccesscontroller = \core_user\access_controller::for($user);
        } else {
            $useraccesscontroller = \core_user\access_controller::for($user, $course);
        }
        $viewprofile = $useraccesscontroller->can_view_profile();
        $userviewurl = null;
        if ($viewprofile) {
            $userviewurl = $useraccesscontroller->get_profile_url();
        }
        ...
        if ($issitecourse) {
            ...
            if ($iscurrentuser || has_capability('moodle/user:viewalldetails', $usercontext)) {
            ...
            // View list of courses of other users.
            if (!$iscurrentuser && (\core_user\access_controller::for($user, null))->can_view_enrolledcourses()) {
    ...
    public function add_course_essentials($coursenode, stdClass $course) {
        ...
        } elseif (count($this->extendforuser) > 0 || $this->page->course->id == $course->id) {
        ...
        switch ($this->branchtype) {
            case 0:
                if ($this->instanceid === 'mycourses') {
                    $this->load_courses_enrolled();
                } elseif ($this->instanceid === 'courses') {
...
class breadcrumb_navigation_node extends navigation_node {
    ...
    public function __construct($navnode) {
        ...
        } elseif ($navnode instanceof navigation_node) {
...
class settings_navigation extends navigation_node {
    ...
    protected function legacy_initialise() {
        ...
        if (isloggedin() && !isguestuser() && (!isset($SESSION->load_navigation_admin) || $SESSION->load_navigation_admin)) {
            ...
            } elseif (!isset($SESSION->load_navigation_admin)) {
                $adminsettings = $this->load_administration_settings();
                $SESSION->load_navigation_admin = (int)($adminsettings->children->count() > 0);

            } elseif ($SESSION->load_navigation_admin) {
                if ($isadminpage) {
                    $adminsettings = $this->load_administration_settings();
                }
            }
        ...
        } elseif ($context->contextlevel == CONTEXT_USER && $usersettings) {
    ...
    protected function load_administration_settings(navigation_node $referencebranch=null, part_of_admin_tree $adminbranch=null) {
        ...
        if ($referencebranch === null) {
        ...
        } elseif ($adminbranch->check_access()) {
            ...
            } elseif ($adminbranch instanceof admin_externalpage) {
                $url = $adminbranch->url;
            } elseif ((!empty($CFG->linkadmincategories) || defined('ADMIN_SEARCH_PAGE')) && $adminbranch instanceof admin_category) {
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        // Add the notifications link if the user has the ability to manage notifications in courses.
        if (notifiable_event_resolver_factory::context_has_resolvers_with_capabilities(extended_context::make_with_context($coursecontext))) {
            // Add the course notifications link.
            $url = new moodle_url('/totara/notification/notifications.php', ['context_id' => $coursecontext->id]);
            $coursenode->add(get_string('notifications', 'admin'), $url, self::TYPE_SETTING, null, null, new \core\output\flex_icon('notification-non-filled', null, false));
        }
        ...
        } elseif (!empty($course->showgrades) && can_access_course($course)) {
            $reportavailable = true;
        }
        ...
        (new \core\hook\navigation_load_course_settings($coursenode, $course))->execute();
    ...
    protected function load_module_settings() {
        ...
        // Add notifications for course modules if the user can manage them, and there are any.
        if (notifiable_event_resolver_factory::context_has_resolvers_with_capabilities(extended_context::make_with_context($this->page->cm->context))) {
            // Add the course module notifications link.
            $params = ['context_id' => $this->page->cm->context->id];
            $url = new moodle_url('/totara/notification/notifications.php', $params);
            $modulenode->add(get_string('notifications', 'admin'), $url, self::TYPE_SETTING, null, null, new \core\output\flex_icon('notification-non-filled', null, false));
        }
        ...
        (new \core\hook\navigation_load_course_module_settings($modulenode, $this->page->cm))->execute();
    ...
    protected function generate_user_settings($courseid, $userid, $gstitle='usercurrentsettings') {
        ...
        if ($gstitle == 'usercurrentsettings') {
            ...
            // Show the grades node.
            if (($issitecourse && $iscurrentuser) || has_capability('moodle/user:viewalldetails', $usercontext)) {
                require_once($CFG->dirroot . '/user/lib.php');
                // Set the grades node to link to the "Grades" page.
                if ($course->id == SITEID) {
                    $url = user_mygrades_url($user->id, $course->id);
        ...
        $edit_own_language = $loggedin_notguest && $currentuser && has_capability('moodle/user:editownlanguage', $usercontext);
        ...
        if ($loggedin_notguest) {
            ...
            } elseif ((has_capability('moodle/user:editprofile', $usercontext) && !is_siteadmin($user)) ||
                    ($currentuser && has_capability('moodle/user:editownprofile', $usercontext))) {
        ...
        if ($userauthplugin && $currentuser && !\core\session\manager::is_loggedinas() && !isguestuser() &&
                has_capability('moodle/user:changeownpassword', $usercontext) && $userauthplugin->can_change_password()) {
        ...
        if ($loggedin_notguest && $currentuser && (new \core_mfa\framework($user->id))->user_can_manage()) {
            $url = new moodle_url('/mfa/user_preferences.php');
            $useraccount->add(
                get_string('manage_multi_factor_authentication', 'mfa'),
                $url,
                self::TYPE_SETTING,
                null,
                'manage_mfa'
        ...
        if ($loggedin_notguest && ($edit_other_profile || $edit_own_language)) {
        ...
        if ($loggedin_notguest && $edit_profile && count(editors_get_enabled_names()) > 1) {
        ...
        if (($currentuser && has_capability('moodle/user:editownmessageprofile', $usercontext)) || ($loggedin_notguest &&
            ...
            $notifications_url = new moodle_url('/totara/notification/user_preferences.php', ['userid' => $user->id]);
    ...
    protected function load_category_settings() {
        ...
        if ($catcontext->contextlevel == CONTEXT_BLOCK) {
            // We can land here while being in the context of a block, in which case we
            // should get the parent context which should be the category/tenant one. See self::initialise().
            $catcontext = $catcontext->get_parent_context();
        }

        if ($catcontext->contextlevel == CONTEXT_TENANT) {
            $tenant = \core\record\tenant::fetch($catcontext->instanceid);
            $catcontext = context_coursecat::instance($tenant->categoryid);
        } elseif ($catcontext->contextlevel != CONTEXT_COURSECAT) {
        ...
        // Notifications
        if (notifiable_event_resolver_factory::context_has_resolvers_with_capabilities(extended_context::make_with_context($catcontext))) {
            $url = new moodle_url('/totara/notification/notifications.php', ['context_id' => $catcontext->id]);
            $categorynode->add(get_string('notifications', 'admin'), $url, self::TYPE_SETTING, null, null,
                new \core\output\flex_icon('notification-non-filled', null, false));
        }

        // Add tenant reports to navigation of parent tenant category
        if (has_capability('totara/reportbuilder:managereports', $catcontext)
            && !empty($catcontext->tenantid) // is a tenant category context
            && $catcontext->get_parent_context()->id === context_system::instance()->id // is not a sub-category
        ) {
            $categorynode->add(
                get_string('reports', 'totara_reportbuilder'),
                new moodle_url(
                    '/totara/reportbuilder/index.php',
                    array('contextid' => $catcontext->id)
                ),
                self::TYPE_SETTING,
                null,
                'report',
...
class navigation_json {
    ...
    protected function convert_child($child, $depth=1) {
        ...
        if ($child->icon instanceof pix_icon) {
            ...
            if (isset($child->icon->attributes['data-generic-navigation-icon'])) {
                ...
                if (!$genericnavigationicon) {
                    ...
                    foreach ($child->icon->attributes as $key => $value) {
                        ...
                        } elseif (!array_key_exists($key, $genericnavigationicon['icon'])) {
            ...
            } elseif (isset($child->icon->attributes['data-generic-settings-icon'])) {
                ...
                if (!$genericsettingsicon) {
                    ...
                    foreach ($child->icon->attributes as $key=>$value) {
                        ...
                        } elseif (!array_key_exists($key, $genericsettingsicon['icon'])) {
                ...
                foreach ($child->icon->attributes as $key => $value) {
                    ...
                    } elseif (!array_key_exists($key, $attributes['icon'])) {
...
class navigation_cache {
    ...
    public function volatile($setting = true) {
        ...
        } elseif (array_key_exists($this->area, self::$volatilecaches)) {
