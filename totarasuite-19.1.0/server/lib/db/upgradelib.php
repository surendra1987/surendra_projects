...
/**
 * Adding support for multiple recipients for notification preferences
 * migrate across the existing recipient fields into multi recipients
 *
 * @return void
 * @throws dml_exception
 */
function totara_notification_migrate_notification_prefs_recipient($batch_limit = 10000) {
    global $DB;

    $offset = 0;
    $has_preferences = true;
    while ($has_preferences) {
        $preferences = $DB->get_recordset('notification_preference', null, 'id ASC', 'id, recipient', $offset, $batch_limit);
        $has_preferences = $preferences->valid();

        foreach ($preferences as $pref) {
            if (!is_null($pref->recipient)) {
                $pref->recipients = json_encode([$pref->recipient]);
                $DB->update_record('notification_preference', $pref);
            }
        }
        $offset += $batch_limit;
        $preferences->close();
...
/**
 * Add external_id mapping to all issuers.
 *
 * @return void
 */
function upgrade_oauth2_user_field_mapping_to_add_external_id(): void {
    global $DB;

    $now = time();
    $issuers = $DB->get_records('oauth2_issuer');
    foreach ($issuers as $issuer) {
        if (!$DB->record_exists('oauth2_user_field_mapping', ['internalfield' => 'external_id', 'issuerid' => $issuer->id])) {
            $DB->insert_record('oauth2_user_field_mapping', [
                'timemodified' => $now,
                'timecreated' => $now,
                'usermodified' => 0,
                'issuerid' => $issuer->id,
                // ms and fb use non-standard 'id' instead of standard OIDC 'sub'
                'externalfield' => ($issuer->type == 'microsoft' || $issuer->type == 'facebook') ? 'id' : 'sub',
