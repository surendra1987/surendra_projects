...
class frontpagerole extends check {
    ...
    public function get_name(): string {
        return get_string('check_frontpagerole_name', 'admin');
    }
    ...
    public function get_result(): result {
        ...
        if (!$frontpagerole = $DB->get_record('role', array('id' => $CFG->defaultfrontpageroleid))) {
            $status  = result::INFO;
            $summary = get_string('check_frontpagerole_notset', 'admin');
            $details = get_string('check_frontpagerole_details', 'admin');
        ...
        if ($riskycount or !$legacyok) {
            ...
            $summary = get_string('check_frontpagerole_error', 'admin', role_get_name($frontpagerole));

        } else {
            $status  = result::OK;
            $summary = get_string('check_frontpagerole_ok', 'admin');
        }
