...
class antivirus extends check {
    ...
    public function get_name(): string {
        return get_string('check_antivirus_name', 'admin');
    }
    ...
    public function get_result(): result {
        global $CFG, $DB;
        $details = \html_writer::tag('p', get_string('check_antivirus_details', 'admin'));

        // If no scanners are enabled, then return an NA status since the results do not matter.
        if (empty($CFG->antiviruses)) {
            $status = result::NA;
            $summary = get_string('check_antivirus_info', 'admin');
        ...
        if (empty($readers)) {
            ...
            $summary = get_string('check_antivirus_logstore_not_supported', 'admin');
            return new result($status, $summary, $details);
        }
        ...
        if (!empty($totalerrors)) {
            $status = result::ERROR;
            $summary = get_string('check_antivirus_error', 'admin', [
                'errors' => $totalerrors,
            ...
            $summary = get_string('check_antivirus_ok', 'admin', [
