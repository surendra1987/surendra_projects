...
class core_date {
    ...
    public static function phpunit_override_default_php_timezone($tz) {
        ...
        // This will either validate & succeed, or throw an exception and fail
        $result = new DateTimeZone($tz ?? '');
    ...
    protected static function init_zones() {
        ...
        // Totara: Replace any broken timezones with working zones
        [self::$bczones, self::$badzones] = self::replace_broken_zones(self::$bczones, self::$badzones);
    ...
    /**
     * If you're running PHP 8.1.14 or 8.2.1, then certain timezones are invalid and can potentially
     * crash the system. This function will substitute them for a valid locational equivalent.
     *
     * @see https://github.com/php/php-src/issues/10218
     * @see https://www.php.net/manual/en/timezones.others.php#timezones.others
     * @param array $bc_zones The key→unused list of valid zones that should drop invalid out.
     * @param array $bad_zones The key→replacement list of zones, where the replacement should be replaced.
     * @return array Returns both arrays back, [$bc_zones, $bad_zones]
     */
    private static function replace_broken_zones(array $bc_zones, array $bad_zones): array {
        // Only run for 8.1.14 and 8.2.1 which are broken.
        if (version_compare(phpversion(), '8.1.14', '!=') && version_compare(phpversion(), '8.2.1', '!=')) {
            return [$bc_zones, $bad_zones];
        }

        // List of bad zones, and their DST-free location equivalent.
        // Etc/GMT+1 actually represents GMT-1, the system maps them backwards.
        $zones_to_replace = [
            ...
            'Etc/GMT+12' => 'Pacific/Midway', // There is no PHP location timezone for GMT-12
        ];

        // $bad_zones are a key→zone array. The zone should be swapped out if it matches one of $invalid_zones.
        foreach ($bad_zones as $zone => $replacement) {
            if (isset($zones_to_replace[$replacement])) {
                $bad_zones[$zone] = $zones_to_replace[$replacement];
            }
        }

        foreach ($zones_to_replace as $zone => $replacement) {
            // $bc_zones are a key→nothing array. The invalid values should be removed
            if (isset($bc_zones[$zone])) {
                unset($bc_zones[$zone]);
            }

            // sometimes the invalid zones themselves are directly called, so make sure they're also replaced.
            $bad_zones[$zone] = $replacement;
        }
