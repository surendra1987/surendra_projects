...
abstract class persistent {
    ...
    public function __construct($id = 0, stdClass $record = null) {
        ...
        // Persistent entities hold their values in plain text, so decrypt any that have
        // come from either method
        $this->decrypt_if_required();
    ...
    /**
     * Get the encrypted properties of the model.
     *
     * @return array
     */
    final public static function encrypted_properties_definition(): array {
        static $cached = [];
        if (isset($cached[static::class])) {
            return $cached[static::class];
        }

        $definitions = array_filter(self::properties_definition(), function ($def) {
            return isset($def['encrypted']) && $def['encrypted'];
        });
        $cached[static::class] = $definitions;
        return $cached[static::class];
    }
    ...
    final public function create() {
        ...
        // Before writing, remove the encrypted values from the record as we need
        // the entity id before we can encrypt
        [$record, $to_encrypt] = $this->extract_encrypted_properties($record, false);

        $id = $DB->insert_record(static::TABLE, $record);
        $this->raw_set('id', $id);

        // Save the encrypted properties if we have any
        if (!empty($to_encrypt)) {
            $this->save_encrypted_properties($id, $to_encrypt);
        }
    ...
    final public function update() {
        ...
        // Before writing, remove the encrypted values from the record so we don't overwrite with unencrypted values
        [$record, $to_encrypt] = $this->extract_encrypted_properties($record, true);
        ...
        // Save the encrypted properties if we have any
        if (!empty($to_encrypt)) {
            $this->save_encrypted_properties($record['id'], $to_encrypt);
        }
    ...
    /**
     * Return the key=>value of properties that should be encrypted while also
     * removing their entries from $record (if $drop_raw is set).
     *
     * $drop_raw is used when updating so we don't blank the database value, but
     * for create we cannot do it as a NULL tends to be frowned upon.
     *
     * @param stdClass $record
     * @param bool $drop_raw Indicate if we should delete the value from $record.
     * @return array
     */
    private function extract_encrypted_properties(stdClass $record, bool $drop_raw = false): array {
        $props = static::encrypted_properties_definition();
        $to_encrypt = [];
        foreach ($props as $property => $def) {
            if (isset($record->$property)) {
                $to_encrypt[$property] = $record->$property;
                if ($drop_raw) {
        ...
        return [$record, $to_encrypt];
    }

    /**
     * Encrypt and save the provided values.
     *
     * @param $id
     * @param mixed $to_encrypt
     * @return void
     */
    private function save_encrypted_properties($id, $to_encrypt): void {
        global $DB;
        $manager = new manager();
        $record = new stdClass();
        $record->id = $id;
        ...
        foreach ($to_encrypt as $property => $clear_value) {
            // Don't encrypt blank
            if ($clear_value === '') {
                // We still want to save the clear value (else you could never remove it)
                $record->{$property} = '';
                $count++;
                continue;
            }

            $compound_value = $manager->encrypt($clear_value, (string) $id, static::class);
            if ($compound_value !== false) {
                $record->{$property} = $compound_value;
                $count++;
            }
        }
        if ($count > 0) {
     ...
     * @return void
     */
    private function decrypt_if_required(): void {
        $encrypted = static::encrypted_properties_definition();
        if (empty($encrypted)) {
            return;
        }
        ...
        foreach ($encrypted as $prop => $def) {
            $cipher_value = $this->raw_get($prop);
            $clear_value = $manager->decrypt($cipher_value, $id, static::class);

            // If we failed to decrypt & the value looks plain text, just return the plaintext instead.
            if ($clear_value === false && !$manager->is_encrypted_value($cipher_value)) {
                continue;
            }

            // We don't want to dirty the model
            $this->data[$prop] = $clear_value;
        }
    }
}