...
/**
 * Software maturity levels used by Totara as allowed for within the semantic versioning 2.0 specification
 * This is designed to be compatible with PHP-standardized versioning as well as composer versioning.
 * Order is: ~Any string not in this list~ < dev < alpha < beta < rc < #
 *
 * The following constants + postfixes have been deprecated:
 *   - MATURITY_EVERGREEN
 */
define('MATURITY_DEV', 10);
...
class core_component {
    ...
    protected static $supportsubplugins = array('mod', 'editor', 'totara', 'tool', 'local', 'perform');
...
';
    ...
    /**
     * Returns list of core subsystems. Subsystems, also known as components, add essential functionality to the product.
     *
     * @return array
     */
    protected static function fetch_subsystems() {
        ...
        $info = array(
            ...
            'mfa'           => $CFG->dirroot.'/mfa',
            'bi'            => $CFG->dirroot.'/integrations/bi',
            'ai'            => $CFG->dirroot.'/integrations/ai',
        );
    ...
    /**
     * Returns list of known plugin types. Plugin types allow Totara, Partners, and other third-parties to introduce
     * new functionality into the product.
     *
     * @return array
     */
    ...
    /**
     * Returns subclasses of a parent within a component.
     *
     * This method was implemented for the case where the implementation classes you are looking for are in the same
     * namespace as the base class, or in a child namespace.
     *
     * Warning: some components may have classes that are unloadable. Doing this search across all of Totara, however
     * useful, will kill the PHP process. Create a namespace architecture that allows you to use get_namespace_classes()
     * instead.
     *
     * @param string $parentclass full name of class that returned classes should be extending
     * @param string $component restrict results to one plugin only, for example 'totara_reportbuilder'
     * @param bool $excludeabstract exclude abstract classes
     * @return string[] list of matching full class names
     */
    public static function get_component_subclasses_of(string $parentclass, string $component, bool $excludeabstract = true) {
        $cached_used = static::init();

        if (!during_initial_install()) {
            $hash_source = $parentclass . $component . (int)$excludeabstract;
            $key = md5($hash_source);

            $cache = \cache::make('core', 'subclasses');
            if ($cached_used) {
        ...
        if (!class_exists($parentclass, true)) {
            debugging('Invalid $parentclass parameter, it must be a name of an existing class: ' . $parentclass, DEBUG_DEVELOPER);
            return array();
        }
        ...
        $classes = array();
        foreach (static::$classmap as $class => $unused) {
            if (strpos($class, $quickmatch) === false) {
                // Optimized match failed.
                continue;
            }
            if (!preg_match($match, $class)) {
                // Regex match failed.
            ...
            $rc = new ReflectionClass($class);
            if ($excludeabstract && $rc->isAbstract()) {
                // This is intended to exclude base classes and helpers.
                continue;
            }
