 ...
 */
class api {
    /** @var int Indicates the system user errored during creation */
    public const SYSTEM_USER_ERROR = 0;
    /** @var int Indicates the system user was successfully created */
    public const SYSTEM_USER_SUCCESS = 1;
    /** @var int Indicates the system user was created, but without automatically loading the OICD information. */
    public const SYSTEM_USER_NAMELESS = -1;
    ...
    /**
     * Build a microsoft ready OAuth 2 service for SMTP
     * @return \core\oauth2\issuer
     */
    private static function init_microsoftsmtp() {
        // Microsoft is a custom setup.
        $record = (object) [
            'name' => 'Microsoft SMTP',
            'type' => 'microsoftsmtp',
            'image' => 'https://www.microsoft.com/favicon.ico',
            'baseurl' => '',
            'loginscopes' => 'https://outlook.office.com/SMTP.Send',
            'loginscopesoffline' => 'https://outlook.office.com/SMTP.Send offline_access',
    ...
    /**
     * Create endpoints for microsoft SMTP issuers.
     * @param issuer $issuer issuer the endpoints should be created for.
     * @return mixed
     * @throws \coding_exception
     * @throws \core\invalid_persistent_exception
     */
    private static function create_endpoints_for_microsoftsmtp($issuer) {
    ...
    public static function init_standard_issuer($type) {
        ...
        } else if ($type == 'microsoftsmtp') {
            return self::init_microsoftsmtp();
        } else {
    ...
    public static function create_endpoints_for_standard_issuer($type, $issuer) {
        ...
        if ($type == 'google') {
            ...
            return self::create_endpoints_for_microsoftsmtp($issuer);
        } else {
            throw new moodle_exception('OAuth 2 service type not recognised: ' . $type);
        }
    }
    ...
    public static function create_standard_issuer($type, $baseurl = false) {
        ...
        if ($type == 'google') {
            ...
            $issuer = self::init_microsoftsmtp();
            $issuer->create();
            return self::create_endpoints_for_microsoftsmtp($issuer);
        } else if ($type == 'facebook') {
    ...
    protected static function discover_endpoints($issuer) {
        ...
        if (!$json = $curl->get($url)) {
            $msg = 'Could not discover end points for identity issuer' . format_string($issuer->get('name'));
    ...
    /**
     * Get the corresponding lang string key for the response's error parameter value.
     *
     * @param string $error The error parameter value
     * @return string
     */
    public static function map_error_to_lang_key(string $error): string {
        $error_keys = [
            'access_denied' => 'oauth2accessdenied',
            'invalid_client' => 'oauth2invalidclient',
            'invalid_grant' => 'oauth2invalidgrant',
            'invalid_request' => 'oauth2invalidrequest',
            'invalid_scope' => 'oauth2invalidscope',
            'server_error' => 'oauth2servererror',
            'temporarily_unavailable' => 'oauth2temporarilyunavailable',
            'unauthorized_client' => 'oauth2unauthorizedclient',
            'unknown_error' => 'oauth2unknownerror',
            'unsupported_grant_type' => 'oauth2unsupportedgranttype',
            'unsupported_response_type' => 'oauth2unsupportedresponsetype',
        ];

        return $error_keys[$error] ?? $error_keys['unknown_error'];
    }
     ...
     * @return int One of the SYSTEM_USER_* constants
     */
    public static function connect_system_account($issuer, $returnurl) {
        ...
        // In certain situations the user information isn't required or provided, and isn't needed for the system account.
        // Default to the username of whomever setup the connection instead and then prompt to set a value afterwards
        // Only if there was no user info endpoint. If there was an endpoint, fail if we cannot load user information.
        ...
        // User was created but without any information, show them the user form instead
        if ($prompt_for_name) {
            return self::SYSTEM_USER_NAMELESS;
        }
        return self::SYSTEM_USER_SUCCESS;
    }
     ...
     * @return system_account|false
     */
    public static function update_system_account($issuer, $username, $email) {
        require_capability('moodle/site:config', context_system::instance());

        $systemaccount = self::get_system_account($issuer);
        $systemaccount->set('username', $username);
        $systemaccount->set('email', $email);
