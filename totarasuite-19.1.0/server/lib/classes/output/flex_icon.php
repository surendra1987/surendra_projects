...
class flex_icon extends \pix_icon {
     ...
     * @param array|null $customdata Optional data to be passed to the rendering (template) context.
     * @param bool $check_exists False to bypass exists check in order to avoid triggering theme loading
     */
    public function __construct($identifier, array $customdata = null, bool $check_exists = true) {
        $this->identifier = (string)$identifier;
        $this->customdata = (array)$customdata;

        if ($check_exists && !self::exists($this->identifier)) {
