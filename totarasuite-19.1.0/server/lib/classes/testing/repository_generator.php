 ...
 */
abstract class repository_generator extends component_generator {
    ...
    public function get_typename() {
        $matches = null;
        if (!preg_match('/^repository_([a-z0-9_]+)\\\\testing\\\\generator$/', get_class($this), $matches)) {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (empty($instanceoptions)) {
            ...
            if (empty($record['contextid'])) {
                throw new coding_exception('contextid must be present in \core\testing\repository_generator::create_instance() $record');
            }

            foreach ($instanceoptions as $option) {
                if (!isset($record[$option])) {
                    throw new coding_exception("$option must be present in \core\testing\repository_generator::create_instance() \$record");
                }
            }
            ...
            if (!in_array($context->contextlevel, array(CONTEXT_SYSTEM, CONTEXT_COURSE, CONTEXT_USER))) {
                throw new coding_exception('Wrong contextid passed in \core\testing\repository_generator::create_instance() $record');
            }
