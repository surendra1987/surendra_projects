...
abstract class base implements \IteratorAggregate {
    ...
    /**
     * Modify the event data to implement the specific logic in the watcher
     *
     * @return modify_event_data
     */
    public function modify_data_by_hook(): modify_event_data {
        $hook = new modify_event_data($this);
        return $hook->execute();
    }
    ...
    /**
     * Add a cached entity that will be most probably used in event observers.
     *
     * This is used to improve performance, but it is required for data
     * that was just deleted.
     *
     * @since Totara 15.2
     *
     * @param \core\orm\entity\entity $entity
     * @return $this
     */
    final public function add_entity_snapshot(\core\orm\entity\entity $entity): self {
        $this->add_record_snapshot($entity::TABLE, $entity->to_record(true));
        return $this;
    }
    ...
    /**
     * Returns a cached entity or fetches the entity from the database if not cached.
     *
     * @since Totara 15.2
     *
     * @param string $entity_class
     * @param int $id
     * @return \core\orm\entity\entity
     */
    final public function get_entity_snapshot(string $entity_class, int $id): \core\orm\entity\entity {
        if (!is_subclass_of($entity_class, \core\orm\entity\entity::class, true)) {
            throw new \coding_exception("$entity_class is not an instance of " . \core\orm\entity\entity::class);
        }

        $record = $this->get_record_snapshot($entity_class::TABLE, $id);
        return new $entity_class($record, false);
    }
