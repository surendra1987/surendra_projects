...
class manager {
    ...
    public static function start() {
        ...
        try {
            ...
            if (!self::$handler->start($requirelock)) {
                ...
                throw new \core\session\exception(get_string('sessionstarterr', 'error'));
            }
    ...
    public static function get_performance_info() {
        ...
        $handler = get_class(static::$handler);
        ...
        if (!empty($CFG->debugsessionlock)) {
            $sessionlock = static::get_session_lock_info();
    ...
    protected static function load_handler() {
        ...
        if (PHPUNIT_TEST) {
            ...
            if (defined('TEST_SESSION_HANDLER') && class_exists(TEST_SESSION_HANDLER)) {
                $class = TEST_SESSION_HANDLER;
            }
    ...
    public static function write_close() {
        ...
        if (static::$sessionactive) {
            $requireslock = static::$handler->use_locking();
            if ($requireslock) {
                ...
                static::update_recent_session_locks($PERF->sessionlock);
                static::sessionlock_debugging();
            }
        }

        if (static::$sessionactive && session_id()) {
    ...
    public static function loginas($userid, \context $context, $generateevent = true) {
        ...
        if ($generateevent) {
            $tenant_id = \context_user::instance($userid)->tenantid;
     ...
     * @return ?array
     */
    public static function get_session_lock_info(): ?array {
        global $PERF;
    ...
    public static function get_locked_page_at($time) {
        $recentsessionlocks = static::get_recent_session_locks();
    ...
    public static function display_blocking_page() {
        ...
        $page = static::get_locked_page_at($PERF->sessionlock['start']);
    ...
    public static function sessionlock_debugging() {
        ...
        if (!empty($CFG->debugsessionlock)) {
            if (isset($PERF->sessionlock['held']) && $PERF->sessionlock['held'] > $CFG->debugsessionlock) {
                $count = number_format($PERF->sessionlock['held'], 3);
                debugging(
                    "Script " . me() . " locked the session for " . $count .
            ...
            if (isset($PERF->sessionlock['wait']) && $PERF->sessionlock['wait'] > $CFG->debugsessionlock) {
                $output = static::display_blocking_page();
    ...
    public static function update_recent_session_locks($sessionlock) {
        ...
        $SESSION->recentsessionlocks = static::get_recent_session_locks();
    ...
    public static function cleanup_recent_session_locks() {
        ...
        $locks = static::get_recent_session_locks();
