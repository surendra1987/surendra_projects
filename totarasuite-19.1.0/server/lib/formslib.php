...
if (!defined('CLI_CONFIGLESS') || !CLI_CONFIGLESS) {
...
abstract class moodleform {
    ...
    function _process_submission($method) {
        ...
        // Totara: Allow the submitted data to be overridden.
        $hook = new \core\hook\moodleform_process_submission($this, $submission, $files);
        $hook->execute();
...
class MoodleQuickForm extends HTML_QuickForm_DHTMLRulesTableless {
    ...
    /** @var string name for location customfield. */
    public $_customlocationfieldname;
        ...
        if (null === $elementList) {
            ...
            foreach (array_keys($this->_elements) as $key) {
                if ($this->_elements[$key]->isFrozen() && !$this->_elements[$key]->_persistantFreeze) {
                    $varname = $this->_elements[$key]->_attributes['name'] ?? null;
                    $value = '';
                    // If we have a default value then export it.
                    if (isset($varname) && isset($this->_defaultValues[$varname])) {
...
class MoodleQuickForm_Renderer extends HTML_QuickForm_Renderer_Tableless{
    ...
    public function __construct() {
        ...
        'legendtemplate' => '<div class="flegend"><legend><span class="legend">{label}<!-- BEGIN required -->{req}<!-- END required -->{advancedimg}</span></legend>{help}</div>',
    ...
    function startGroup(&$group, $required, $error){
        ...
        if (!$fromtemplate) {
            ...
            // Totara: Attribute properties may return null, cast them to an empty string.
    ...
    function renderElement(&$element, $required, $error){
        ...
        if (!$fromtemplate) {
            ...
            // Totara: Attribute & name properties may return null, cast them to an empty string.
...
MoodleQuickForm::registerElementType('tagsaiautocomplete', "$CFG->libdir/form/tagsaiautocomplete.php", 'MoodleQuickForm_tagsaiautocomplete');
...
MoodleQuickForm::registerElementType('integer', "$CFG->libdir/form/integer.php", 'MoodleQuickForm_integer');
MoodleQuickForm::registerElementType('decimal', "$CFG->libdir/form/decimal.php", 'MoodleQuickForm_decimal');
