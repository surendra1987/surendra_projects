...
function self_completion_form($cmorid, $course = null) {
    ...
    // Execute a hook to allow form overrides, notably used by the pathway format to remove the form.
    $hook = new \core_completion\hook\override_activity_self_completion_form($cmdata->completionstate, $cm, $course);
    $hook->execute();

    $self_completion_form = $hook->get_form();
    if (is_null($self_completion_form)) {
        // If the form was not overriden by the hook, load the default form.
        ...
        $self_completion_form = $OUTPUT->render_from_template('core/notification_info', $notificationdata);
    }

    return $self_completion_form;
}
...
class completion_info {
    ...
    /**
     * Get a list of users who have a completion for a course.
     *
     * @param int $course_id Course id, if not provided use current course
     * @return array The list of users who have completion records stored for this course
     */
    public function get_course_completions_users($course_id = null) : array {
        global $DB;
        ...
        $sql = 'SELECT DISTINCT userid FROM {course_completions} WHERE course = ?';
        $params = array($course_id);

        return $DB->get_records_sql_unkeyed($sql, $params);
    }
     ...
     */
    public function delete_course_completion_data(int $userid = null) {
        $userids = ($userid == null) ? [] : [$userid];
        $this->delete_course_completion_data_bulk($userids);
    }

    /**
     * Deletes all course completion data for a list of users
     *
     * When called without a userids, course completion data will be deleted, EXCLUDING RPL completion related records.
     * When called with a userids, course completion data will be deleted, INCLUDING RPL completion related records.
     *
     * @param int[] $userids If provided, all course completion data for these users and course will be deleted,
     *                       including RPL course and activity records.
     */
    public function delete_course_completion_data_bulk(array $userids = []) {
        global $DB, $CFG;
        ...
        if (empty($userids)) {
            ...
            $criteriasql .= " AND {course_completion_crit_compl}.userid {$usersql}";
            $statssql .= " AND {block_totara_stats}.userid {$usersql}";
            $coursesql .= " AND {course_completions}.userid {$usersql}";
        ...
        if (empty($userids)) {
            \core_completion\helper::save_completion_log($this->course_id, null, $message);
        } else {
    ...
    public function invalidatecache($courseid = null, $userid = null, $unused = null) {
        ...
        if (isset($courseid) && isset($userid)) {
            ...
            $completioncachekeys = [];
            $progressinfocachekeys = [];

            $users = $this->get_course_completions_users($courseid);

            foreach ($users as $user) {
                $completioncachekeys[] = $user->userid . '_' . $courseid;
                $progressinfocachekeys[] = $this->get_progressinfo_cache_key($user->userid);
            }

            $cache->delete_many($completioncachekeys);
            $this->get_progressinfo_cache()->delete_many($progressinfocachekeys);
        } else {
            $cache->purge();
            $this->get_progressinfo_cache()->purge();
        }
