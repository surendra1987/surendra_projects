 ...
 */
class core_files_file_storage_test extends \core_phpunit\testcase {
    ...
    public function test_copy_area_files_to_new_context() {
        ...
        // Create a new page.
        $page2 = $this->getDataGenerator()->create_module('page', array('course'=>$course1->id));
        $page2context = context_module::instance($page2->cmid);
        ...
        // Copy the files.
        $fs->copy_area_files_to_new_context($page1context->id, $page2context->id, 'mod_page', 'content');

        // Page2 filearea should no longer be empty.
        $this->assertFalse($fs->is_area_empty($page2context->id, 'mod_page', 'content'));

        // Page1 filearea should still have files.
        $this->assertFalse($fs->is_area_empty($page1context->id, 'mod_page', 'content'));

        $page2files = $fs->get_area_files($page2context->id, 'mod_page', 'content', 0, 'sortorder', false);
        $copiedfile = reset($page2files);

        // The two files should have the same content hash.
        $this->assertEquals($copiedfile->get_contenthash(), $originalfile->get_contenthash());
    }

    /**
     * @covers ::copy_area_files
     */
    public function test_copy_area_files(): void {
        // Set up some resources to use the context ID of
        $dg = $this->getDataGenerator();
        $course1 = $dg->create_course();
        $page1 = $dg->create_module('page', array('course' => $course1->id));
        $page1context = context_module::instance($page1->cmid);
        $page2 = $dg->create_module('page', array('course' => $course1->id));
        $page2context = context_module::instance($page2->cmid);
        $page3 = $dg->create_module('page', array('course' => $course1->id));
        $page3context = context_module::instance($page3->cmid);
        ...
        $count = $fs->copy_area_files(
            $page1context->id, 'mod_page', 'content', 5,
            $page2context->id, 'mod_page', 'content', 9
        );

        $this->assertEquals(1, $count);

        $old_area_files = $fs->get_area_files($page1context->id, 'mod_page', 'content', 5, 'id', false);
        $new_area_files = $fs->get_area_files($page2context->id, 'mod_page', 'content', 9, 'id', false);
        // both areas should now have the file
        $this->assertEquals(1, count($old_area_files));
        $this->assertEquals(1, count($new_area_files));
        $this->assertEquals(reset($old_area_files)->get_contenthash(), reset($new_area_files)->get_contenthash());

        // test when not specifying itemid
        $count = $fs->copy_area_files(
            $page1context->id, 'mod_page', 'content', null,
            $page3context->id, 'mod_page', 'content', null
        );
        $this->assertEquals(1, $count);
        $old_area_files = $fs->get_area_files($page1context->id, 'mod_page', 'content', 5, 'id', false);
        $new_area_files = $fs->get_area_files($page3context->id, 'mod_page', 'content', 5, 'id', false);
        $this->assertEquals(1, count($old_area_files));
        $this->assertEquals(1, count($new_area_files));
        $this->assertEquals(reset($old_area_files)->get_contenthash(), reset($new_area_files)->get_contenthash());
    }
    ...
    public function test_get_local_path_from_storedfile(): void {
        global $CFG;
        $fs = get_file_storage();
        self::setAdminUser();
        $contextid = context_user::instance(\core\entity\user::logged_in()->id)->id;
        ...
        $user_file = $fs->create_file_from_string($file, "file content");
        self::assertInstanceOf('stored_file', $user_file);
        $path = $fs->get_local_path_from_storedfile($user_file, true);

        self::stringContains($CFG->dataroot, $path);
