...
class help_icon implements renderable, templatable {
    ...
    /**
     * Lang string variable parameter for the help text
     *
     * @var mixed
     */
    private $a;
     ...
     * @param mixed $a Object used within translation strings
     */
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (!empty($this->a)) {
            $url_params['a'] = json_encode($this->a);
        }

        $data->url = (new moodle_url('/help.php', $url_params))->out(false);
...
class pix_icon implements renderable, templatable {
    ...
    public function __construct($pix, $alt, $component='moodle', array $attributes = null) {
        ...
        $altattributes = is_null($alt) ? null : json_decode($alt, true);
...
class html_writer {
     ...
     * @param array $attributes - html select element attributes
     * @param string $field_name Additional text to attach to the label (to improve accessibility)
     * @return HTML fragment
     */
    public static function select_time($type, $name, $currenttime = 0, $step = 5, array $attributes = null, string $field_name = '') {
        global $OUTPUT;
        ...
        $attributes = (array) $attributes;
        $label = trim($field_name) === '' ? get_string(substr($type, 0, -1), 'form') : $field_name . ':' . get_string(substr($type, 0, -1), 'form');
        $data = (object) [
            'name' => $name,
...
class html_table implements renderable, templatable {
    ...
    /**
     * @var string Class to add to particular rows
     *
     */
    public $rowclass;
    ...
    /** @var array processed menu items. */
...
class block_contents {
    ...
    /** @var string Accessible title of block. Plain text, not HTML. Optional. */
    public $title_aria_label = null;
    ...
    /**
     * If true, the block header will not be rendered and if show_header setting not initialize, the default
     * value is true.
     *
     * @var bool
     */
    ...
    /**
     * If true the block header will be collapsed. Totara specific property.
     * @var bool
     */
    public $header_collapsible;
...
class action_menu_link extends action_link implements renderable {
    ...
    /**
     * Get whether the item is active (selected).
     *
     * @return void
     */
    ...
    /**
     * Set whether the item is active (selected).
     *
     * @param bool $active
     * @return void
     */
