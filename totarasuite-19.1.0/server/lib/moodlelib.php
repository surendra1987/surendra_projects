...
use core_mfa\escalation\login as login_escalation;
use core_mfa\framework as mfa_framework;
...
function validate_param($param, $type, $allownull=NULL_NOT_ALLOWED, $debuginfo='') {
    ...
    if ($type == PARAM_FLOAT) {
        ...
        if (is_float($param) or is_int($param)) {
            // These always fit.
        } elseif (!is_numeric($param) or !preg_match('/^[\+-]?[0-9]*\.?[0-9]*(e[-+]?[0-9]+)?$/i', (string)$param)) {
            throw new invalid_parameter_exception($debuginfo);
        }
    } elseif ((string)$param !== (string)$cleaned) {
    ...
    switch ($type) {
        ...
        case PARAM_BOOL:
            ...
            if ($tempstr === 'on' or $tempstr === 'yes' or $tempstr === 'true') {
                $param = 1;
            } elseif ($tempstr === 'off' or $tempstr === 'no'  or $tempstr === 'false') {
                $param = 0;
            } else {
        ...
        case PARAM_TEXT:
            ...
            // Totara: $param is expected to be a string (not null)
            $param = fix_utf8($param) ?? '';
            ...
            do {
                ...
                } elseif (strpos($param, '</span>') !== false) {
        ...
        case PARAM_AUTH:
            ...
            if (empty($param)) {
                return '';
            } elseif (exists_auth_plugin($param)) {
                return $param;
            } else {
                return '';
            }
        ...
        case PARAM_THEME:
            ...
            if (empty($param)) {
                return '';
            } elseif (file_exists("$CFG->dirroot/theme/$param/config.php")) {
                return $param;
            } elseif (!empty($CFG->themedir) and file_exists("$CFG->themedir/$param/config.php")) {
                return $param;
            } else {
...
function html_is_blank($string) {
    // Totara: null is automatically empty, skip the extra checks
    if ($string === null) {
        return true;
    }
...
function get_config($plugin, $name = null) {
    ...
    } elseif (isset($CFG->forced_plugin_settings[$plugin])) {
...
/**
 * Set a volatile flag for a batch of names
 *
 * @param string $type the "type" namespace for the key
 * @param array $names the keys to set
 * @param string $value the value to set (without magic quotes)
 * @param null $expiry (optional) epoch indicating expiry - defaults to now()+ 24hs
 * @return bool Always returns true
 */
function set_cache_flag_bulk(string $type, array $names, string $value, $expiry = null): bool {
    global $DB;
    ...
    foreach ($names as $name) {
        if ($f = $DB->get_record('cache_flags', array('name' => $name, 'flagtype' => $type), '*', IGNORE_MULTIPLE)) {
            if ($f->value == $value && $f->expiry == $expiry && $f->timemodified == $timemodified) {
    ...
    if (!empty($names_to_update)) {
        [$in_sql, $params] = $DB->sql_in($names_to_update);
        $sql = "
            UPDATE {cache_flags} 
                SET value = :value, expiry = :expiry, timemodified = :timemodified
    ...
    if (!empty($records_to_insert)) {
        $DB->insert_records('cache_flags', $records_to_insert);
    }
...
function check_user_preferences_loaded(stdClass $user, $cachelifetime = 120) {
    ...
    if (isset($loadedusers[$user->id]) and isset($user->preference) and isset($user->preference['_lastloaded'])) {
        ...
        } elseif (!get_cache_flag('userpreferenceschanged', $user->id, $user->preference['_lastloaded'])) {
...
function get_user_preferences($name = null, $default = null, $user = null) {
    ...
    if (is_null($name)) {
        // All prefs.
    } elseif (is_numeric($name) or $name === '_lastloaded') {
    ...
    } elseif (isset($user->preference[$name])) {
...
function date_format_string($date, $format, $tz = 99) {
    ...
    // Totara: Call the strftime replacement since the default is deprecated
    $datestring = \totara_core\strftime::format($format, $date);
 ...
 *
 * @param bool $nossoredirect If true, login URL will include nosso=1 to prevent additional
                              redirects. This may be necessary in some SSO login flows to prevent
                              infinite redirects when $CFG->loginpageredirect is enabled.
 * @return string login url
 */
function get_login_url($nossoredirect = false) {
    global $CFG;

    $url = "$CFG->wwwroot/login/index.php";

    if ($nossoredirect && !empty(get_config(null, 'loginpageredirect'))) {
        $url .= '?nosso=1';
    }
 ...
 * If they are not logged in and have a pending MFA verification, then it redirects
 * to the MFA verification screen.
 * If $autologinguest is set and {@link $CFG}->autologinguests is set to 1
 * they will be automatically logged in as guests.
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if (!isloggedin()) {
        if ($autologinguest and !empty($CFG->guestloginbutton) and !empty($CFG->autologinguests)) {
            ...
            // If this is a real course (not SITE course) we should check to
            // see that the guest user is allowed to access the course. If not, they'll
            // need to log in anyway so redirecting to the login screen makes more sense.
            if ($course->id != SITEID) {
                if (!course_has_guest_enrolment_plugin_enabled($course->id)) {
                    // Implement a hidden setting to enable sites to always redirect to login when a course
                    // does not have guest enrolment enabled. Normally we would just auto login as guest and
                    // then show the enrolment options page, which might provide other ways for a guest to
                    // access the course.
                    if (!empty($CFG->autologinnoguestaccess) && $CFG->autologinnoguestaccess) {
                       redirect(get_login_url());
                       exit; // Never reached.
                    }
                }
    ...
    if (get_user_preferences('auth_forcepasswordchange') && !\core\session\manager::is_loggedinas()) {
        ...
        } elseif ($userauth->can_change_password()) {
    ...
    // Create the post login hook for execution.
    $after_hook = new \core\hook\after_require_login($courseorid, $autologinguest, $cm, $setwantsurltome, $preventredirect);
    ...
    if (empty($CFG->enablesitepolicies) && !$USER->policyagreed and !is_siteadmin()) {
        ...
        } elseif (!empty($CFG->sitepolicyguest) and isguestuser()) {
            if ($preventredirect) {
    ...
    if ($course->id == SITEID) {
        ...
        } elseif (is_viewing($coursecontext, $USER)) {
...
/**
 * Determine if course uses guest enrolment plugin or not.
 *
 * @param integer $courseid ID of course to check.
 * @return bool True if course has guest enrolment enabled.
 */
function course_has_guest_enrolment_plugin_enabled($courseid) {
    global $CFG, $DB;
    if (!in_array('guest', explode(',', $CFG->enrol_plugins_enabled))) {
        // Guest enrolment plugin not enabled at site level.
        return false;
    }
    $params = array('enrol' => 'guest', 'courseid' => $courseid, 'status' => ENROL_INSTANCE_ENABLED);
    return $DB->record_exists('enrol', $params);
}
...
function require_course_login($courseorid, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    } elseif ($issite && !empty($cm) and !$cm->uservisible) {
 ...
 * certain ajax scenarios when the user cannot / should not be
...
function fullname($user, $override=false) {
    ...
    if ($CFG->debugdeveloper && !$override) {
...
function update_user_record_by_id($id, $sync=false) {
    ...
    if ($newinfo = $userauth->get_userinfo($oldinfo->username)) {
        ...
        if ($newuser) {
            ...
            // Trigger event user suspended.
            if (isset($newuser['suspended']) && (int)$newuser['suspended'] == 1) {
...
function delete_user(stdClass $user) {
    ...
    if ($CFG->authdeleteusers === 'fullproper') {
        ...
        } while ($DB->record_exists('user', array('username' => $updateuser->username)));
        $updateuser->email = '';
        $updateuser->idnumber = '';
        $updateuser->password = AUTH_PASSWORD_NOT_CACHED;

    } elseif ($CFG->authdeleteusers === 'partial') {
...
function authenticate_user_login($username, $password, $ignorelockout=false, &$failurereason=null) {
    ...
    } elseif (!empty($CFG->authloginviaemail)) {
 ...
 *
 * @param stdClass $user
 * @param complete_login_callback|null $complete_login_callback
 * @param bool $persistent_login
 * @return stdClass A {@link $USER} object - BC only, do not use
 */
function complete_user_login($user, ?complete_login_callback $complete_login_callback = null, bool $persistent_login = false) {
    global $CFG, $USER, $SESSION;
    require_once("$CFG->libdir/authlib.php");

    // Halt process if MFA is required.
    $mfa_framework = new mfa_framework($user->id);
    if (!$persistent_login
        && !CLI_SCRIPT
        && $mfa_framework->is_mfa_required()
    ) {
        (new login_escalation($user->id))->trigger(['callback' => $complete_login_callback]);

        // We should redirect to the mfa verify page.
        redirect($CFG->wwwroot .'/mfa/verify.php');
    }

    login_attempt_valid($user);

    // When called after mfa verification, load the post_login_callback.
    ...
    // 1. user is THE guest.
    // 2. We can redirect to password change URL only in browser.
    // 3. Totara: do not interfere with device registration.
    if (isguestuser() || CLI_SCRIPT || !empty($SESSION->totara_mobile_device_registration)) {
        // Execute complete login callback
        if ($complete_login_callback) {
            $complete_login_callback->execute();
        }
        return $USER;
    }
...
/**
 * Callback execute after complete login is called on login page
 * @param $remember_username_checked
 * @param $site_fullname
 * @param $loginsite
 * @return void
 */
function login_page_complete_login_callback($remember_username_checked, $site_fullname, $loginsite) {
    ...
    if (!empty($CFG->persistentloginenable)) {
        if (!empty($remember_username_checked)) {
    ...
    } elseif (!empty($CFG->nolastloggedin)) {
        // do not store last logged in user in cookie
        // auth plugins can temporarily override this from loginpage_hook()
        // do not save $CFG->nolastloggedin in database!
    } elseif (empty($CFG->rememberusername) or ($CFG->rememberusername == 2 and empty($remember_username_checked))) {
    ...
    if (!isguestuser() and !empty($userauth->config->expiration) and $userauth->config->expiration == 1) {
        ...
        $PAGE->set_title("$site_fullname: $loginsite");
        $PAGE->set_heading("$site_fullname");
...
/**
 * Callback execute after complete login is called in confirm page
 * @param $redirect
 * @return void
 */
function confirm_page_complete_login_callback($redirect) {
...
/**
 * Callback execute after complete login is called in core_login_process_password_set
 * @returns void
*/
function password_set_complete_login_callback() {
    global $USER, $SESSION;
...
/**
 * Callback execute after complete login is called in admin setup
 * @param int $adminuser_id
 * @return void
 */
function admin_setup_complete_login_callback(int $adminuser_id) {
    global $CFG;

    redirect("$CFG->wwwroot/user/editadvanced.php?id=$adminuser_id"); // Edit thyself
}
 ...
 * @param int $mnethostidignored ignored since Totara 14.0
 * @return mixed False, or A {@link $USER} object.
 */
function get_complete_user_data($field, $value, $mnethostidignored = null) {
    global $CFG, $DB;
 ...
 * @param string $errmsg the HTML error message to display when the password doesn't comply with the policy.
...
/**
 * For a given password, returns a list of policy violations, if any.
 *
 * @param string|null $password
 * @return array
 * @throws coding_exception
 */
function get_password_policy_errors(?string $password): array {
    global $CFG;
    $err_msg = [];

    if (is_null($password) || $password === '') {
        $err_msg[] = get_string('errorpasswordblank', 'auth');
    }
...
function reset_course_userdata($data) {
    ...
    } elseif ($data->timeshift > 0 && $data->reset_end_date_old) {
    ...
    } elseif (!empty($data->reset_gradebook_grades)) {
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
    ...
    if (is_string($from)) {
        ...
        $fromstring = \core_text::entities_to_utf8(fullname($from));
    }
    ...
    $temprecipients[] = array($user->email, \core_text::entities_to_utf8(fullname($user, $override_fullname_display)));
    ...
    $context = array(
        ...
        'subject' => $subject,
        'to' => $user->email,
        'toname' => fullname($user, $override_fullname_display),
        'from' => $mail->From,
    ...
    if (!empty($user->mailformat) && $user->mailformat == 1) {
        ...
        if (!$messagehtml) {
            // If no html has been given auto convert the text to html and trim it.
            $messagehtml = trim(text_to_html($messagetext));
        }

        $context['body'] = $messagehtml;

        // Always pass HTML through the email template.
        $messagehtml = $renderer->render_from_template('core/email_html', $context);
    }

    // Note: It is vital that we set body here again for plain text emails. Both HTML and Plain text use the same context object :(
    ...
    // Merge single attachment and multiple attachments if both are provided (shouldn't happen, but just in case)
    if ($attachment && $attachname) {
        if (!in_array($attachname, array_keys($attachment_list))) {
            $attachment_list[$attachname] = $attachment;
        }
    }

    $ical_attachments = [];
    foreach ($attachment_list as $attachment_name => $the_attachment) {
        if (preg_match("~\\.\\.~", $the_attachment)) {
            // Security check for ".." in dir path.
            $mail->addStringAttachment('Error in attachment.  User attempted to attach a filename with a unsafe name.', 'error.txt', '8bit', 'text/plain');
        } else {
            require_once($CFG->libdir.'/filelib.php');
            $mimetype = mimeinfo('type', $attachment_name);

            $attachmentpath = $the_attachment;
            ...
            if (!file_exists($attachmentpath)) {
            ...
            } elseif (substr($attachment_name, -4) === ".ics") {
                $ical_attachments[] = [
                ...
                $mail->addAttachment($attachmentpath, $attachment_name, 'base64', $mimetype);
            }
        }
    }

    // If there is only 1 ical attachment, attach it as an inline attachment. Else attach all as file attachments
    if (count($ical_attachments) === 1) {
        $mail->Ical = file_get_contents($ical_attachments[0]['path']);
        // This encoding will allow foreign characters in an embedded calendar booking to remain unchanged.
        // (Whereas '64bit' could make an email get detected as spam, and 'quoted-printable' will escape foreign characters.)
        $mail->Encoding = '8bit';
    } else {
        foreach ($ical_attachments as $detail) {
            $mail->addAttachment($detail['path'], $detail['name'], 'base64', $detail['mimetype']);
        }
    }
...
/**
 * Send an email to a specified user with multiple attachments
 *
 * @param stdClass $user  A {@link $USER} object
 * @param stdClass $from A {@link $USER} object
 * @param string $subject plain text subject line of the email
 ...
 * @return bool Returns true if mail was sent OK and false if there was an error.
 */
function email_to_user_multiple_attachments($user, $from, $subject, $messagetext, $messagehtml, $attachment_list,
    $usetrueaddress = true, $replyto = '', $replytoname = '', $wordwrapwidth = 79
) {
    return email_to_user($user, $from, $subject, $messagetext, $messagehtml, '', '',
        $usetrueaddress, $replyto, $replytoname, $wordwrapwidth, $attachment_list);
}
...
function email_is_not_allowed($email) {
    ...
    if (!empty($CFG->allowemailaddresses)) {
        ...
        foreach ($allowed as $allowedpattern) {
            ...
            } elseif (strpos(strrev($email), strrev('@'.$allowedpattern)) === 0) {
                return false;
            }
        }
        ...
        foreach ($denied as $deniedpattern) {
            ...
            } elseif (strpos(strrev($email), strrev('@'.$deniedpattern)) === 0) {
                return get_string('emailnotallowed', '', $CFG->denyemailaddresses);
            }
        }
    }
...
function get_max_upload_sizes($sitebytes = 0, $coursebytes = 0, $modulebytes = 0, $custombytes = null) {
    ...
    } elseif ($coursebytes && ($coursebytes < $sitebytes || $sitebytes == 0)) {
...
function current_language() {
    ...
    } elseif (!empty($COURSE->id) and $COURSE->id != SITEID and !empty($COURSE->lang)) {
...
function get_string($identifier, $component = '', $a = null, $lazyload = false) {
    ...
    // Totara: Don't pass a null into a string function
...
function moodle_setlocale($locale='') {
    ...
    } elseif (!empty($CFG->locale)) { // Override locale for all language packs.
    ...
    if ($result === false) {
        ...
        } elseif (stripos($currentlocale, '.UTF8') !== false) {
    ...
    foreach ($lines as $linematchings) {
        ...
        if (!empty($linematchings[1])) {
            ...
            if (!preg_match('/^<(\s*.+?\/\s*|\s*(img|br|input|hr|area|base|basefont|col|frame|isindex|link|meta|param)(\s.+?)?)>$/is', $linematchings[1])) {
                ...
                } elseif (preg_match('/^<!--\[if\s.*?\]>$/s', $linematchings[1], $tagmatchings)) {
                    $tagdetails[] = (object) array(
                            'open' => true,
                            'tag'  => core_text::strtolower('if'),
                            'pos'  => core_text::strlen($truncate),
                    );
                } elseif (preg_match('/^<!--<!\[endif\]-->$/s', $linematchings[1], $tagmatchings)) {
                    $tagdetails[] = (object) array(
                            'open' => false,
                            'tag'  => core_text::strtolower('if'),
                            'pos'  => core_text::strlen($truncate),
        ...
        if ($totallength + $contentlength > $ideal) {
            ...
            if ($breakpos == 0) {
                ...
                $breakpos = $left + $entitieslength;
            } elseif ($breakpos > $left + $entitieslength) {
...
function get_performance_info() {
    ...
    $info['html'] .= '<ul class="list-unstyled ml-3 row">';
    ...
    $info['html'] .= '</ul><ul class="list-unstyled ml-3 row">';
    ...
    if ($data->session !== null) {
        $info['sessionsize'] = $data->session->size;
        $info['html'] .= "<li class=\"serverload col-sm-4\">" . $data->session->html . "</li>";
        $info['txt'] .= $data->session->txt;
    }

    // Display session wait time if it's known
    if (!empty($PERF->sessionlock['wait'])) {
        $sessionwait = number_format($PERF->sessionlock['wait'], 3) . ' secs';
        $info['html'] .= "<li class=\"sessionwait col-sm-4\">Session wait: " . $sessionwait . "</li>";
    ...
    if (!empty($data->cache)) {
        $stats = $data->cache;
        $html = '<ul class="cachesused list-unstyled ml-3 row">';
        $html .= '<li class="cache-stats-heading font-weight-bold">Caches used (hits/misses/sets)</li>';
        $html .= '</ul><ul class="cachesused list-unstyled ml-3">';
        ...
        foreach ($stats as $definition => $details) {
            ...
            $html .= '<ul class="cache-definition-stats list-unstyled ml-3 cache-mode-'.$modeclass.' card d-inline-block">';
            $html .= '<li class="cache-definition-stats-heading pt-3 card-header bg-inverse font-weight-bold">' .
                $definition . $mode.'</li>';
            $text .= "$definition {";
...
function get_home_page() {
    ...
    $gridcatalog = ($defaulthomepage == HOMEPAGE_TOTARA_GRID_CATALOG && totara_catalog_admin_setting_catalogtype::is_totara());
    ...
    if (!empty($USER->tenantid) and $CFG->tenantsisolated) {
        ...
        if ($pref == -1) {
            ...
            if ($pref == HOMEPAGE_TOTARA_GRID_CATALOG && totara_catalog_admin_setting_catalogtype::is_totara()) {
                return HOMEPAGE_TOTARA_GRID_CATALOG;
            }
        }
        return HOMEPAGE_TOTARA_DASHBOARD;
    ...
    if ($pref == -1) {
        ...
        if ($pref == HOMEPAGE_SITE) {
            return HOMEPAGE_SITE;
        } elseif ($pref == HOMEPAGE_TOTARA_GRID_CATALOG && totara_catalog_admin_setting_catalogtype::is_totara()) {
            return HOMEPAGE_TOTARA_GRID_CATALOG;
        }
    }
...
/**
 * Moderately safe method for unserializing given input that is expected to contain only a serialized instance of an stdClass object
 *
 ...
 *
 * PLEASE DO NOT SERIALIZE DATA FOR STORAGE IN TOTARA.
 * This function is retained for Moodle compatibility purposes only.
 *
 * @param string $input
 * @return stdClass
 */
...
class lang_string {
    ...
    public function __construct($identifier, $component = '', $a = null, $lang = null) {
        ...
        if (!empty($a)) {
            if (is_scalar($a)) {
                $this->a = $a;
            } elseif ($a instanceof lang_string) {
                $this->a = $a->out();
            } elseif (is_object($a) or is_array($a)) {
                $a = (array)$a;
                $this->a = array();
