 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_session_manager_test extends \core_phpunit\testcase {
    public function test_start() {
        // Session must be started only once...

        // Session is not active.
        self::assertFalse(\core\session\manager::is_session_active());

        $reflection = new \ReflectionProperty('\core\session\manager', 'sessionactive');
    ...
    public function test_init_empty_session() {
        ...
        $this->assertEqualsCanonicalizing(array('id' => 0), (array)$USER);
    ...
    public function test_session_exists() {
        ...
        if (defined('TEST_SESSION_HANDLER')) {
            // We don't mark as skipped.
            // Testing is limited when using a custom session handler for testing because the actual session
            // existence is being mocked in these tests, and the API for session handlers doesn't allow for
            // mocking presently.
            return;
        }
    ...
    public function sessionlock_history(): array {
        return [
            ...
            [
                'url'      => '/good.php?id=5',
                'start'    => 1500000000.000,
                'gained'   => 1500000000.100,
                'released' => 1500000001.500,
                'wait'     => 0.1
            ],
        ];
     ...
     * @param string|null $url Session lock page url.
     * @param float $time Session lock time.
     */
    ...
    public function test_cleanup_recent_session_locks(): void {
        ...
        $SESSION->recentsessionlocks = $this->sessionlock_history();
        $this->assertCount(7, $SESSION->recentsessionlocks);
    ...
    /**
     * Test display blocking page
     */
    public function test_display_blocking_page(): void {
        ...
        $output = \core\session\manager::display_blocking_page();
        $this->assertNotEmpty($output);
        $this->assertStringContainsString($pages[0]['url'], $output);
    }
}
