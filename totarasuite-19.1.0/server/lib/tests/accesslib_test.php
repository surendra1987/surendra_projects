 ...
 */
class core_accesslib_test extends \core_phpunit\testcase {
    /**
    ...
    public function test_get_all_roles() {
        ...
        $this->assertCount(17, $allroles); // there are 17 roles in standard totara install
    ...
    public function test_get_role_archetypes() {
        ...
        $this->assertCount(19, $archetypes); // there are 15+1+2+1 archetypes in standard totara install
    ...
    public function test_allow_assign() {
        ...
        $expectedlegacylog = array(SITEID, 'role', 'edit allow assign', str_replace($CFG->wwwroot . '/', '', $baseurl));
        $this->assertEventLegacyLogData($expectedlegacylog, $event);
        $this->assertSame("The user with id '0' modified the role with id '{$otherid}' to allow assigning role '{$student->id}' to users", $event->get_description());

        $sink = $this->redirectEvents();
        allow_assign($otherid, $student->id, false);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $sink->close();
        $this->assertFalse($DB->record_exists('role_allow_assign', array('roleid' => $otherid, 'allowassign' => $student->id)));
        ...
        $expectedlegacylog = array(SITEID, 'role', 'edit allow assign', str_replace($CFG->wwwroot . '/', '', $baseurl));
        $this->assertEventLegacyLogData($expectedlegacylog, $event);
        $this->assertSame("The user with id '0' modified the role with id '{$otherid}' to disallow assigning role '{$student->id}' to users", $event->get_description());
    }
    ...
    public function test_allow_override() {
        ...
        $expectedlegacylog = array(SITEID, 'role', 'edit allow override', str_replace($CFG->wwwroot . '/', '', $baseurl));
        $this->assertEventLegacyLogData($expectedlegacylog, $event);
        $this->assertSame("The user with id '0' modified the role with id '{$otherid}' to allow overriding role '{$student->id}'", $event->get_description());

        $sink = $this->redirectEvents();
        allow_override($otherid, $student->id, false);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $sink->close();
        $this->assertFalse($DB->record_exists('role_allow_override', array('roleid' => $otherid, 'allowoverride' => $student->id)));
        ...
        $expectedlegacylog = array(SITEID, 'role', 'edit allow override', str_replace($CFG->wwwroot . '/', '', $baseurl));
        $this->assertEventLegacyLogData($expectedlegacylog, $event);
        $this->assertSame("The user with id '0' modified the role with id '{$otherid}' to disallow overriding role '{$student->id}'", $event->get_description());
    }
    ...
    public function test_allow_switch() {
        ...
        $expectedlegacylog = array(SITEID, 'role', 'edit allow switch', str_replace($CFG->wwwroot . '/', '', $baseurl));
        $this->assertEventLegacyLogData($expectedlegacylog, $event);
        $this->assertSame("The user with id '0' modified the role with id '{$otherid}' to allow users to switch to role '{$student->id}'", $event->get_description());
        ...
        $expectedlegacylog = array(SITEID, 'role', 'edit allow switch', str_replace($CFG->wwwroot . '/', '', $baseurl));
        $this->assertEventLegacyLogData($expectedlegacylog, $event);
        $this->assertSame("The user with id '0' modified the role with id '{$otherid}' to disallow users to switch to role '{$student->id}'", $event->get_description());
    }
    ...
    public function test_mark_user_dirty_bulk_with_invalid_user_id(): void {
        $this->expectException(coding_exception::class);
        $this->expectExceptionMessage('Invalid user parameter supplied for mark_user_dirty_bulk() function.');

        mark_user_dirty_bulk([1, 2, 3, null, 4, 5]);
    }

    public function test_mark_user_dirty_bulk_sets_cache_flags(): void {
        global $CFG;
        $original_cfg_value = $CFG->sessiontimeout;
        $CFG->sessiontimeout = 100;
        $now = time();

        builder::table('cache_flags')->delete();

        mark_user_dirty_bulk([111, 222, 333]);
        ...
        foreach ($records as $record) {
            $this->assertEquals('accesslib/dirtyusers', $record->flagtype);
            $this->assertEquals(1, $record->value);
            $this->assertGreaterThanOrEqual($now, $record->timemodified);
            $this->assertGreaterThanOrEqual($now + 100, $record->expiry);
        }
        ...
        $CFG->sessiontimeout = $original_cfg_value;
    }
    public function test_mark_user_dirty_bulk_sets_accesslib_private(): void {
        global $ACCESSLIB_PRIVATE;

        $ACCESSLIB_PRIVATE->accessdatabyuser[111] = true;
        $ACCESSLIB_PRIVATE->accessdatabyuser[222] = true;
        $ACCESSLIB_PRIVATE->accessdatabyuser[333] = true;
        $ACCESSLIB_PRIVATE->accessdatabyuser[444] = true;

        $ACCESSLIB_PRIVATE->dirtyusers[111] = 0;
        $ACCESSLIB_PRIVATE->dirtyusers[222] = 0;
        $ACCESSLIB_PRIVATE->dirtyusers[333] = 0;
        $ACCESSLIB_PRIVATE->dirtyusers[444] = 0;

        mark_user_dirty_bulk([111, 222, 333]);

        $this->assertArrayNotHasKey(111, $ACCESSLIB_PRIVATE->accessdatabyuser);
        $this->assertArrayNotHasKey(222, $ACCESSLIB_PRIVATE->accessdatabyuser);
        $this->assertArrayNotHasKey(333, $ACCESSLIB_PRIVATE->accessdatabyuser);
        $this->assertArrayHasKey(444, $ACCESSLIB_PRIVATE->accessdatabyuser);

        $this->assertEquals(1, $ACCESSLIB_PRIVATE->dirtyusers[111]);
        $this->assertEquals(1, $ACCESSLIB_PRIVATE->dirtyusers[222]);
        $this->assertEquals(1, $ACCESSLIB_PRIVATE->dirtyusers[333]);
        $this->assertEquals(0, $ACCESSLIB_PRIVATE->dirtyusers[444]);
    }
}
