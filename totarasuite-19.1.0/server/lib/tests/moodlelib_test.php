...
class core_moodlelib_test extends \core_phpunit\testcase {
    ...
    public function test_get_string_limitation() {
        ...
        try {
            ...
            $this->fail('Expected warning or error');
        } catch (Throwable $ex) {
            $this->assertTrue(true);
        }
    }
    ...
    public function test_format_float() {
        ...
        // Custom number of decimal places.
        $this->assertEquals('50', format_float(50.43000, 0, false, true));
        $this->assertEquals('50', format_float(50.000, 0, false, true));
        $this->assertEquals('50.4', format_float(50.43000, 1, false, true));
        $this->assertEquals('50.43', format_float(50.43000, 5, false, true));
    ...
    public function test_complete_user_login() {
        ...
        $this->assert_complete_user_login($user, $events);
    }

    /**
     * Asserts the login event is triggered and the user is set up correctly.
     *
     * @param $user
     * @param $events
     * @return void
     */
    private function assert_complete_user_login($user, $events): void {
        global $USER, $DB;
    ...
    /**
     * Test callback passed to complete_user_login is executed
     *
     * @return void
     */
    public function test_callback_is_executed_in_complete_user_login(): void {
        global $CFG;
        ...
        try {
            ...
            // Assert the redirect in callback was executed.
            $this->assertEquals('redirecterrordetected', $exception->errorcode);
            $this->assertEquals($CFG->wwwroot . "?no=" . $user->id, $exception->link);
    ...
    public function test_unserialize_object() {
        // Copy all the unserialize_array() tests
        $a = [1, 2, 3];
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        $a = ['a' => 1, 2 => 2, 'b' => 'cde'];
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        ...
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        $a = (object)['a' => 1, 2 => 2, 'b' => 'cde'];
        $this->assertEquals($a, unserialize_object(serialize($a)));
        $a = ['a' => 1, 2 => 2, 'b' => (object)['a' => 'cde']];
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        $a = array('aggregatesonly' => [51, 34], 'gradesonly' => [21, 45, 78]);
        $this->assertEquals((object) $a, unserialize_object(serialize($a)));
        ...
        // Some new tests
        $input = 'O:14:"core\lock\lock":2:{s:3:"key";O:22:"core_availability\tree":1:{s:8:"children";O:23:"core\dml\recordset_walk":2:{s:8:"callback";s:8:"readfile";s:9:"recordset";O:13:"ArrayIterator":3:{i:0;i:0;i:1;a:1:{i:0;s:13:"../config.php";}i:2;a:0:{}}}}s:8:"infinite";i:1;}';
        $output = unserialize_object($input);
        $this->assertInstanceOf('stdClass', $output);
        $this->assertInstanceOf('__PHP_Incomplete_Class', $output->key);

        $a = new stdClass();
        $a->array = ['apple', 'banana'];
        $a->object = new stdClass();
        $a->object->menu = 'pizza';
        $this->assertEquals($a, unserialize_object(serialize($a)));
    }

    /**
     * Test function that determines if a course has guest enrolment enabled.
     * @return void
     */
    public function test_course_has_guest_enrolment_plugin_enabled() {
        set_config('guestloginbutton', 1);
        set_config('autologinguests', 1);
        $course = $this->getDataGenerator()->create_course();

        // Should be false if guest enrolment is not enabled.
        set_config('enrol_plugins_enabled', 'manual');
        $this->assertFalse(course_has_guest_enrolment_plugin_enabled($course->id));

        set_config('enrol_plugins_enabled', 'manual,guest');
        // Still false if there is no guest enrolment instance.
        $this->assertFalse(course_has_guest_enrolment_plugin_enabled($course->id));

        // Still false if there is a guest enrolment instance, but it is not enabled.
        $enrolplugin = enrol_get_plugin('guest');
        $data = [
            'courseid' => $course->id,
            'status' => ENROL_INSTANCE_DISABLED,
            'contextid' => \context_course::instance($course->id)->id,
            'name' => 'Disabled guest enrolment instance'
        ];
        $enrolplugin->add_instance($course, $data);
        $this->assertFalse(course_has_guest_enrolment_plugin_enabled($course->id));

        // True if there is a guest enrolment instance, and it is enabled.
        $data = [
            'courseid' => $course->id,
            'status' => ENROL_INSTANCE_ENABLED,
            'contextid' => \context_course::instance($course->id)->id,
            'name' => 'Enabled guest enrolment instance'
        ];
        $enrolplugin->add_instance($course, $data);
        $this->assertTrue(course_has_guest_enrolment_plugin_enabled($course->id));

        set_config('enrol_plugins_enabled', 'manual');
        // Check still false when plugins disabled at site level, even when enabled on a course.
        $this->assertFalse(course_has_guest_enrolment_plugin_enabled($course->id));
    }
    ...
    /**
     * Test that {@link get_callable_name()} describes the callable as expected.
     * This test expands test_get_callable_name and includes a closure, as closures cannot be passed
     * via
     *
     * @return void
     */
    public function test_get_callable_name_closure(): void {
        $closure = function($a) { return $a; };
        $this->assertSame('Closure::__invoke', get_callable_name($closure));
    }
    ...
    public static function callable_names_provider() {
        return [
            ...
            'static_method_of_object' => [
                [static::class, 'my_foobar_method'],
                'core_moodlelib_test::my_foobar_method',
            ],
            'method_of_object' => [
    ...
    public static function password_policy_errors_provider(): array {
        // password, [expected errors]
        return [
            'blank' => ['', [
                'passwordblank',
                'minpasswordlength',
            ...
            'nonalpha and numbers' => ['########90123456', [
                'minpasswordlower',
                'minpasswordupper',
            ]],
            'no uppers' => ['abc#####90123456', [
                'minpasswordupper',
            ]],
            'perfect' => ['abC#####90123456', [
            ]],
            'consecutive identicals' => ['abC############6', [
    ...
    /**
     * @dataProvider password_policy_errors_provider
     */
    public function test_get_password_policy_errors($password, $expected_strings) {
        global $CFG;
        $CFG->maxconsecutiveidentchars = 9;
        $expected = [];
        foreach ($expected_strings as $error_string) {
            $a = '';
            if (isset($CFG->{$error_string})) {
                $a = $CFG->{$error_string};
            }
            $expected[] = get_string('error' . $error_string, 'auth', $a);
        }
        $actual = get_password_policy_errors($password);
     ...
     * @return array
     */
    public static function set_cache_flag_data_provider(): array {
        return [[true], [false]];
    }

    /**
     * @dataProvider set_cache_flag_data_provider
     * @param bool $use_bulk_version
     * @return void
     */
    public function test_set_cache_flag_insert(bool $use_bulk_version): void {
        builder::table('cache_flags')->delete();

        $now = time();
        $expiry = $now + DAYSECS;

        // Bulk version should work the same.
        if ($use_bulk_version) {
            set_cache_flag_bulk('test_type', ['test_name'], 'test_value', $expiry);
        } else {
            set_cache_flag('test_type', 'test_name', 'test_value', $expiry);
        }

        $records = builder::table('cache_flags')->get();
        $this->assertEquals(1, $records->count());

        $record = $records->first();
        $this->assertEquals('test_type', $record->flagtype);
        $this->assertEquals('test_name', $record->name);
        $this->assertEquals('test_value', $record->value);
        $this->assertGreaterThanOrEqual($now, $record->timemodified);
        $this->assertEquals($expiry, $record->expiry);
    }
     ...
     * @return void
     */
    public function test_set_cache_flag_update(bool $use_bulk_version): void {
        builder::table('cache_flags')->delete();
        ...
        if ($use_bulk_version) {
            set_cache_flag_bulk('test_type', ['test_name'], 'test_value_modified', $expiry + HOURSECS);
        } else {
            set_cache_flag('test_type', 'test_name', 'test_value_modified', $expiry + HOURSECS);
        }
        ...
        $this->assertEquals('test_type', $record->flagtype);
        $this->assertEquals('test_name', $record->name);
        $this->assertEquals('test_value_modified', $record->value);
        $this->assertGreaterThanOrEqual($now, $record->timemodified);
        $this->assertEquals($expiry + HOURSECS, $record->expiry);
    }

    /**
     * @return array
     */
    public static function set_cache_flag_default_expiry_data_provider(): array {
        $now = time();
    ...
    /**
     * @dataProvider set_cache_flag_default_expiry_data_provider
     * @param int|null $expiry_param
     * @param bool $use_bulk_version
     * @return void
     */
    public function test_set_cache_flag_default_expiry(?int $expiry_param, bool $use_bulk_version): void {
        builder::table('cache_flags')->delete();

        $now = time();

        // Check without expiry param.
        if ($use_bulk_version) {
            set_cache_flag_bulk('test_type', ['test_name'], 'test_value', $expiry_param);
        } else {
            set_cache_flag('test_type', 'test_name', 'test_value', $expiry_param);
        }
        ...
        $this->assertEquals('test_value', $record->value);
        $this->assertGreaterThanOrEqual($now, $record->timemodified);
        $this->assertGreaterThanOrEqual($now + 24 * 60 * 60, $record->expiry);
    }

    public function test_set_cache_flag_bulk_insert(): void {
        builder::table('cache_flags')->delete();

        $now = time();
        $expiry = $now + DAYSECS;

        set_cache_flag_bulk('test_type', ['test_name1', 'test_name2', 'test_name3'], 'test_value', $expiry);
        ...
        // Insert for same names but different type.
        set_cache_flag_bulk('other_test_type', ['test_name1', 'test_name2', 'test_name3'], 'test_value', $expiry);
        $records = builder::table('cache_flags')->get();
        $this->assertEquals(6, $records->count());

        $records = builder::table('cache_flags')->where('flagtype', 'other_test_type')->get();
        $this->assertEquals(3, $records->count());

        foreach ($records as $record) {
            $this->assertEquals('other_test_type', $record->flagtype);
            $this->assertEquals('test_value', $record->value);
            $this->assertGreaterThanOrEqual($now, $record->timemodified);
            $this->assertEquals($expiry, $record->expiry);
        }
    ...
    public function test_set_cache_flag_bulk_update(): void {
        builder::table('cache_flags')->delete();
        ...
        // Insert
        set_cache_flag_bulk('test_type', ['test_name1', 'test_name2', 'test_name3'], 'test_value', $expiry);
        $this->assertEquals(3, builder::table('cache_flags')->count());

        // Update
        set_cache_flag_bulk('test_type', ['test_name1', 'test_name2', 'test_name3'], 'new_test_value', $expiry + HOURSECS);
        $records = builder::table('cache_flags')->get();
        $this->assertEquals(3, $records->count());

        foreach ($records as $record) {
            $this->assertEquals('test_type', $record->flagtype);
            $this->assertEquals('new_test_value', $record->value);
            $this->assertGreaterThanOrEqual($now, $record->timemodified);
            $this->assertEquals($expiry + HOURSECS, $record->expiry);
        }
    ...
    public function test_set_cache_flag_bulk_insert_and_update_mixed(): void {
        builder::table('cache_flags')->delete();
        ...
        set_cache_flag_bulk(
            'test_type',
            ['test_name1', 'test_name2', 'test_name4', 'test_name5'],
            'new_test_value',
            $expiry + HOURSECS
        );
        $records = builder::table('cache_flags')->get();
        $this->assertEquals(5, $records->count());

        // Check the newly inserted and updated.
        $records = builder::table('cache_flags')->where('name', '!=', 'test_name3')->get();
        ...
        // Check the one that was not updated.
        $record = builder::table('cache_flags')->where('name', 'test_name3')->one(true);
        $this->assertEquals('test_type', $record->flagtype);
        $this->assertEquals('test_value', $record->value);
        $this->assertGreaterThanOrEqual($now, $record->timemodified);
        $this->assertEquals($expiry, $record->expiry);
