...
class core_filelib_test extends \core_phpunit\testcase {
    ...
    public function test_curl_blocked_redirect() {
        ...
        // Block a host.
        // Note: totaralearning.com is the URL redirected to when test_redir.php has the param extdest=1 set.
        set_config('curlsecurityblockedhosts', 'www.totaralearning.com');
...
EOF;
     ...
     */
    public static function data_make_content_disposition_valid_case() {
        $case = [];
     ...
     */
    public static function data_make_content_disposition_invalid_case() {
    ...
    /**
     * Mobile browser support for the filename*=utf-8'' is not very good and created issues, therefore we fall back to
     * the non-encoded filename in this case
     */
    public function test_special_mobile_condition() {
        // Simulate Chrome 100 on Windows
        $useragent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4758.102 Safari/537.36';
        ...
        $this->assertEquals($header, "Content-Disposition: attachment; filename*=utf-8''{$encodename}");

        // Simulate Chrome; iPhone 7 Plus; iOS 11.
        $useragent = 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_3 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) CriOS/61.0.3163.73 Mobile/14G60 Safari/602.1';
        core_useragent::instance(true, $useragent);

        $header = make_content_disposition('attachment', $filename);
        $this->assertEquals($header, "Content-Disposition: attachment; filename=\"{$filename}\"");
    }

    /**
     * Assert that HTML will appear/not appear in the download list depending
     * on the correct config options.
     *
     * @return void
     */
    public function test_totara_tweak_always_download_files(): void {
        global $CFG;

        // No HTML allowed (must return)
        $CFG->allow_inline_uploaded_html = 0;
        $expected = ['image/svg+xml', 'text/html'];
        $result = totara_tweak_always_download_files();
    ...
    /**
     * Data provider for the test_totara_tweak_content_security_policy test
     *
     * @return array[]
     */
    public static function totara_tweak_content_security_policy_data(): array {
        return [
            // Setting, Allow XSS, Mime Type, Expected CSP
            // Setting = 1, html never shows policy
            [true, null, 'text/html', false],
            [true, false, 'text/html', false],
            [true, true, 'text/html', false],
            // Setting = 0, html shows policy based on allowxss
            [false, null, 'text/html', true],
            [false, false, 'text/html', true],
            [false, true, 'text/html', false],
            // Image/png shows based on allowxss
            [false, null, 'image/png', true],
            [false, false, 'image/png', true],
            [false, true, 'image/png', false],
            [true, null, 'image/png', true],
            [true, false, 'image/png', true],
            [true, true, 'image/png', false],
        ];
    }

    /**
     * @param bool $setting
     * @param bool|null $allow_xss
     * @param string $mimetype
     * @param bool $expect_csp
     * @return void
     * @dataProvider totara_tweak_content_security_policy_data
     */
    public function test_totara_tweak_content_security_policy(bool $setting, ?bool $allow_xss, string $mimetype, bool $expect_csp): void {
        global $CFG;

        $options = $allow_xss !== null ? ['allowxss' => $allow_xss] : [];

        $CFG->allow_inline_uploaded_html = $setting;
        $result = totara_tweak_content_security_policy($options, $mimetype);
        $this->assertIsArray($result);

        if ($expect_csp) {
            $this->assertEquals(['Content-Security-Policy: script-src none;'], $result);
        } else {
