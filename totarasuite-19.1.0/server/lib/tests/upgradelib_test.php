 ...
 */
class core_upgradelib_test extends \core_phpunit\testcase {
    ...
    /**
     * test totara_notification_migrate_notification_prefs_recipient
     * should encode any strings, and leave nulls as nulls
     *
     * @return void
     * @throws dml_exception
     */
    public function test_totara_notification_migrate_notification_prefs_recipient() {
        global $DB;
        $context = \totara_core\extended_context::make_system();
        // create a couple of recipient records = one with a recipient set - the other null (built-ins).
        $params = [
            'recipient' => totara_notification_mock_recipient::class,
            'context_id' => $context->get_context_id(),
            'time_created' => time(),
        ];
        $mocked_record_id = $DB->insert_record_raw(\totara_notification\entity\notification_preference::TABLE, $params);
        $mocked_recipient = $DB->get_field(\totara_notification\entity\notification_preference::TABLE, 'recipient', ['id' => $mocked_record_id]);
        self::assertSame(totara_notification_mock_recipient::class, $mocked_recipient);
        ...
        $nulled_record_id = $DB->insert_record_raw(\totara_notification\entity\notification_preference::TABLE, $params);
        $nulled_recipient = $DB->get_field(\totara_notification\entity\notification_preference::TABLE, 'recipient', ['id' => $nulled_record_id]);
        self::assertNull($nulled_recipient);

        // run the upgrade function
        totara_notification_migrate_notification_prefs_recipient();

        // ensure the nulls remain as nulls
        $nulled_recipient_check = $DB->get_field(\totara_notification\entity\notification_preference::TABLE, 'recipients', ['id' => $nulled_record_id]);
        self::assertNull($nulled_recipient_check);
        // ensure the singles come through as json_encoded_arrays
        $mocked_recipient_check = $DB->get_field(\totara_notification\entity\notification_preference::TABLE, 'recipients', ['id' => $mocked_record_id]);
        self::assertSame(json_encode([totara_notification_mock_recipient::class]), $mocked_recipient_check);
    }

    public function test_upgrade_oauth2_user_field_mapping_to_add_external_id(): void {
        global $CFG, $DB;
        ...
        $issuers = [
            ...
            'microsoft' => \core\oauth2\api::create_standard_issuer('microsoft'),
            'custom' => \core\oauth2\api::create_issuer((object) [
        ...
        // Delete auto created external_id mappings to simulate older DB
        $DB->delete_records('oauth2_user_field_mapping', ['internalfield' => 'external_id']);

        // Run upgrade step
        upgrade_oauth2_user_field_mapping_to_add_external_id();

        // Verify mappings have been created
        $external_id_mappings = collection::new(
            $DB->get_records('oauth2_user_field_mapping', ['internalfield' => 'external_id'])
        )->key_by('issuerid');
        foreach ($issuers as $key => $issuer) {
            $name = $issuer->get('name');
            $mapping = $external_id_mappings[$issuer->get('id')] ?? null;
            $this->assertNotNull($mapping, "external_id mapping must exist for issuer '$name'");
            $expected_field = ($key == 'microsoft' || $key == 'facebook') ? 'id' : 'sub';
            $this->assertEquals($expected_field, $mapping->externalfield);
        }
    }
}
