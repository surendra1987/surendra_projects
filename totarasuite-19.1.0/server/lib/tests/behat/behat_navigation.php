...
class behat_navigation extends behat_base {
    ...
    /**
     * Opens the course enrol page.
     *
     * @Given /^I am on "(?P<coursefullname_string>(?:[^"]|\\")*)" course enrol page$/
     * @throws coding_exception
     * @param string $coursefullname The full name of the course.
     * @return void
     */
    public function i_am_on_course_enrol_page($coursefullname) {
        \behat_hooks::set_step_readonly(false);
        global $DB;
        $course = $DB->get_record("course", array("fullname" => $coursefullname), 'id', MUST_EXIST);
        $url = new moodle_url('/course/enrol.php', ['id' => $course->id]);
        $this->getSession()->visit($this->locate_path($url->out_as_local_url(false)));
        $this->wait_for_pending_js();
    }
