...
class behat_accessibility extends behat_base {
    ...
    protected function run_axe_for_tags(array $standardtags = [], array $extratags = []): void {
        if (!$this->running_javascript()) {
            throw new DriverException("Javascript is required for this step.");
        }
...
EOF;
        ...
        for ($i = 0; $i < self::get_extended_timeout() * 10; $i++) {
            $axe_result = $this->getSession()->getDriver()->evaluateScript($getresults);
            if (!empty($axe_result)) {
                $results = json_decode($axe_result);
                break;
            }
