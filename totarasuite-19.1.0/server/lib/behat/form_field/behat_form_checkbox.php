...
class behat_form_checkbox extends behat_form_field {
     ...
     * Click field.
     */
    protected function click_field() {
        try {
            $this->field->click();
        } catch (\Throwable $th1) {
            // probably complaining "element click intercepted", try click on the label
            $id = $this->field->getAttribute('id');
            try {
                $this->field->find('xpath', '../label[@for="' . $id . '"]')->click();

                // Trigger the onchange event as triggered when 'checking' the checkbox.
                $this->trigger_on_change();
            } catch (\Throwable $th2) {
                // didn't work, just throw the original exception again
                throw $th1;
            }
        }
    }
