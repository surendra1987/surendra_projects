...
class behat_form_field {
    ...
    public function set_value($value) {
        ...
        $retval = $instance->set_value($value);
        $this->blur();
        return $retval;
    }

    /**
     * Causes the javascript blur event (handling validation)
     */
    ...
    public function key_press($char, $modifier = null) {
        ...
        // dealing with a fgroup element.
        $instance = $this->guess_type();
        $modifier != null && $instance->field->keyDown($modifier);

        if ($char == '13') {
            $char = WebDriverKeys::RETURN_KEY;
        }

        try {
            $instance->field->keyPress($char, $modifier);
            $modifier != null && $instance->field->keyUp($modifier);
