...
class behat_base extends Behat\MinkExtension\Context\RawMinkContext {
     ...
     * be enough.
     *
     * @deprecated since Totara 14.0 TL-28745 / MDL-64979 - please use get_reduced_timeout() instead
     ...
     *
     * @deprecated since Totara 14.0 TL-28745 / MDL-64979 - please use get_timeout() instead
     ...
     *
     * @deprecated since Totara 14.0 TL-28745 / MDL-64979 - please use get_extended_timeout() instead
     ...
     * @return NodeElement[] NodeElements list
     */
    protected function find_all($selector, $locator, $exception = false, $node = false, $timeout = false) {
        ...
        if (!$exception) {
            ...
            $exception = new ElementNotFoundException($this->getSession()->getDriver(), $exceptiontype, null, $exceptionlocator);
        }
    ...
    protected function click_node($node) {
        try {
            ...
            if (preg_match('/^element click intercepted.*Other element would receive the click/', $message) || preg_match('/^element not interactable.*/', $message)) {
                $driver = $this->getSession()->getDriver();
    ...
    /**
     * Transforms all dynamic dates in given text.
     * Example: "Due on ## +3 days ## d M ##" will be converted to a string
     * like "Due on 25 Dec"
     *
     ...
     */
    protected function transform_dates_in_text(string $text): string {
        $matches = [];
        $expanded = $text;

        // Quick exit if there's nothing to transform
        ...
        if (preg_match_all('/##(.*)##(.*)##/', $expanded, $matches, PREG_SET_ORDER) == 1) {
            foreach ($matches as $match) {
                ...
                $expanded = $this->transform_date($expanded, $pattern, $offset, $format);
            }
        }
        ...
        if (preg_match_all('/##(.*)##/', $expanded, $matches, PREG_SET_ORDER) == 1) {
            foreach ($matches as $match) {
                ...
                $expanded = $this->transform_date($expanded, $pattern, $offset);
            }
        }
     ...
     */
    private function transform_date($text, $pattern, $offset, $format = DATE_ISO8601): string {
