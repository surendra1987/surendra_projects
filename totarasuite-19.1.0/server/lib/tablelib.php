...
class flexible_table {
    ...
    /**
     * @var array List of columns that should render as a th
     */
    ...
    /**
     * Message to display if no records found
     *
     * @var string
     */
    ...
    /**
     * Indicates the given $column index is header for this row.
     * @param string $column
     * @param bool $is_header
     * @return void
     */
    public function column_header(string $column, bool $is_header): void {
        $this->column_headers[$column] = $is_header;
    }
    ...
    function setup() {
        ...
        if ($this->persistent) {
            // Totara: Pass an empty string instead of null, the result is the same but there's no deprecation thrown
    ...
    function print_nothing_to_display() {
        ...
        echo html_writer::div($this->get_no_records_message(), 'table-no-entries');
    }
    ...
    public function get_row_html($row, $classname = '') {
        ...
        if ($row === NULL) {
            ...
            foreach ($row as $index => $data) {
                ...
                if (isset($this->column_headers[$column]) && $this->column_headers[$column]) {
                    $tag = 'th';
                    $attributes['scope'] = 'row';
                }

                $html .= html_writer::tag($tag, $content, $attributes);
            }
        }
    ...
    /**
     * Get the no_records_message property.
     *
     * @return string the value of property no_records_message.
     */
    function get_no_records_message() :string {
    ...
    /**
     * Set the no_records_message property.
     *
     * @param string $message value to set property to
     * @return void
     */
    function set_no_records_message($message) :void {
