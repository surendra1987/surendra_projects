...
class tagcollsearchable extends \core\output\inplace_editable {
    ...
    public function __construct($tagcoll) {
        if (!core_tag_collection::is_editable($tagcoll->id)) {
            // Totara: Preventing the editing on searchable box, for special tag collections.
            $editable = false;
        } else {
    ...
    public function export_for_template(\renderer_base $output) {
        if ($this->value) {
            $this->displayvalue = $output->flex_icon('tags-searchable', array('alt'=>get_string('yes')));
        } else {
            $this->displayvalue = $output->flex_icon('tags-unsearchable', array('alt'=>get_string('no')));
        }
