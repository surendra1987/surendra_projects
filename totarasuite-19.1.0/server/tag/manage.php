...
if ($tagcoll) {
    ...
    if (null !== $url) {
        // Totara: redirecting to a special management page, if the collection is a special collection
        redirect($url);
    }
}
...
if (!$tagcoll) {
    $add_tag_collection = new component('core_tag/components/add_collection/AddCollection');
    $add_tag_collection->register($PAGE);

    echo $OUTPUT->header();
    echo $OUTPUT->page_main_heading(get_string('managetags', 'core_tag'));
    ...
    echo $OUTPUT->heading(get_string('tagcollections', 'core_tag') . $OUTPUT->help_icon('tagcollection', 'tag'), 2);
    echo html_writer::table($colltable);
    $url = new moodle_url($manageurl, array('action' => 'colladd'));
    echo html_writer::div($OUTPUT->render($add_tag_collection),
            'mdl-right addtagcoll');

    echo $OUTPUT->heading(get_string('tagareas', 'core_tag'), 2);
...
$add_stardard_tags = new component('core_tag/components/add_standard_tags/AddStandardTags',
    ['id' => $tagcollid]
);
$add_stardard_tags->register($PAGE);
echo $OUTPUT->header();

// Tag collection is specified. Manage tags in this collection.
echo $OUTPUT->page_main_heading(core_tag_collection::display_name($tagcoll));

// Form to filter tags.
print('<form class="tag-filter-form" method="get" action="'.$CFG->wwwroot.'/tag/manage.php">');
print('<div class="tag-management-form generalbox form-inline"><label class="accesshide" for="id_tagfilter">'. get_string('search') .'</label>'.
    '<input type="hidden" name="tc" value="'.$tagcollid.'" />'.
    '<input type="hidden" name="perpage" value="'.$perpage.'" />'.
    '<input id="id_tagfilter" name="filter" type="text" value="' . s($filter) . '" class="form-control mr-2">'.
...
echo '<div class="addstandardtags visibleifjs">' .
    $OUTPUT->render($add_stardard_tags) .
    '</div>';
