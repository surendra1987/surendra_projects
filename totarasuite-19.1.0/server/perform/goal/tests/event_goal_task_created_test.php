...
/**
 * Unit tests for the goal_task_created event.
 */
class perform_goal_event_goal_task_created_test extends testcase {

    public function test_event_goal_task_created_when_goal_with_same_subject_user(): void  {
        settings_helper::enable_perform_goals();
        $owner = self::getDataGenerator()->create_user();
        $this->setUser($owner);
        $goal_config = goal_generator_config::new(['user_id' => $owner->id,
            'owner_id' => $owner->id]);
        $goal = goal_generator::instance()->create_goal($goal_config);

        $sink = $this->redirectEvents();
        // Operate
        $goal_task = goal_task_model::create($goal->id, "goal task 1");

        $events = $sink->get_events();
        $events = array_filter($events, function (base $event) {
            return $event instanceof goal_task_created;
        });
        ...
        $expected_description = "The user with id '{$owner->id}' has created a goal task with id '{$goal_task->id}'";
        $this->assertSame($expected_description, $event->get_description());
        $this->assertEventContextNotUsed($event);
        $this->assertSame(get_string('event_goal_task_created', 'perform_goal'), $event->get_name());
    }

    public function test_event_goal_task_created_when_goal_with_different_subject_user(): void  {
        global $DB;
        settings_helper::enable_perform_goals();
