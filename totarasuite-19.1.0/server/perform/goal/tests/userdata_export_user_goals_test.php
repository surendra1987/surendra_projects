...
use perform_goal\userdata\export_user_goals;
use totara_tenant\local\util as tenant_util;
use totara_userdata\userdata\target_user;

class perform_goal_userdata_export_user_goals_test extends testcase {
    public function test_export_user_goals() {
        $fs = get_file_storage();
        ...
        $goal_task_config = goal_task_generator_config::new([
            'goal_id' => $goal1->id,
            'description' => "task 1 of goal 1 for subject user 1, no resource"
        ]);
        $goal_task1 = goal_generator::instance()->create_goal_task($goal_task_config);
        self::assertEquals($goal1->id, $goal_task1->goal_id);
        $course = $this->getDataGenerator()->create_course();
        $goal_task_config = goal_task_generator_config::new([
            'goal_id' => $goal1->id,
            'description' => "task 2 of goal 1 for subject user 1 with resource",
            'resource_id' => $course->id,
            'resource_type' => course_type::TYPECODE
        ]);
        $goal_task2 = goal_generator::instance()->create_goal_task($goal_task_config);
        self::assertEquals($goal1->id, $goal_task2->goal_id);
        $goal_task_resource = goal_generator::instance()->create_goal_task_resource($goal_task2, $goal_task_config);
        self::assertEquals($goal_task2->id, $goal_task_resource->task_id);
        ...
        $goal_task_config = goal_task_generator_config::new([
            'goal_id' => $goal3->id,
            'description' => "task 1 of goal 3 for subject user 2, no resource"
        ]);
        $goal_task = goal_generator::instance()->create_goal_task($goal_task_config);
        self::assertEquals($goal3->id, $goal_task->goal_id);

        $course = $this->getDataGenerator()->create_course();
        $goal_task_config = goal_task_generator_config::new([
            'goal_id' => $goal3->id,
            'description' => "task 2 of goal 3 for subject user 2 with resource",
            'resource_id' => $course->id,
            'resource_type' => course_type::TYPECODE
        ]);
        $goal_task = goal_generator::instance()->create_goal_task($goal_task_config);
        self::assertEquals($goal3->id, $goal_task->goal_id);
        $goal_task_resource = goal_generator::instance()->create_goal_task_resource($goal_task, $goal_task_config);
        self::assertEquals($goal_task->id, $goal_task_resource->task_id);
         ...
         * - goal tasks = 2 (for subject user 1, one task with resource and one task without resource)
         * - goal task resource(for subject user 1, one task with resource)
        ...
        $this->assertEquals(
            ...
            (goal_entity::repository()
                ->where('user_id', $targetuser1->id)
                ->with('tasks')
                ->with('tasks.resource')
            )->count()
        );

        // Execute userdata export
        $export = export_user_goals::execute_export($targetuser1, context_user::instance($subject1->id));

        // Test
        $this->assertCount(2, $export->data);
        foreach ($export->data as $data) {
            $this->assertEquals($subject1->id, $data['user_id']);
            foreach ($data['tasks'] as $task) {
                self::assertArrayHasKey(
                    $task['id'],
                    [
                        $goal_task1->id => $goal_task1->description,
                        $goal_task2->id => $goal_task2->description
                    ]
                );
            }
        }
        // Expected 1 file from 2 files
        $this->assertCount(1, $export->files);
    }

    public function test_export_user_goals_with_adhoc_task_inputs(): void {
        $subject1 = self::getDataGenerator()->create_user();
        $owner = self::getDataGenerator()->create_user();
        self::setUser($owner);

        $context_user = context_user::instance($subject1->id);
        $goal_test_config1 = goal_generator_config::new([
            'context' => $context_user,
            'name' => 'Test user goal 1',
            'owner_id' => $owner->id,
            'user_id' => $subject1->id,
            'id_number' => 'goal_1',
            'description' => 'Test goal 1 description',
        ]);
        $goal1 = goal_generator::instance()->create_goal($goal_test_config1);

        $context_system =  context_system::instance();
        $goal_test_config2 = goal_generator_config::new([
            'context' => $context_system,
            'name' => 'Test system goal 2',
            'owner_id' => $owner->id,
            'user_id' => $subject1->id,
            'id_number' => 'goal_2',
            'description' => 'Test goal 2 description',
        ]);
        $goal2 = goal_generator::instance()->create_goal($goal_test_config2);

        // Execute userdata export. An adhoc export user data task gets a system context value from a totara_userdata_export
        // record and will pass it in. However, we need to check that the export will succeed (context is ignored).
        $targetuser1 = new target_user($subject1);
        $export1 = export_user_goals::execute_export($targetuser1, $context_user); // finding user's goals with system user context
        $export2 = export_user_goals::execute_export($targetuser1, $context_system); // finding user's goals with system context

        // Test
        $this->assertCount(2, $export1->data);
        $this->assertCount(2, $export2->data);
    }

    public function test_export_tenant_user_goals_with_adhoc_task_inputs(): void {
        $this->setAdminUser();
        ...
        $subject1 = self::getDataGenerator()->create_user();
        $owner = self::getDataGenerator()->create_user();
        tenant_util::migrate_user_to_tenant($subject1->id, $tenant1->id);
        tenant_util::migrate_user_to_tenant($owner->id, $tenant1->id);
        self::setUser($owner);
        ...
        $goal_test_config = goal_generator_config::new([
            ...
            'user_id' => $subject1->id,
            'id_number' => 'goal_1',
            'description' => 'Tenant goal 1 description',
        ]);
        $goal1 = goal_generator::instance()->create_goal($goal_test_config);

        $context_tenant_user = context_user::instance($subject1->id);
        $goal_test_config2 = goal_generator_config::new([
            'context' => $context_tenant_user,
            'name' => 'Tenant goal 2',
            'owner_id' => $owner->id,
            'user_id' => $subject1->id,
            'id_number' => 'goal_2',
            'description' => 'Tenant goal 2 description',
        ]);
        $goal2 = goal_generator::instance()->create_goal($goal_test_config2);
        ...
        // record and will pass it in. However, we need to check that the export will succeed (context is ignored).
        $targetuser1 = new target_user($subject1);
        $export1 = export_user_goals::execute_export($targetuser1, $context_tenant); // finding user's goals with tenant context
        $export2 = export_user_goals::execute_export($targetuser1, $context_tenant_user); // finding user's goals with user context for a tenant member
