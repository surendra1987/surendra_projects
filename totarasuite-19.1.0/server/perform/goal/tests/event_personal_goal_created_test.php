...
class perform_goal_event_personal_goal_created_test extends testcase {

    public function test_event_goal_created_no_subject_user() {
        ...
        $event = personal_goal_created::create_from_instance($perform_goal);
        $event->trigger();
        ...
        $this->assertSame(
            "The user with id '{$perform_goal->owner_id}' has created a personal goal with id '{$perform_goal->id}'",
            $event->get_description()
        );
        $this->assertEventContextNotUsed($event);
    }

    public function test_event_goal_created_with_subject_user() {
        ...
        $this->assertSame(perform_goal_entity::TABLE, $event->objecttable);
        $this->assertSame(
            "The user with id '{$perform_goal->owner_id}' has created a personal goal with id '{$perform_goal->id}' for the user with id '{$perform_goal->user_id}'",
            $event->get_description()
        );
        $this->assertEventContextNotUsed($event);
    }
