...
/**
 * External API tests for the perform_goal_create_goal mutation.
 */
class perform_goal_webapi_resolver_mutation_create_goal_external_api_test extends perform_goal_create_goal_testcase {
    ...
    public function test_with_invalid_field_external_api(): void {
        $args = $this->setup_env();
        $args['input']['context_id'] = 999999;

        self::assert_external_operation_failed(
            self::make_external_api_request(self::MUTATION, $args),
            'Invalid or missing context_id'
        );
    }
    ...
    public function test_with_invalid_manage_same_user_capability_external_api(): void {
        global $DB;

        $args = $this->setup_env();
        $api_user = self::helper_set_auth_header_get_api_user(false);

        $subject_id = $args['input']['user']['id'];
        assign_capability(
            'perform/goal:manageownpersonalgoals',
            CAP_PREVENT,
            $DB->get_record('role', ['shortname' => 'user'])->id,
            $args['input']['context_id'],
            true
        );

        self::setUser($subject_id);
        self::assert_external_operation_failed(
            self::make_external_api_request(self::MUTATION, $args, $api_user),
            'Sorry, but you do not currently have permissions to do that (create a goal in this context).'
        );
