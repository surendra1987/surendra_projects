 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_backup_restore_gradebook_structure_step_test extends \core_phpunit\testcase {
    ...
    public static function rewrite_step_backup_file_for_legacy_freeze_provider() {
        ...
        foreach ($iterator as $file) {
            $sourcefile = $file->getPathname(); // Convert SplFileInfo to string path
            if (!preg_match('/\.test$/', $sourcefile)) {
                continue;
            }

            $expectfile = preg_replace('/\.test$/', '.expectation', $sourcefile);
            $tests[basename($sourcefile)] = [$sourcefile, $expectfile];
        }
