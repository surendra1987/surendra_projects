...
class restore_course_structure_step extends restore_structure_step {
    ...
    public function process_course($data) {
        global $CFG, $DB, $TOTARA_COURSE_TYPES;
        ...
        // Restore course type
        if (!isset($data->coursetype) || !in_array($data->coursetype, $TOTARA_COURSE_TYPES)) {
            $data->coursetype = TOTARA_COURSE_TYPE_ELEARNING;
            $this->log(
                "Mapped course type not found in the course type list, assign the Totara course type default value",
                backup::LOG_DEBUG
            );
        }
        ...
        if ($DB->record_exists('totara_mobile_compatible_courses', ['courseid' => $data->id])) {
            if (empty($data->is_totara_mobile_compatible)) {
                $DB->delete_records('totara_mobile_compatible_courses', ['courseid' => $data->id]);
            }
        } else if (!empty($data->is_totara_mobile_compatible)) {
            $DB->insert_record('totara_mobile_compatible_courses', ['courseid' => $data->id]);
        }

        if ($DB->record_exists('catalog_exclusions', ['objectid' => $data->id, 'objecttype' => 'course'])) {
            if (empty($data->exclude_from_course)) {
                $DB->delete_records('catalog_exclusions', ['objectid' => $data->id, 'objecttype' => 'course']);
            }
        } elseif (!empty($data->exclude_from_course)) {
            $DB->insert_record('catalog_exclusions', ['objectid' => $data->id, 'objecttype' => 'course']);
        }
    ...
    public function process_custom_role_name($data) {
        ...
        if (!$role = $DB->get_record('role', array('shortname' => $data->role_name))) {
            $msg = "Custom role name [{$data->custom_name}] cannot be restored because it doesn't exist in the target database";
            debugging($msg);

            $this->log($msg, backup::LOG_DEBUG);
            return;
        }
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('totara_customfield', 'course_filemgr', 'totara_course_custom_field', $this->task->get_old_system_contextid());
        // Restore files for text area custom field types.
        $this->add_related_files('totara_customfield', 'course', 'totara_course_custom_field', $this->task->get_old_system_contextid());
...
class restore_ras_and_caps_structure_step extends restore_structure_step {
    ...
    public function process_override($data) {
        ...
        // Totora: Have to change the $newroleid check so it works with
        //         PHP 5 used in older Totara versions.
    ...
    public function process_course_completion_history($data) {
        ...
        if (!empty($userid)) {
            ...
            $params = array(
                ...
                'timecompleted' => $data->timecompleted,
                'grade' => $data->grade,
                'grademax' => isset($data->grademax) ? $data->grademax : $course_item->grademax,
                'grademin' => isset($data->grademin) ? $data->grademin : $course_item->grademin
            );
...
class restore_create_categories_and_questions extends restore_structure_step {
    ...
    protected function process_question_category($data) {
        ...
        if (!$mapping->newitemid) {
            ...
            // We still need it for doing second time of restoring
