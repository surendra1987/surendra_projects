...
// Protected API starts here
    ...
    protected function define_settings() {
        ...
        $defaultvalue = false;
        $changeable = false;
        if (isset($rootsettings['hvp_libraries']) && $rootsettings['hvp_libraries']) { // Only enabled when available
            $defaultvalue = true;
            $changeable = true;
        }
        $groups = new restore_groups_setting('hvp_libraries', base_setting::IS_BOOLEAN, $defaultvalue);
        $groups->set_ui(new backup_setting_ui_checkbox($groups, get_string('rootsettinghvplibraries', 'backup')));
        $groups->get_ui()->set_changeable($changeable);
        $this->add_setting($groups);
