...
class core_badges_renderer extends plugin_renderer_base {
    ...
    protected function render_badgeclass(\core_badges\output\badgeclass $badge) {
        $badgedata = $badge->export_for_template($this);

        $now = time();
        $expiration = $badgedata->expiredate ?? $now + 8400;
        $badgeimage = $badgedata->badgeimage;
        $output = '';
        $output .= html_writer::start_tag('div', array('id' => 'badge'));
        $output .= html_writer::start_tag('div', array('id' => 'badge-image'));
        $output .= html_writer::empty_tag('img', array('src' => $badgeimage, 'alt' => $badgedata->badgename, 'width' => '100'));
        if ($expiration < $now) {
            $output .= $this->output->pix_icon('i/expired',
        ...
        $output .= $this->output->heading(get_string('issuerdetails', 'badges'), 3);
        $dl = array();
        $dl[get_string('issuername', 'badges')] = $badgedata->issuedby;
        if (isset($badgedata->issuedbyemailobfuscated)) {
            $dl[get_string('contact', 'badges')] = $badgedata->issuedbyemailobfuscated;
        }
        ...
        $output .= $this->output->heading(get_string('badgedetails', 'badges'), 3);
        $dl = array();
        $dl[get_string('name')] = $badgedata->badgename;
        if (!empty($badgedata->version)) {
            $dl[get_string('version', 'badges')] = $badgedata->version;
        }
        if (!empty($badgedata->language)) {
            $dl[get_string('language')] = $badgedata->language;
        }
        $dl[get_string('description', 'badges')] = $badgedata->badgedescription;
        if (!empty($badgedata->imageauthorname)) {
            $dl[get_string('imageauthorname', 'badges')] = $badgedata->imageauthorname;
        }
        if (!empty($badgedata->imageauthoremail)) {
            $dl[get_string('imageauthoremail', 'badges')] =
                html_writer::tag('a', $badgedata->imageauthoremail, array('href' => 'mailto:' . $badge->imageauthoremail));
        }
        if (!empty($badgedata->imageauthorurl)) {
            $dl[get_string('imageauthorurl', 'badges')] =
                html_writer::link($badgedata->imageauthorurl, $badgedata->imageauthorurl, array('target' => '_blank'));
        }
        if (!empty($badgedata->imagecaption)) {
            $dl[get_string('imagecaption', 'badges')] = $badgedata->imagecaption;
        }

        if (isset($badgedata->coursefullname)) {
            $dl[get_string('course')] = $badgedata->coursefullname;
        }
        $dl[get_string('bcriteria', 'badges')] = $badgedata->criteria;
        $output .= $this->definition_list($dl);

        if (isset($badgedata->expiredate)) {
            $output .= $this->output->heading(get_string('issuancedetails', 'badges'), 3);
            $dl = array();
            if ($badgedata->expiredate < $now) {
                $dl[get_string('expirydate', 'badges')] = userdate($badgedata->expiredate . get_string('warnexpired', 'badges'));
            } else {
                $dl[get_string('expirydate', 'badges')] = userdate($badgedata->expiredate);
            }
            $output .= $this->definition_list($dl);
        }
        if ($badge->badge->get_endorsement()) {
            $output .= self::print_badge_endorsement($badge->badge);
        }

        $relatedbadges = $badge->badge->get_related_badges(true);
        $items = array();
        foreach ($relatedbadges as $related) {
            $relatedurl = new moodle_url('/badges/overview.php', array('id' => $related->id));
            $items[] = html_writer::link($relatedurl->out(), $related->name, array('target' => '_blank'));
    ...
    protected function render_external_badge(\core_badges\output\external_badge $ibadge) {
        ...
        if (!empty($issued->image) && is_object($issued->image)) {
    ...
    public function render_test_backpack_result(int $backpackid): string {
        ...
        $result = $this->page_main_heading(get_string('testbackpack', 'badges', $backpack->backpackweburl));
