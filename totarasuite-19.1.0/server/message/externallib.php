...
class core_message_external extends external_api {
    ...
    public static function data_for_messagearea_search_users_parameters() {
        return new external_function_parameters(
            array(
                'userid' => new external_value(PARAM_INT, 'The id of the user who is performing the search'),
                'search' => new external_value(PARAM_RAW, 'The string being searched'),
                'limitnum' => new external_value(PARAM_INT, 'Limit number', VALUE_DEFAULT, 0, NULL_NOT_ALLOWED)
            )
        );
    }
    ...
    public static function data_for_messagearea_search_users($userid, $search, $limitnum = 0) {
        ...
        // Default of zero (= no limit) is not acceptable anymore to prevent denial of service. We silently change
        // it to the max limit.
        if (empty($limitnum)) {
            $limitnum = self::get_messagearea_search_max_limitnum();
        }
        self::validate_messagearea_search_limitnum($limitnum);
    ...
    /**
     * Mark a single notification as read, this will trigger a 'notification_viewed' event.
     * Note: This function is no longer interrupted by user-to-user messaging settings
    ...
    /**
     * Restrict the limitnum parameter for message area searches to prevent denial of service.
     *
     * @param int $limitnum
     * @return void
     * @throws invalid_parameter_exception
     */
    public static function validate_messagearea_search_limitnum(int $limitnum): void {
        $max_limitnum = self::get_messagearea_search_max_limitnum();
        if ($limitnum > $max_limitnum || $limitnum <= 0) {
            throw new invalid_parameter_exception('The limitnum parameter must be between 1 and ' . $max_limitnum);
        }
    }

    /**
     * Get the value for the maximum allowed number of returned results for the message area search.
     *
     * @return int
     */
    public static function get_messagearea_search_max_limitnum(): int {
        global $CFG;

        return empty($CFG->message_area_search_max_limitnum) ? 20 : (int)$CFG->message_area_search_max_limitnum;
    }
}
