...
class migrate_message_data extends \core\task\adhoc_task {
    ...
    private function migrate_notification($notification, $isread) {
        ...
        // Totara: migrate record of message metadata, this is properly the best way that I could think of to
        // decouple between the plugin totara_message and this core_message subsystem
        if (file_exists("{$CFG->dirroot}/totara/message/lib.php")) {
            require_once("{$CFG->dirroot}/totara/message/lib.php");

            if (function_exists('totara_message_migrate_message_metadata_to_notification')) {
                totara_message_migrate_message_metadata_to_notification($notification->id, $newid, $isread);
            }
        }
    }
     ...
     * @return void
     */
    public static function queue_task(int $userid): void {
