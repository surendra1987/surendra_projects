 ...
 */
namespace message_totara_alert\watcher;

use core_message\hook\purge_check_notification_hook;
use message_output_totara_alert;
...
class purge_notification_watcher {
    ...
    public static function check_notification_for_purge(purge_check_notification_hook $hook): void {
        ...
        if (!class_exists('message_output_totara_alert')) {
            require_once("{$CFG->dirroot}/message/output/totara_alert/message_output_totara_alert.php");
        }
        ...
        $metadata = $repository->find_message_metadata_from_notification_id(
            $notification->id,
            message_output_totara_alert::get_processor_id()
        );
