...
class message_output_email extends message_output {
    ...
    function send_message($eventdata) {
        ...
        if (core_user::is_real_user($eventdata->userto->id) && (!isset($eventdata->use_central_notification) || $eventdata->use_central_notification === false)) {
        ...
        $attachment_list = static::get_attachment_list($eventdata);
        ...
        $result = email_to_user_multiple_attachments($recipient, $eventdata->userfrom, $eventdata->subject, $eventdata->fullmessage,
            $eventdata->fullmessagehtml, $attachment_list, true, $replyto, $replytoname
        );
    ...
    /**
     * Return the list of checked attachments to send
     * Keeping it public to allow for testing
     *
     * @param object $eventdata the event data submitted by the message sender
     */
    public static function get_attachment_list($eventdata) {
        global $CFG;
        
        // Now allowing for a single attachment passed in the attachment / attachname paramers as well as a list of attachments passed in attachment_list
        if (empty($CFG->allowattachments) || (empty($eventdata->attachment) && empty($eventdata->attachment_list))) {
            return [];
        }

        $attachment_list = $eventdata->attachment_list ?? [];
        $checked_attachments = [];

        // Adding the single attachment to the list if name not already in the list
        if (!empty($eventdata->attachment)) {
            if (empty($eventdata->attachname)) {
                // Attachment needs a file name.
                debugging('Attachments should have a file name. Some attachments are not sent.', DEBUG_DEVELOPER);
            } else {
                if (!isset($attachment_list[$eventdata->attachname])) {
                    $attachment_list[$eventdata->attachname] = $eventdata->attachment;
                }
            }
        }

        foreach ($attachment_list as $name => $file) {
            if (!is_string($name)) {
                debugging('Attachments should have a file name. Some attachments are not sent.', DEBUG_DEVELOPER);
                continue;
            } else if (!($file instanceof stored_file)) {
                // Attachment should be of a type stored_file.
                debugging('Attachments should be of type stored_file. Attachment ' . $name . ' not sent.', DEBUG_DEVELOPER);
                continue;
            }
            ...
            $attachname = clean_filename($name);

            $checked_attachments[$attachname] = $attachment;
        }

        return $checked_attachments;
    }
}
