...
foreach($events as $event) {
    ...
    // Check room location if the current module is face-to-face.
    if ($event->modulename == "facetoface" && !empty($event->uuid)) {
        // Search valid room. From Totara t13, a session can have multiple rooms.
        $sql = "
        SELECT fr.*
        ...
        $filtered_rooms = $DB->get_records_sql($sql, $params);

        // Get the location data from custom fields if they exist.
        $location = [];
        $delimiter = get_string('icallocationstringdelimiter', 'facetoface');
        foreach ($filtered_rooms as $filtered_room) {
            $place = [];

            $room = new \mod_facetoface\room($filtered_room->id);
        ...
        $location = str_replace('\n', $delimiter, messaging::ical_escape(implode(';', $location), true));

        // Possibility of multiple commas, replaced with the single one.
        $location = preg_replace("/{$delimiter}+/", $delimiter, $location);

        // Possibility of multiple backslash remove.
        $location = str_replace("\\", '', $location);

        // Possibility of multiple tabs removes because of multiple rooms with long location names.
        $location = preg_replace('/(\v|\s)+/', ' ', $location);
