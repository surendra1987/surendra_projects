...
class qtype_ddmarker_edit_form extends qtype_ddtoimage_edit_form_base {
    ...
    public function validation($data, $files) {
        ...
        // Check that drop slots are not removed after answers are recorded.
        if ($data['id']) {
            $new_count = 0;
            foreach ($data['drops'] as $drop) {
                if (!empty($drop['coords'])) {
                    $new_count++;
                }
            }

            $old_count = $DB->count_records('qtype_ddimageortext_drops', ['questionid' => $data['id']]);
            if ($new_count != $old_count) {
                // The number of answers is being changed. Only prevent if answers have already been recorded.
                $submissions_exist = $DB->record_exists('question_attempts', ['questionid' => $data['id']]);

                if ($submissions_exist) {
                    // The 'min' of the two counts causes the validation warning to appear on the most appropriate line.
                    $errors['drops[' . min($new_count, $old_count) . ']'] = get_string('cannoteditafterattempts', 'mod_quiz', $old_count);
                }
            }
        }
