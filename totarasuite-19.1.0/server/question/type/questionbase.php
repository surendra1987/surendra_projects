...
abstract class question_definition {
    ...
    public function format_text($text, $format, $qa, $component, $filearea, $itemid,
        ...
        $formatoptions->noclean = !$clean; // Allow content to be cleaned, TL-20750 will replace the need for this flag.
        $formatoptions->para = false;
        $text = $qa->rewrite_pluginfile_urls($text, $component, $filearea, $itemid);

        // Avoid a possible error later: avoid a call for Weka editor to filter_json_content for quiz
        // questions and feedback, if this text is not in JSON format.
        if ($format == FORMAT_JSON_EDITOR && !document_helper::looks_like_json($text)) {
            $format = FORMAT_PLAIN;
        }
