...
Y.extend(DDIMAGEORTEXT_FORM, M.qtype_ddimageortext.dd_base_class, {
    ...
    constrain_xy: function(draginstanceno, bgimgxy) {
        var drag = this.doc.drag_item(draginstanceno);
        var xleftconstrained = bgimgxy[0];
        var ytopconstrained = bgimgxy[1];

        // Drag not set here
        if (drag) {
            xleftconstrained =
                Math.min(xleftconstrained, this.doc.bg_img().get('width') - drag.get('offsetWidth'));
            ytopconstrained =
                Math.min(ytopconstrained, this.doc.bg_img().get('height') - drag.get('offsetHeight'));
        }
