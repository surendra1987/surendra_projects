...
class qtype_ddtoimage_renderer_base extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        $img = html_writer::empty_tag('img', array(
                'src' => $bgimage, 'class' => 'dropbackground img-responsive',
        ...
        foreach ($question->places as $placeno => $place) {
            ...
            [$fieldname, $html] = $this->hidden_field_for_qt_var($qa, $varname);
            $hiddens .= $html;

            // Totara: Don't modify the base object, rather gather the drop points specifically
            $placeinfo = (array) $place;
            $placeinfo['fieldname'] = $fieldname;

            if (isset($placeinfo['text'])) {
                $placeinfo['text'] = format_string($placeinfo['text']);
            }
            $dropplaces[$placeno] = (object) $placeinfo;
        }
        $output .= html_writer::tag('div',
                $droparea . $dragitems . $dropzones . $hiddens, array('class' => 'ddarea'));
        $topnode = 'div#' . $qa->get_outer_question_div_unique_id() . ' div.ddarea';
        $params = array('drops' => $dropplaces,
                        'topnode' => $topnode,
