 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qformat_xml_xmlformat_test extends question_testcase {
    private function assert_same_xml($expectedxml, $xml) {
        $this->assertEquals(str_replace("\r\n", "\n", $expectedxml),
                str_replace("\r\n", "\n", $xml));
    }

    private function make_test_question() {
        global $USER;
        $q = new stdClass();
        $q->id = 0;
        $q->contextid = 0;
     ...
     */
    private function remove_irrelevant_form_data_fields($expectedq) {
