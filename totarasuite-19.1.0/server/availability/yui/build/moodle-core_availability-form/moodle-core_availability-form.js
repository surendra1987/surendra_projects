...
M.core_availability.List = function(json, root, parentRoot) {
    ...
    this.node = Y.Node.create('<div class="availability-list"><h3 class="accesshide"></h3>' +
            ...
            '<div class="availability-header"><span class="pl-3">' +
            ...
            ' </span><select class="availability-neg custom-select mx-3"' +
            ...
            '<select class="availability-op custom-select mx-3"' +
    ...
    if (!root) {
        ...
        noneNode.appendChild(Y.Node.create('<span class="mt-3 label label-warning">' +
                M.util.get_string('invalid', 'availability') + '</span>'));
    }

    // Create the button and add it.
    var button = Y.Node.create('<button type="button" class="btn btn-default mt-3">' +
...
M.core_availability.List.prototype.clickAdd = function() {
    ...
    dialogRef.dialog.addButton({
        label: M.util.get_string('cancel', 'moodle'),
        section: Y.WidgetStdMod.FOOTER,
        action: function() {
            dialogRef.dialog.destroy();
            // Reset keyboard focus back to the triggering element
            this.inner.one('> .availability-button').one('button').focus();
        }.bind(this),
        classNames: 'btn'
    });
...
M.core_availability.Item = function(json, root) {
    ...
    this.node.appendChild(Y.Node.create('<span class="mt-3 label label-warning"/>'));
};
...
M.core_availability.DeleteIcon = function(toDelete) {
    var that = this;
    this.span = Y.Node.create('<a class="d-inline-block col-form-label availability-delete px-3" href="#" role="button">');
    require(['core/str', 'core/templates'], function(stringlib, templatelib) {
