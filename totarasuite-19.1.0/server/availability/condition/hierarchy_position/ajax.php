...
if ($filter !== '') {
    ...
    $sql = "SELECT id, fullname FROM {pos} WHERE {$searchsql}";
    $params = array('fullname' => $searchparam);
    $positions = $DB->get_records_sql($sql, $params, 0, 50);
    ...
    foreach ($positions as $key => $pos) {
        $name = format_string($pos->fullname);
    ...
    $position = $DB->get_record('pos', array('id' => $positionid), 'id,fullname');
    if ($position) {
        $item = new stdClass();
        $item->label = format_string($position->fullname);
