 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_cohort_cohortlib_test extends \core_phpunit\testcase {
    ...
    public function test_cohort_delete_cohort_event() {
        ...
        $this->assertContains(
            pre_delete_cohort_check::class,
            $hooks_triggered,
            "cohort_delete_cohort needs to trigger the pre_delete_cohort_check hook"
        );
    ...
    /**
     * Update cohort role category context
     *
     * @return void
     * @throws coding_exception
     */
    public function test_update_cohort_role_context(): void {
        global $DB, $USER;
        $this->setAdminUser();
        $category = $this->getDataGenerator()->create_category();

        $sub_cat = $this->getDataGenerator()->create_category([
        ...
        $cohort = $this->getDataGenerator()->create_cohort(
            ['contextid' => context_coursecat::instance($sub_cat->id)->id]
        );

        $cohort_context = context::instance_by_id($cohort->contextid);

        $assignable_roles = get_assignable_roles($cohort_context, ROLENAME_BOTH);

        $role_ids = [];
        foreach ($assignable_roles as $role_key => $role_value) {
            $role_ids[] = $role_key;
        }

        $sliced_role_ids = array_slice($role_ids,0, 2);

        $expected_cohort_roles = [];
        foreach ($sliced_role_ids as $role_value) {
            $now = time();
            $cohortrole = new \stdClass();
            $cohortrole->cohortid = $cohort->id;
            $cohortrole->usermodified = $USER->id;
            $cohortrole->roleid = $role_value;
            $cohortrole->contextid = $sub_cat->get_context()->id;
            $cohortrole->timecreated = $now;
            $cohortrole->timemodified = $now;

            $DB->insert_record('cohort_role', $cohortrole);

            $expected_cohort_roles[$role_value] = context_coursecat::instance($category->id)->id;
        }

        update_cohort_role_category_context($sub_cat);

        $cohort_roles = $DB->get_records('cohort_role', ['cohortid' => $cohort->id], '', 'roleid, contextid');

        $actual_cohort_roles = [];
        foreach ($cohort_roles as $cohort_role) {
            $actual_cohort_roles[$cohort_role->roleid] = $cohort_role->contextid;
        }

        $this->assertSame(2, $DB->count_records('cohort_role', ['cohortid' => $cohort->id]));
        $this->assertEquals($expected_cohort_roles, $actual_cohort_roles);
    }
}
