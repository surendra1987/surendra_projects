...
if (\core_course\local\grade_helper::does_course_need_regrade($courseid)) {
    ...
    if (\core_course\local\grade_helper::use_async_course_regrade($courseid)) {
        ...
        // Also reset item as we cannot select an item that needs to be regraded.
        $itemtype = $defaulttype;
        $itemid = null;
    } else {
...
// If grades needs regrading return notification.
if (grade_needs_regrade_final_grades($course->id)) {
    echo $OUTPUT->notification(get_string('course_regrade_not_complete', 'totara_core'), \core\output\notification::NOTIFY_WARNING);
}
