...
if ($courseid and $scales = grade_scale::fetch_all_local($courseid)) {
    ...
    foreach($scales as $scale) {
        $line = array();
        $header = new html_table_cell(format_string($scale->name).'<div class="scale_options">'.str_replace(",",", ",$scale->scale).'</div>');
        $header->header = true;
        $header->scope = 'row';
...
if ($courseid) {
    ...
    echo $OUTPUT->page_main_heading(get_string('scale_link', 'core'));
}

echo $OUTPUT->heading($strcustomscales, 2, 'main');
echo html_writer::table($table);
echo $OUTPUT->heading($strstandardscale, 2, 'main');
