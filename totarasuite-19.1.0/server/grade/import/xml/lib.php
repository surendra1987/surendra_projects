...
/**
 * This functionality comes from grade/import/xml/import.php(deprecated) to avoid CSRF vulnerability
 * validating sesskey first(see index.php)
 *
 * @param \stdClass $data
 * @return void
 ...
 */
function import_xml_grades_from_url($data) {
    global $OUTPUT;

    $text = download_file_content($data->url);
    if ($text === false) {
        \core\notification::error(get_string('cannotreadfile', 'error', $data->url));
        $redirect = new moodle_url('/grade/import/xml/index.php', array('id' => $data->id));
        redirect($redirect);
    }

    $error = '';
    $course = array('id' => $data->id);
    $importcode = import_xml_grades($text, (object)$course, $error);

    if ($importcode !== false) {
        // Commit the code if we are up this far.
        if (defined('USER_KEY_LOGIN')) {
            if (grade_import_commit($data->id, $importcode, $data->feedback, false)) {
                echo 'ok';
                die;
            } else {
            ...
            print_grade_page_head($data->id, 'import', 'xml', get_string('importxml', 'grades'));
            grade_import_commit($data->id, $importcode, $data->feedback);
            echo $OUTPUT->footer();
            die;
        }
    }
