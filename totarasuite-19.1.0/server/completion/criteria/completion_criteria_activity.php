...
class completion_criteria_activity extends completion_criteria {
    ...
    public function config_form_display(&$mform, $data = null) {
        $modnames = \container_course\course_helper::get_all_modules_with_context_component_area(
            "core_completion",
            "criteria_activity_form"
        );
    ...
    /**
     * Retrieve user's progress towards completing this activity from
     * the 'progress' field in 'course_modules_completion' table where present.
     *
     * @since Totara 15.2
     *
     * @param completion_criteria_completion $completion
     *
     * @return float|number Percentage completed (0.0 - 1.0)
     */
    public function get_progress($completion) {
        global $DB;

        $conditions = ['coursemoduleid' => $this->moduleinstance, 'userid' => $completion->userid];
        $progress = $DB->get_field('course_modules_completion', 'progress', $conditions);
        if (!is_null($progress)) {
            return round($progress / 100, 3);
        }

        // Fall back on generic progress results.
        return parent::get_progress($completion);
    }
}
