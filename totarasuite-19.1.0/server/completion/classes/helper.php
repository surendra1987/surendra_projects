...
final class helper {
    ...
    public static function write_module_completion($modulecompletion, $message = '') {
        ...
        // Mark progress caches stale
        program_progress_cache::mark_user_cache_stale($modulecompletion->userid);
    ...
    public static function log_course_completion($courseid, $userid, $message = '', $changeuserid = null, \stdClass $coursecompletion = null, \stdClass $prevcoursecompletion = null) {
        ...
        if ($prevcoursecompletion !== null) {
            ...
            if ($coursecompletion->status === $prevcoursecompletion->status
                ...
                && $coursecompletion->duedate === $prevcoursecompletion->duedate
            ) {
                return;
            }
        }
    ...
    public static function get_course_completion_log_description($coursecompletion, $message = '') {
        ...
        $description = $message . '<br/>' .
            ...
            '<li>Reaggregate: ' . self::format_log_date($coursecompletion->reaggregate) . '</li>' .
            '<li>Due date: ' . self::format_log_date($coursecompletion->duedate) . '</li></ul>';
    ...
    public static function get_course_completion_history_log_description($historycompletion, $message = '') {
        ...
        if (isset($historycompletion->grademax) && is_numeric($historycompletion->grademax)) {
            $grademax = (float)$historycompletion->grademax;
        } else {
            $grademax = "Empty (non-numeric)";
        }

        if (isset($historycompletion->grademin) && is_numeric($historycompletion->grademin)) {
            $grademin = (float)$historycompletion->grademin;
        } else {
            $grademin = "Empty (non-numeric)";
        }
        ...
        $description = $message . '<br/><ul>' .
            ...
            '<li>Grade: ' . $grade . '</li>'.
            '<li>Maximum grade: ' . $grademax . '</li>'.
            '<li>Minimum grade: ' . $grademin . '</li></ul>';
