@totara @block @block_totara_trending @ml_recommender @engage @totara_engage @javascript
Feature: Test Trending Content Block
  ...
  Scenario: Visible items are initially controlled via admin settings
    ...
    And I add the "Trending" block to the "main" region
    And I press "Stop customising this page"

    Then I should see the "Trending" block
    And I should see "Trending Survey 2" in the "Trending" "block"
    And I should see "Trending Playlist 2" in the "Trending" "block"
    And I should see "Trending Survey 1" in the "Trending" "block"
    And I should not see "Trending Article 2" in the "Trending" "block"
    And I should not see "Trending Article 1" in the "Trending" "block"
    And I should not see "Trending Playlist 1" in the "Trending" "block"

    And I log out

    # Change the number to something bigger
    When I log in as "admin"
    And I navigate to "Plugins > Blocks > Trending" in site administration
    ...
    Then I should see "Trending Survey 2" in the "Trending" "block"
    And I should see "Trending Playlist 2" in the "Trending" "block"
    And I should see "Trending Survey 1" in the "Trending" "block"
    And I should see "Trending Article 2" in the "Trending" "block"
    And I should see "Trending Article 1" in the "Trending" "block"
    And I should not see "Trending Playlist 1" in the "Trending" "block"
