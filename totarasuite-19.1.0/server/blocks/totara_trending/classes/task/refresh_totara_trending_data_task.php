...
namespace block_totara_trending\task;
...
class refresh_totara_trending_data_task extends scheduled_task {
    ...
    public function get_name() {
        return get_string('refresh_totara_trending_data_task', 'block_totara_trending');
    }
    ...
    public function execute() {
        ...
        $totara_trending_repository = new totara_trending_repository('block_totara_trending\entity\totara_trending');

        // If nothing is enabled just move on.
        if (!advanced_feature::is_enabled('engage_resources')) {
            return;
        }

        $components = ['engage_article', 'engage_survey', 'totara_playlist'];
        $records = $totara_trending_repository->get_trending_components($components, $CFG->block_totara_trending_dayctr);
        $trending_items = $totara_trending_repository->from_records($records);

        // Delete existing cache records.
        $totara_trending_repository->truncate_totara_trending();
