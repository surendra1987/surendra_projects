...
/**
 * Test the endpoints for recommending playlists by playlist or user from the ML service
 */
class ml_recommender_get_service_recommended_playlists_test extends \core_phpunit\testcase {
    /**
    ...
    protected function set_recommended_data(?array $data): void {
        ...
        if (null !== $data) {
            ...
            $mock_helper
                ->method('get_similar_items')
                ->willReturn($data);
        }

        $reflection = new ReflectionClass(playlists_loader::class);
