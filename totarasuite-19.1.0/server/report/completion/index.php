...
if (!$csv) {
    ...
    echo $OUTPUT->page_main_heading(get_string('coursecompletion', 'moodle'));
    echo $OUTPUT->heading(get_string('allparticipants').": {$total_header}", 2);
...
foreach ($progress as $user) {
    ...
    if ($csv) {
        $row = array();
        $row[] = $user->id;
        $row[] = core_text::entities_to_utf8(fullname($user));
        foreach ($extrafields as $field) {
            $row[] = $user->{$field};
        }
    } else {
