...
class report_log_table_log extends table_sql {
    ...
    /** @var string[] core event sources */
    public const ORIGINS = ['api', 'cli', 'mobile', 'restore', 'web', 'ws'];
    ...
    public function col_fullnameuser($event) {
        ...
        if (!empty($override_value = $hook->get_override_value())) {
    ...
    public function col_description($event) {
        // Description.
        return clean_string($event->get_description());
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if (isset($this->filterparams->origin) && ($this->filterparams->origin != '')) {
            if ($this->filterparams->origin !== '---') {
                ...
                list($originsql, $originparams) = $DB->get_in_or_equal(static::ORIGINS,
