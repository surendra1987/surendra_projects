...
class report_log_renderer extends plugin_renderer_base {
    ...
    public function report_selector_form(report_log_renderable $reportlog) {
        $select_attrs = ['class' => 'w-auto mr-1 mb-1'];
        ...
        if (!empty($courses) && $reportlog->showcourses) {
            ...
            echo html_writer::select($courses, "id", $selectedcourseid, null, $select_attrs);
        } else {
            $courses = array();
            $courses[$selectedcourseid] = get_course_display_name_for_list($reportlog->course) . (($selectedcourseid == SITEID) ?
                ' (' . get_string('site') . ') ' : '');
            echo html_writer::label(get_string('selectacourse'), 'menuid', false, array('class' => 'accesshide'));
            echo html_writer::select($courses, "id", $selectedcourseid, false, $select_attrs);
        ...
        if (!empty($groups)) {
            ...
            echo html_writer::select($groups, "group", $reportlog->groupid, get_string("allgroups"), $select_attrs);
        }
        ...
        if ($reportlog->showusers) {
            ...
            echo html_writer::select($users, "user", $reportlog->userid, get_string("allparticipants"), $select_attrs);
        } else {
            $users = array();
            ...
            echo html_writer::label(get_string('selctauser'), 'menuuser', false, array('class' => 'accesshide'));
            echo html_writer::select($users, "user", $reportlog->userid, false, $select_attrs);
            $a = new stdClass();
            $a->url = new moodle_url('/report/log/index.php', array('chooselog' => 0,
                'group' => $reportlog->get_selected_group(), 'user' => $reportlog->userid,
                'id' => $selectedcourseid, 'date' => $reportlog->date, 'modid' => $reportlog->modid,
            ...
            $a->url = $a->url->out(false);
            echo html_writer::start_span('mx-3');
        ...
        echo html_writer::select($dates, "date", $reportlog->date, get_string("alldays"), $select_attrs);
        ...
        echo html_writer::select($activities, "modid", $reportlog->modid, get_string("allactivities"), $select_attrs);

        // Add actions selector.
        echo html_writer::label(get_string('actions'), 'menumodaction', false, array('class' => 'accesshide'));
        echo html_writer::select($reportlog->get_actions(), 'modaction', $reportlog->action, get_string("allactions"), $select_attrs);
        ...
        echo html_writer::select($origin, 'origin', $reportlog->origin, false, $select_attrs);
        ...
        echo html_writer::select($edulevel, 'edulevel', $reportlog->edulevel, false, $select_attrs).$this->help_icon('edulevel');
        ...
        if (!empty($readers)) {
            if (count($readers) == 1) {
                $attributes = array('type' => 'hidden', 'name' => 'logreader', 'value' => key($readers), 'class' => 'w-auto mr-1 mb-1');
                echo html_writer::empty_tag('input', $attributes);
            } else {
                echo html_writer::label(get_string('selectlogreader', 'report_log'), 'menureader', false,
                        array('class' => 'accesshide'));
                echo html_writer::select($readers, 'logreader', $reportlog->selectedlogreader, false, $select_attrs);
            }
