 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class repository_dropbox_api_test extends \core_phpunit\testcase {
    /**
     * Data provider for has_additional_results.
     *
     * @return array
     */
    public static function has_additional_results_provider() {
        return [
            'No more results' => [
                (object) [
                    'has_more'  => false,
     ...
     * @return array
     */
    public static function check_and_handle_api_errors_provider() {
        return [
            '200 http_code' => [
                ['http_code' => 200],
                '',
     ...
     * @return array
     */
    public static function supports_thumbnail_provider() {
        $tests = [
    ...
    public function test_fetch_dropbox_data_recurse_on_additional_records() {
        ...
        $mock->expects($matcher)
            ->method('get_api_endpoint')
            ->willReturnCallback(function ($endpoint) use ($matcher) {
                ...
                $this->fail('Excess call to get_api_endpoint');
            });
    ...
    public function test_fetch_dropbox_content() {
        ...
        $mock->expects($matcher)
            ->method('setHeader')
            ->willReturnCallback(function ($value) use ($matcher, $data) {
                match ($matcher->numberOfInvocations()) {
                    1 => $this->assertEquals('Content-Type: ', $value),
                    2 => $this->assertEquals('Dropbox-API-Arg: ' . json_encode($data), $value),
                };
            });
    ...
    public function test_get_file_share_info_new() {
        ...
        $mock->expects($matcher)
            ->method('fetch_dropbox_data')
            ->willReturnCallback(function ($endpoint, $data = []) use ($matcher, $id, $file) {
                switch ($matcher->numberOfInvocations()) {
                    case 1:
                        $this->assertEquals('sharing/list_shared_links', $endpoint);
                        $this->assertEquals(['path' => $id], $data);
                        return (object) ['links' => []];
                    case 2:
                        $this->assertEquals('sharing/create_shared_link_with_settings', $endpoint);
                        $this->assertEquals([
                            'path' => $id,
                            'settings' => [
                ...
                $this->fail('Excess call to fetch_dropbox_data');
            });
    ...
    public function test_get_file_share_info_new_failure() {
        ...
        $mock->expects($matcher)
            ->method('fetch_dropbox_data')
            ->willReturnCallback(function ($endpoint, $data = []) use ($matcher, $id) {
                switch ($matcher->numberOfInvocations()) {
                    case 1:
                        $this->assertEquals('sharing/list_shared_links', $endpoint);
                        $this->assertEquals(['path' => $id], $data);
