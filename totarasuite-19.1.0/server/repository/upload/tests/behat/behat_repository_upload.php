...
class behat_repository_upload extends behat_files {
    ...
    /**
     * Uploads a file to the filemanager inside the specified selector.
     *
     * @When /^I upload "(?P<filepath_string>(?:[^"]|\\")*)" file to the filemanager in the "([^"]*)" "([^"]*)"$/
     * @throws ExpectationException Thrown by behat_base::find
     * @param string $filepath
     * @param string $group
     * @param string $selector
     */
    public function i_upload_file_to_filemanager_in_the_container($filepath, $group, $selector) {
        \behat_hooks::set_step_readonly(false);

        $parent_node = $this->find(...$this->transform_selector($selector, $group));
        $filemanager_node = $this->find(
            'css',
            '.ffilepicker',
            new ExpectationException("File manager within \"$group\" \"$selector\" could not be found", $this->getSession()),
            $parent_node
        );

        $this->upload_file_to_filemanager($filepath, $filemanager_node, new TableNode(array()), false);
    }
     ...
     * @param string|NodeElement $filemanagerelement The string label for the element, or the actual node itself.
     * @param TableNode $data Data to fill in upload form
    ...
    protected function upload_file_to_filemanager($filepath, $filemanagerelement, TableNode $data, $overwriteaction = false) {
        ...
        $filemanagernode = $filemanagerelement;
        if (!$filemanagernode instanceof NodeElement) {
