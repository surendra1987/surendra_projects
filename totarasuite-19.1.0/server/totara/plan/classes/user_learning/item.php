...
class item extends item_base implements item_has_progress {
    ...
    public static function completed($userorid): array {
        ...
        $plans = \dp_get_plans($user->id, DP_PLAN_STATUS_COMPLETE);
        foreach ($plans as $plan) {
            $items[] = new self($user, $plan);
        }
            ...
            if (!empty($content['course'])) {
                ...
                if (!\completion_info::is_enabled_for_site()) {
                    ...
                    $sql = 'SELECT c.* FROM {course} c
                        ...
                        WHERE c.id IN ('.implode(',', array_keys($courseids)).') AND
                        (c.enablecompletion = 0 OR cc.status IS NULL OR cc.status NOT IN 
