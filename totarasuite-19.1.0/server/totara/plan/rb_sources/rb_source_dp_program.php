...
class rb_source_dp_program extends rb_base_source {
    ...
    protected function define_joinlist() {
        ...
        $joinlist = array(
            ...
            new rb_join(
                ...
                'LEFT', // type of join
                '(SELECT pua2.*, pa.assignmenttype, pa.assignmenttypeid
                  ...
                  INNER JOIN {prog_user_assignment} AS pua2
                    ON pua2.id = pua.id
                  INNER JOIN {prog_assignment} AS pa
                    ON pa.id = pua2.assignmentid)',
    ...
    protected function define_columnoptions() {
        ...
        $this->add_assignment_columns($columnoptions, 'base.id', 'prog_user_assignment');
    ...
    function rb_display_recurring_status($programid, $row) {
        ...
        $program_content = new program_content($programid);
    ...
    protected function define_paramoptions() {
        ...
        $paramoptions[] = new rb_param_option(
            'exceptionstatus',
            'CASE WHEN prog_user_assignment.exceptionstatus IN (' . program::PROGRAM_EXCEPTION_RAISED . ',' . program::PROGRAM_EXCEPTION_DISMISSED .')
                THEN 1 ELSE 0 END',
    ...
    private function add_audiencevisibility_joins() {
        $this->requiredjoins[] = new rb_join(
            ...
            'INNER',
            '{prog_completion}',
            'base.id = program_completion.programid AND program_completion.coursesetid = 0',
            REPORT_BUILDER_RELATION_ONE_TO_MANY,
            'base'
        );
    ...
    public function post_config(reportbuilder $report) {
        ...
        if ($report->get_param_value('userid')) {
            ...
            $completionstatus = $report->get_field('program_completion', 'status', 'program_completion.status');
            $wheresql = "(({$visibilitysql}) OR ({$completionstatus} > :incomplete))";
            $whereparams['incomplete'] = program::STATUS_PROGRAM_INCOMPLETE;
