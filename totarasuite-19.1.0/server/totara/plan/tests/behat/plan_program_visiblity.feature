...
Feature: See that audience based visibility doesn't effect a program showing in a Learning Plan.
  ...
  Scenario: Audience based visibility where visibility is set to No users.
    Given the "mylearning" user profile block exists
    And I log in as "admin"
    When I navigate to "Shared services settings" node in "Site administration > System information > Configure features"
    And I set the field "Enable audience-based visibility" to "1"
    ...
    # Check reassigning the program more than once.
    And I click on "Add programs" "button"
    And I click on "Miscellaneous" "link"
    And I click on "Visibility Test Program 1" "link"
    And I click on "Save" "button" in the "Add programs" "totaradialogue"

    # Check that the course is not visible in the plan.
    Then I should not see "Visibility Test Program 1" in the "#dp-component-update-table" "css_element"
