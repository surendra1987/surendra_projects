...
class totara_dashboard_edit_form extends moodleform {
    public function definition() {
        ...
        if (version_specific_feature::is_enabled('totara_19_1_totara_dashboard_report')) {
            $sortorder_group = array();
            $sortorder_group[0] = $mform->createElement('text', 'sortorder', get_string('sortorder', 'totara_dashboard'));
            $sortorder_field = $sortorder_group[0];
            $sortorder_field->updateAttributes(['type' => 'number', 'min' => '0', 'max' => totara_dashboard::get_last_sortorder(), 'step' => '1']);
            $sortorder_group[] = $mform->createElement('static', 'sortorderdescription', '', '<p class="mt-4">' . get_string('sortorderdescription', 'totara_dashboard') . '</p>');
            $mform->addElement('group', 'sortorder', get_string('sortorder', 'totara_dashboard'), $sortorder_group, false);
            $mform->addHelpButton('sortorder', 'sortorder', 'totara_dashboard');
            $mform->setType('sortorder[sortorder]', PARAM_TEXT);
        }
        ...
        if ($dashboard->id) {
            ...
            if (version_specific_feature::is_enabled('totara_19_1_totara_dashboard_report')) {
                $mform->setDefault('sortorder', ['sortorder' => $dashboard->sortorder]);
            }
        } else {
            ...
            if (version_specific_feature::is_enabled('totara_19_1_totara_dashboard_report')) {
                $new_sortorder = totara_dashboard::get_new_sortorder();
                $sortorder_field->updateAttributes(['max' => $new_sortorder]);
                $mform->setDefault('sortorder', ['sortorder' => $new_sortorder]);
            }
        }
