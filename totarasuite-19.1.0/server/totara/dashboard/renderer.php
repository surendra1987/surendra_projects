...
class totara_dashboard_renderer extends plugin_renderer_base {
    ...
    public function dashboard_manage_report() {
        global $DB, $PAGE;

        // Debug reportbuilder.
        $debug = optional_param('debug', false, PARAM_BOOL);
        // Before any actions make sure user may actually access this report.
        $report_short_name = 'dashboard_dashboards';
        $report_record = $DB->get_record('report_builder', array('shortname' => $report_short_name));
        $global_restriction_set = rb_global_restriction_set::create_from_page_parameters($report_record);
        $config = (new rb_config())->set_global_restriction_set($global_restriction_set);
        $report = reportbuilder::create_embedded($report_short_name, $config);
        if (!$report) {
        ...
        list($report_html, $debug_html) = $renderer->report_html($report, $debug);

        return array(
            'report' => $report,
            'debug_html' => $debug_html,
            'report_html' => $report_html,
            'count_html' => $renderer->result_count_heading($report)
        );
    }
}