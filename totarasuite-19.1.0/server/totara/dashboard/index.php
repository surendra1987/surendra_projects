...
$is_home_page = false;
$dashboard_home = get_home_page() == HOMEPAGE_TOTARA_DASHBOARD;
...
$PAGE->set_main_skip_target_id('region-main');

if (!$id) {
    $title = $SITE->shortname. ': ' . get_string('dashboard', 'totara_dashboard');
    $heading = get_string('dashboard', 'totara_dashboard');
    ...
    $title = $SITE->shortname. ': ' . $dashboard->name;
    $heading = $dashboard->name;

    // Home page just show the site name
    if ($is_home_page) {
    ...
    // If we're on the home page, remove the breadcrumb links
    if ($is_home_page) {
        /** @var breadcrumb_navigation_node[] $items */
    ...
    if ($PAGE->user_allowed_editing() && !$dashboard->is_locked()) {
        if ($reset !== null) {
            ...
            // If the user has just disabled editing ($edit === false), then redirect here to prevent the "Confirm resubmission" browser warning
    ...
    if (has_capability('totara/dashboard:manage', context_system::instance())) {
        ...
        $managebutton = $OUTPUT->single_button(new moodle_url("/totara/dashboard/manage.php"), $managestring, 'get');
    }
