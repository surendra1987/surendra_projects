...
class totara_dashboard {
    ...
    /**
     * Previous order of the dashboard
     *
     * @var int | null
     */
    private $previous_order = null;
    ...
    /**
     * Change dashboard order to first position
     */
    public function move_top() {
        db_reorder($this->id, 0, 'totara_dashboard');
    }
    ...
    /**
     * Returns the maximum sortorder number
     * @return int
     */
    public static function get_last_sortorder() {
        global $DB;
        $record =  $DB->get_record_sql('SELECT MAX(sortorder) maxsort FROM {totara_dashboard}');
        return $record->maxsort;
    }

    /**
     * Returns a new dashboard's position
     * @return int
    */
    public static function get_new_sortorder() {
        $last_position = static::get_last_sortorder();
        return  $last_position == null ? 0 : $last_position+1;
    }
    ...
    public function save() {
        ...
        if ($this->id > 0) {
            ...
            $this->id = $id;
            if (version_specific_feature::is_disabled('totara_19_1_totara_dashboard_report')) {
        ...
        //Reorder the dashboards when a dashboard has been added or sortorder is changed.
        if (($this->sortorder <> $this->previous_order) && version_specific_feature::is_enabled('totara_19_1_totara_dashboard_report')) {
            db_reorder($this->id, $this->sortorder, 'totara_dashboard');
        }
    ...
    public function set_from_form(stdClass $data) {
        ...
        if (version_specific_feature::is_enabled('totara_19_1_totara_dashboard_report')) {
            $empty_sortorder = $this->get_last_sortorder();
            $new_position = $this->get_new_sortorder();
            $this->previous_order = $new_position;
            if (isset($data->sortorder["sortorder"])) {
                $sortorder =  $data->sortorder["sortorder"];
                if ($this->id <= 0) {
                    $empty_sortorder = $new_position;
                } else {
                    $this->previous_order = $this->sortorder;
                }
                $this->sortorder = $sortorder == "" ? $empty_sortorder : (int)$sortorder;
            } else {
                $this->sortorder = $new_position;
            }
        }
