 ...
 * @param string $shortname Return data for this field only
 * @return array Array with the customfield and its associated value
 */
function customfield_get_data($item, $tableprefix, $prefix, $indexfullname = true, $itemextradata = array(), string $shortname = '') {
    global $DB, $CFG;
    $out = array();
    ...
    $sql = "SELECT tif.*, tid.id AS dataid, tid.data
    ...
    if (!empty($shortname)) {
        $sql .= " AND tif.shortname = :shortname";
        $params['shortname'] = $shortname;
    }
    $sql .= " ORDER BY tif.sortorder ASC";
    ...
    foreach ($fields as $field) {
        ...
        switch ($field->datatype) {
            ...
            case 'multiselect':
                ...
                foreach ($dataparams as $dataparam) {
                    if (isset($datavalue[$dataparam->value])) {
                        ...
                        $values[] = format_string($option['option']);
                    }
                }
                $data = implode(', ', $values);
                break;
            ...
            case 'text':
                ...
                $data = \customfield_text::display_item_data($data);
                break;
            case 'integer':
                require_once($CFG->dirroot.'/totara/customfield/field/integer/field.class.php');
                $data = \customfield_integer::display_item_data($data);
                break;
            case 'decimal':
                require_once($CFG->dirroot.'/totara/customfield/field/decimal/field.class.php');
                $data = \customfield_decimal::display_item_data($data, (array)$field);
                break;
        }
...
/**
 * Retrieves the definition for an item's custom field's implementation
 *
 * @param string $item_type The item type to get the definition of. E.g. `course`
 * @return array An associative array, with the following keys: \n
 *    - item_table_name => the item's table name. E.g. for course, it would be the course table `course`
 *    - info_table_prefix => the item's prefix for the custom field tables. E.g. for course, the tables are `course_info_field` and `course_info_data`, this makes the prefix
 *     `course`
 *    - info_data_item_id_key => the foriegn key to the item table on the `{info_table_prefix}_info_data` table. E.g. for course, the foriegn key for `course_info_data` is
 *     `courseid`
 *    - info_field_type_id_key => (Optional) the type ID for the info_field table. This is used to filter down customfields to a particular type. For example, positions and
 *      organisations have custom fields defined on the position/organisation type, and then implemented on the specific positions/organisations assigned to those types.
 *    - item_type_id_key => (Optional) Similar to the `info_field_type_id_key`, this relates to the types utilised by some implementations of custom fields. This value defines
 *      the type ID column on the specific item's table.
 * @throws coding_exception
 */
function customfield_get_item_information(string $item_type): array {
    switch ($item_type) {
        case 'course':
            return [
                'item_table_name' => 'course',
                'info_table_prefix' => 'course',
                'info_data_item_id_key' => 'courseid',
            ];
        case 'facetofacesession':
            return [
                'item_table_name' => 'facetoface_sessions',
                'info_table_prefix' => 'facetoface_session',
                'info_data_item_id_key' => 'facetofacesessionid',
            ];
        case 'facetofaceroom':
            return [
                'item_table_name' => 'facetoface_room',
                'info_table_prefix' => 'facetoface_room',
                'info_data_item_id_key' => 'facetofaceroomid',
            ];
        case 'facetofacesignup':
            return [
                'item_table_name' => 'facetoface_signups',
                'info_table_prefix' => 'facetoface_signup',
                'info_data_item_id_key' => 'facetofacesignupid',
                'item_type_id_key' => 'dataid',
            ];
        case 'facetofacecancellation':
            return [
                'item_table_name' => 'facetoface_signups',
                'info_table_prefix' => 'facetoface_cancellation',
                'info_data_item_id_key' => 'facetofacecancellationid',
            ];
        case 'organisation':
            return [
                'item_table_name' => 'org',
                'info_table_prefix' => 'org_type',
                'info_data_item_id_key' => 'organisationid',
                'info_field_type_id_key' => 'typeid',
                'item_type_id_key' => 'typeid',
            ];
        case 'position':
            return [
                'item_table_name' => 'pos',
                'info_table_prefix' => 'pos_type',
                'info_data_item_id_key' => 'positionid',
                'info_field_type_id_key' => 'typeid',
                'item_type_id_key' => 'typeid',
            ];
        default:
            throw new coding_exception("Unsupported item type `{$item_type}`");
    }
}

/**
 * Retrieves custom fields and their respective data for an item
 *
 * @param string $item_type The item the custom field is defined for (e.g courses).
 * To use this method the definition in `customfield_get_item_information()` needs to have been defined
 * @param array $item_ids The ids of the items to get the custom fields for.
 *
 * @return array An associative array, keyed by the item ID and the value for each key being an array of custom field data
 * @throws coding_exception
 * @throws dml_exception
 */
function customfield_get_custom_fields_and_data_for_items(string $item_type, array $item_ids): array {
    global $DB;
    ...
    $item_ids = array_map(function ($id) {
        return intval($id);
    }, $item_ids);

    $item_information = customfield_get_item_information($item_type);
    $item_table_name = $item_information['item_table_name'];
    $info_table_prefix = $item_information['info_table_prefix'];
    $info_data_item_id_key = $item_information['info_data_item_id_key'];
    $info_field_type_id_key = $item_information['info_field_type_id_key'] ?? null;
    $item_type_id_key = $item_information['item_type_id_key'] ?? null;

    $item_custom_fields = [];

    // If custom fields are separated by a type id, then we filter by type ID
    $type_sql = '';
    if ($info_field_type_id_key !== null) {
        $type_sql = "AND info_field.{$info_field_type_id_key} = item.{$item_type_id_key}";
    }

    // We chunk the item_ids up so that we don't hit an sql error with too many ids used in the IN clause
    // This can happen if a request comes in with a limit larger than what the DB can handle.
    // We allow this to continue but log a debug message to warn the admin of this issue
    $items_count = count($item_ids);
    $max_in_params = $DB->get_max_in_params();
    if ($items_count > $max_in_params) {
        debugging("The number of parameters passed ({$items_count}) exceeds maximum number allowed ({$max_in_params}). Query will be chunked, consider calling with less items", DEBUG_DEVELOPER);
    }

    $item_id_sets = array_chunk($item_ids, $max_in_params);
    foreach ($item_id_sets as $item_id_set) {
        list($in_sql, $params) = $DB->get_in_or_equal($item_id_set);
        $sql = "SELECT item.id item_id, info_field.id field_id, info_data.id data_id,
      info_field.fullname, info_field.shortname, info_field.datatype, info_field.description,
      info_field.defaultdata, info_data.data
      FROM {{$item_table_name}} item
      CROSS JOIN {{$info_table_prefix}}_info_field info_field
      LEFT JOIN {{$info_table_prefix}}_info_data info_data ON info_data.{$info_data_item_id_key} = item.id
          AND info_data.fieldid = info_field.id
      WHERE item.id {$in_sql}
      {$type_sql}
      ORDER BY item.id, info_field.sortorder";

        $custom_fields = $DB->get_recordset_sql($sql, $params, 0, 0);

        foreach ($custom_fields as $custom_field) {
            $item_id = $custom_field->item_id;

            if (!isset($item_custom_fields[$item_id])) {
                $item_custom_fields[$item_id] = [];
            }
            ...
            $item_custom_fields[$item_id][] = [
                'definition' => [
                    ...
                    'raw_default_value' => $custom_field->defaultdata,
                    'default_value' => $custom_field->defaultdata,
                ],
                'value' => $custom_field->data,
                'raw_value' => $custom_field->data,
            ];
        }
        $custom_fields->close();
    }
