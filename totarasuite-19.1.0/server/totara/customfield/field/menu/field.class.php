...
class customfield_menu extends customfield_base {
    ...
    function __construct($fieldid, $item, $prefix, $tableprefix, $addsuffix = false, $suffix = '') {
        ...
        // Set default value to the choosedots option if '' or null.
        $this->datakey = ($this->data === '' || $this->data === null)
            ? ''
            : (int)array_search($this->data, $this->options);
    }
    ...
    public function edit_field_add(&$mform) {
        ...
        if ($this->itemid != 0 && $this->is_locked()) {
            $data = ($this->data === '' || $this->data === null)
                ? get_string('readonlyemptyfield', 'totara_customfield')
                : format_string($this->data);
