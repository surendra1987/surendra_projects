...
class engage_article_multi_tenancy_contribution_loader_of_tenant_member_test extends \core_phpunit\testcase {
    /***
     * @var stdClass|null
     */
    private $system_user;
    ...
    public function test_loading_shared_resources_by_system_user_without_isolation(): void {
        ...
        $article_generator->share_article($public_article, [new user($this->tenant_user->id)]);
     ...
     * @return void
     */
    public function test_loading_shared_resources_by_tenant_participant_without_isolation(): void {
        $article_generator = $this->get_article_generator();
        $public_article = $article_generator->create_public_article(['userid' => $this->tenant_participant->id]);
     ...
     * @return void
     */
    public function test_loading_shared_resource_by_tenant_participant_with_isolation(): void {
        $article_generator = $this->get_article_generator();
        $public_article = $article_generator->create_public_article(['userid' => $this->tenant_participant->id]);
    ...
    public function test_loading_shared_resources_for_suspended_tenant_member(): void {
        global $CFG;
        $article_generator = $this->get_article_generator();
        $public_article = $article_generator->create_article([
            'userid' => $this->tenant_user->id,
            'access' => access::PUBLIC
        ]);

        $article_generator->share_article($public_article, [(new user($this->tenant_participant->id)),
            (new user($this->system_user->id))
        ]);

        $query = new query();
        $query->set_component('totara_engage');
        $query->set_area('shared');
        $query->set_share_recipient_id($this->tenant_participant->id);

        $this->setUser($this->tenant_participant);
        require_once("{$CFG->dirroot}/user/lib.php");
        user_suspend_user($this->tenant_user->id);
        ...
        // We still expect to fetch a shared resource even if the tenant member user is suspended.
        self::assertEquals(1, $cards->count());
    }

    public function test_loading_shared_resources_for_suspended_tenant_participant(): void {
        global $CFG;
        // Let's make a new tenant member and participant for fresh testing.
        $tenant_generator = self::getDataGenerator()->get_plugin_generator('totara_tenant');
        $tenant_generator->enable_tenants();
        $tenant2 = $tenant_generator->create_tenant();
        $tenant_member = self::getDataGenerator()->create_user(['tenantid' => $tenant2->id,]);
        $tenant_participant = self::getDataGenerator()->create_user();
        $tenant_generator->set_user_participation($tenant_participant->id, [$tenant2->id]);

        $article_generator = $this->get_article_generator();
        $public_article = $article_generator->create_article([
            'userid' => $tenant_participant->id,
            'access' => access::PUBLIC
        ]);
        $article_generator->share_article($public_article, [new user($tenant_member->id),
            new user($this->system_user->id)
        ]);
        $query = new query();
        $query->set_component('totara_engage');
        $query->set_area('shared');
        $query->set_share_recipient_id($tenant_member->id);

        $this->setUser($tenant_member);
        require_once("{$CFG->dirroot}/user/lib.php");
        user_suspend_user($tenant_participant->id);
        ...
        // We still expect to fetch a shared resource even if the tenant participant user is suspended.
        self::assertEquals(1, $cards->count());
    }
}