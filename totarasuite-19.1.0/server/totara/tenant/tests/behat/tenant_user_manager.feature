@totara @tenant @totara_tenant @javascript @totara_reportbuilder
...
#       what tenants are there just by looking at course categories anyway.
    ...
    And I set the following Totara form fields to these values:
      ...
      | Tenant users audience name | First T Audience |
    ...
    And I set the field "addselect" to "Tenant User Manager"
    And I press "Add"
    And I navigate to "Tenant users" node in "First tenant"
    And I log out
    ...
    And I press "Create tenant member"
    ...
    Then I should see "Tenant users"
    ...
    And I click on "Tenant User Manager" "link" in the "Tenant User Manager" "table_row"
    And I set the field "addselect" to "Testovaci Uzivatel"
    And I press "Add"
    Then I navigate to "Tenant users" node in "First tenant"
  ...
  Scenario: Tenant domain manager with system capabilities cannot edit or delete user from different tenant
    ...
    And the following "role assigns" exist:
      ...
      | t1user      | apiuser | System       |           |
    And the following "permission overrides" exist:
      | capability                           | permission | role          | contextlevel | reference |
      | moodle/user:update                   | Allow      | apiuser       | System       |           |
      | moodle/user:delete                   | Allow      | apiuser       | System       |           |

    When I log in as "t1user"
    And I am on profile page for user "t2user"
    And I should see "The details of this user are not available to you"
    And I click on "[aria-label='Show admin menu window']" "css_element"
    And I click on "Users" "link"
    And "[data-flex-icon='settings']" "css_element" should not exist in the "t2user" "table_row"
    And "[data-flex-icon='delete']" "css_element" should not exist in the "t2user" "table_row"
