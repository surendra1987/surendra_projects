...
/**
  * Webhook filter factory class
  */
class totara_webhook_filter_factory implements filter_factory {
    /**
     * @inheritDoc
     */
    public function create(string $key, $value, ?int $user_id = null): ?filter {
        switch ($key) {
            case 'name':
                return static::create_name_filter($value);
            case 'endpoint':
                return static::create_endpoint_filter($value);
    ...
    /**
     * Returns an instance of a created_at after filter for Webhook.
     *
    ...
    public static function create_created_at_after_filter(string $value): filter {
        return (new greater_equal_than_filter('created_at'))
            ->set_value($value)
            ->set_entity_class(totara_webhook::class);
    }

    /**
     * Returns an instance of a updated_at after filter for Webhook.
     *
     * @param string $value the matching value(s).
     *
     * @return filter the filter instance.
    ...
    /**
     * Returns an instance of a name filter for Webhook.
     *
     * @param string $value the matching value(s).
     *
     * @return filter the filter instance.
    ...
    public static function create_name_filter(string $value): filter {
        return (new like_filter('name'))
            ->set_value($value)
            ->set_entity_class(totara_webhook::class);
    }

    /**
     * Returns an instance of a endpoint filter for Webhook.
     *
     * @param string $value the matching value(s).
     *
     * @return filter the filter instance.
     */
    public static function create_endpoint_filter(string $value): filter {
        return (new like_filter('endpoint'))
            ->set_value($value)
            ->set_entity_class(totara_webhook::class);
    }
