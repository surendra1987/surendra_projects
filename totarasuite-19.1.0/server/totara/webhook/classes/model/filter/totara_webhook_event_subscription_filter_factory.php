...
/**
  * Webhook Event Subscription filter factory class
  */
class totara_webhook_event_subscription_filter_factory implements filter_factory {
    /**
     * @inheritDoc
     */
    public function create(string $key, $value, ?int $user_id = null): ?filter {
        switch ($key) {
            case 'event':
                return static::create_event_filter($value);
    ...
    /**
     * Returns an instance of a created_at after filter for Webhook Event Subscription.
     *
    ...
    public static function create_created_at_after_filter(string $value): filter {
        return (new greater_equal_than_filter('created_at'))
            ->set_value($value)
            ->set_entity_class(totara_webhook_event_subscription::class);
    }

    /**
     * Returns an instance of a updated_at after filter for Webhook Event Subscription.
     *
     * @param string $value the matching value(s).
     *
     * @return filter the filter instance.
    ...
    /**
     * Returns an instance of a event filter for Webhook Event Subscription.
     *
     * @param string $value the matching value(s).
     *
     * @return filter the filter instance.
     */
    public static function create_event_filter(string $value): filter {
        return (new like_filter('event'))
            ->set_value($value)
            ->set_entity_class(totara_webhook_event_subscription::class);
    }
