@totara @totara_mobile @totara_program @_file_upload @javascript @editor @editor_weka
Feature: Test the totara_mobile_program query
  ...
  Scenario: Test the query with a program that has JSON summary and endnote
    ...
    And I click on "Save changes" "button"
    And I activate the weka editor with css "#fitem_id_summary_editor .tui-weka"
    And I select the text "HTML" in the weka editor
    And I replace the selection with "JSON" in the weka editor
    Then I should not see "HTML" in the "#fitem_id_summary_editor .tui-weka" "css_element"
    And I activate the weka editor with css "#fitem_id_endnote_editor .tui-weka"
    And I select the text "LMTH" in the weka editor
    And I replace the selection with "NOSJ" in the weka editor
    Then I should not see "LMTH" in the "#fitem_id_endnote_editor .tui-weka" "css_element"
    And I click on "Save changes" "button"
    And I log out
    When I am using the mobile emulator
    Then I should see "Device emulator loading..."
