...
/**
 * Tests the mobile_completedlearning item type resolver
 */
class mobile_completedlearning_webapi_resolver_type_item_test extends \core_phpunit\testcase {

    use webapi_phpunit_helper;

    private function resolve($field, $learning_item, array $args = []) {
        return $this->resolve_graphql_type('mobile_completedlearning_item', $field, $learning_item, $args);
    }
    ...
    private function get_learning_items(int $userid) {
        ...
        foreach ($items as $item) {
            ...
            $item->download_friendly = $item->get_download_friendly();
    ...
    public function test_resolve_download_friendly(): void {
        ...
        // Program.
        $item = array_pop($items);
        $value = $this->resolve('download_friendly', $item);
        $this->assertFalse($value);
