...
use core_phpunit\testcase;use mobile_findlearning\filter_handler;
 ...
 */
class mobile_findlearning_mobile_webapi_resolver_query_filter_catalog_v2_test extends testcase {

    use webapi_phpunit_helper;

    protected const QUERY = 'mobile_findlearning_filter_catalog_v2';
    ...
    public function test_resolve_guest_user_with_filter() {
        ...
        if (builder::get_db()->get_dbfamily() == 'mssql') {
            $this->markTestSkipped('MSSQL does not return result as consistency with fts.');
        }
        $this->create_faux_catalog_items();
        $this->setGuestUser();
        ...
        $result = $this->resolve_graphql_query(
            ...
            [
                'limit_from' => 0,
                'filter_data' => [
                    'filter_structure' => '{ "catalog_fts" : "beta"}'
                ]
            ]
        );
    ...
    public function test_embedded_query() {
        ...
        $result = \totara_webapi\graphql::execute_operation(
            \core\webapi\execution_context::create('mobile', self::QUERY),
            [
                'limit_from' => 0,
                'filter_data' => []
            ]
     ...
     * Test the results of the embedded mobile query through the GraphQL stack.
     */
    public function test_embedded_query_with_filter_fallback() {
        // MSSQL does not return results consistently with fts.
        if (builder::get_db()->get_dbfamily() == 'mssql') {
            $this->markTestSkipped('MSSQL does not return results consistently with fts.');
        }
        $users = $this->create_faux_catalog_items();
        $this->setUser($users['u1']->id);

        $config = config::instance();
        $config->update(['search_fallback' => config_general::SEARCH_FALLBACK_NO_RESULTS]);
        $result = \totara_webapi\graphql::execute_operation(
            \core\webapi\execution_context::create('mobile', self::QUERY),
            [
                'limit_from' => 0,
                'filter_data' => [
                    'filter_structure' => '{ "catalog_fts" : "bet"}'
                ]
            ]
        );
        ...
        // Disable fallback
        $config->update(['search_fallback' => config_general::SEARCH_FALLBACK_NONE]);
        $result = \totara_webapi\graphql::execute_operation(
            \core\webapi\execution_context::create('mobile', self::QUERY),
            [
                'limit_from' => 0,
        ...
        $this->assertEmpty($data['catalogPage']['items']);
    }
    ...
    public function test_resolve_filters(): void {
        ...
        $result = $this->execute_graphql_operation(
            ...
            [
                'limit_from' => 0,
                'filter_data' => [
                    'filter_structure' => '{ "catalog_learning_type_panel" : ["playlist"]}'
                ]
            ],
            \totara_webapi\graphql::TYPE_MOBILE);

        $data = $result->data;
        $items = $data['catalogPage']['items'];
        foreach ($items as $item) {
            self::assertEquals('playlist', $item['itemType']);
        }
        ...
        foreach ($options as $option) {
            ...
            if ($option['id'] == 'playlist') {
                // Playlist is selected, active must be true.
