...
class appraisal_answer_form extends moodleform {
    ...
    public function definition_after_data() {
        ...
        if ($button) {
            if (appraisal::is_read_only()) {
                $action_button = new single_button(
                    new moodle_url('/totara/appraisal/index.php'),
                    get_string($button, 'totara_appraisal')
                );
                $action_button->disabled = true;
                $mform->addElement('html', $OUTPUT->render($action_button));
            } else {
