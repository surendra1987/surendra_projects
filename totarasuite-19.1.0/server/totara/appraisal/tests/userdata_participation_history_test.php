...
use totara_appraisal\userdata\participation_history;
 ...
 * vendor/bin/phpunit --verbose totara_appraisal_userdata_participation_history_test totara/appraisal/tests/userdata_participation_history_test.php
 */
class totara_appraisal_userdata_participation_history_test extends appraisal_testcase {
    ...
    public function test_get_compatible_context_levels() {
        $this->assertEquals(array(CONTEXT_SYSTEM), participation_history::get_compatible_context_levels());
    }
    ...
    public function test_is_purgeable() {
        $this->assertFalse(participation_history::is_purgeable(target_user::STATUS_ACTIVE));
        $this->assertFalse(participation_history::is_purgeable(target_user::STATUS_SUSPENDED));
        $this->assertFalse(participation_history::is_purgeable(target_user::STATUS_DELETED));
    }
    ...
    public function test_is_exportable() {
        $this->assertTrue(participation_history::is_exportable());
    }
    ...
    public function test_is_countable() {
        $this->assertTrue(participation_history::is_countable());
    }
    ...
    public function test_count() {
        ...
        // Do the count.
        $result = participation_history::execute_count($data->targetuser, context_system::instance());
        $this->assertEquals(2, $result);

        $result = participation_history::execute_count(new target_user($data->user1), context_system::instance());
    ...
    public function test_export() {
        ...
        $expected->data = $DB->get_records_select('appraisal_role_changes', $select,
            array('userid1' => $data->targetuser->id, 'userid2' => $data->targetuser->id));

        $actual = participation_history::execute_export($data->targetuser, context_system::instance());
