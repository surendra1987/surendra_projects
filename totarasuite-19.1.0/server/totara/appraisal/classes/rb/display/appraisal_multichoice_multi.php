...
class appraisal_multichoice_multi extends base {
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        $result = array();
        foreach ($ids as $id) {
            // If there's an invalid value we don't have the name for make sure we log that
            if (!isset(\rb_source_appraisal_detail::$appraisalmultichoicenamecache[$report->src->appraisalid][$id])) {
                debugging('Missing cache value in rb_source_appraisal_detail::$appraisalmultichoicenamecache for value "'.$id.'"', DEBUG_ALL);
                continue;
            }
