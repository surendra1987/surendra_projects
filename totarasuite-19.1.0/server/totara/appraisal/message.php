...
switch ($action) {
    case 'edit':
        if (empty($messageid)) {
            appraisal::read_only_debugging('Create new message of legacy appraisal');
        }

        $form = new appraisal_message_form(null, array('appraisalid' => $id, 'messageid' => $messageid, 'readonly' => !$isdraft || $isreadonly));
        ...
        if ($isdraft && $data = $form->get_data()) {
            ...
            $msg->save();
            notification::success(get_string('messagesaved', 'totara_appraisal'));
            redirect($returnurl);
    ...
    case 'delete':
        ...
        if ($messageid && $confirm) {
            ...
            notification::success(get_string('messagedeleted', 'totara_appraisal'));
            redirect($returnurl);
        }
        break;
}
