 ...
 */
class scheduled_rule_name extends base {
    ...
    public static function display($value, $format, stdClass $row, rb_column $column, reportbuilder $report): string {
        ...
        if ($format !== 'html') {
            return self::to_plaintext(format_string($extra->name));
        }

        $url = new moodle_url('/totara/useraction/scheduled_actions.php');
        return html_writer::link($url->out(true), format_string($extra->name));
    }
