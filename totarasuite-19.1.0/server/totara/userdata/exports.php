...
admin_externalpage_setup('userdataexports');

$config = (new rb_config())->set_sid($sid);
$report = reportbuilder::create_embedded('userdata_exports', $config);
...
echo $output->page_main_heading(get_string('sourcetitle', 'rb_source_userdata_exports'));
...
foreach ($params as $param) {
    ...
    if ($param->name === 'exporttypeid') {
        $exporttype = $DB->get_record('totara_userdata_export_type', array('id' => $param->value));
        if ($exporttype) {
            $paraminfo[] = get_string('exporttype', 'totara_userdata') . ': ' . format_string($exporttype->fullname);
        }
    }
}
if ($paraminfo) {
