...
final class util {
    ...
    public static function graphql_error_formatter(Throwable $error): array {
        $category = client_aware_exception::CATEGORY_INTERNAL; // Errors default to internal
        ...
        if (client_aware_exception_helper::exception_registered($error)) {
            ...
            // When the previous error is registered as client_aware, then return the formatted error
    ...
    public static function graphql_error_handler(array $errors, callable $formatter) {
        $debug = static::retrieve_debug_flag($formatter);
        foreach ($errors as $error) {
            /** @var Throwable $error */
            $prev = $error->getPrevious();
            if (!$prev) {
                if ($debug === DebugFlag::INCLUDE_DEBUG_MESSAGE) {
                    static::overwrite_exception_message($error);
                }
                continue;
            }
            self::log_exception($prev);
            // we only want the debug message if the debug flag INCLUDE_TRACE and above
            if ($debug === DebugFlag::INCLUDE_DEBUG_MESSAGE) {
                static::overwrite_exception_message($error);
            }
        }
    ...
    /**
     * Used when trying to figure out the external API client's debug mode
     *
     * @param callable $formatter
     * @return int
     * @throws \ReflectionException
     */
    private static function retrieve_debug_flag(callable $formatter): int {
        $formatter_callable = $formatter;
        if (is_array($formatter)) {
            $rf = new \ReflectionMethod(...$formatter);
        } else {
            $rf = new ReflectionFunction($formatter_callable);
        }
        $formatter_vars = $rf->getStaticVariables();
        $debug_flag = $formatter_vars['debug'] ?? DebugFlag::NONE;
        return intval($debug_flag);
    }
    ...
    private static function overwrite_exception_message(Throwable $exception): void {
        ...
        try {
            ...
            $error_ref_msg->setAccessible(true);
