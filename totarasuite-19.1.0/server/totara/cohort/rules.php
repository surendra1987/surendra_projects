...
echo $OUTPUT->page_main_heading(format_string($cohort->name));
...
function display_approval_action_box($cohortid, $debug=false, $style='display:block') {
    ...
    $message = html_writer::start_tag('form', $attrs);
    $message .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'id', 'value' => $cohortid));
    $message .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
    if ($debug) {
        $message .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'debug', 'value' => 1));
    }
    $message .= html_writer::start_tag('strong');

    $message .= get_string('cohortruleschanged', 'totara_cohort');
    $message .= html_writer::end_tag('strong');
    if (get_config('cohort', 'applyinbackground')) {
        $message .= html_writer::start_tag('p');
        $message .= get_string('cohortruleschangedadhocnote', 'totara_cohort');
        $message .= html_writer::end_tag('p');
    } else {
        $message .= html_writer::start_tag('p');
        $message .= get_string('cohortruleschangednote', 'totara_cohort');
        $message .= html_writer::end_tag('p');
    }
    $attrs = array('type' => 'submit', 'name' => 'approverulechanges', 'value' => get_string('approvechanges', 'totara_cohort'));
    $message .= html_writer::empty_tag('input', $attrs);
    $attrs = array('type' => 'submit', 'name' => 'cancelrulechanges', 'value' => get_string('cancelchanges', 'totara_cohort'));
    $message .= html_writer::empty_tag('input', $attrs);

    $message .= html_writer::end_tag('form');
    $format = new stdClass();
    $format->message = $OUTPUT->container($message);
    echo $OUTPUT->render_from_template('core/notification_warning', $format);
