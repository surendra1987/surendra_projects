...
class rb_source_cohort_orphaned_users extends rb_source_user {
            ...
            where
                not exists (
                    ...
                    from {cohort_members} cm
                        inner join {cohort} c
                        on cm.cohortid=c.id ####tenant_report####
                    where
                        cm.userid=u.id
    ...
    public function post_params(reportbuilder $report) {
        ...
        if (!empty($report->tenantid)) {
            $alias = uniqid('chm_');
            $tenant = \core\record\tenant::fetch($report->tenantid);
            $context_id = $tenant->category_context->id;

            $tenant_sql = "INNER JOIN {context_map} $alias
                    ON $alias.childid != c.contextid AND $alias.parentid = $context_id and cm.cohortid != $tenant->cohortid";
        }
        $base = str_replace('####tenant_report####', $tenant_sql, $report->src->base);
        $report->src->base = $base;
    }
}
