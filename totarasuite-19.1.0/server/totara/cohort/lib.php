...
use totara_program\exception\manager;
    ...
    if ($value == COHORT_ASSN_VALUE_ENROLLED) {
        switch ($instancetype) {
            ...
            case COHORT_ASSN_ITEMTYPE_PROGRAM:
                ...
                $todb->assignmenttype = assignments::ASSIGNTYPE_COHORT;
    ...
    switch ($instancetype) {
        ...
        case COHORT_ASSN_ITEMTYPE_CERTIF:
            ...
            if (!empty($record)) {
                ...
                manager::delete_exceptions_by_assignment($assid);
    ...
    switch ($instancetype) {
        ...
        case COHORT_ASSN_ITEMTYPE_PROGRAM:
            if ($value == COHORT_ASSN_VALUE_VISIBLE) {
                ...
                $sqlparams = array(assignments::ASSIGNTYPE_COHORT, $cohortid);
            }
            break;
        case COHORT_ASSN_ITEMTYPE_CERTIF:
            if ($value == COHORT_ASSN_VALUE_VISIBLE) {
        ...
        default:
            ...
            if ($value == COHORT_ASSN_VALUE_VISIBLE) {
                ...
                $sql = "SELECT " .$DB->sql_concat("'c'", 'c.id') . " AS instanceid, c.id, c.fullname AS fullname, 'course' AS type
                    ...
                    WHERE pa.assignmenttype = ?
                    AND pa.assignmenttypeid = ?";
                $sqlparams = array($cohortid, assignments::ASSIGNTYPE_COHORT, $cohortid);
            }
            break;
    }
...
class cohort {
     ...
     * @return string
     */
    public static function display_goal_table($cohort, $can_edit, $display_warning = false, $warning_message = '') {
        global $OUTPUT;
        ...
        if ($display_warning) {
            // This warning attempts to match the mustache template for 'notification_warning'.
            $out .= html_writer::start_tag('div', ['class' => 'alert alert-warning alert-with-icon fade in']);
            $out .= html_writer::start_tag('div', ['class' => 'alert-icon']);
            $out .= html_writer::start_tag('span', [
                'aria-hidden' => 'true',
                'data-flex-icon' => 'core|notification-warning',
                'class' => 'flex-icon ft-fw ft tfont-var-exclamation-triangle-fill ft-size-200'
                ]
            );
            $out .= html_writer::end_tag('span');
            $out .= html_writer::end_tag('div');
            $out .= html_writer::nonempty_tag('div', $warning_message, ['class' => 'alert-message']);
            $out .= html_writer::end_tag('div');
            $out .= html_writer::end_tag('div');
        }
 ...
 * @param $removeoldmembers integer The new value for the option to enable / disable new members being removed from the audience.
 ...
 * @param bool $force If set to true, this function will be run even if it has already been run in this request.
 * @return void
 */
function totara_cohort_check_and_update_dynamic_cohort_members($courseid, progress_trace $trace, $cohortid = NULL, bool $force = false) {
    global $DB;

    if (!$force && defined('TOTARA_COHORT_SYNCED')) {
        // We have already run this for all courses+cohorts in this request (hence the define), and we're not being forced to run it again.
        return;
    }

    // Attempt to acquire a lock with a one-second time out, and a maximum lifetime of 30 minutes.
    $lockfactory = \core\lock\lock_config::get_lock_factory('totara_cohort_update_dynamic');
    $key = 'updating_' . (string)(int)$courseid . '_' . (string)(int)$cohortid;
    $lock = $lockfactory->get_lock($key, 1, 1800);
    if (!$lock) {
        // We failed to get the lock within one second, this process must already be running!
        return;
    }
    // OK, we have the lock. We can run the task.
    ...
    if (empty($courseid) and empty($cohortid)) {
        ...
        if (!defined('TOTARA_COHORT_SYNCED') && !PHPUNIT_TEST) {
            define('TOTARA_COHORT_SYNCED', true);
        }
    }
    $lock->release();
}
...
function totara_cohort_program_completion_link($cohortid, $programid){
    ...
    $item = $DB->get_record_sql(
        "SELECT pa.assignmenttypeid AS id,
                     pa.completiontime,
                     pa.completionevent,
                     pa.completioninstance,
                     pa.completionoffsetamount,
                     pa.completionoffsetunit,
                     p.availableuntil
                FROM {prog_assignment} pa
                JOIN {prog} p
                  ON p.id = pa.programid
               WHERE pa.assignmenttypeid = :assignmenttypeid
                 AND pa.programid = :programid
                 AND pa.assignmenttype = :assignmenttype",
        [
            'assignmenttypeid' => $cohortid,
            'programid' => $programid,
    ...
    if (!$item) {
        ...
        $can_update = !($item->availableuntil > 0 && $item->availableuntil < time());
    }
    return $cat->get_completion($item, $programid, $can_update);
}
