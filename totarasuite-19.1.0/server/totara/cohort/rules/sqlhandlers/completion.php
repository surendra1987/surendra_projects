        ...
        $sqlhandler->sql = "{$goalnum} {$operator}
                  ...
                  SELECT count(*)
                   ...
                   WHERE pc.userid = u.id
                     ...
                     AND pc.status = " . program::STATUS_PROGRAM_COMPLETE . "
...
class cohort_rule_sqlhandler_course_completion_history_list extends cohort_rule_sqlhandler_completion_list {
    ...
    protected function construct_sql_snippet($goalnum, $operator, $lov) {
        ...
        $sqlhandler->sql = "{$goalnum} {$operator}
                  (
                  SELECT count(DISTINCT cch.courseid)
            ...
            $sqlchunks[] = "(
                EXISTS (
                    SELECT 1
                      ...
                      JOIN {certif_completion} cc ON cc.certifid = c.id
                     WHERE p.id = {$certifid}
                       AND cc.userid = u.id AND cc.timecompleted != 0 AND  cc.timecompleted {$comparison}
                 )
                 OR
                 EXISTS (
                    SELECT 1
                      FROM {prog} p
                      JOIN {certif} c ON c.id = p.certifid
                      JOIN {certif_completion_history} cch ON cch.certifid = c.id
                     WHERE p.id = {$certifid}
                       AND cch.userid = u.id AND cch.timecompleted != 0 AND cch.timecompleted {$comparison}
                 )
            )";
