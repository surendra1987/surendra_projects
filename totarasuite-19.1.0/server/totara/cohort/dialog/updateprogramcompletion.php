...
$data->id = $programid;
$data->item = array(
    assignments::ASSIGNTYPE_COHORT => array(
        $cohortid=>1,
    )
);
...
if ($completionevent != assignments::COMPLETION_EVENT_NONE) {
    ...
    if (!empty($num) && !empty($unit)) {
        $data->completionoffsetamount = [assignments::ASSIGNTYPE_COHORT => [$cohortid => $num]];
        $data->completionoffsetunit = [assignments::ASSIGNTYPE_COHORT => [$cohortid => $unit]];
        $data->completiontime = [assignments::ASSIGNTYPE_COHORT => [$cohortid => null]];
    }
}
