 ...
 *
 */
class totara_cohort_position_rules_test extends \core_phpunit\testcase {

    protected $users = null;
    protected $userspos = null;
    private $pos1 = null;
    private $pos2 = null;
    private $pos3 = null;
    private $pos4 = null;
    ...
    public function setUp(): void {
        ...
        for ($i = 1; $i <= self::TEST_POSITION_COUNT_MEMBERS; $i++) {
            ...
            \totara_job\job_assignment::create_default($this->users[$i]->id, $data);
            array_push($this->userspos[$pos], $this->users[$i]->id);
        }

        $this->userspos['pos1'] = array_flip($this->userspos['pos1']);
        $this->userspos['pos2'] = array_flip($this->userspos['pos2']);
        $this->userspos['pos3'] = array_flip($this->userspos['pos3']);
    ...
    public function test_position_name_rule() {
        ...
        $this->assertEmpty(array_diff_key(array_flip($members), $this->userspos['pos1']));
    }
     ...
     */
    public static function data_position_date_rules() {
    ...
    public function test_position_date_rules($rulename, $params, $usercount, $sourcemembersmatched) {
        ...
        foreach ($sourcemembersmatched as $pos) {
            $usersinposition = $usersinposition + $this->userspos[$pos];
        }
     ...
     */
    public static function data_manager_hierarchy() {
        $data = array(
     ...
     */
    public static function data_position_hierarchy() {
        $data = array(
    ...
    public function test_position_rule($params, $positions, $usercount, $sourcemembersmatched) {
        ...
        $this->userspos['pos4'] = array_flip(array($newuser1->id, $newuser4->id, $newuser5->id));
        ...
        foreach ($sourcemembersmatched as $pos) {
            $membersincohort = $membersincohort + $this->userspos[$pos];
        }
    ...
    public function __get(string $name) {
        if (preg_match('/^user(\d+)$/', $name, $matches)) {
            return $this->users[$matches[1]] ?? null;
        }
