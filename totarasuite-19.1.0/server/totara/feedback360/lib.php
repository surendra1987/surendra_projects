...
class feedback360 {
    ...
    public function save_answers(stdClass $formdata, feedback360_responder $resp) {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Save answer');
    ...
    public static function duplicate($feedback360id) {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Clone feedback 360');
    ...
    public function activate() {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Activate feedback');
        // Only draft and closed feedback can be activated
    ...
    /**
     * Debuging code for T18
     *
     * @param null|string $functionname
     * @return void
     * @throws moodle_exception
     */
    public static function read_only_debugging($functionname = null) {
        ...
        if (!empty($functionname) && !$is_test) {
            debugging($functionname . ' has been deprecated since Totara 18.0.', DEBUG_DEVELOPER);
            print_error('feedback360readonly', 'totara_feedback360');
        }
    }
    ...
    /**
     * Since Totara 18 this always returns true.
     *
     * @return bool
     */
     ...
     * @throws moodle_exception
     */
    public static function check_read_only_enabled() {
        if (self::is_read_only()) {
            print_error('error:readonly_enabled', 'totara_feedback360');
        }
    }

    public static function add_read_only_notification(bool $is_read_only, bool $is_draft) {
        if ($is_read_only && $is_draft) {
            $key = manager::get_adhoc_tasks(close_feedback360_task::class)
                ? 'readonly_adhoc_scheduled_admin_activity'
                : 'legacy_feedback_read_only';

            notification::info(get_string($key, 'totara_feedback360'));
        }
    }
}
...
class feedback360_question extends question_storage {
    ...
    public function save() {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Save feedback 360 question');
    ...
    public function duplicate($feedback360id) {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Duplicate legacy feedback 360 question');
    ...
    public static function reorder($questionid, $pos) {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Reorder the position');
...
class feedback360_responder {
    ...
    public function save() {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Save response assignment');
    ...
    public static function update_system_assignments($new, $cancel, $userformid, $duedate, $asmanager = false) {
        // Check if 360 feedback is read only
        feedback360::read_only_debugging('Create the records for feedback360_resp_assignment');
        global $DB, $CFG, $USER;
