...
if ($USER->id == $userid) {
    ...
    $PAGE->set_privacy_aware_title(get_string('userxfeedback360', 'totara_feedback360', get_string('userx', 'moodle', $user->id)));
...
if (feedback360::is_read_only()) {
    $key = manager::get_adhoc_tasks(close_feedback360_task::class)
        ? 'readonly_adhoc_scheduled_user_table'
        : 'legacy_feedback_read_only_allfeedback';
