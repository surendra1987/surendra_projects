...
switch ($action) {
    ...
    case 'edit':
        if ($mnewform->is_submitted()) {
            ...
            if (!$newelement) {
                notification::error(get_string('error:choosedatatype', 'totara_question'));
                redirect($returnurl);
            }
        }
        ...
        $preset = array('question' => $question, 'feedback360id' => $feedback360->id, 'readonly' => !$isdraft || $isreadonly);
        ...
        if ($isdraft && $fromform = $meditform->get_data()) {
            feedback360::read_only_debugging('Creating content of feedback 360');
            $question->set($fromform)->save();
            if (is_ajax_request($_SERVER)) {
                ajax_result();
                return;
            } else {
                notification::success(get_string('contentupdated', 'totara_feedback360'));
                redirect($returnurl);
            }
        } else {
        ...
        break;
    case 'delete':
        feedback360::read_only_debugging('Deleting content of feedback 360');
        if ($question->id < 1) {
            notification::error(get_string('error:elementnotfound', 'totara_question'));
            redirect($returnurl);
        }
        ...
        if ($confirm == 1) {
            ...
            notification::success(get_string('deletedquestion', 'totara_question'));
            redirect($returnurl);
        }
        break;
    case 'clone':
        feedback360::read_only_debugging('Duplicating content of feedback 360');
