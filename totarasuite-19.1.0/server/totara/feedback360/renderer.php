...
class totara_feedback360_renderer extends plugin_renderer_base {
    ...
    public function feedback360_manage_table($feedback360s = array(), $userid = null) {
        ...
        foreach ($feedback360s as $feedback360) {
            ...
            $canmanagefeedback360 = has_capability('totara/feedback360:managefeedback360', $systemcontext, $userid);
            $row = array();
            // 360 feedback name
            $row[] = $canmanagefeedback360 ? html_writer::link($editurl, $name) : $name;
            // Assignments
            ...
            if (has_capability('totara/feedback360:manageactivation', $systemcontext, $userid)) {
                ...
                } else if ($feedback360->status == feedback360::STATUS_DRAFT && !feedback360::is_read_only()) {
    ...
    public function display_assigned_groups($assignments, $itemid) {
        ...
        if (empty($assignments)) {
            ...
            foreach ($assignments as $assign) {
                ...
                if (feedback360::is_draft($feedback360) && !feedback360::is_read_only()) {
    ...
    public function display_feedback_header(feedback360_responder $resp, $subjectuser, $anonymous, $numresponders) {
        ...
        if ($anonmessage && !feedback360::is_read_only()) {
            if ($anonymous) {
    ...
    public function feedback360_additional_actions($status, $id) {
        ...
        } else if (!feedback360::is_read_only() && (feedback360::is_draft($feedback360) || $feedback360->status == feedback360::STATUS_CLOSED)) {
        ...
        foreach ($user_assignments as $user_assignment) {
            ...
            if ($canmanage) {
                if ($user_assignment->status != feedback360::STATUS_ACTIVE) {
                    ...
                    $options = $this->output->single_button($url, get_string('requestfeedback360', 'totara_feedback360'), 'get', $attributes);
                    ...
                    $edit = $this->output->action_icon($editurl, new pix_icon('/t/edit', $editstr, 'moodle'), null, $attributes);
                    ...
                    $remind = $this->output->action_icon($remindurl, new pix_icon('/t/email', $remindstr, 'moodle'), null, $attributes);
                    ...
                    if (!empty($user_assignment->anonymous)) {
                        ...
                        $cancel = $this->output->action_icon($cancelurl, new pix_icon('/t/stop', $strstop, 'moodle'), null, $attributes);
                    }
        ...
        if (empty($resp_assignments)) {
            ...
            foreach ($resp_assignments as $resp_assignment) {
                ...
                if (!empty($completed)) {
                    ...
                    if (!$hidebutton) {
                        $options = $this->output->single_button($answerurl, get_string('answernow', 'totara_feedback360'), 'get', $attributes);
                    } else {
                        $options = '';
                    }
