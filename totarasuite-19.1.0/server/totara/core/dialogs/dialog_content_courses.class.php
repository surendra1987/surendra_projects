...
class totara_dialog_content_courses extends totara_dialog_content {
    ...
    /**
     * Flag to indicate that visibility should be checked
     *
     * @access  public
     * @var     bool
     */
    ...
    public function load_courses($where = false, $userid = 0) {
        ...
        if ($this->categoryid) {
            ...
            if ($where === false) {
                ...
                $this->courses = $coursecat->get_course_records($where, $params, array(), $this->check_visibility);
            } else {
                $options = [
                    'check_visibility' => $this->check_visibility,
                ];
                $this->courses = $coursecat->get_courses($options);
            }
        }
    }
