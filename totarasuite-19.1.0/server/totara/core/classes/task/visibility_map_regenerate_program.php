 ...
 */
class visibility_map_regenerate_program extends \core\task\adhoc_task {

    /**
     * Queues a map update for either all certifications, or the given program.
     *
    ...
    public function get_name() {
        return get_string('task_visibility_map_regenerate_program', 'totara_core');
    }
    ...
    public function execute() {
        ...
        $map = visibility_controller::program()->map();
        $start = microtime(true);
        if (!$quiet) {
            mtrace('Updating program visibility map at '.time() . ' ...');
        }
        if ($instanceid !== null) {
