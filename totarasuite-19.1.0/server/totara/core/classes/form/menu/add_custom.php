...
final class add_custom extends \totara_form\form {
    public function definition() {
        ...
        // Icon picker.
        helper::add_totara_form_icon_picker($this->model, '');

        // Custom CSS class.
        $this->model->add(helper::totara_form_customclass_text());
        ...
        $url->set_attributes(array('maxlength' => 1333));
        $this->model->add($url);
    ...
    public function validation(array $data, array $files) {
        ...
        if ($data['customclass']) {
            if ($error = helper::validate_customclass($data['customclass'], $data['parentid'])) {
                $errors['customclass'] = $error;
            }
        }
