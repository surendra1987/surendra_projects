...
function totara_program_is_viewable($programorid, $userid = null): bool {
    ...
    $now = time();
    $isavailable = (
        $program->available == program::AVAILABILITY_TO_STUDENTS &&
...
function totara_certification_is_viewable($certificationorid, $userid = null): bool {
    ...
    $now = time();
    $isavailable = (
        $certification->available == program::AVAILABILITY_TO_STUDENTS &&
...
function totara_major_version() {
    ...
    if (str_starts_with($TOTARA->version, 'evergreen')) {
        return 'evergreen';
    } else if (preg_match('/^[0-9]+/', $TOTARA->version, $matches)) {
        return $matches[0];
    } else {
        return false;
    }
...
function totara_version_info() {
    ...
    // Also track specific version.
    $a->oldcfgversion = $CFG->version;
    $a->newcfgversion = $version;
    ...
    if ($version < $CFG->version) {
        if (!empty($CFG->totara_release)) {
            ...
            $a->totaraupgradeerror = 'error:cannotupgradefromnewertotara_specific';
            return $a;
...
function totara_icon_url_and_alt($type, $icon, $alt = '') {
    ...
    if (empty($src)) {
        $iconpath = $type . 'icons/';
        $imagelocation = $PAGE->theme->resolve_image_location($iconpath. $icon, $component, true);
...
function get_my_reports_list() {
    $reports = get_my_reports_data();
    return $reports['reports'] ?? [];
}

/**
 * @deprecated since Totara 18.0
 * @return array Contains 'reports' and 'errors'
 */
function get_my_reports_list_and_errors() {
    debugging('get_my_reports_list_and_errors has been deprecated, please call get_my_reports_data instead', DEBUG_DEVELOPER);
    return get_my_reports_data();
}

/**
 * @return array Contains 'reports', 'tenants', and 'errors'
 */
function get_my_reports_data() {
    global $USER;
        ...
        if ($reportrecord->embedded) {
            try {
                ...
                if ($e->errorcode == "nopermission") {
                    ...
                    $reports_with_errors[] = $reportrecord->fullname;
                    unset($reportbuilder_permittedreports[$key]);

                    if (!defined('BEHAT_SITE_RUNNING')) {
                        debugging(
                            sprintf("Embedded report '%s' has errors: %s", $reportrecord->fullname, $e->getMessage()),
                            DEBUG_DEVELOPER,
                            $e->getTrace()
                        );
                    }
    ...
    $tenant_reports = [];
    foreach ($reportbuilder_permittedreports as $key => $report) {
        $tenant_reports[$report->tenantid ?? 0][] = $report;
    }

    if (has_capability('totara/tenant:view', context_system::instance())) {
        $tenants = \core\entity\tenant::repository()
            ->order_by('name')
            ->get()
            ->map(function ($x) {
                return (object)[
        ...
        $report_tenant_ids = array_unique(array_column($reportbuilder_permittedreports, 'tenantid'), SORT_NUMERIC);
        $tenant_ids = array_unique([
            ...\totara_tenant\util::get_user_tenant_ids($USER->id),
            ...$report_tenant_ids
        ], SORT_NUMERIC);
        $tenants = \core\entity\tenant::repository()
            ->where_in('id', $tenant_ids)
            ->order_by('name')
            ->get()
            ->map(function ($tenant) {
                $can_edit = has_capability(
                    'totara/reportbuilder:managereports',
                    context_coursecat::instance($tenant->categoryid)
                );
                return (object)[
                    'id' => $tenant->id,
                    'name' => $tenant->name,
                    'can_edit' => $can_edit,
                    'can_create' => $can_edit && !advanced_feature::is_disabled('user_reports'),
                ];
            })
            ->filter(function ($tenant) use ($report_tenant_ids) {
                // show tenants in the list if you can either create a report in the tenant, or there are reports to view in it
                return $tenant->can_create || in_array($tenant->id, $report_tenant_ids);
            })
            ->key_by('id')
            ->all(true);
    }

    $can_edit = has_capability('totara/reportbuilder:managereports', context_system::instance());
    $can_create = $can_edit && !advanced_feature::is_disabled('user_reports');
    if (isset($tenant_reports[0]) || $can_create) {
        $tenants = [
            'NONE' => (object)[
                'id' => 'NONE',
    ...
    return [
        'reports' => $reportbuilder_permittedreports,
        'errors' => $reports_with_errors,
        'tenants' => $tenants,
    ];
}
...
function get_my_scheduled_reports_list($sqlclause=array()) {
    ...
    $sql = "SELECT rbs.*, rb.fullname, rb.tenantid
 ...
 *
 * @param bool $plain_text return strings as plain text rather than HTML
 * @return array tree of menu item objects
 */
function totara_build_menu($plain_text = false) {
    global $SESSION, $USER, $CFG, $DB;

    // HTML and plain format menus must be cached separately
    $cache_field = $plain_text ? 'mymenu_plain' : 'mymenu';

    $rev = \totara_core\totara\menu\helper::get_cache_revision();
    $lang = current_language();
    if (!empty($CFG->menulifetime) and !empty($SESSION->$cache_field['lang']) and isset($SESSION->$cache_field['rev'])) {
        if ($SESSION->$cache_field['id'] == $USER->id and $SESSION->$cache_field['lang'] === $lang and $SESSION->$cache_field['rev'] == $rev) {
            if ($SESSION->$cache_field['c'] + $CFG->menulifetime > time()) {
                $tree = $SESSION->$cache_field['tree'];
    ...
    unset($SESSION->$cache_field);

    $allnodes = $DB->get_records('totara_navigation', array(), 'parentid ASC, sortorder ASC, id ASC');
    $tree = totara_build_menu_descendants(0, $allnodes, 0, $plain_text);

    if (!empty($CFG->menulifetime)) {
        $SESSION->$cache_field = array(
            'id' => $USER->id,
            'lang' => $lang,
            'c' => time(),
            'tree' => $tree,
 ...
 * @param bool $plain_text return text as plain text instead of HTML
 * @return array
 */
function totara_build_menu_descendants($parentid, $allrecords, $parentdepth = 0, $plain_text = false) {
    $tree = [];
    ...
    foreach ($allrecords as $id => $record) {
        ...
        if (($node instanceof \totara_core\totara\menu\container)) {
            $descendants = totara_build_menu_descendants($record->id, $allrecords, $currentdepth, $plain_text);
            if (!$descendants) {
        ...
        $node = (object)array(
            ...
            'is_selected' => false,
            'customclass' => $node->get_customclass(),
            'icon_content' => $node->get_icon_content()
        );

        if ($plain_text) {
            $node->linktext = html_to_text($node->linktext, 0, false);
        }
 ...
 */
function encrypt_data(string $plaintext, string $key = ''): mixed {
    $default = false;
    if (empty($key)) {
    ...
    try {
        /** @var \phpseclib3\Crypt\RSA\PublicKey $rsa_key */
        $rsa_key = \phpseclib3\Crypt\RSA::loadPublicKey($key);
    } catch (\phpseclib3\Exception\NoKeyLoadedException $ex) {
        $state = $default ? 'was ' . PUBLIC_KEY_PATH : 'was custom key';
        debugging('Cannot load public key for encrypt_data (' . $state . ')', DEBUG_DEVELOPER);
        return false;
    }

    // For compatibility with the old style
    $rsa_key = $rsa_key
        ->withHash('sha1')
        ->withMGFHash('sha1')
        ->withPadding(\phpseclib3\Crypt\RSA::ENCRYPTION_PKCS1 | \phpseclib3\Crypt\RSA::SIGNATURE_PKCS1)
    ;
    $length = ($rsa_key->getLength() - 88) >> 3;

    $plaintext = str_split($plaintext, $length);
    $ciphertext = '';
    foreach ($plaintext as $chunk) {
        $ciphertext .= $rsa_key->encrypt($chunk);
    }
