 ...
 */
class totara_core_totara_test extends \core_phpunit\testcase {
    public function test_totara_major_version() {
        global $CFG;

        $majorversion = totara_major_version();
        $this->assertIsString($majorversion);
        $this->assertMatchesRegularExpression('/^(evergreen|[0-9]+)$/', $majorversion);
     ...
     */
    public static function visibility_data() {
        $data = array(
