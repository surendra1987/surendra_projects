...
if (!$program->is_certif()) {
    print_error('This is not a valid certification');
}
...
$PAGE->set_url(new moodle_url('/totara/certification/edit_content.php', array('id' => $id)));
...
if ($data = $contenteditform->get_data()) {
    if (isset($data->savechanges) || $submitdata) {
        ...
        if (!$programcontent->save_content()) {
            ...
            if (isset($data->savechanges) || $submitdata) {
                $str = get_string('certificationcontentsaved', 'totara_certification');
...
$event = program_viewed::create_from_data($dataevent)->trigger();
...
if (has_capability('totara/program:cloneprogram', $program->get_context())) {
