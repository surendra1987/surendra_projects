...
use totara_certification\totara_notification\resolver\due_date as due_date_resolver;
 ...
 */
class totara_certification_totara_notification_due_date_test extends totara_certification_totara_notification_base {
    ...
    public function test_resolver() {
        ...
        self::verify_notification_logs([
            [
                ...
                'event_name' => get_string('notification_log_due_date_resolver', 'totara_certification', [
    ...
    public function test_get_scheduled_events(): void {
        ...
        $cert_set_1 = certif_load_completion($data->program1->id, $data->user1->id, MUST_EXIST);

        program_generator::instance()->assign_program($data->program2->id, [$data->user1->id]);
        $cert_set_2 = certif_load_completion($data->program2->id, $data->user1->id, MUST_EXIST);

        $cert_set_1[1]->timedue = $now + DAYSECS * 3;
        $cert_set_2[1]->timedue = $now + DAYSECS * 6;

        certif_write_completion($cert_set_1[0], $cert_set_1[1]);
        certif_write_completion($cert_set_2[0], $cert_set_2[1]);
        ...
        self::assert_scheduled_events($resolver_class_name, $now + DAYSECS * 3, $now + DAYSECS * 4, [
            ['program_id' => $data->program1->id, 'user_id' => $data->user1->id, 'time_due' => $now + DAYSECS * 3],
        ]);
        // Result expected for min_time < time due.
        ...
        self::assert_scheduled_events($resolver_class_name, $now + DAYSECS * 2, $now + DAYSECS * 7, [
            ['program_id' => $data->program1->id, 'user_id' => $data->user1->id, 'time_due' => $now + DAYSECS * 3],
            ['program_id' => $data->program2->id, 'user_id' => $data->user1->id, 'time_due' => $now + DAYSECS * 6],
        ]);
