...
if ($prefix === 'goal' && $class === 'personal' && settings_helper::is_perform_goals_transition_mode_enabled()) {
    notification::warning(get_string('error:transition_mode_on_for_goal_type', 'totara_hierarchy'));
    redirect(new moodle_url('/totara/hierarchy/type/index.php', ['prefix' => 'goal', 'class' => $class]));
}
...
// update data
    ...
    if ($typenew->id == 0) {
        ...
        /** @var \totara_hierarchy\event\type_created $eventname */
    ...
    // Existing type.
        ...
        $typenew = $DB->get_record($shortprefix.'_type', array('id' => $typenew->id));
        /** @var \totara_hierarchy\event\type_updated $eventname */
...
if ($type->id == 0) {
    echo $OUTPUT->page_main_heading(get_string('addtype', 'totara_hierarchy'));
} else {
    echo $OUTPUT->page_main_heading(get_string('editgeneric', 'totara_hierarchy', format_string($type->fullname)));
}
