...
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(get_string($prefix . 'types', 'totara_hierarchy'));

$templatedata = new stdClass();

$heading = '';
if ($prefix == 'goal') {
    $managegoaltypes = settings_helper::is_perform_goals_transition_mode_enabled()
        ? get_string('manage' . $class . 'goaltypes_legacy', 'totara_hierarchy')
        : get_string('manage' . $class . 'goaltypes', 'totara_hierarchy');
    $templatedata->heading = $OUTPUT->heading($managegoaltypes . ' ' .
...
if ($can_add) {
    ...
    $templatedata->can_add = $hierarchy->get_add_type_button(0, $class);
}
