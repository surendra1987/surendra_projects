...
use hierarchy_goal\entity\personal_goal_type;
 ...
 * @group totara_goal
 */
class totara_hierarchy_webapi_resolver_type_personal_goal_type_test extends testcase {

    use webapi_phpunit_helper;

    private const TYPE = 'totara_hierarchy_personal_goal_type';

    public function test_invalid_input(): void {
        $this->expectException(coding_exception::class);
        $this->expectExceptionMessage("Only personal goal type entities are accepted");
    ...
    public function test_resolve(): void {
        ...
        $type1 = $hierarchy_generator->create_personal_goal_type($goal_type_data);

        $goal_type = new personal_goal_type($type1->id);
        $goal_type_data['id'] = $type1->id;
