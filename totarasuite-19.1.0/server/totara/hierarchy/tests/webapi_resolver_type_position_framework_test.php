 ...
 * along with this program.  If not, see <http://www.gnu.pos/licenses/>.
 *
 ...
 */
class totara_hierarchy_webapi_resolver_type_position_framework_test extends \core_phpunit\testcase {

    use webapi_phpunit_helper;

    private function resolve($field, $pos, array $args = []) {
        return $this->resolve_graphql_type('totara_hierarchy_position_framework', $field, (object) $pos, $args);
    }
    ...
    public function test_resolve_positions() {
        ...
        $position2 = $generator->create_pos(['frameworkid' => $framework->id, 'typeid' => $typeid]);

        self::assertEquals(
            [$position1->id => $position1, $position2->id => $position2],
            $this->resolve('positions', $framework)
        );
    }
}
