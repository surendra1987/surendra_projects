...
class company_goal_types extends query_resolver {
    ...
    public static function resolve(array $args, execution_context $ec) {
        ...
        // Default will retrieve the company goal type which has been assigned to a company goal.
        $repo = company_goal_type::repository();
        if (!isset($args['input']['check_goal_exist']) || $args['input']['check_goal_exist'] !== 0) {
            $repo->where_exists(
                    builder::table(company_goal::TABLE)
                        ->where_field(company_goal_type::TABLE . '.id', company_goal::TABLE . '.typeid')
                );
        }
