...
class goal extends hierarchy {
    ...
    public function update_user_assignments($goalid, $assignmenttype, $assignment) {
        ...
        $new_user_assignment->timecreated = time();
     ...
     *
     * @deprecated since Totara 13 - use update_user_assignments instead.
     *
    ...
    public static function update_goal_item($todb, $scope) {
        ...
        if ($scope == self::SCOPE_COMPANY) {
            ...
            if (settings_helper::is_perform_goals_transition_mode_enabled()) {
                debugging(
                    '"Totara goals transition mode" setting is enabled, ensure that you adjusted your code. It is not recommended changing the personal goals.',
                    DEBUG_DEVELOPER
                );
            }
    ...
    public static function insert_goal_item($todb, $scope) {
        ...
        if ($scope == self::SCOPE_COMPANY) {
            ...
            if (settings_helper::is_perform_goals_transition_mode_enabled()) {
                debugging(
                    '"Totara goals transition mode" setting is enabled, ensure that you adjusted your code. It is not recommended creating the new personal goals.',
                    DEBUG_DEVELOPER
                );
            }
    ...
    public function get_permissions($context = null, $user = null) {
        ...
        if (!empty($user)) {
            ...
            if (has_capability('totara/hierarchy:managegoalassignments', $syscontext)) {
                ...
                $permissions['can_edit_manager'] = has_capability('totara/hierarchy:managemanagerassignedgoal', $context);
                $permissions['can_edit'] = array(
                    GOAL_ASSIGNMENT_INDIVIDUAL => $permissions['can_edit_company'],
                    GOAL_ASSIGNMENT_SELF => $permissions['can_edit_personal'],
                    GOAL_ASSIGNMENT_MANAGER => $permissions['can_edit_manager'],
    ...
    public function display_hierarchy_item($record, $include_custom_fields=false, $indicate_depth=true, $cfields=array(), $types=array()) {
        ...
        $can_view_goal_details = false;
        if (isset($record->is_personal) && $record->is_personal === true) {
            $permissions = $this->get_permissions(null, $record->userid);
            $can_view_goal_details = $permissions['can_view_personal'] ?? false;
        } else {
            $permissions = $this->get_permissions();
            $can_view_goal_details = $permissions['canviewitems'] ?? false;
        }

        if ($can_view_goal_details) {
            if (isset($record->name)) {
                $out .= $OUTPUT->action_link(
                    new moodle_url(
                        '/totara/hierarchy/prefix/goal/item/view.php',
                        array('id' => $record->id)
                    ), $record->name, null, array('class' => $cssclass)
                );
            } else {
                $out .= $OUTPUT->action_link(
                    new moodle_url(
                        '/totara/hierarchy/item/view.php',
                        array('prefix' => $this->prefix, 'id' => $record->id)
     ...
     */
    public function get_add_type_button($page=0, $class='') {
        if ($class === 'personal' && settings_helper::is_perform_goals_transition_mode_enabled()) {
            return '';
        }
        global $OUTPUT;
        $type = '';
        ...
        return $OUTPUT->single_button($url, get_string('add' . $type . 'type', 'totara_hierarchy'), 'get');
    }
    ...
    public static function get_personal_custom_fields($personalgoaltype) {
        global $DB;
        return $DB->get_records('goal_user_info_field', array('typeid' => $personalgoaltype->id), 'sortorder ASC');
    }
    ...
    /**
     * Return extra fields from goals.
     * @return array
     */
    protected function get_extra_item_select_fields(): array {
        return ['hier.proficiencyexpected'];
    }
}
