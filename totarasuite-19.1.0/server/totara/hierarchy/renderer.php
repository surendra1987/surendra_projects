...
class totara_hierarchy_renderer extends plugin_renderer_base {
    ...
    public function competency_view_evidence($item, $evidence=null, $can_edit=false) {
        if (advanced_feature::is_enabled('competency_assignment')) {
            debugging('This method has been deprecated in Totara Perform. Please use totara/competency/classes/controllers/competency_controller for display and management of linked courses');
        }
    ...
    public function competency_view_related($item, $can_edit=false, $related=null) {
        if (advanced_feature::is_enabled('competency_assignment')) {
            debugging('This method has been deprecated in Totara Perform. The functionality is no longer supported');
        }
    ...
    */
     ...
     */
    public function assigned_goals($prefix, $shortprefix, $addgoalurl, $itemid, $display_warning = false, $warning_message = '') {
        global $CFG;
        $templatedata = new stdClass();
     ...
     * @param string $warning_message
     */
    public function mygoals_company_table($userid, $can_edit, $display = false, $display_warning = false, $warning_message = '') {
        global $DB, $PAGE;

        $assignments = goal::get_user_assignments($userid, $can_edit, true);
        ...
        foreach ($assignments as $goalid => $assignment) {
            ...
            $type = '';
            if (isset($goaltypes[$assignment->goal->typeid])) {
                $type = format_string($goaltypes[$assignment->goal->typeid]->fullname);
            }

            $cells['name'] = new html_table_cell($namedata);
            $cells['type'] = new html_table_cell($type);
    ...
    public function show_delete_prevented_warning(string $header, string $title, string $sub_title, string $reason, moodle_url $cancel_url): string {
        $output = html_writer::div(html_writer::tag('h2', $header), 'page-heading');
        $output .= $this->box_start('hierarchy-prevent-delete generalbox modal modal-dialog modal-in-page show');
        ...
        $cancel = html_writer::link($cancel_url, get_string('cancel'), ['class' => 'btn singlecancel']);
    ...
    public function mygoals_personal_table($userid, $can_edit, $display = false) {
        ...
        foreach ($assignments as $goalid => $assignment) {
            ...
            if ($can_edit[$assignment->assigntype]) {
                ...
                $edit_button = '';
                if (settings_helper::is_perform_goals_transition_mode_disabled()) {
                    $edit_url = new moodle_url('/totara/hierarchy/prefix/goal/item/edit_personal.php', array('id' => $goalid));
                    $edit_str = get_string('edit');
                    $edit_button = $this->output->action_icon($edit_url, new pix_icon('t/editstring', $edit_str));
                }
            ...
            $type = '';
            if ($personalgoaltype) {
                $type = format_string($personalgoaltype->fullname);
            }
