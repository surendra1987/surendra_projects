...
/**
 * Class totara_completionimport_importcertification_test
 *
 * @group totara_completionimport
 * @group totara_evidence
 */
class totara_completionimport_importcertification_test extends completionimport_advanced_testcase {
    ...
    public function test_import_assignments() {
        ...
        $program_generator->assign_to_program($program->id, assignments::ASSIGNTYPE_COHORT, $cohort->id, null, true);

        // Assign some users as individual to the certification - (users: 6 and 7).
        $program_generator->assign_to_program($program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $users[6]->id, null, true);
        $program_generator->assign_to_program($program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $users[7]->id, null, true);

        // Assign user 8 as an individual but set completion date in the future.
        $record = array('completiontime' => '15 2'  , 'completionevent' => assignments::COMPLETION_EVENT_FIRST_LOGIN);
        $program_generator->assign_to_program($program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $users[8]->id, $record, true);
        ...
        foreach ($cohortassignments as $assignmenttype) {
            $this->assertEquals(assignments::ASSIGNTYPE_COHORT, $assignmenttype,
        ...
        foreach ($individualassignments as $assignmenttype) {
            $this->assertEquals(assignments::ASSIGNTYPE_INDIVIDUAL, $assignmenttype,
        ...
        $params = array('programid' => $program->id, 'assignmenttype' => assignments::ASSIGNTYPE_INDIVIDUAL, 'assignmenttypeid' => $users[8]->id);
    ...
    private function setup_import_action_tests($recertifydatetype) {
        ...
        // Assign audience to the certification.
        $program_generator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_COHORT, $this->cohort->id, null, true);

        // Assign some users as individual to the certification - (users: 11 and 12).
        $program_generator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[11]->id, null, true);
        $program_generator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[12]->id, null, true);

        // Assign users 13 and 14 as individual but set completion date to require login.
        $record = array('completiontime' => '15 2'  , 'completionevent' => assignments::COMPLETION_EVENT_FIRST_LOGIN);
        $program_generator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[13]->id, $record, true);
        $record = array('completiontime' => '15 2'  , 'completionevent' => assignments::COMPLETION_EVENT_FIRST_LOGIN);
        $program_generator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[14]->id, $record, true);
        ...
        $DB->set_field('prog_completion', 'timedue', assignments::COMPLETION_TIME_UNKNOWN,
        ...
        $err = assignments::COMPLETION_TIME_UNKNOWN;
        $not = assignments::COMPLETION_TIME_NOT_SET;
    ...
    public static function data_provider_recertifydatetypes() {
    ...
    private function certif_set_state_certified($programid, $userid, $timecompleted) {
        ...
        $base = get_certiftimebase($certification->recertifydatetype, $certcompletion->baselinetimeexpires,
            ...
            get_timewindowopens($certcompletion->baselinetimeexpires, $certification->windowperiod));
