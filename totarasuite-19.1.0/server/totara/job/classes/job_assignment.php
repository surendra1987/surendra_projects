...
class job_assignment {
    ...
    public static function create($data) {
        ...
        $additionalfields = array_diff(array_keys($data), [
            ...
            'tempmanagerjaid', 'tempmanagerexpirydate', 'appraiserid', 'totarasync', 'synctimemodified', 'suppressnotifications'
        ]);
        ...
        try {
            ...
            // Update manager and temporary manager.
            $suppressnotifications = (isset($data['suppressnotifications']) && $data['suppressnotifications']);
            $jobassignment->updated_manager(null);
            $jobassignment->updated_temporary_manager(null, null, $suppressnotifications);
        ...
        $event = job_assignment_created::create(
            array(
                ...
                'other' => array(
                    ...
                    'newappraiserid' => $jobassignment->appraiserid,
                    'newtempmanagerjaid' => $jobassignment->tempmanagerjaid,
                ),
            )
        );
        $event->trigger();
    ...
    private function update_internal(array $data) {
        ...
        $oldappraiserid = $this->appraiserid;
        $oldtempmanagerjaid = $this->tempmanagerjaid;
        ...
        try {
            ...
            $this->updated_temporary_manager($oldtempmanagerjaid, $oldtempmanagerexpirydate, $suppressnotifications);
        ...
        $event = job_assignment_updated::create(
            array(
                ...
                'other' => array(
                    ...
                    'oldappraiserid' => $oldappraiserid,
                    'oldtempmanagerjaid' => $oldtempmanagerjaid,
                    ...
                    'newappraiserid' => $this->appraiserid,
                    'newtempmanagerjaid' => $this->tempmanagerjaid,
                ),
            )
        );
        $event->trigger();
     ...
     * @param bool $supressnotifications Whether to suppress notifications or not.
     */
    private function updated_temporary_manager($oldtempmanagerjaid, $oldtempmanagerexpirydate, $supressnotifications = false) {
        global $CFG, $DB, $USER;
        ...
        if ($supressnotifications) {
            // No need to continue. We don't want to send notifications.
            return;
        }
    ...
    public static function update_to_empty_by_criteria($field, $matchingvalue) {
        ...
        $fields = 'id, userid, managerjaid, managerjapath, positionid, organisationid, appraiserid, tempmanagerjaid';
            ...
            $event = job_assignment_updated::create(
                array(
                    ...
                    'other' => array(
                        ...
                        'oldappraiserid' => $record->appraiserid,
                        'oldtempmanagerjaid' => $record->tempmanagerjaid,
                        ...
                        'newappraiserid' => $field == 'appraiserid' ? null : $record->appraiserid,
                        'newtempmanagerjaid' => $record->tempmanagerjaid,
                    ),
                )
            );
            $event->trigger();
     ...
     *
     * This is true if:
     *   - User 1 is managed (temporary or permanently) or appraised by user 2
     *   - User 2 is managed (temporary or permanently) or appraised by user 1
     *   - User 1 and User 2 are manager (or temporary manager) and appraiser for the same user.
     *
     * @param int $userid1
     * @param int $userid2
     * @return bool
    ...
    public static function users_share_relation(int $userid1, int $userid2): bool {
        ...
        $sql = 'SELECT 1
             ...
             LEFT JOIN "ttr_job_assignment" tmja ON tmja.id = ja.tempmanagerjaid AND ja.tempmanagerexpirydate > :now
                 WHERE (ja.userid = :userid1 AND (mja.userid = :managerid1 OR ja.appraiserid = :appraiser1 OR tmja.userid = :tempmanager1)) 
                    OR (ja.userid = :userid2 AND (mja.userid = :managerid2 OR ja.appraiserid = :appraiser2 OR tmja.userid = :tempmanager2)) 
                    OR (mja.userid = :managerid3 AND ja.appraiserid = :appraiser3) 
                    OR (mja.userid = :managerid4 AND ja.appraiserid = :appraiser4) 
                    OR (tmja.userid = :tempmanagerid5 AND ja.appraiserid = :appraiser5) 
                    OR (tmja.userid = :tempmanagerid6 AND mja.userid = :managerid6)
                    OR (tmja.userid = :tempmanagerid7 AND ja.appraiserid = :appraiser7) 
                    OR (tmja.userid = :tempmanagerid8 AND mja.userid = :managerid8)                    
             ';

        $params = [
            'now' => time(),
            // Test one: User 1 is managed or appraised or temporary managed by user 2
            'userid1' => $userid1,
            'managerid1' => $userid2,
            'appraiser1' => $userid2,
            'tempmanager1' => $userid2,

            // Test two: User 2 is managed or appraised or temporary managed by user 1
            'userid2' => $userid2,
            'managerid2' => $userid1,
            'appraiser2' => $userid1,
            'tempmanager2' => $userid1,
            ...
            // Test five:  User 1 is a temporary manager and user 2 is appraiser
            'tempmanagerid5' => $userid1,
            'appraiser5' => $userid2,

            // Test six: User 1 is a temporary manager and user 2 is manager
            'tempmanagerid6' => $userid1,
            'managerid6' => $userid2,

            // Test seven: User 2 is a temporary manager and user 1 is appraiser
            'tempmanagerid7' => $userid2,
            'appraiser7' => $userid1,

            // Test eight: User 2 is a temporary manager and user 1 is manager
            'tempmanagerid8' => $userid2,
            'managerid8' => $userid1,
        ];
    ...
    /**
     * Fetches a list of all possible users that could be selectable as appraisers. If a tenant_id is passed in,
     * limit the results to only users that are available within the specified tenant.
     *
     * @param int $user_id
     * @param int|null $tenant_id
     * @return array
     */
    public static function get_potential_appraisers_for_job_assignment(int $user_id, ?int $tenant_id = null): array {
        global $CFG, $DB;

        // Get guest user for exclusion purposes
        $guest = guest_user();

        // Load potential appraisers for this user.
        $usernamefields = totara_get_all_user_name_fields(true, 'u');
        $username_extra_fields = get_extra_user_fields_sql(context_system::instance(), 'u', '', totara_get_all_user_name_fields());
        $order = totara_get_all_user_name_fields(true, 'u', null, null, true);

        $sql = "SELECT u.id, u.email, {$usernamefields} {$username_extra_fields}
          FROM {user} u
         WHERE u.deleted = 0
           AND u.suspended = 0
           AND u.id != :guestid
        ...
        if (!empty($CFG->tenantsenabled)) {
            if ($tenant_id) {
                $sql .= " AND EXISTS (
                    ...
                    JOIN {tenant} t ON t.cohortid = cm.cohortid
                    WHERE t.id = :tenantid
                    AND cm.userid = u.id
                    )";
