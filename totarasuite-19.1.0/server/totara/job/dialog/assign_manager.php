...
class totara_job_dialog_assign_manager extends totara_dialog_content {
        ...
        foreach ($managers as $manager) {
            ...
            if (empty($jobassignments)) {
                ...
                if ($canedit) {
                    ...
                    $item->name = totara_job_display_user_job($manager, null, null, true);
                    $item->userid = $manager->id;
                    $item->jaid = null;
                    $item->displaystring = $item->name;
                } else {
                    $item->name = totara_job_display_user_job($manager, null, null, false);
                ...
                $item->name = totara_job_display_user_job($manager, $firstjob);
                $item->displaystring = $item->name;
                $this->managers[$item->id] = $item;
            } else {
                ...
                // an expandable node.
                $displaystring = $manager->fullname;
                $displayfields = [];

                // We need a fallback in case the context is not set
                $context = $this->context ?? context_system::instance();

                $extrafields = get_extra_user_fields($this->context);
                if ($extrafields) {
                    foreach ($extrafields as $field) {
                        if (!empty($manager->{$field})) {
                            // TOTARA - Escape potential XSS in extra identity fields.
                            $displayfields[] = s($manager->{$field});
                        }
                    }
                }
                if (!empty($displayfields)) {
                    $displaystring .= ' (' . implode(', ', $displayfields) . ')';
                }
    ...
    private function load_job_assignments() {
        ...
        foreach ($jobassignments as $jobassignment) {
            ...
            $item->displaystring = totara_job_display_user_job($manager, $jobassignment);
        ...
        if ($this->allowcreate && totara_job_can_edit_job_assignments($manager->id)) {
            ...
            if ($this->allowmultiple || empty($jobassignments)) {
                ...
                $item->displaystring = totara_job_display_user_job($manager, null, null, true);
    ...
    private function get_managers_from_db($managerid = null) {
        ...
        $sql = "SELECT DISTINCT u.id, {$usernamefields} {$username_extra_fields}";
        ...
        foreach ($results as $result) {
            ...
            if ($this->allowcreate && isset($addcreateitem)) {
                if (($result->userid != $addcreateitem->userid) && totara_job_can_edit_job_assignments($addcreateitem->userid) && $this->allowmultiple) {
                    ...
                    $previtem->fullname = totara_job_display_user_job($addcreateitem, null, null, true);
            ...
            if ($result->jaid) {
                $job = new stdClass();
                $job->fullname = $result->jafullname;
                $job->idnumber = $result->jaidnumber;
                $item->fullname = totara_job_display_user_job($result, $job);
                ...
                $item->fullname = totara_job_display_user_job($result, null, null, true);
                $addcreateitem = null;
            } else {
                $item->fullname = totara_job_display_user_job($result, null);
                $addcreateitem = null;
                $this->disabled_items[$item->id] = $item;
            }
