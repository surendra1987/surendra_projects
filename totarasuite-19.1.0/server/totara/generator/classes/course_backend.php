...
class totara_generator_course_backend extends tool_generator_course_backend {
    ...
    /**
     * @var array|null IDs of system course categories
     */
    private static $system_category_ids = null;
    ...
    private function create_course() {
        ...
        $record['category'] = totara_generator_util::get_random_record_id(
            'course_categories',
            true,
            self::get_system_category_ids()
        );
    ...
    /**
     * Get the ids of system course categories. These should be excluded when generating courses.
     *
     * @return array
     */
    public static function get_system_category_ids(): array {
        global $DB;

        if (is_null(self::$system_category_ids)) {
            self::$system_category_ids = $DB->get_fieldset_select('course_categories', 'id', 'issystem = 1');
        }

        return self::$system_category_ids;
    }
}
