...
class helper {
    ...
    /**
     * These strings are for filtering using the names of database tables related to Engage.
     */
    public const TABLES_PREFIXES_ENGAGE_RELATED = ['ml_recommender', 'workspace', 'totara_msteams_', 'playlist'];
    ...
    /**
     * Check table belongs to flavour or not.
     *
     * @param string $table_name
     * @param string $flavour
     * @return bool
     */
    public static function is_table_belong_to_flavour(string $table_name, string $flavour): bool {
        $flavours = [];
        foreach (core_component::get_plugin_list('flavour') as $fname => $unused) {
            $flavours[] = $fname;
        }
        ...
        if (!in_array($flavour, $flavours)) {
            throw new coding_exception("Invalid flavour.");
        }

        $tables = builder::get_db()->get_tables();
        if (!in_array($table_name, array_values($tables))) {
            throw new coding_exception("This table does not exsit.");
        }

        $result = false;
        foreach (explode('_', $flavour) as $value) {
            foreach (array_merge(['perform'], self::TABLES_PREFIXES_ENGAGE_RELATED) as $table_prefix) {
                if ($value === 'engage' && $table_prefix !== 'perform') {
                    if (strpos($table_name, $table_prefix) !== false) {
                        $result = true;
                    }
                } else if ($value === 'perform' && $table_prefix === 'perform') {
                    if (strpos($table_name, $table_prefix) !== false) {
                        $result = true;
                    }
                } else if ($value === 'learn' || $value === 'professional') {
                    if (strpos($table_name, $table_prefix) !== false) {
                        $result = false;
                    } else {
                        $result = true;
    ...
    /**
     * Indicates whether the given feature is allowed in the specified flavour.
     * This is determined as follows in this order:
     * - if the feature is in the flavour's prohibited settings list, it is not
     *   allowed.
     * - if the feature's value is advanced_feature::DISABLED in the flavour's
     *   enforced settings, it is disallowed.
     * - if the feature is not present in the flavour's default settings, it is
     *   disallowed.
     *
     * Note: this just checks whether the flavour _allows_ the feature; callers
     * still need to check the feature's configuration values as to whether it
     * is _enabled_ or not.
     *
     * @return bool
     */
    public static function is_allowed(definition $flavour, string $feature): bool {
        $setting = "enable$feature";

        foreach ($flavour->get_prohibited_settings() as $settings) {
            if (in_array($setting, array_keys($settings))) {
                return false;
            }
        }

        // Feature is not prohibited.
        foreach ($flavour->get_enforced_settings() as $settings) {
            if (in_array($setting, array_keys($settings))) {
                return $settings[$setting] === advanced_feature::ENABLED;
            }
        }

        // Feature is not prohibited and does not have an enforced value.
        foreach ($flavour->get_default_settings() as $settings) {
            if (in_array($setting, array_keys($settings))) {
                return true;
            }
        }
