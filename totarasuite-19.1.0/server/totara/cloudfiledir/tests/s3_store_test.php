...
require_once(__DIR__ . '/store_testcase.php');
 ...
 * Test S3 store.
 */
final class totara_cloudfiledir_s3_store_test extends totara_cloudfiledir_store_testcase {
    protected function prepare_store_config(array $config): array {
        ...
        require_once(__DIR__ . '/s3_provider_test.php');
        $config['provider'] = 's3';
        $config['options'] = \totara_cloudfiledir_s3_provider_test::get_provider_options();
        $config['bucket'] = \totara_cloudfiledir_s3_provider_test::get_provider_bucket();
    ...
    protected function get_provider(array $config): totara_cloudfiledir\local\provider\base {
        return new s3($config['options'], $config['idnumber'], $config['bucket'], '');
    }
}
