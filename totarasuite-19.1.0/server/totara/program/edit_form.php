...
use totara_program\hook\program_edit_form_definition_complete;
...
//--------------------------------------------------------------------------------
        ...
        $mform->addElement('text', 'fullname', get_string('fullname', 'totara_program'), 'maxlength="1333" size="50"');
        ...
        $hook = new program_edit_form_definition_complete($this, $program->id, $action);
        if ($iscertif) {
...
class program_content_nonedit_form extends moodleform {
    ...
    function display_time_allowed(&$mform, $content, $certifpath) {
        ...
        if ($total_time_allowed > 0) {
            ...
            $numperiod = utils::get_duration_num_and_period($total_time_allowed);
...
class program_assignments_nonedit_form extends moodleform {
    ...
    function definition() {
        ...
        /** @var \totara_program\program $program */
        $program = $this->_customdata['program'];
        /** @var \totara_program\assignments\assignments $assignments */
