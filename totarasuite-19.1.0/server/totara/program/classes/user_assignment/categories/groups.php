...
/**
 * A representation of a group assignment to a program
 */
class groups extends user_assignment {

    /**
     * @inheritDoc
     */
    public function display_criteria(): string {
        $group_name = group::create_from_group_id($this->assignment->assignmenttypeid)
            ->get_name();

        $out = html_writer::start_tag('li', array('class' => 'assignmentcriteria'));
        $out .= html_writer::start_tag('span', array('class' => 'criteria'));
        $out .= get_string('memberofgroup', 'totara_program', $group_name);
