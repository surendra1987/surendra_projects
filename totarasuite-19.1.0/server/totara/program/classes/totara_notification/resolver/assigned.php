...
class assigned extends notifiable_event_resolver implements scheduled_event_resolver, permission_resolver, audit_resolver {
    ...
    public static function get_scheduled_events(int $min_time, int $max_time): moodle_recordset {
        return builder::table('prog_user_assignment')
            ->select(['programid as program_id', 'userid as user_id', 'MIN(timeassigned) as first_assigned'])
            ->group_by(['userid', 'programid'])
            ->having('MIN(timeassigned)', '>=', $min_time)
            ->having('MIN(timeassigned)', '<', $max_time)
            ->get_lazy();
    }
    ...
    public function get_fixed_event_time(): int {
        return (int) $this->event_data['first_assigned'];
    }
