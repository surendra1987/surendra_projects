...
/**
 * Handles the "totara_program_group_self_enrol" GraphQL mutation.
 */
final class group_self_enrol extends mutation_resolver {
    /**
     * {@inheritdoc}
     */
    public static function resolve(array $args, execution_context $ec) {
        try {
            ...
            $can_self_enrol_result =
                group_interactor::from($parsed->group, $parsed->context)
                    ->can_self_enrol();

            if ($can_self_enrol_result->success) {
                $parsed->group->add_users($parsed->user);
                return static::success_result(true);
            } else {
                return static::success_result(false, $can_self_enrol_result->message, $can_self_enrol_result->code);
            }
     ...
     *
     * @param array<string,mixed> $arg incoming arguments.
    ...
    private static function parse(
        ...
        if (!$parsed->group) {
            $parsed->error = (object) [
                ...
                'message' => get_string(
                    'group:error:invalid_group_reference',
                    'totara_program'
                )
            ];
        }
