...
/**
 * Event triggered when a users completion state is changed via the completion editor within a program.
 *
 * @property-read array $other
 * Extra information about the event.
 * - oldstate The old user completion status
 * - newstate The new user completion status
 *
 */
class program_completionstateedited extends \core\event\base {
    ...
    public static function get_name() {
        return get_string('completionstateedited', 'totara_program');
    }
    ...
    public function get_description() {
        ...
        $statuses = array(
            program::STATUS_PROGRAM_COMPLETE => 'complete',
            program::STATUS_PROGRAM_INCOMPLETE => 'incomplete',
        );
        ...
        return "The user with id '{$this->userid}' had their completion state changed from '{$oldstate}' to '{$newstate}' for program '{$this->objectid}' via the completion editor by user with the id '{$changedby}'";
    }
