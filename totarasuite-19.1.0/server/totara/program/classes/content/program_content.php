...
class program_content {
    ...
    const CONTENTTYPE_MULTICOURSE = 1;
    const CONTENTTYPE_COMPETENCY = 2;
    const CONTENTTYPE_RECURRING = 3;
    ...
    private $courseset_classnames = array(
        self::CONTENTTYPE_MULTICOURSE => '\totara_program\content\course_sets\multi_course_set',
        self::CONTENTTYPE_COMPETENCY  => '\totara_program\content\course_sets\competency_course_set',
        self::CONTENTTYPE_RECURRING   => '\totara_program\content\course_sets\recurring_course_set',
    );

    /**
     * program_content constructor.
     * @param $programid
     */
     ...
     * @return void
     */
    protected function ensure_coursesets_loaded(): void {
        global $DB;
        ...
        if ($this->coursesets === null) {
            ...
            foreach ($sets as $set) {
                if (!array_key_exists($set->contenttype, $this->courseset_classnames)) {
                    throw new ProgramException(get_string('contenttypenotfound', 'totara_program'));
                }
     ...
     */
    public static function cmp_set_sortorder($a, $b): int {
     ...
     * @return array
     */
    public function get_course_sets_path(int $pathtype): array {
        $csc = array();
     ...
     */
    public function get_courseset_by_id(int $coursesetid) {
     ...
     * @return void
     */
    public function fix_set_sortorder(&$coursesets = null): void {
        if ($coursesets === null) {
        ...
        usort($coursesets, array('totara_program\content\program_content', 'cmp_set_sortorder'));
     ...
     * @return bool
     */
    public function setup_content($formdata): bool {
     ...
     * @return void
     */
    function copy_coursesets_to_recert($formdata): void {
    ...
    /**
     * Load course_sets and fix sortorder.
     *
     * @return void
     */
     ...
     * @return array
     */
    public function get_courseset_prefixes($formdata): array {
        $setprefs = array();
        foreach (array('_ce','_rc') as $suffix) {
     ...
     * @return array
     */
    public function get_deleted_coursesets($formdata): array {
     ...
     * @param object $formdata The submitted form data
     * @return int|\stdClass|false Returns set order number if a matching action was found or false for no action
     */
    public function check_set_action(string $action, $formdata) {
    ...
    /**
     * Saves course_sets to database and triggers event.
     *
     * @return bool
     */
    public function save_content(): bool {
        global $DB;
        $this->ensure_coursesets_loaded();
        $this->fix_set_sortorder($this->coursesets);
     ...
     *
     * @param int $settomove_sortorder
     * @return bool
     */
    public function move_set_up(int $settomove_sortorder): bool {
     ...
     * @return bool
     */
    public function move_set_down(int $settomove_sortorder): bool {
     ...
     * @return bool
     */
    public function add_set(int $contenttype): bool {
        $this->ensure_coursesets_loaded();
     ...
     * saved
     *
     * @param int $settodelete_sortorder
     * @return bool
     */
    public function delete_set(int $settodelete_sortorder): bool {
        ...
        foreach ($this->get_course_sets() as $courseset) {
            if ($courseset->sortorder == $settodelete_sortorder) {
                ...
                if (isset($courseset->islastset) && $courseset->islastset) {
                    ...
                    if ($courseset->nextsetoperator == course_set::NEXTSETOPERATOR_THEN) {
                        if (is_object($previous_set)) {
                            $previous_set->nextsetoperator = course_set::NEXTSETOPERATOR_THEN;
                        }
                    }
                }
    ...
    /**
     * Similar to update content, but for use with sort order operations.
     *
     * @param int $set_pos Does nothing but provided for symmetry with other methods.
     * @return void
     */
    public function update_set(int $set_pos): void {
        $this->ensure_coursesets_loaded();
        $this->fix_set_sortorder($this->coursesets);
    }
     ...
     */
    public function delete_courseset_by_id(int $coursesetid): bool {
     ...
     *
     * @param int $set_sortorder
     * @param \stdClass $formdata
     * @return bool
     */
    public function add_course(int $set_sortorder, $formdata): bool {
        foreach ($this->get_course_sets() as $courseset) {
     ...
     * @return bool
     */
    public function delete_course(int $set_sortorder, int $courseid, $formdata): bool {
        foreach ($this->get_course_sets() as $courseset) {
            if ($courseset->sortorder == $set_sortorder) {
     ...
     * @return bool
     */
    public function add_competency(int $set_sortorder, $formdata): bool {
        foreach ($this->get_course_sets() as $courseset) {
            if ($courseset->sortorder == $set_sortorder) {
     ...
     */
    public function get_total_time_allowance(string $certifpath): int {
     ...
     * @return array of courseset_groups
     */
    public function get_courseset_groups(string $certifpath, bool $trimoptional = false): array {

        $coursesets = $this->get_course_sets();
        $courseset_groups = $this->group_coursesets($certifpath, $coursesets);
            ...
            foreach ($courseset_groups as $key => $courseset_group) {
                ...
                foreach ($courseset_group as $courseset) {
                    ...
                    if ($completionoptional === null) {
                        ...
                        if ($previousoperator == course_set::NEXTSETOPERATOR_AND) {
                            $operator = ' && ';
                        } else {
                            $operator = ' || ';
                        }
     ...
     *
     * @param string $certifpath the path that the sets are part of
     * @param array $coursesets An array of program coursesets
     * @return array An array of grouped sets.
     */
    public static function group_coursesets(string $certificationpath, array $coursesets): array {
        ...
        foreach ($coursesets as $courseset) {
            ...
            if (in_array($courseset->nextsetoperator, array(course_set::NEXTSETOPERATOR_AND, course_set::NEXTSETOPERATOR_OR))) {
     ...
     * @return void
     */
    public function set_courseset_group_timedue(array $courseset_group, int $userid): void {
        global $DB;
        ...
        if ($timeallowance>0) {
            if (!$cc = $DB->get_record('prog_completion', array('programid' => $this->programid, 'userid' => $userid, 'coursesetid' => $courseset_selected->id))) {
                ...
                $cc->status = program::STATUS_COURSESET_INCOMPLETE;
                $cc->timecreated = $now;
     ...
     * @return void
     */
    public function set_courseset_group_timedue_bulk(array $courseset_group, array $userids): void {
        global $DB;
    ...
    /**
     * Gets the form template for some coursesets.
     *
     * @param $mform Moodle form of some sort
     * @param array $template_values
     * @param array|null $coursesets
     * @param bool $updateform
     * @param false $iscertif
     * @param int $certifpath
     * @return string
     */
    public function get_content_form_template(&$mform, array &$template_values, array $coursesets = null, bool $updateform = true, bool $iscertif = false, int $certifpath = CERTIFPATH_CERT) {
        global $OUTPUT;
        ...
        if (!$recurring) {
            ...
            if ($updateform) {
                ...
                $contentoptions = array(
                    self::CONTENTTYPE_MULTICOURSE => get_string('setofcourses', 'totara_program'),
                );
                if (!$iscertif) {
                    if (advanced_feature::is_enabled('competencies')) {
                        $contentoptions[self::CONTENTTYPE_COMPETENCY] = get_string('competency', 'totara_program');
                    }
                    if ($numcoursesets == 0) { // don't allow recurring course to be added if the program already has other content
                        $contentoptions[self::CONTENTTYPE_RECURRING] = get_string('recurringcourse', 'totara_program');
                    }
                }
     ...
     */
    public function get_visibility_coursesets(int $operator, int $visibility): array {
        $courseaudiencevisibility = array();
        foreach ($this->get_course_sets() as $set) {
            if (get_class($set) === $this->courseset_classnames[self::CONTENTTYPE_MULTICOURSE]) {
                $courseaudiencevisibility += totara_search_for_value($set->courses, 'audiencevisible', $operator, $visibility);
            } else if (get_class($set) === $this->courseset_classnames[self::CONTENTTYPE_COMPETENCY]){
    ...
    /**
     * Clone the program content
     *
     * @param program $to_program
     * @return bool
     */
    public function clone(program $to_program): bool {
        global $DB;

        if ($DB->record_exists('prog_courseset', array('programid' => $to_program->id))) {
            throw new \coding_exception("Program content can only be cloned into a program with empty content");
        }

        $coursesets = $DB->get_records('prog_courseset', array('programid' => $this->programid), 'id ASC');

        if ($coursesets) {
            $trans = $DB->start_delegated_transaction();

            foreach ($coursesets as $courseset) {
                $existing_coursesetid = $courseset->id;
                unset($courseset->id);
                $courseset->programid = $to_program->id;

                $coursesetid = $DB->insert_record('prog_courseset', $courseset);

                $courseset_courses = $DB->get_records('prog_courseset_course', array('coursesetid' => $existing_coursesetid), 'id ASC');

                foreach ($courseset_courses as $courseset_course) {
                    unset($courseset_course->id);
                    $courseset_course->coursesetid = $coursesetid;

                    $DB->insert_record('prog_courseset_course', $courseset_course);
                }
            }
