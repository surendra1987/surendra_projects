...
class availability_checks_task extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        $now = time();
        $debugging = debugging();
        $unavailable = $DB->get_records('prog', array('available' => program::AVAILABILITY_NOT_TO_STUDENTS));
        $available = $DB->get_records('prog', array('available' => program::AVAILABILITY_TO_STUDENTS));
        ...
        foreach ($available as $program) {
            ...
            if ((!empty($program->availablefrom) && $program->availablefrom >= $now) ||
                ...
                $program->available = program::AVAILABILITY_NOT_TO_STUDENTS;
                $DB->update_record('prog', $program);
