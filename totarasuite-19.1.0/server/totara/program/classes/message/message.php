...
abstract class message {
     ...
     * @var int
     */
    private $completiontime = assignments::COMPLETION_TIME_NOT_SET;
    ...
    public function __construct($programid, $messageob=null, $uniqueid=null) {
        ...
        $tiggertime = utils::duration_explode($this->triggertime);
    ...
    public function init_form_data($formnameprefix, $formdata) {
        ...
        $this->triggertime = utils::duration_implode($this->triggernum, $this->triggerperiod);
    }
            ...
            switch ($placeholder) {
                ...
                case 'completioncriteria':
                    ...
                    if ($progassignment) {
                        ...
                        if ($progassignment->completionevent == assignments::COMPLETION_EVENT_NONE) {
                            $ccriteria = get_string('completioncriterianotdefined', 'totara_program');
                            if ($time != assignments::COMPLETION_TIME_NOT_SET) {
                                $formatedtime = trim(userdate($time, get_string('strftimedatefulllong', 'langconfig'), core_date::get_user_timezone($recipient), false));
                                $ccriteria = category::build_completion_string($formatedtime);
                            }
                        } else {
                            $ccriteria = category::build_relative_completion_string($offsetamount, $offsetunit, $event, $instance);
                        }
     ...
     */
    abstract public function get_message_form_template(&$mform, &$template_values, &$formdataobject, $updateform = true) : string;
     ...
     * @return <type>
     */
    public function get_generic_hidden_fields_template(&$mform, &$template_values, &$formdataobject, $updateform = true) : string {
