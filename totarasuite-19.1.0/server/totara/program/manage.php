...
if ((!empty($hide) or !empty($show)) && confirm_sesskey()) {
    if (empty($CFG->audiencevisibility)) {
        ...
        $event = program_updated::create_from_data($dataevent)->trigger();
    }
}
...
// Start output.
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(($viewtype == 'program') ? get_string('manageprograms', 'admin') : get_string('managecertifications', 'totara_certification'));
...
if ($canmanage) {
    echo $OUTPUT->container_start('totara_program__categories buttons');
...
if (!$programs) {
    ...
    foreach ($programs as $aprogram) {
        ...
        if (has_capability('totara/program:configuredetails', $programcontext)) {
            ...
            $settings_string = ($viewtype == 'certification') ? get_string('editcertificationsettings', 'totara_certification', $programname) : get_string('editprogramsettings', 'totara_program', $programname);
            $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/edit', $settings_string));
        }
        ...
        if (has_capability('totara/program:configureassignments', $programcontext)) {
            ...
            $assignments_string = ($viewtype == 'certification') ? get_string('manageassignments', 'totara_certification', $programname) : get_string('manageassignments', 'totara_program', $programname);
            $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/enrolusers', $assignments_string));
        }
        ...
        if (has_capability($capability, $programcontext)) {
            ...
            $del_string = $viewtype === 'certification' ? get_string('deletecertification', 'totara_certification', $programname) : get_string('deleteprogram', 'totara_program', $programname);
            $icons[] = $OUTPUT->action_icon(
                $url,
                new pix_icon('t/delete', $del_string)
            );
        }
        ...
        if (!empty($CFG->audiencevisibility) && has_capability('totara/coursecatalog:manageaudiencevisibility', $context)) {
            ...
            $icons[] = $OUTPUT->action_icon($url, new pix_icon("t/{$icon}", get_string('manageaudincevisibilitya', 'totara_cohort', $programname)));
        } else {
            ...
            if (has_any_capability($capabilities, $programcontext)) {
                if (!empty($aprogram->visible)) {
                    ...
                    $hide = $viewtype === 'certification' ? get_string('hidecertification', 'totara_certification', $programname) : get_string('hideprogram', 'totara_program', $programname);
                    $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/hide', $hide));
                } else {
                    $url = new moodle_url($baseurl, array('show' => $aprogram->id));
                    $show = $viewtype === 'certification' ? get_string('showcertification', 'totara_certification', $programname) : get_string('showprogram', 'totara_program', $programname);
                    $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/show', $show));
                }
            }
        }
        ...
        if ($canmanage) {
            if ($up && empty($searchcriteria)) {
                ...
                $move_up_string = ($viewtype == 'certification') ? get_string('movecertificationup', 'totara_certification', $programname) : get_string('moveprogramup', 'totara_program', $programname);
                $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/up', $move_up_string));
            }
            if ($down && empty($searchcriteria)) {
                $url = new moodle_url($baseurl, array('movedown' => $aprogram->id));
                $move_down_string = ($viewtype == 'certification') ? get_string('movecertificationdown', 'totara_certification', $programname) : get_string('moveprogramdown', 'totara_program', $programname);
                $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/down', $move_down_string));
            }
...
echo html_writer::start_tag('div', array('class' => 'totara_program__category_footer buttons'));
...
if ($canmanage && $numprograms > 1 && empty($searchcriteria)) {
    ...
    echo $OUTPUT->single_button($url, get_string('resortprogramsbyname', 'totara_program'), 'get', ['class' => 'totara_program__category_button-primary singlebutton']);
}
...
function print_category_edit(html_table $table, coursecat $category, $catlist, $viewtype = 'program', $depth = -1, $up = false, $down = false) {
    ...
    $str = new stdClass;
    $str->edit = new lang_string('editcategorya', 'core', $category->name);
    $str->delete = new lang_string('deletecategory', 'core', $category->name);
    $str->moveup = new lang_string('movecategoryup', 'core', $category->name);
    $str->movedown = new lang_string('movecategorydown', 'core', $category->name);
    $str->hide = new lang_string('hidecategory', 'core', $category->name);
    $str->show = new lang_string('showcategorya', 'core', $category->name);
    $str->cohorts = new lang_string('cohortsa', 'cohort', $category->name);
