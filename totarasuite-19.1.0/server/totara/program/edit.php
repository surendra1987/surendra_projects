...
use totara_program\hook\program_edit_form_save_changes;
...
if ($action == 'edit') {
    // Take them straight to overview page if they do not have permissions to edit.
    if (!has_capability('totara/program:configuredetails', $programcontext)) {
        redirect(new moodle_url('/totara/program/edit.php', array('id' => $id)));
    }
    $currenttab = 'details';
...
if ($data = $detailsform->get_data()) {
    ...
    if (isset($data->edit)) {
        ...
        $hook = new program_edit_form_save_changes($data, $program->id);
