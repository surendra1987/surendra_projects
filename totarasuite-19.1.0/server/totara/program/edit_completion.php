...
if ($progcompletion && empty($exceptions) && !$dismissedexceptions) {
    ...
    $currentformdata->timeduenotset = ($progcompletion->timedue == assignments::COMPLETION_TIME_NOT_SET) ? 'yes' : 'no';
    $currentformdata->timedue = ($progcompletion->timedue == assignments::COMPLETION_TIME_NOT_SET) ? 0 : $progcompletion->timedue;
    ...
    if ($deletehistory) {
        ...
        if (prog_write_completion($newprogcompletion, 'Completion manually edited')) {
            if ($progcompletion->status == program::STATUS_PROGRAM_COMPLETE && $newprogcompletion->status == program::STATUS_PROGRAM_INCOMPLETE) {
...
program_progress_cache::mark_progressinfo_stale($id, $userid);
