 ...
 */
class totara_program_deprecated_program_class_test extends reportcache_advanced_testcase {
    ...
    private function get_program_objects($type) {
         if ($type === 'program') {
            ...
            $program1 = $this->program_generator->create_deprecated_program();
            $this->assertDebuggingCalledCount(3);

            /** @var program $program2 */
            $program2 = $this->program_generator->create_deprecated_program();
            $this->assertDebuggingCalledCount(3);
        } else {
    ...
    public function test_debugging_messages() {
        program::create([]);
        $this->assertDebuggingCalled(array(
            'program has been deprecated, please use totara_program\program instead.',
            'prog_content has been deprecated, please use totara_program\content\program_content instead.',
            'prog_assignments has been deprecated, please use totara_program\assignments instead.',
        ));
    }
    ...
    public function test_display_required_assignment_reason($type) {
        ...
        foreach($assignmentdata as $userassigned) {
            ...
            // Lets just ignore the debugging messages.
            $this->assertDebuggingCalledCount(count($this->getDebuggingMessages()));
    ...
    public function test_construction_from_id() {
        ...
        $program_created = $this->program_generator->create_deprecated_program($detail);
        $this->assertDebuggingCalledCount(3);
    ...
    public function test_construction_from_incomplete_object() {
        ...
        try {
            ...
            $this->assertSame(
                'Coding error detected, it must be fixed by a programmer: Program created with incomplete program record (Missing: visible,availablefrom,availableuntil,available,timecreated,timemodified,usermodified,icon,allowextensionrequests)',
                $e->getMessage());
        }
    ...
    public function test_construction_from_incorrect_object() {
        ...
        try {
            ...
            $this->assertSame(
                'Coding error detected, it must be fixed by a programmer: Program created with incomplete program record (Missing: endnote,availablefrom,availableuntil,available,usermodified,allowextensionrequests)',
                $e->getMessage());
        }

        $this->assertDebuggingCalled('program has been deprecated, please use totara_program\program instead.');
    ...
    public function test_get_progress() {
        ...
        $program = $this->program_generator->create_deprecated_program($detail);
        $this->assertDebuggingCalledCount(3);
    ...
    public function test_delete_certification() {
        ...
        try {
            ...
            $this->assertSame(
                "Coding error detected, it must be fixed by a programmer: Property 'available' is automatically calculated based on the given from and until dates and should not be manually specified",
                $e->getMessage());
        }

        // prog_update_completion() is expecting namespaced program object.
        // We use the new program object for the simple purpose of updating completion and for this test to continue.
        prog_update_completion($user_complete->id, new \totara_program\program($certification->id));
        prog_update_completion($user_incomplete->id, new \totara_program\program($certification->id));
    ...
    public function test_clone_program(): void {
        ...
        foreach ($cloned_program as $prop => $val) {
            if (in_array($prop, ['id', 'fullname', 'shortname', 'timestarted', 'timecreated', 'timemodified', 'usermodified', 'sortorder', 'content', 'visible', 'available', 'idnumber'])) {
                continue;
            }
