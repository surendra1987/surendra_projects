...
/**
 * Class totara_program_program_class_test
 *
 * Tests the methods in the program class
 */
class totara_program_program_class_test extends reportcache_advanced_testcase {
     ...
     *  - This means that if the 1st assignment method is assignments::ASSIGNTYPE_ORGANISATION,
    ...
    private function get_assignment_data() {
        $user_data_full =  array(
            array(0, array(assignments::ASSIGNTYPE_INDIVIDUAL), array($this->users[0])),
            array(1, array(assignments::ASSIGNTYPE_INDIVIDUAL), array($this->users[1])),
            array(2, array(assignments::ASSIGNTYPE_ORGANISATION), array($this->organisations[0])),
            array(3, array(assignments::ASSIGNTYPE_ORGANISATION), array($this->organisations[0])),
            array(4, array(assignments::ASSIGNTYPE_ORGANISATION), array($this->organisations[1])),
            array(5, array(assignments::ASSIGNTYPE_ORGANISATION), array($this->organisations[1])),
            array(6, array(assignments::ASSIGNTYPE_POSITION), array($this->positions[0])),
            array(7, array(assignments::ASSIGNTYPE_POSITION), array($this->positions[0])),
            array(8, array(assignments::ASSIGNTYPE_POSITION), array($this->positions[1])),
            array(9, array(assignments::ASSIGNTYPE_POSITION), array($this->positions[1])),
            array(10, array(assignments::ASSIGNTYPE_COHORT), array($this->audiences[0])),
            array(11, array(assignments::ASSIGNTYPE_COHORT), array($this->audiences[0])),
            array(12, array(assignments::ASSIGNTYPE_COHORT), array($this->audiences[1])),
            array(13, array(assignments::ASSIGNTYPE_COHORT), array($this->audiences[1])),
            array(14, array(assignments::ASSIGNTYPE_MANAGERJA), array($this->managerjas[0])),
            array(15, array(assignments::ASSIGNTYPE_MANAGERJA), array($this->managerjas[0])),
            array(16, array(assignments::ASSIGNTYPE_MANAGERJA), array($this->managerjas[1])),
            array(17, array(assignments::ASSIGNTYPE_MANAGERJA), array($this->managerjas[1])),
            array(18, array(assignments::ASSIGNTYPE_ORGANISATION, assignments::ASSIGNTYPE_POSITION), array($this->organisations[0], $this->positions[1])),
            array(19, array(assignments::ASSIGNTYPE_ORGANISATION, assignments::ASSIGNTYPE_POSITION, assignments::ASSIGNTYPE_COHORT),
                array($this->organisations[1], $this->positions[1], $this->audiences[0])),
            array(20, array(assignments::ASSIGNTYPE_COHORT, assignments::ASSIGNTYPE_MANAGERJA), array($this->audiences[1], $this->managerjas[1])),
            array(21, array(assignments::ASSIGNTYPE_COHORT, assignments::ASSIGNTYPE_INDIVIDUAL), array($this->audiences[1], $this->users[21])),
    ...
    private function assign_users_to_program($program, $user_data_full) {
        ...
        foreach($user_data_full as $user_data) {
            foreach($user_data[1] as $key => $assignment_method) {
                ...
                // Groupid could actually be the individual user id in the case of assignments::ASSIGNTYPE_INDIVIDUAL
    ...
    private function get_expected_assignment_reason_strings() {
        $expectedstrings = array(
            assignments::ASSIGNTYPE_COHORT => 'Member of audience',
            assignments::ASSIGNTYPE_INDIVIDUAL => 'Assigned as an individual',
            assignments::ASSIGNTYPE_MANAGERJA => 'Part of',
            assignments::ASSIGNTYPE_ORGANISATION => 'Member of organisation',
            assignments::ASSIGNTYPE_POSITION => 'Hold position of ',
        );
    ...
    public function test_display_completion_record_reason_learningplan($type) {
        ...
        plan_assignment::update_plan_assignments($user->id, $plan->id);
    ...
    public function test_display_completion_record_reason_unknown() {
        ...
        $progcompletion25->timedue = assignments::COMPLETION_TIME_NOT_SET;
    ...
    private function compare_programs($prog_a, $prog_b) {
        ...
        $this->assertInstanceOf('totara_program\program', $prog_a);
        $this->assertInstanceOf('totara_program\program', $prog_b);
        ...
        $class = new ReflectionClass('totara_program\program');
    ...
    public function test_get_content() {
        ...
        $this->assertInstanceOf('totara_program\content\program_content', $content);
        $coursesets = $content->get_course_sets();
        $this->assertCount(3, $coursesets);
        foreach ($coursesets as $courseset) {
            $this->assertInstanceOf('totara_program\content\course_sets\multi_course_set', $courseset);
    ...
    public function test_get_exceptionsmanager() {
        ...
        $this->assertInstanceOf(exception_manager::class, $manager);
    ...
    public function test_assigned_learners_are_enrollable_in_courses() {
        ...
        foreach ($users as $user) {
            ...
            $this->assertObjectHasProperty('enroled', $result);
            $this->assertTrue($result->enroled);
            $this->assertObjectHasProperty('program_context', $result);
            $this->assertEquals($program->get_context(), $result->program_context);
            ...
            // Now when triggered again the user should still be enrolled
            $result = prog_can_enter_course($user, $course);
            $this->assertObjectHasProperty('enroled', $result);
            $this->assertTrue($result->enroled);
            $this->assertObjectHasProperty('program_context', $result);
            $this->assertNull($result->program_context);
        }
    ...
    public function test_unassign_learners_completion_records() {
        ...
        $this->program_generator->assign_to_program($cert1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $this->program_generator->assign_to_program($cert1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        $this->program_generator->assign_to_program($cert2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $this->program_generator->assign_to_program($cert2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);

        // Add the users to the programs.
        $this->program_generator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $this->program_generator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        $this->program_generator->assign_to_program($prog2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $this->program_generator->assign_to_program($prog2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
    ...
    public function test_user_is_assigned() {
        ...
        foreach ($users as $user) {
            ...
            $this->assertTrue(utils::user_is_assigned($program->id, $user->id));
        }

        $this->assertFalse($program->user_is_assigned($unassigneduser->id));
        $this->assertFalse(utils::user_is_assigned($program->id, $unassigneduser->id));

        $this->assertFalse($program->user_is_assigned(0));
        $this->assertFalse(utils::user_is_assigned($program->id, 0));

        $this->assertFalse($program->user_is_assigned(-1));
        $this->assertFalse(utils::user_is_assigned($program->id, -1));

        $this->assertFalse($program->user_is_assigned(null));
        $this->assertFalse(utils::user_is_assigned($program->id, null));
    }
    ...
    public function test_get_all_programs_with_incomplete_users() {
        ...
        foreach ($programs as $program) {
            $this->assertInstanceOf('totara_program\program', $program);
    ...
    public function test_get_program_learners() {
        ...
        $learners = $program->get_program_learners(program::STATUS_PROGRAM_INCOMPLETE);
        ...
        $learners = $program->get_program_learners(program::STATUS_PROGRAM_COMPLETE);
        $this->assertCount(1, $learners);
        $learner = reset($learners);
    ...
    public function test_delete() {
        ...
        // Add a prog_group and assign it.
        $group = $this->program_generator->create_group();
        ...
        // Check that the prog_group was deleted (no need to check control - the delete function is tested thoroughly).
        self::assertEquals(0, prog_group::repository()->count());
    }
    ...
    public function test_duration_implode() {
        ...
        $this->assertSame(0, utils::duration_implode(0, utils::TIME_SELECTOR_HOURS));
        $this->assertSame(1 * $hour, utils::duration_implode(1, utils::TIME_SELECTOR_HOURS));
        $this->assertSame(17 * $hour, utils::duration_implode(17, utils::TIME_SELECTOR_HOURS));

        $this->assertSame(0, utils::duration_implode(0, utils::TIME_SELECTOR_DAYS));
        $this->assertSame(1 * $day, utils::duration_implode(1, utils::TIME_SELECTOR_DAYS));
        $this->assertSame(23 * $day, utils::duration_implode(23, utils::TIME_SELECTOR_DAYS));

        $this->assertSame(0, utils::duration_implode(0, utils::TIME_SELECTOR_WEEKS));
        $this->assertSame(1 * $week, utils::duration_implode(1, utils::TIME_SELECTOR_WEEKS));
        $this->assertSame(5 * $week, utils::duration_implode(5, utils::TIME_SELECTOR_WEEKS));

        $this->assertSame(0, utils::duration_implode(0, utils::TIME_SELECTOR_MONTHS));
        $this->assertSame(1 * $month, utils::duration_implode(1, utils::TIME_SELECTOR_MONTHS));
        $this->assertSame(51 * $month, utils::duration_implode(51, utils::TIME_SELECTOR_MONTHS));

        $this->assertSame(0, utils::duration_implode(0, utils::TIME_SELECTOR_YEARS));
        $this->assertSame(1 * $year, utils::duration_implode(1, utils::TIME_SELECTOR_YEARS));
        $this->assertSame(42 * $year, utils::duration_implode(42, utils::TIME_SELECTOR_YEARS));

        $this->assertSame(0, utils::duration_implode(0, utils::TIME_SELECTOR_NOMINIMUM));
        $this->assertSame(0, utils::duration_implode(1, utils::TIME_SELECTOR_NOMINIMUM));
        $this->assertSame(0, utils::duration_implode(18, utils::TIME_SELECTOR_NOMINIMUM));
    }
    ...
    public function test_duration_explode() {
        ...
        $result = utils::duration_explode(0);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_NOMINIMUM, $result->period);
        $this->assertEquals(0, $result->num);
        $this->assertEquals('no minimum time', $result->periodstr);

        $result = utils::duration_explode(1);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(0, $result->period);
        $this->assertEquals(0, $result->num);
        $this->assertEquals('', $result->periodstr);

        $result = utils::duration_explode($hour * 7);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_HOURS, $result->period);
        $this->assertEquals(7, $result->num);
        $this->assertEquals('hour(s)', $result->periodstr);

        $result = utils::duration_explode($day * 3);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_DAYS, $result->period);
        $this->assertEquals(3, $result->num);
        $this->assertEquals('day(s)', $result->periodstr);

        $result = utils::duration_explode($day * 7);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_WEEKS, $result->period);
        $this->assertEquals(1, $result->num);
        $this->assertEquals('week(s)', $result->periodstr);

        $result = utils::duration_explode($day * 21);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_WEEKS, $result->period);
        $this->assertEquals(3, $result->num);
        $this->assertEquals('week(s)', $result->periodstr);

        $result = utils::duration_explode($week * 3);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_WEEKS, $result->period);
        $this->assertEquals(3, $result->num);
        $this->assertEquals('week(s)', $result->periodstr);

        $result = utils::duration_explode($week * 7);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_WEEKS, $result->period);
        $this->assertEquals(7, $result->num);
        $this->assertEquals('week(s)', $result->periodstr);

        $result = utils::duration_explode($month * 7);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_MONTHS, $result->period);
        $this->assertEquals(7, $result->num);
        $this->assertEquals('month(s)', $result->periodstr);

        $result = utils::duration_explode($month * 18);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_MONTHS, $result->period);
        $this->assertEquals(18, $result->num);
        $this->assertEquals('month(s)', $result->periodstr);

        $result = utils::duration_explode($month * 24);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_MONTHS, $result->period);
        $this->assertEquals(24, $result->num);
        $this->assertEquals('month(s)', $result->periodstr);

        $result = utils::duration_explode($year * 7);
        $this->assertInstanceOf('stdClass', $result);
        $this->assertEquals(utils::TIME_SELECTOR_YEARS, $result->period);
        $this->assertEquals(7, $result->num);
    ...
    public function test_get_standard_time_allowance_options() {
        $options_limited = utils::get_standard_time_allowance_options();
        $options_all = utils::get_standard_time_allowance_options(true);
        ...
        $expected = array(
            utils::TIME_SELECTOR_DAYS => 'Day(s)',
            utils::TIME_SELECTOR_WEEKS => 'Week(s)',
            utils::TIME_SELECTOR_MONTHS => 'Month(s)',
            utils::TIME_SELECTOR_YEARS => 'Year(s)',
        );
        $this->assertSame($expected, $options_limited);
        $expected[utils::TIME_SELECTOR_NOMINIMUM] = 'No minimum time';
    ...
    public function test_print_duration_selector() {
        $html = utils::print_duration_selector('t_', 'name_test', utils::TIME_SELECTOR_WEEKS, 'number_test', 7);
    ...
    public function test_update_exceptions() {
        ...
        $this->program_generator->assign_to_program($program1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[0]->id, null, true);
        $assignment = $DB->get_record('prog_assignment', ['programid' => $program1->id, 'assignmenttype' => assignments::ASSIGNTYPE_INDIVIDUAL, 'assignmenttypeid' => $this->users[0]->id]);
    ...
    public function test_notification_preference_removed_when_program_deleted(): void {
        ...
        // Note: There are 5 potential messages, but the current user doesn't have
        //       a manager so only 3 of them are sent.
        $messages = $this->messagesink->get_messages();
     ...
     */
    public function test_clone_metadata(): void {
        $creator = $this->data_generator->create_user();
        $this->setUser($creator);
        $program = $this->program_generator->create_program();
        $program_metadata = new totara_catalog\search_metadata\search_metadata();
        $program_metadata->set_pluginname('program');
        $program_metadata->set_plugintype('totara');
        $program_metadata->set_value('testing');
        $program_metadata->set_instanceid($program->id);
        $program_metadata->save();

        $cloner = $this->data_generator->create_user();
        $this->setUser($cloner);

        $p2 = $this->program_generator->create_program();
        $program->clone_metadata($p2);
        $p2_metadata = \totara_catalog\search_metadata\search_metadata_helper::find_searchmetadata('totara_program', $p2->id);
        $this->assertSame('testing', $p2_metadata->value);
    }
     ...
     * @throws moodle_exception
     */
    public function test_clone_tags(): void {
        $user = $this->data_generator->create_user();
        $this->setUser($user);
        $program = $this->program_generator->create_program();

        $tag_col = new \core_tag\entity\tag_collection();
        $tag_col->name = 'test_col';
        $tag_col->set_updated_timestamp(true);
        $tag_col->save();

        $tag_definition = new \core_tag\entity\tag();
        $tag_definition->userid = $user->id;
        $tag_definition->tagcollid = $tag_col->id;
        $tag_definition->save();

        $tag = new \core_tag\entity\tag_instance();
        $tag->tagid = $tag_definition->id;
        $tag->component = 'totara_program';
        $tag->itemtype = 'prog';
        $tag->itemid = $program->id;
        $tag->save();
        ...
        $p2 = $this->program_generator->create_program();
        $program->clone_tags($p2);

        $cloned_tags = \core_tag\entity\tag_instance::repository()
            ->where('component', '=', 'totara_program')
            ->where('itemtype', '=', 'prog')
            ->where('itemid', '=', $p2->id)
            ->get();

        foreach ($cloned_tags as $cloned_tag) {
            $this->assertEquals($cloned_tag->itemid, $p2->id);
            $this->assertEquals($cloned_tag->tagid, $tag_definition->id);
        }
    }
     ...
     * @throws moodle_exception
     */
    public function test_clone_custom_fields(): void {
        global $DB;
        $creator = $this->data_generator->create_user();
        $this->setUser($creator);
        $program = $this->program_generator->create_program();
        ...
        $customfield = $this->generate_customfield(['datatype' => 'text']);
        $DB->insert_record('prog_info_data', ['programid' => $program->id, 'data' => 'txtdata', 'fieldid' => $customfield]);

        $customfield = $this->generate_customfield(['datatype' => 'text', 'forceunique' => 1]);
        $DB->insert_record('prog_info_data', ['programid' => $program->id, 'data' => 'txtdata', 'fieldid' => $customfield]);

        $p2 = $this->program_generator->create_program();
        $program->clone_custom_fields($p2);
        ...
        $cloned_custom_fields = $DB->get_records_sql($sql, $params);

        foreach ($cloned_custom_fields as $cloned_custom_field) {
            if ($cloned_custom_field->forceunique) {
                $this->assertNull($cloned_custom_field->data);
                continue;
            }
            $cloned_custom_field->data = 'txtdata';
        }
    }
     ...
     * @throws moodle_exception
     */
    public function test_clone_program(): void {
        global $CFG;

        $extended_context = extended_context::make_system();
        $creator = $this->data_generator->create_user();
        $this->setUser($creator);
        $program = $this->program_generator->create_program(['visible' => 1, 'idnumber' => 'test101']);

        $cloner = $this->data_generator->create_user();
        $cloner_role_id = $this->getDataGenerator()->create_role();
        $this->getDataGenerator()->role_assign($cloner_role_id, $cloner->id, $extended_context->get_context_id());

        // ensure cloner can clone programs
        assign_capability(
        ...
        $this->assertNotSame($program->id, $cloned_program->id);
        $cloned_program_fullname = get_string('cloneprogramnameprefix', 'totara_program', $program->fullname);
        $this->assertSame($cloned_program_fullname, $cloned_program->fullname);
        $cloned_program_shortname = get_string('cloneprogramnameprefix', 'totara_program', $program->shortname);
        $this->assertSame($cloned_program_shortname, $cloned_program->shortname);
        // Note - this is assuming program was the last program saved, the sortorder will actually be calculated by max value in prog table.
        $this->assertEquals($program->sortorder + 1, $cloned_program->sortorder);
        $this->assertSame($program->category, $cloned_program->category);
        // The cloned program is hidden (basic visibility) on the creation.
        $this->assertEquals(0, $cloned_program->visible);
        $this->assertEquals($program->audiencevisible, $cloned_program->audiencevisible); // Default
        // idnumber is not cloned
        $this->assertEmpty($cloned_program->idnumber);

        // program creator should be updated to the cloning user
        $this->assertSame($cloned_program->usermodified, $cloner->id);
        ...
        foreach ($cloned_program as $prop => $val) {
            if (in_array($prop, $properties)) {
                continue;
            }
            $this->assertEquals($program->$prop, $val);
        }

        // Do a second clone with audience visibility
        $CFG->audiencevisibility = 1;
        $clone2 = $program->clone_program_details();

        $this->assertNotSame($program->id, $clone2->id);
        $clone2_fullname = get_string('cloneprogramnameprefix', 'totara_program', $program->fullname);
        $this->assertSame($clone2_fullname, $clone2->fullname);
        $clone2_shortname = get_string('cloneprogramnameprefix', 'totara_program', $program->shortname);
        $this->assertSame($clone2_shortname, $clone2->shortname);
        // Note - this is assuming program was the last program saved, the sortorder will actually be calculated by max value in prog table.
        $this->assertEquals($program->sortorder + 2, $clone2->sortorder);
        $this->assertSame($program->category, $clone2->category);
        // The cloned program is hidden (basic visibility) on the creation.
        $this->assertEquals(1, $clone2->visible); // Default
        $this->assertEquals(COHORT_VISIBLE_NOUSERS, $clone2->audiencevisible);
        // idnumber is not cloned
        $this->assertEmpty($clone2->idnumber);

        // program creator should be updated to the cloning user
        $this->assertSame($clone2->usermodified, $cloner->id);

        // compare all other properties
        foreach ($clone2 as $prop => $val) {
            if (in_array($prop, $properties)) {
                continue;
            }
            $this->assertEquals($program->$prop, $val);
     ...
     * @throws coding_exception
     */
    public function test_shallow_clone_program(): void {
        $extended_context = extended_context::make_system();
        $creator = $this->data_generator->create_user();
        $this->setUser($creator);
        $program = $this->program_generator->create_program(['visible' => 1, 'summary' => 'origin summary']);
        ...
        $this->setUser($cloner);
        $cloned_program = $program->shallow_clone_program_details();

        // shallow clone should copy fullname, shortname and the category only
        $cloned_program_fullname = get_string('cloneprogramnameprefix', 'totara_program', $program->fullname);
        $this->assertSame($cloned_program_fullname, $cloned_program->fullname);
        $cloned_program_shortname = get_string('cloneprogramnameprefix', 'totara_program', $program->shortname);
        $this->assertSame($cloned_program_shortname, $cloned_program->shortname);
        $this->assertSame($program->category, $cloned_program->category);
        $this->assertFalse(property_exists($cloned_program, 'summary'));
        // The cloned program is hidden on the creation.
        $this->assertEquals(0, $cloned_program->visible);
        $this->assertFalse(isset($cloned_program->audiencevisible));
    }
     ...
     * @throws moodle_exception
     */
    public function test_clone_fullname_shortname_max_length(): void {
        $creator = $this->data_generator->create_user();
        $this->setUser($creator);
        $fullname = str_pad("fullname", 1333, "-");
        $shortname = str_pad("shortname", 100, "-");
        $program = $this->program_generator->create_program([
            'fullname' => $fullname,
            'shortname' => $shortname,
        ]);
        ...
        $cloned_program = $program->clone_program_details();
        $fullname_langstring = get_string('cloneprogramnameprefix', 'totara_program', "fullname");
        $shortname_langstring = get_string('cloneprogramnameprefix', 'totara_program', "shortname");
        $this->assertSame(str_pad($fullname_langstring, 1333, "-"), $cloned_program->fullname);
        $this->assertSame(str_pad($shortname_langstring, 100, "-"), $cloned_program->shortname);
    }
     ...
     * @throws moodle_exception
     */
    public function test_clone_image(): void {
        global $CFG, $USER;
        ...
        $program->clone_image($cloned_program);
        $original_image = $program->get_image();
        $cloned_image = $cloned_program->get_image();
        $this->assertNotEquals($original_image, $cloned_image);

        // check the file contents
        $fs = get_file_storage();
        $origin_files = $fs->get_area_files($program->get_context()->id, 'totara_program', 'images', $program->id);
        $cloned_files = $fs->get_area_files($cloned_program->get_context()->id, 'totara_program', 'images', $cloned_program->id);
        $origin_image = reset($origin_files);
        $clone_image = reset($cloned_files);
        $this->assertSame($origin_image->get_content(), $clone_image->get_content());
    }
     ...
     * @throws moodle_exception
     */
    public function test_clone_overview_files(): void {
        global $CFG, $USER;
        ...
        // Upload an overview file for the program.
        $context = context_program::instance($program->id);
        $draftfile = core_files_external::upload(
            $usercontext->id,
            'user',
        ...
        $program->clone_overviewfiles($cloned_program);

        // check the file contents
        $fs = get_file_storage();
        $origin_files = $fs->get_area_files($program->get_context()->id, 'totara_program', 'overviewfiles', 0);
        $cloned_files = $fs->get_area_files($cloned_program->get_context()->id, 'totara_program', 'overviewfiles', 0);
     ...
     */
    public function test_clone_summary_files(): void {
        // clone summary images
        global $CFG, $USER;
        ...
        // create second program to clone files to
        $cloner = $this->data_generator->create_user();
        $this->setUser($cloner);
        $cloned_program = $this->program_generator->create_program();
        $program->clone_summary_files($cloned_program);

        // check the file contents
        $fs = get_file_storage();
        $origin_files = $fs->get_area_files($program->get_context()->id, 'totara_program', 'summary', 0);
        $cloned_files = $fs->get_area_files($cloned_program->get_context()->id, 'totara_program', 'summary', 0);
     ...
     * @throws stored_file_creation_exception
     */
    public function test_clone_endnote_files(): void {
        // clone summary images
        global $CFG, $USER;
        ...
        // create second program to clone to
        $cloner = $this->data_generator->create_user();
        $this->setUser($cloner);
        $cloned_program = $this->program_generator->create_program();
        $program->clone_endnote_files($cloned_program);

        // check the file contents
        $fs = get_file_storage();
        $origin_files = $fs->get_area_files($program->get_context()->id, 'totara_program', 'endnote', 0);
        $cloned_files = $fs->get_area_files($cloned_program->get_context()->id, 'totara_program', 'endnote', 0);
    ...
    /**
     * Will create a custom field for programs
     *
     * @param $data
     * @return int
     * @throws coding_exception
     */
    private function generate_customfield($data): int {
