...
use totara_program\assignments\categories\cohorts as cohorts_category;
 ...
 * vendor/bin/phpunit totara_program_update_learner_assignments_test totara/program/tests/update_learner_assignments_test.php
 *
 */
class totara_program_update_learner_assignments_test extends reportcache_advanced_testcase {
    ...
    private function set_individual_assignments(
        program $program,
        $users,
        $completiontime = assignments::COMPLETION_TIME_NOT_SET,
        ...
        $data = new stdClass();
        $data->id = $program->id;
        $data->item = array(assignments::ASSIGNTYPE_INDIVIDUAL => array());
        $data->completiontime = array(assignments::ASSIGNTYPE_INDIVIDUAL => array());
        $data->completionoffsetamount = array(assignments::ASSIGNTYPE_INDIVIDUAL => array());
        $data->completionoffsetunit = array(assignments::ASSIGNTYPE_INDIVIDUAL => array());
        $data->completionevent = array(assignments::ASSIGNTYPE_INDIVIDUAL => array());
        $data->completioninstance = array(assignments::ASSIGNTYPE_INDIVIDUAL => array());

        foreach ($users as $user) {
            $data->item[assignments::ASSIGNTYPE_INDIVIDUAL][$user->id] = 1;
            $data->completiontime[assignments::ASSIGNTYPE_INDIVIDUAL][$user->id] = $completiontime;
            $data->completionoffsetamount[assignments::ASSIGNTYPE_INDIVIDUAL][$user->id] = $completionoffsetamount;
            $data->completionoffsetunit[assignments::ASSIGNTYPE_INDIVIDUAL][$user->id] = $completionoffsetunit;
            $data->completionevent[assignments::ASSIGNTYPE_INDIVIDUAL][$user->id] = $completionevent;
            $data->completioninstance[assignments::ASSIGNTYPE_INDIVIDUAL][$user->id] = $completioninstance;
        }

        $category = new totara_program\assignments\categories\individuals();
    ...
    private function set_audience_assignment(
        ...
        $data = new stdClass();
        $data->id = $program->id;
        $data->item = array(assignments::ASSIGNTYPE_COHORT => array());
        $data->completiontime = array(assignments::ASSIGNTYPE_COHORT => array());
        $data->completionoffsetamount = array(assignments::ASSIGNTYPE_COHORT => array());
        $data->completionoffsetunit = array(assignments::ASSIGNTYPE_COHORT => array());
        $data->completionevent = array(assignments::ASSIGNTYPE_COHORT => array());
        $data->completioninstance = array(assignments::ASSIGNTYPE_COHORT => array());
        // The lines below could be moved into the above arrays.
        $data->item[assignments::ASSIGNTYPE_COHORT][$audience->id] = 1;
        $data->completiontime[assignments::ASSIGNTYPE_COHORT][$audience->id] = $completiontime;
        $data->completionoffsetamount[assignments::ASSIGNTYPE_COHORT][$audience->id] = $completionoffsetamount;
        $data->completionoffsetunit[assignments::ASSIGNTYPE_COHORT][$audience->id] = $completionoffsetunit;
        $data->completionevent[assignments::ASSIGNTYPE_COHORT][$audience->id] = $completionevent;
        $data->completioninstance[assignments::ASSIGNTYPE_COHORT][$audience->id] = 0;
    ...
    public function test_assigning_individuals() {
        ...
        $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion0->status);
        $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion1->status);
        ...
        $this->assertEquals(program::STATUS_COURSESET_INCOMPLETE, $progcoursesetcompletion0->status);
        $this->assertEquals(program::STATUS_COURSESET_INCOMPLETE, $progcoursesetcompletion1->status);
    ...
    public function test_unassigning_individuals() {
        ...
        // Check prog_assignment records.
        $progassignment0 = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_INDIVIDUAL, 'assignmenttypeid' => $user0->id));
        $this->assertNotEmpty($progassignment0);

        $progassignment1 = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_INDIVIDUAL, 'assignmenttypeid' => $user1->id));
        $this->assertNotEmpty($progassignment1);
    ...
    public function test_assigning_audience() {
        ...
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_COHORT, 'assignmenttypeid' => $audience->id));
    ...
    public function test_deferred_assignments() {
        ...
        $usercount = program::PROG_UPDATE_ASSIGNMENTS_DEFER_COUNT + 1;
        ...
        $this->assertEquals(program::PROG_UPDATE_ASSIGNMENTS_DEFERRED, $result);
        ...
        // Check that the returned value was correct and that the record's assignmentsdeferred flag is correct.
        $this->assertEquals(program::PROG_UPDATE_ASSIGNMENTS_COMPLETE, $result);
    ...
    public function test_decrease_due_date_relative_dates() {
        ...
        $this->set_audience_assignment(
            $this->program,
            $audience,
            assignments::COMPLETION_TIME_NOT_SET,
            assignments::COMPLETION_EVENT_ENROLLMENT_DATE,
            20,
        ...
        $this->assertEquals(assignments::COMPLETION_EVENT_ENROLLMENT_DATE, $progassignment->completionevent);
        $this->assertEquals(0, $progassignment->completioninstance);
    ...
    public function test_increase_due_date_when_complete() {
        ...
        $DB->execute("UPDATE {prog_completion}
            ...
            array('statuscomplete' => program::STATUS_PROGRAM_COMPLETE,
        ...
        foreach ($audienceusers as $user) {
            ...
            $this->assertEquals(0, $progcompletion->coursesetid);
            $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion->status); // Hacked to complete.
            $this->assertGreaterThanOrEqual($timebefore, $progcompletion->timecreated);
            $this->assertLessThanOrEqual($timeafter, $progcompletion->timecreated);
            $this->assertEquals(0, $progcompletion->timestarted);
            $this->assertEquals($originalduedate, $progcompletion->timedue); // Still original!!!
    ...
    public function test_multiple_assignments_with_assignment_due_dates() {
        ...
        $audienceprogassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_COHORT,
                'assignmenttypeid' => $audience->id));
        ...
        $individualprogassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_INDIVIDUAL,
    ...
    public function test_adding_additional_assignments() {
        ...
        $this->assertEquals(assignments::COMPLETION_EVENT_ENROLLMENT_DATE, $individualprogassignment->completionevent);
        $this->assertEquals(0, $individualprogassignment->completioninstance);
        ...
        foreach ($audienceusers as $user) {
            ...
            $this->assertEquals(0, $progcompletion->coursesetid);
            $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion->status);
            $this->assertGreaterThanOrEqual($timebefore, $progcompletion->timecreated);
            $this->assertLessThanOrEqual($timeafter, $progcompletion->timecreated);
    ...
    public function test_assigning_manager_hierarchy() {
        ...
        $topmanager1ja = job_assignment::create_default($topmanager1->id);
        $topmanager2 = $this->getDataGenerator()->create_user();
        $topmanager2ja = job_assignment::create_default($topmanager2->id);
        $manager1 = $this->getDataGenerator()->create_user();
        $manager1ja = job_assignment::create_default($manager1->id, array('managerjaid' => $topmanager1ja->id));
        $manager2 = $this->getDataGenerator()->create_user();
        $manager2ja = job_assignment::create_default($manager2->id, array('managerjaid' => $topmanager1ja->id));
        $manager3 = $this->getDataGenerator()->create_user();
        $manager3ja = job_assignment::create_default($manager3->id);
        $manager4 = $this->getDataGenerator()->create_user();
        $manager4ja = job_assignment::create_default($manager4->id,  array('managerjaid' => $topmanager2ja->id));
        // Test a user with manager assigned via first (sortorder=1) job assignment only.
        $user1 = $this->users[1];
        $user1ja = job_assignment::create_default($user1->id, array('managerjaid' => $manager1ja->id));
        // Test a user with the same manager assigned via two job assignments.
        $user2 = $this->users[2];
        $user2ja1 = job_assignment::create_default($user2->id, array('managerjaid' => $manager1ja->id));
        $user2ja2 = job_assignment::create_default($user2->id, array('managerjaid' => $manager1ja->id));
        // Test a user with the manager assigned via their second (sortorder=2) job assignment only.
        $user3 = $this->users[3];
        $user3ja1 = job_assignment::create_default($user3->id, array());
        $user3ja2 = job_assignment::create_default($user3->id, array('managerjaid' => $manager2ja->id));
        $user4 = $this->users[4];
        $user4ja = job_assignment::create_default($user4->id, array('managerjaid' => $manager3ja->id));
        $user5 = $this->users[5];
        $user5ja = job_assignment::create_default($user5->id, array('managerjaid' => $manager4ja->id));
        ...
        $this->programgenerator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_MANAGERJA, $topmanager1ja->id,
            array('includechildren' => 1), true);
        $this->programgenerator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_MANAGERJA, $topmanager2ja->id, null, true);
        ...
        // Check prog_assignment records.
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_MANAGERJA, 'assignmenttypeid' => $topmanager1ja->id));
        $this->assertNotEmpty($progassignment);
        ...
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_MANAGERJA, 'assignmenttypeid' => $topmanager2ja->id));
        $this->assertNotEmpty($progassignment);
        $this->assertEquals(0, $progassignment->includechildren);
        $this->assertEquals(-1, $progassignment->completiontime);
        $this->assertEquals(0, $progassignment->completionevent);
    ...
    public function test_change_in_manager_hierarchy() {
        ...
        $user1ja = job_assignment::create_default($user1->id, array('managerjaid' => $manager1ja->id));
        $user2 = $this->users[2];
        $user2ja = job_assignment::create_default($user2->id, array('managerjaid' => $manager1ja->id));
        $user3 = $this->users[3];
        $user3ja = job_assignment::create_default($user3->id, array('managerjaid' => $manager2ja->id));
        $user4 = $this->users[4];
        $user4ja = job_assignment::create_default($user4->id, array('managerjaid' => $manager3ja->id));
        $user5 = $this->users[5];
        $user5ja = job_assignment::create_default($user5->id, array('managerjaid' => $manager4ja->id));
    ...
    public function test_assigning_position() {
        ...
        // Update_learner_assignments is also run within the assign_to_program generator methods.
        $this->programgenerator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_POSITION, $position1->id,
            array('includechildren' => 1), true);
        $this->programgenerator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_POSITION, $position3->id,
        ...
        // Check prog_assignment records.
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_POSITION, 'assignmenttypeid' => $position1->id));
        $this->assertNotEmpty($progassignment);
        $this->assertEquals(1, $progassignment->includechildren);
        $this->assertEquals(-1, $progassignment->completiontime);
        $this->assertEquals(0, $progassignment->completionevent);
        ...
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_POSITION, 'assignmenttypeid' => $position3->id));
    ...
    public function test_assigning_organisation() {
        ...
        $user2ja2 = job_assignment::create_default($user2->id, array('organisationid' => $organisation1->id));
        // Test a user with the organisation assigned via their second (sortorder=2) job assignment only.
        $user3 = $this->users[3];
        $user3ja1 = job_assignment::get_first($user3->id);
        $user3ja2 = job_assignment::create_default($user3->id, array('organisationid' => $organisation1->id));
        $user4 = $this->users[4];
        ...
        // Update_learner_assignments is also run within the assign_to_program generator methods.
        $this->programgenerator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_ORGANISATION, $organisation1->id,
            array('includechildren' => 1), true);
        $this->programgenerator->assign_to_program($this->program->id, assignments::ASSIGNTYPE_ORGANISATION, $organisation3->id,
            array('includechildren' => null), true);
        ...
        // Check prog_assignment records.
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_ORGANISATION, 'assignmenttypeid' => $organisation1->id));
        $this->assertNotEmpty($progassignment);
        $this->assertEquals(1, $progassignment->includechildren);
        $this->assertEquals(-1, $progassignment->completiontime);
        $this->assertEquals(0, $progassignment->completionevent);
        ...
        $progassignment = $DB->get_record('prog_assignment',
            array('programid' => $this->program->id, 'assignmenttype' => assignments::ASSIGNTYPE_ORGANISATION, 'assignmenttypeid' => $organisation3->id));
    ...
    public function test_already_complete_when_assigned() {
        ...
        $this->assertEquals(0, $progcompletion0->coursesetid);
        $this->assertEquals(0, $progcompletion1->coursesetid);
        $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion0->status);
        $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion1->status);
        $this->assertGreaterThanOrEqual($timebefore, $progcompletion0->timecreated);
        $this->assertGreaterThanOrEqual($timebefore, $progcompletion1->timecreated);
        $this->assertLessThanOrEqual($timeafter, $progcompletion0->timecreated);
        $this->assertLessThanOrEqual($timeafter, $progcompletion1->timecreated);
        ...
        $this->assertNotEquals(0, $progcoursesetcompletion0->coursesetid);
        $this->assertNotEquals(0, $progcoursesetcompletion1->coursesetid);
        $this->assertEquals(program::STATUS_COURSESET_COMPLETE, $progcoursesetcompletion0->status);
        $this->assertEquals(program::STATUS_COURSESET_COMPLETE, $progcoursesetcompletion1->status);
    ...
    /**
     * When exceptions occur immediately on assignment, the timedue will be assignments::COMPLETION_TIME_NOT_SET. A bug was causing invalid
    ...
    public function test_exception_results_in_valid_timedue() {
        ...
        $this->set_individual_assignments(
            ...
            array($user),
            assignments::COMPLETION_TIME_NOT_SET,
            assignments::COMPLETION_EVENT_PROGRAM_COMPLETION,
            $otherprogid,
            1,
            utils::TIME_SELECTOR_DAYS
        );
        ...
        $this->assertEquals(assignments::COMPLETION_TIME_NOT_SET, $progcompletion->timedue);
    }
