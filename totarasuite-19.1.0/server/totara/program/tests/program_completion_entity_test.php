...
class totara_program_program_completion_entity_test extends testcase {
    ...
    public function test_entity_relation(): void {
        ...
        /** @var program_completion $row */
        $row = program_completion::repository()->where('programid', $this->program1->id)->get()->first();

        /** @var program $completion_program */
        $completion_program = $row->program()->get()->first();
        self::assertEquals($this->program1->id, $completion_program->id);
        self::assertEquals($this->program1->fullname, $completion_program->fullname);
    }
