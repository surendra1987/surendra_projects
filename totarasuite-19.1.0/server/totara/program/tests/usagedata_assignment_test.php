...
class totara_program_usagedata_assignment_test extends testcase {
    ...
    public function test_export(): void {
        ...
        $generator_program->assign_to_program($program->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user4->id);
        $generator_program->assign_to_program($program1->id, assignments::ASSIGNTYPE_COHORT, $cohort->id);
        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user5->id);
        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user6->id);

        $generator_program->assign_to_program($program->id, assignments::ASSIGNTYPE_GROUP, $group->id);
        $generator_program->assign_to_program($program1->id, assignments::ASSIGNTYPE_GROUP, $group1->id);
        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_GROUP, $group2->id);

        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_ORGANISATION, $organisation->id);
        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_POSITION, $position1->id);
        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_POSITION, $position2->id);
        $generator_program->assign_to_program($program2->id, assignments::ASSIGNTYPE_MANAGERJA, $managerja->id);

        $cert1 = $generator_program->create_certification();
        $cert2 = $generator_program->create_certification();
        $generator_program->assign_to_program($cert1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user5->id);
        $generator_program->assign_to_program($cert2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user6->id);

        $results = (new \totara_program\usagedata\assignment())->export();
        ...
        builder::table('prog_assignment')->where('programid', $program->id)
            ->update(['completionevent' => 6, 'completionoffsetamount' => 1, 'completionoffsetunit' => 1]);
        builder::table('prog_assignment')->where('programid', $program1->id)
        ...
        $program->update_learner_assignments(true);
        $program = new program($program2->id);
        $program->update_learner_assignments(true);
