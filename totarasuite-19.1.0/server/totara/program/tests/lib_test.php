 ...
 * vendor/bin/phpunit --verbose totara_program_lib_test totara/program/tests/lib_test.php
 */
class totara_program_lib_test extends reportcache_advanced_testcase {
    ...
    public function test_prog_update_completion_progs_and_certs() {
        ...
        $program_generator->assign_to_program($certification->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        ...
        $progcompletion->status = program::STATUS_PROGRAM_INCOMPLETE;
    ...
    public function test_prog_update_completion_specific_prog() {
        ...
        $program_generator->assign_to_program($program1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        $program_generator->assign_to_program($program2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        $program_generator->assign_to_program($program3->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        $program_generator->assign_to_program($program4->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        $program_generator->assign_to_program($program5->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        $program_generator->assign_to_program($program6->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        ...
        $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion->status);
        $progcompletion = prog_load_completion($program2->id, $user->id);
        $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status);
        $progcompletion = prog_load_completion($program3->id, $user->id);
        $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status);
        $progcompletion = prog_load_completion($program4->id, $user->id);
        $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status);
    ...
    public function test_prog_reset_course_set_completions() {
        ...
        foreach ($users as $user) {
            $program_generator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
            $program_generator->assign_to_program($prog2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
            $program_generator->assign_to_program($cert1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
            $program_generator->assign_to_program($cert2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
        }
        $endassigntime = time();
        $this->waitForSecond();
        ...
        foreach ($progcompletionnonzeropre as $pcnonzero) {
            $this->assertEquals(program::STATUS_COURSESET_COMPLETE, $pcnonzero->status);
            ...
            if ($pcnonzero->programid == $prog1->id && $pcnonzero->userid == $user1->id ||
                ...
                $pcnonzero->status = program::STATUS_COURSESET_INCOMPLETE;
    ...
    public function test_prog_update_available_enrolments_with_learning_plan() {
        ...
        plan_assignment::update_plan_assignments($user5->id, $plan[$user5->id]->id);
        $DB->delete_records('dp_plan_program_assign', array('planid' => $plan[$user6->id]->id));
        plan_assignment::update_plan_assignments($user6->id, $plan[$user6->id]->id);

        // 4) User7 and user8 are not assigned and their enrolment is suspended.
        $DB->delete_records('dp_plan_program_assign', array('planid' => $plan[$user7->id]->id));
        plan_assignment::update_plan_assignments($user7->id, $plan[$user7->id]->id);
        $DB->delete_records('dp_plan_program_assign', array('planid' => $plan[$user8->id]->id));
        plan_assignment::update_plan_assignments($user8->id, $plan[$user8->id]->id);
        $DB->set_field('user_enrolments', 'status', ENROL_USER_SUSPENDED, array('userid' => $user7->id));
        $DB->set_field('user_enrolments', 'status', ENROL_USER_SUSPENDED, array('userid' => $user8->id));
    ...
    public function setup_fix_completions() {
        ...
        foreach ($this->fixusers as $user) {
            foreach ($this->fixprograms as $prog) {
                $program_generator->assign_to_program($prog->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user->id, null, true);
            }
        }
    }
    ...
    public function test_prog_fix_completions_only_selected() {
        ...
        $DB->execute($sql, array('timedueunknown' => assignments::COMPLETION_TIME_UNKNOWN));
    ...
    /**
     * Test prog_fix_timedue. Set the program due date to \totara_program\assignments\assignments::COMPLETION_TIME_NOT_SET.
     */
    public function test_prog_fix_timedue() {
        ...
        $expectedprogcompletion->status = program::STATUS_PROGRAM_INCOMPLETE;
        $expectedprogcompletion->timestarted = 0;
        $expectedprogcompletion->timedue = assignments::COMPLETION_TIME_NOT_SET;
        ...
        $progcompletion->timedue = assignments::COMPLETION_TIME_UNKNOWN;
    ...
    public function test_prog_load_all_completions() {
        ...
        $program_generator->assign_to_program($cert1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        $program_generator->assign_to_program($cert2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $program_generator->assign_to_program($cert2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);

        // Add the users to the programs.
        $program_generator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $program_generator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        $program_generator->assign_to_program($prog2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $program_generator->assign_to_program($prog2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
    ...
    public function test_prog_conditionally_delete_completion() {
        ...
        $program_generator->assign_to_program($prog3->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $program_generator->assign_to_program($prog3->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        ...
        $program_generator->assign_to_program($prog4->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $program_generator->assign_to_program($prog4->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        $DB->delete_records('prog_user_assignment', array('programid' => $prog4->id));
    ...
    public function test_prog_display_progress_assignment_with_program() {
        ...
        $program_generator->assign_to_program($prog->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        ...
        $program_generator->assign_to_program($prog->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
    ...
    public function test_prog_display_progress_assignment_with_certification() {
        ...
        $program_generator->assign_to_program($cert->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        ...
        $program_generator->assign_to_program($cert->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        ...
        $program_generator->assign_to_program($cert->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user3->id, null, true);
        ...
        $program_generator->assign_to_program($cert->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user4->id, null, true);
        ...
        $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion->status); // Show that prog_completion still exists in correct state.
        list($certcompletion, $progcompletion) = certif_load_completion($cert->id, $user6->id, false);
        ...
        $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status); // Show that prog_completion still exists in correct state.
        list($certcompletion, $progcompletion) = certif_load_completion($cert->id, $user7->id, false);
        $this->assertEmpty($certcompletion); // Certif_load_completion needs both records to exist to work, so we know this record is gone.
        $this->assertEmpty($progcompletion);
    ...
    public function test_prog_fix_orphaned_exceptions_assign_with_programs() {
        ...
        // Break all the records, progs and certs.
        $DB->set_field('prog_user_assignment', 'exceptionstatus', program::PROGRAM_EXCEPTION_RAISED);

        // Check that all of the records are broken, progs and certs.
        $expectedfixedcount = 0;
        $this->assertEquals($expectedfixedcount,
            $DB->count_records('prog_user_assignment', array('exceptionstatus' => program::PROGRAM_EXCEPTION_RESOLVED)));
        list($fulllist, $aggregatelist, $totalcount) = prog_get_all_completions_with_errors();
        $this->assertCount($this->numtestusers * $this->numtestprogs - $expectedfixedcount, $fulllist);
        list($fulllist, $aggregatelist, $totalcount) = certif_get_all_completions_with_errors();
        $this->assertCount($this->numtestusers * $this->numtestcerts, $fulllist);
    ...
    public function test_prog_fix_orphaned_exceptions_recalculate_with_programs() {
        ...
        $expectedfixedcount = 0;
        $this->assertEquals($expectedfixedcount,
            $DB->count_records('prog_user_assignment', array('exceptionstatus' => program::PROGRAM_EXCEPTION_NONE)));
        list($fulllist, $aggregatelist, $totalcount) = prog_get_all_completions_with_errors();
        $this->assertCount($this->numtestusers * $this->numtestprogs - $expectedfixedcount, $fulllist);
        list($fulllist, $aggregatelist, $totalcount) = certif_get_all_completions_with_errors();
        $this->assertCount($this->numtestusers * $this->numtestcerts, $fulllist);
    ...
    public function test_prog_fix_orphaned_exceptions_recalculate_with_exceptions() {
        ...
        // Break all the records, progs and certs.
        $DB->set_field('prog_user_assignment', 'exceptionstatus', program::PROGRAM_EXCEPTION_RAISED);
        $totalraised = $DB->count_records('prog_user_assignment', array('exceptionstatus' => program::PROGRAM_EXCEPTION_RAISED));
        ...
        $this->assertEquals($totalraised,
            $DB->count_records('prog_user_assignment', array('exceptionstatus' => program::PROGRAM_EXCEPTION_RAISED)));
    ...
    public function test_prog_create_completion() {
        ...
        $data = array(
            'status' => program::STATUS_PROGRAM_COMPLETE,
            'timestarted' => 123,
            'timecreated' => 234,
            'timedue' => 345,
            'timecompleted' => 456,
    ...
    public function test_prog_create_courseset_completion() {
        ...
        $this->assertEquals(program::STATUS_COURSESET_INCOMPLETE, $cscompletion->status);
        ...
        $this->assertEquals(assignments::COMPLETION_TIME_NOT_SET, $cscompletion->timedue);
    ...
    public function test_prog_set_status_complete() {
        ...
        $programgenerator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user1->id, null, true);
        $programgenerator->assign_to_program($prog1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        $programgenerator->assign_to_program($prog2->id, assignments::ASSIGNTYPE_INDIVIDUAL, $user2->id, null, true);
        ...
        $DB->set_field('prog_completion', 'status', program::STATUS_PROGRAM_COMPLETE, ['programid' => $prog1->id, 'userid' => $user2->id, 'coursesetid' => 0]);
