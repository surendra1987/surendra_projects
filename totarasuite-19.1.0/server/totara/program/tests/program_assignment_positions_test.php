...
use totara_program\assignment\manager;
use totara_program\assignments\assignments;
use totara_program\assignment\position;
use totara_program\task\user_assignments_task;
use totara_program\utils;

class totara_program_program_assignment_positions_test extends \core_phpunit\testcase {
    ...
    public function test_show_in_ui() {
        ...
        $assignment = base::create_from_instance_id($this->programs[1]->id, assignments::ASSIGNTYPE_POSITION, $this->positions[1]->id);
    ...
    public function test_create_from_id() {
        ...
        $this->create_position_data();
        $this->programgenerator->assign_to_program($this->programs[1]->id, assignments::ASSIGNTYPE_POSITION, $this->positions[1]->id);
        ...
        $assignment = position::create_from_id($assignmentrecord->id);
    ...
    public function test_create_from_instance_id() {
        ...
        $this->assertEquals(assignments::ASSIGNTYPE_POSITION, $property->getValue($assignment));
    ...
    public function test_get_duedate() {
        ...
        $assignmenttoprog = assignments::factory($positiontype);
        $assignmenttoprog->update_assignments($data, false);

        // Get assignment record
        $assignmentrecord = $DB->get_record('prog_assignment', ['programid' => $this->programs[1]->id, 'assignmenttype' => assignments::ASSIGNTYPE_POSITION, 'assignmenttypeid' => $this->positions[1]->id]);
        $assignment = position::create_from_id($assignmentrecord->id);

        // Check due date is correct
        $expected = new \stdClass();
        $expected->string = userdate($timedue->getTimestamp(), 'Complete by %d %b %Y at %H:%M');
        ...
        $completiontime = '3 ' . utils::TIME_SELECTOR_WEEKS;
    ...
    public function test_position_assignment_delete() {
        ...
        $this->programgenerator->assign_to_program($this->programs[1]->id, assignments::ASSIGNTYPE_POSITION, $this->positions[1]->id);
        $this->programgenerator->assign_to_program($program1_id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[5]->id);
        ...
        $position_assignment = $DB->get_record('prog_assignment', ['assignmenttype' => assignments::ASSIGNTYPE_POSITION, 'assignmenttypeid' => $this->positions[1]->id]);
        $assignment = manager::create_from_id($position_assignment->id);
        $assignment->remove();
