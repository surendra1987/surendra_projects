...
class totara_program_program_extension_entity_test extends testcase {
    ...
    public function test_entity_relation(): void {
       ...
       $DB->insert_record('prog_extension', $extension);
        ...
        /** @var program_extension $row */
        $row = program_extension::repository()->where('programid', $this->program1->id)->one();

        /** @var program $extention_program */
        $extention_program = $row->program()->get()->first();
        self::assertEquals($this->program1->id, $extention_program->id);
        self::assertEquals($this->program1->fullname, $extention_program->fullname);

        /** @var user $extention_user */
        $extention_user = $row->user()->get()->first();
        self::assertEquals($this->user1->id, $extention_user->id);
        self::assertEquals($this->user1->firstname, $extention_user->firstname);
    }
