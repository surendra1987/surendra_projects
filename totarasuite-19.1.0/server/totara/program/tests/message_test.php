 ...
 */
class totara_program_message_test extends \core_phpunit\testcase {
    ...
    private function send_noneventbased_message($user, $program, $message = null, $subject = null) {
        ...
        /** @var totara_program\message\non_eventbased_message $stub - this will be a subclass of the given abstract class */
        $stub = $this->getMockForAbstractClass('totara_program\message\non_eventbased_message', array($program->id, $messageob));
    ...
    public function test_noneventbased_message_replacements_duedate_notset() {
        /** @var \totara_program\testing\generator $programgenerator */
        $programgenerator = $this->getDataGenerator()->get_plugin_generator('totara_program');
        $programgenerator->assign_to_program($this->program1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->user1->id,
            array('completiontime' => assignments::COMPLETION_TIME_NOT_SET), true);
    ...
    public function test_noneventbased_message_replacements_completioncriteria_set() {
        ...
        $programgenerator = $this->getDataGenerator()->get_plugin_generator('totara_program');
        $programgenerator->assign_to_program($this->program1->id, assignments::ASSIGNTYPE_INDIVIDUAL, $this->user1->id,
            array('completiontime' => '5 ' . utils::TIME_SELECTOR_DAYS, 'completionevent' => assignments::COMPLETION_EVENT_ENROLLMENT_DATE), true);
    ...
    public function test_noneventbased_message_replacements_completioncriteria_multiple() {
        ...
        $programgenerator->assign_to_program($this->program1->id, assignments::ASSIGNTYPE_COHORT, $audience1->id,
            array('completiontime' => '8 ' . utils::TIME_SELECTOR_DAYS, 'completionevent' => assignments::COMPLETION_EVENT_ENROLLMENT_DATE), true);
