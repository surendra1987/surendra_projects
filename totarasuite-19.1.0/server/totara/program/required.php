...
if (isset($program) && $program->user_is_assigned($userid)) {
    if (prog_is_accessible($program)) {
        ...
        if (!empty($courseid)) {
            ...
            // Check for active existing enrolment, no point trying to enrol someone who is already enrolled.
            $course_context = \context_course::instance($courseid);
            if (is_enrolled($course_context, $userid, '', true)) {
                redirect($url);
            }
        ...
        echo $OUTPUT->page_main_heading(format_string($heading));
    ...
    if (($filter == 'all' || $filter == 'program') && advanced_feature::is_enabled('programs')) {
        ...
        echo $OUTPUT->heading(get_string('programs', 'totara_program'), 2);
    ...
    if (($filter == 'all' || $filter == 'certification') && advanced_feature::is_enabled('certifications')) {
        ...
        echo $OUTPUT->heading(get_string('certifications', 'totara_program'), 2);
