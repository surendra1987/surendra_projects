...
$html .= html_writer::start_tag('div');
if ($action == exception_manager::SELECTIONACTION_NONE) {
    echo get_string('pleaseselectoption', 'totara_program');
    die();
}
else if ($action == exception_manager::SELECTIONACTION_AUTO_TIME_ALLOWANCE) {
    $html .= get_string('choseautomaticallydetermine', 'totara_program');
}
else if ($action == exception_manager::SELECTIONACTION_OVERRIDE_EXCEPTION) {
    $html .= get_string('choseoverrideexception', 'totara_program');
}
else if ($action == exception_manager::SELECTIONACTION_DISMISS_EXCEPTION) {
    $html .= get_string('exceptionchosendismiss', 'totara_program');
}
