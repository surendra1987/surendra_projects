...
final class course_editor {
    ...
    public static function get_current_completion_from_data($data) {
        ...
        if (!empty($data->timeenrolled)) {
            $course = course::load_by_id($data->courseid);
            $coursecompletion->duedate = $course->calculate_duedate_from_time($coursecompletion->timeenrolled);
        } else {
            $coursecompletion->duedate = null;
        }
