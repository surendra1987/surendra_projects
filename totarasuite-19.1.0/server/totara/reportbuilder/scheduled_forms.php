...
class scheduled_reports_new_form extends moodleform {
    function definition() {
        ...
        // Export type options.
        $exportformatselect = $report->get_report_export_options();
        ...
        $export_file_group = [];
        $export_file_group[] =& $mform->createElement('select', 'format', get_string('export', 'totara_reportbuilder'), $exportformatselect);
        $export_file_group[] = $mform->createElement('static', null, null, html_writer::empty_tag('br'));
        $export_file_group[] =& $mform->createElement('advcheckbox', 'compress', null, get_string('compress_file', 'totara_reportbuilder'), null, array(
            0, 1
        ));
        $export_file_group[] = $mform->createElement('static', null, null, html_writer::empty_tag('br'));
        $export_file_group[] = $mform->createElement(
            'static',
            'compress_help',
            '',
            html_writer::tag(
                'span',
                get_string('compress_file_help', 'totara_reportbuilder', $limit),
                array('class' => 'totara_reportbuilder__scheduled_form-compress_help'),
            )
        );
        $mform->addGroup($export_file_group, 'export_file', get_string('export', 'totara_reportbuilder'));
    ...
    public function set_data($data) {
        ...
        $mform->getElement('export_file')->getElements()[0]->setValue($data->format);
        $mform->getElement('export_file')->getElements()[2]->setValue($data->compress);
    ...
    public function get_data() {
        ...
        if ($data) {
            $data->format = $data->export_file['format'];
            $data->compress = $data->export_file['compress'] ?? 0;
        }
        return $data;
    }
}
