...
trait report_testing {
     ...
     * @param bool $hidden
     * @param bool $rowheader
     * @return array of records
     */
    protected function add_column(\reportbuilder $report, $type, $value, $transform, $aggregate, $heading, $hidden, $rowheader = false) {
        global $DB;

        $column = $report->src->new_column_from_option($type, $value, $transform, $aggregate, $heading, !empty($heading), $hidden, $rowheader);
     ...
     */
    protected function create_report($source, $fullname, $showtotalcount = false, $embedded = 0, $tenantid = null) {
        global $DB;
        ...
        if (!$todb->embedded && $tenantid) {
            $todb->tenantid = $tenantid;
            $todb->accessmode = 1;
        }
        ...
        if (!$tenantid) {
            ...
            $tdm_roleid = $DB->get_field('role', 'id',['shortname' => 'tenantdomainmanager']);
            if ($manager_roleid && $tdm_roleid) {
                $accessdata->role_activeroles = [$manager_roleid => 1, $tdm_roleid => 1];
            }
            $access = new \totara_reportbuilder\rb\access\tenant();
            $access->form_process($todb->id, $accessdata);
        }
