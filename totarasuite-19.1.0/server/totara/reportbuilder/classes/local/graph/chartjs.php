...
final class chartjs extends base {
    ...
    protected $colors;
    /** @var array SVG colours associated with categories or series. */
    protected array $category_colours;
    /** @var int Track how many regular colours have been assigned (different from $processedcount) */
    ...
    protected function init(): void {
        ...
        if (isset($this->usersettings['categoryColors'])) {
            $this->category_colours = $this->usersettings['categoryColors'] ?? [];
            unset($this->usersettings['categoryColors']);
        }
        ...
        if (!$this->is_pie_chart()) {
            ...
            foreach ($this->values as $k => $val) {
                $series = $this->values[$k]['label'];

                // If this is a named series, override the colour
                if (!empty($this->category_colours[$series])) {
                    $color = $this->category_colours[$series];
                } else {
                    $color = $this->colors[$i % count($this->colors)];
                    $i++;
                }

                $this->values[$k]['backgroundColor'] = $color;

                if ($this->graphrecord->type === 'line') {
                    $this->values[$k]['borderColor'] = $color;
                }
            }
        }
    ...
    protected function process_data(array $data): void {
        ...
        foreach ($values as $k => $val) {
            ...
            if ($this->is_pie_chart()) {
                // Override any wedges with custom colours
                if (!empty($this->category_colours[$label])) {
                    $color = $this->category_colours[$label];
                } else {
                    // Default colours
                    $color = $this->colors[$this->colour_count % count($this->colors)];
                    $this->colour_count++;
                }
                $this->values[$k]['backgroundColor'][] = $color;
            }
    ...
    private function fix_pie_colors() {
        ...
        // Only run if we haven't defined category colours, else this can juggle already assigned colours around
        if (!empty($this->category_colours)) {
            return;
        }
    ...
    /**
     * Checks if the chart should be rendered or not.
     *
     * @return bool True if the chart should be rendered, false otherwise.
     */
    private function should_render(): bool {
        // Pie charts don't have empty states, let's hide them
        if ($this->is_pie_chart() && !$this->has_renderable_data()) {
            return false;
        }
    ...
    /**
     * Checks if the chart has any renderable data or not.
     *
     * @return bool True if the chart has at least one non-empty data value, false otherwise.
     */
    private function has_renderable_data(): bool {
