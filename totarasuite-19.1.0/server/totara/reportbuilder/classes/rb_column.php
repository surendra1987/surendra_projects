...
class rb_column {
    ...
    /**
     * Determines if the column is used in row headers or not.
     * @var bool
     */
    public $rowheader;
    ...
    protected static function is_static_field($field) {
        ...
        // If it's a json data we will treat it as static. We don't want to do anything with it.
        if (strpos($field, 'jsondata') !== false) {
            return true;
        }
     ...
     * @param \context|null $context Context to check capability in. Defaults to system context
     * @return boolean True if the column should be shown, false otherwise
     */
    function display_column($isexport=false, \context $context = null) {
