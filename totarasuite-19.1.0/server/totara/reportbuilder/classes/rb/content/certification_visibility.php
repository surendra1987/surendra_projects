...
/**
 * Restrict content by a certification visibility settings
 *
 * NOTE: This restriction requires 'certif' and 'ctx' joins to be present
 * in the report source.
 */
final class certification_visibility extends base {

    const TYPE = 'certification_visibility_content';
     ...
     *                          on the fields from the 'certif' and 'ctx' joins.
    ...
    public function text_restriction($title, $reportid) {
        return get_string('certification_visibility', 'totara_reportbuilder');
    }
    ...
    public function form_template(&$mform, $reportid, $title) {
        $mform->addElement('header', 'certification_visibility_header', get_string('certification_visibility', 'totara_reportbuilder'));
        $mform->addHelpButton('certification_visibility_header', 'certification_visibility', 'totara_reportbuilder');
        $mform->setExpanded('certification_visibility_header');

        $enable = \reportbuilder::get_setting($reportid, self::TYPE, 'enable');
        $mform->addElement('checkbox', 'certification_visibility_enable', '', get_string('certification_visibility_checkbox', 'totara_reportbuilder'));
        $mform->setDefault('certification_visibility_enable', $enable);
        $mform->disabledIf('certification_visibility_enable', 'contentenabled', 'eq', 0);
    }
    ...
    public function form_process($reportid, $fromform) {
        ...
        $visibilityenable = $fromform->certification_visibility_enable ?? 0;
