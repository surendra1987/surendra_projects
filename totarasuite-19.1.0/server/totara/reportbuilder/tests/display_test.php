...
use totara_reportbuilder\rb\display\course_type_icon;
 ...
 */
class totara_reportbuilder_display_test extends \core_phpunit\testcase {
    ...
    public function test_aggregation() {
        ...
        // We want to test the API format's plaintext functionality, too
        $report->columns['user-custom_field_'.$field->id]->displayfunc = 'plaintext';
        $processed = $report->src->process_data_row($row, 'api', $report);

        $this->assertSame((int)$row->user_id, $processed[0]);
        $this->assertSame('January', $processed[1]);
        $this->assertSame('1357812000', $processed[2]);
        $this->assertSame('1357812000', $processed[3]);
        $this->assertSame('Řízek', $processed[4]);
        $this->assertSame('Monday', $processed[5]);
        $this->assertSame('Some html <strong>text</strong><script></script>', $processed[6]);
        ...
        // Get id from html string
        $pattern = '/id="(.*?)"\s/';
        preg_match($pattern, $processed[3], $id_match);
        ...
        $this->assertSame('<img src="https://www.example.com/moodle/theme/image.php/_s/' . $theme . '/totara_core/1/courseicons/business-modelling" id="' . $id_match[1] . '" class="course_icon" alt="volba1" title="volba1" />', $processed[3]);
    ...
    public function test_user_timezone(): void {
        ...
        // Create report.
        $rid = $this->create_report('user', 'Test user timezones');
        $config = (new rb_config())->set_nocache(true);
        $report = reportbuilder::create($rid, $config);

        // Mock objects to use in the display function.
        $column = $this->getMockBuilder('\rb_column')
            ->setConstructorArgs(array('user_timezone', 'UTC', 'Heading', 'user.timezone'))
            ->getMock();
        $format = "html";
        $row = new stdClass();
        $extrafieldrow = reportbuilder_get_extrafield_alias($column->type, $column->value, 'timezone');
        ...
        $display = \totara_reportbuilder\rb\display\user_timezone::display('Pacific/Auckland', $format, $row, $column, $report);
        $this->assertEquals('Australia/Perth', $display);

        $CFG->forcetimezone = 99;

        $display = \totara_reportbuilder\rb\display\user_timezone::display('Pacific/Auckland', $format, $row, $column, $report);
        $this->assertEquals('Pacific/Auckland', $display);
    ...
    public function test_course_type_icon() {
        // Mock objects to use in the display function.
        $column = $this->getMockBuilder('\rb_column')
            ->setConstructorArgs(
                [
        ...
        $report = $this->getMockBuilder('\reportbuilder')
            ->disableOriginalConstructor()
            ->getMock();

        $row = new stdClass();

        $display = course_type_icon::display(TOTARA_COURSE_TYPE_ELEARNING, "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/elearning-regular', $display);
        $this->assertStringContainsString(get_string('elearning', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display(TOTARA_COURSE_TYPE_ELEARNING, "csv", $row, $column, $report);
        $this->assertEquals(get_string('elearning', 'rb_source_dp_course'), $display);

        // DB queries mostly return values as string so try that too
        $display = course_type_icon::display("0", "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/elearning-regular', $display);
        $this->assertStringContainsString(get_string('elearning', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display("0", "csv", $row, $column, $report);
        $this->assertEquals(get_string('elearning', 'rb_source_dp_course'), $display);

        $display = course_type_icon::display(TOTARA_COURSE_TYPE_BLENDED, "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/blended-regular', $display);
        $this->assertStringContainsString(get_string('blended', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display(TOTARA_COURSE_TYPE_BLENDED, "csv", $row, $column, $report);
        $this->assertEquals(get_string('blended', 'rb_source_dp_course'), $display);

        // DB queries mostly return values as string so try that too
        $display = course_type_icon::display("1", "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/blended-regular', $display);
        $this->assertStringContainsString(get_string('blended', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display("1", "csv", $row, $column, $report);
        $this->assertEquals(get_string('blended', 'rb_source_dp_course'), $display);

        $display = course_type_icon::display(TOTARA_COURSE_TYPE_FACETOFACE, "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/facetoface-regular', $display);
        $this->assertStringContainsString(get_string('facetoface', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display(TOTARA_COURSE_TYPE_FACETOFACE, "csv", $row, $column, $report);
        $this->assertEquals(get_string('facetoface', 'rb_source_dp_course'), $display);

        // DB queries mostly return values as string so try that too
        $display = course_type_icon::display("2", "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/facetoface-regular', $display);
        $this->assertStringContainsString(get_string('facetoface', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display("2", "csv", $row, $column, $report);
        $this->assertEquals(get_string('facetoface', 'rb_source_dp_course'), $display);

        // Nw test this with an unknown type
        $display = course_type_icon::display(999, "html", $row, $column, $report);
        $this->assertStringContainsString('totara_core|msgicons/course-request', $display);
        $this->assertStringContainsString(get_string('unknowntype', 'rb_source_dp_course'), $display);
        $display = course_type_icon::display(999, "csv", $row, $column, $report);
        $this->assertEquals(get_string('unknowntype', 'rb_source_dp_course'), $display);

        // Nw test this with null
        $display = course_type_icon::display(null, "html", $row, $column, $report);
        $this->assertEquals('', $display);
        $display = course_type_icon::display(null, "csv", $row, $column, $report);
        $this->assertEquals('', $display);
    }
}
