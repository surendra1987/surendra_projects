...
class totara_reportbuilder_rb_content_program_visibility_test extends \core_phpunit\testcase {
    ...
    public function test_program_report_source_without_tenants() {
        ...
        $program1 = $program_gen->create_program(['visible' => 1, 'audiencevisible' => COHORT_VISIBLE_ALL]);
        $program2 = $program_gen->create_program(['visible' => 0, 'audiencevisible' => COHORT_VISIBLE_NOUSERS]);
        $program3 = $program_gen->create_program(['visible' => 1, 'audiencevisible' => COHORT_VISIBLE_AUDIENCE]);
        totara_cohort_add_association($audience->id, $program3->id, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);

        $program_gen->assign_program($program1->id, [$learner1->id, $learner2->id, $learner3->id]);
        $program_gen->assign_program($program2->id, [$learner2->id, $learner3->id]);
        $program_gen->assign_program($program3->id, [$learner3->id]);
        ...
        $reportid = self::create_report('program_completion', 'Test Program Completion Report');
        $report = reportbuilder::create($reportid, (new rb_config())->set_nocache(true));
        $this->add_column($report, 'prog', 'shortname', null, null, null, 0);
        ...
        reportbuilder::update_setting($reportid, 'program_visibility_content', 'enable', 1);
    ...
    public function test_program_report_source_with_nonisolated_tenants() {
        ...
        $program1 = $program_gen->create_program(['visible' => 1, 'audiencevisible' => COHORT_VISIBLE_ALL, 'category' => $tenant1->categoryid]);
        $program2 = $program_gen->create_program(['visible' => 0, 'audiencevisible' => COHORT_VISIBLE_NOUSERS]);
        $program3 = $program_gen->create_program(['visible' => 1, 'audiencevisible' => COHORT_VISIBLE_AUDIENCE, 'category' => $tenant2->categoryid]);
        totara_cohort_add_association($audience->id, $program3->id, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
