...
define(['jquery', 'core/config', 'core/str'], function ($, mdlcfg, mdlstrings) {
    ...
    var handler = {
         ...
         * @param {boolean} checkaccess Optional, to skip filter access check
         */
        init: function(filter, value, type, name, reportid, checkaccess) {
            ...
            handler.checkaccess = checkaccess;
            ...
            $.each(waitingitems, function () {
                switch (this.filter) {
                    ...
                    case "course_multi":
                        ...
                        break;
                    case "tenant":
                        handler.rb_load_tenant_filters();
                        break;
                }
            });
            ...
            $('div.rb-org-add-link a').each(function() {
                ...
                mdlstrings.get_string('chooseorgplural', 'totara_reportbuilder').done(function (chooseorgplural) {
                    totaraMultiSelectDialogRbFilter(
                        id,
                        chooseorgplural,
                        url + 'find.php?reportid=' + self.reportid + '&checkaccess=' + self.checkaccess,
            ...
            $('input.rb-filter-choose-course').each(function() {
                ...
                mdlstrings.get_string('coursemultiitemchoose', 'totara_reportbuilder').done(function (coursemultiitemchoose) {
                    totaraMultiSelectDialogRbFilter(
                        ...
                        url + 'find.php?sesskey=' + mdlcfg.sesskey + '&report_id=' + self.reportid,
                        url + 'save.php?filtername=' + id + '&sesskey=' + mdlcfg.sesskey +'&ids='
                    );
                });
        ...
        rb_load_tenant_filters: function() {
            var self = this;
            // Bind multitenant report filter.
            $('div.rb-user-add-link a').each(function() {
                var id = $(this).attr('id');
                // Remove 'show-' and '-dialog' from ID.
                id = id.substr(5, id.length - 12);
                ...
                var url = mdlcfg.wwwroot + '/totara/tenant/assignfilter/';

                mdlstrings.get_string('choosetenants', 'totara_reportbuilder').done(function (choosetenants) {
                    totaraMultiSelectDialogRbFilter(
                        id,
                        choosetenants,
                        url + 'find.php?reportid=' + self.reportid,
