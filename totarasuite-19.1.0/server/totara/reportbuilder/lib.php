...
use totara_reportbuilder\event\scheduled_report_task_failed;
...
class reportbuilder {
    ...
    public $fullname, $shortname, $source, $hidden, $searchcolumns, $requiredcolumns, $requiredjoins, $initialdisplay, $userlinkcheck;
    ...
    public $_id, $recordsperpage, $defaultsortcolumn, $defaultsortorder, $tenantid;
    ...
    /**
     * @var \core\record\tenant|null Tenant record for a tenant report
     */
    private $tenant;

    /**
     * @var bool If false sessions are closed before this report is generated.
     */
    public $keep_sessions_open = false;
    ...
    public function __construct($report, $config = null, $checkaccess = true) {
        ...
        $this->userlinkcheck = $report->userlinkcheck;
        ...
        $this->keep_sessions_open = $report->embedded || $config->get_keep_session_open();
    ...
    private function initialise() {
        ...
        if ($this->_sid) {
            if (!$this->restore_saved_search()) {
                // The saved search was not available, reset back to the default;
                $this->_sid = 0;
                \core\notification::error(get_string('saved_search_not_found', 'totara_reportbuilder'));
            }
        }
        ...
        $this->requiredjoins = array();
        if (!empty($this->src->requiredjoins)) {
            foreach ($this->src->requiredjoins as $join) {
                $this->requiredjoins[] = $join;
            }
        }
    ...
    public function get_context(): context {
        return empty($this->tenantid)
            ...
            : \core\record\tenant::fetch($this->tenantid)->category_context;
    }

    /**
     * Get the tenant record if a tenant report.
     *
     * @return \core\record\tenant|null
     */
    public function get_tenant(): ?\core\record\tenant {
        if (empty($this->tenantid)) {
            return null;
        }
        if (empty($this->tenant)) {
            $this->tenant = \core\record\tenant::fetch($this->tenantid);
        }
    ...
    public function get_caching_problems() {
        ...
        if (!$DB->is_create_table_from_select_supported()) {
            ...
            $problems[] = get_string('caching_reports_with_create_table_from_select', 'totara_reportbuilder');
        }

        if (!empty($CFG->tenantsenabled)) {
            // Caching is not compatible.
            $problems[] = get_string('caching_reports_with_multitenancy', 'totara_reportbuilder');
        }

        // Report caching not enabled when database is supported and multitenancy is turned off
        if (empty($CFG->enablereportcaching) && $DB->is_create_table_from_select_supported() && empty($CFG->tenantsenabled)) {
            $enablelink = new moodle_url("/".$CFG->admin."/settings.php", array('section' => 'optionalsubsystems'));
            $problems[] = get_string('reportcachingdisabled', 'totara_reportbuilder', $enablelink->out());
        }
    ...
    protected static $cache_sourcegroups = [];
    ...
    public static function get_source_groups($includenonselectable = false, ?bool $tenant_sources_only = false) {
        // If we've run this function before, return the cached output
        $for = empty($tenant_sources_only) ? 'site' : 'tenant';
        if (!empty(self::$cache_sourcegroups) && !empty(self::$cache_sourcegroups[$for])) {
            return self::$cache_sourcegroups[$for];
        }
        ...
        foreach (self::find_source_dirs() as $dir) {
            if (is_dir($dir) && $dh = opendir($dir)) {
                while (($file = readdir($dh)) !== false) {
                    ...
                    if ($for === 'tenant' && class_exists($classname) && !$classname::is_source_tenant_compatible()) {
                        continue;
                    }
        ...
        self::$cache_sourcegroups[$for] = $output;
        return $output;
    }
     ...
     *
     * @return Boolean True if user can view or false if the search was not found
     */
    function restore_saved_search() {
        ...
        if ($missing) {
            // The saved search no longer exists for this user, forget about it.
            $DB->delete_records(
                'report_builder_saved_user_default',
                ['userid' => $this->reportfor, 'reportid' => $this->get_id(), 'savedid' => $this->_sid]
            );
            return false;
        }
    ...
    private function process_filters() {
        ...
        $mformtoolbar = new report_builder_toolbar_search_form(null,
            array('fields' => $this->has_toolbar_filter(), 'cansave' => $this->can_save_search()));
        $adddatatoolbar = $mformtoolbar->get_data(false);
        // Get submitted data as get_data could result in NUll if validation fails.
        $toolbarsubmitteddata = $mformtoolbar->get_submitted_data();

        // Save the search.
        if (isset($toolbarsubmitteddata->toolbarsavesearchbutton)) {
            $params = $this->get_current_url_params();
            $params['id'] = $this->_id;
            redirect(new moodle_url("/totara/reportbuilder/save.php", $params));
        }
    ...
    public function display_sidebar_search() {
        ...
        // If this is an embedded report then don't activate instant filters.
    ...
    */
    ...
    /**
     * Returns an array of defined reportbuilder enabled access plugins
     *
     * @param int $reportid
     * @return array
     */
    public static function get_enabled_access_plugins(int $reportid): array {
        $plugins = self::get_all_access_plugins();

        $report_setting_info = \core\orm\query\builder::table('report_builder_settings')
            ->where('name', 'enable')
            ->where('value', '1')
        ...
        foreach ($report_setting_info as $id => $plugin) {
            $ptype = $plugin->type;
            if (!isset($plugins[$ptype])) {
                continue;
            }
    ...
    public static function get_permitted_reports($userid = null, $showhidden = false) {
        ...
        if ($reports) {
            foreach ($reports as $report) {
                ...
                if ($report->accessmode == REPORT_BUILDER_ACCESS_MODE_NONE && is_null($report->tenantid)) {
                    $permitted_reports[$report->id] = $report;
                    continue;
                }
                if ($report->accessmode == REPORT_BUILDER_ACCESS_MODE_ANY || $report->tenantid != null) {
    ...
    function get_content_restrictions($cache = false) {
        ...
        if (isset($this->contentoptions) && is_array($this->contentoptions)) {
            foreach ($this->contentoptions as $option) {
                ...
                $class = new $classname($this->reportfor, $this);
    ...
    /**
     * Returns the names of all column joins.
     * For every column that is not part of the base of a report,
     * a join relationship can be retrieved using the get_column_joins method
     *
     * @return array Array of column join names.
     */
    function get_columnjoins_names() {
        $joinnames = array();
        $columnjoinlist = $this->get_column_joins();
        foreach ($columnjoinlist as $item) {
            $joinnames[] = $item->name;
        }
        return $joinnames;
    }
    ...
    /**
     * Return a join from the requiredjoins by name
     *
     * @param string $name Join name to get from the join list
     * @return object|false {@link rb_join} object for the matching join, or false
     */
    function get_requiredjoins_item($name) {
        $requiredjoinlist = $this->get_required_joins();
        foreach ($requiredjoinlist as $item) {
     ...
     * @return array An array of {@link rb_join} objects containing join information
     */
    function get_required_joins() {
        if (empty($this->requiredjoins)) {
            return array();
        }

        // Get dependencies join.
        $dependeciesjoins = array();
        $joinlistnames = $this->get_joinlist_names();
        foreach ($this->requiredjoins as $join) {
            if (isset($join->dependencies) && in_array($join->dependencies, $joinlistnames)) {
                $dependeciesjoins[] = $join->dependencies;
            }
        }

        $item = new stdClass();
        $item->joins = $dependeciesjoins;
        $dependencies =  $this->get_joins($item, 'dependencies');
        $this->requiredjoins = array_merge($this->requiredjoins, $dependencies);
     ...
     * @return void
     */
    protected function get_tenant_joins(): array {
        global $CFG, $USER;
        // not a tenant report.
        if (empty($this->tenantid)) {
            return [];
        }
        $is_tenant_member = !empty($USER) && !empty($USER->tenantid);
        return $this->src->limit_to_tenant($this, $CFG->tenantsisolated && $is_tenant_member);
    }
    ...
    function build_query($countonly = false, $filtered = false, $allowcache = true) {
        ...
        // Apply Tenant where SQL clauses defined by the source
        if (!empty($this->tenantid) && !empty($this->src->tenantwhere)) {
            $where[] = $this->src->tenantwhere;
            if (!empty($this->src->tenantparams)) {
                $sqlparams = array_merge($sqlparams, $this->src->tenantparams);
            }
        }
    ...
    protected function collect_joins($filtered, $countonly = false) {
        ...
        // set tenant joins as required joins for tenant reports.
        $tenantjoins = $this->get_tenant_joins();
        $requiredjoins = $this->get_required_joins();
        ...
        $joins = array_merge($tenantjoins, $requiredjoins, $columnjoins, $filterjoins, $paramjoins, $contentjoins, $sourcejoins, $globalrestrictionjoins);
    ...
    public function get_filtered_count($nocache = false) {
        ...
        // Use cached value if present.
        if (is_null($this->_filteredcount) || $nocache) {
     ...
     * @param bool|null $validate_sesskey Validate the session key provided by the form
     * @param lock|bool $lock The execution lock to be closed at the completion of exection
     *
     * @return void No return but initiates save dialog
     */
    function export_data($format, ?bool $validate_sesskey = true, $lock = false) {
        global $PAGE;
        ...
        // We want to be able to release the lock so make sure we terminate after sending ourselves
        $writer->set_terminate_after_sending(false);
    ...
    public function display_table($return = false) {
        ...
        $context = null;
        if (!empty($this->embedobj) && !empty($this->embedobj->embeddedparams['context'])) {
            $context = $this->embedobj->embeddedparams['context'];
        }
        ...
        foreach ($columns as $column) {
            ...
            $type = $column->type;
            $value = $column->value;
            if ($column->display_column(false, $context)) {
        ...
        if (!$this->hidetoolbar && $this->toolbarsearch && $this->has_toolbar_filter()) {
            ...
            // If can save search but no standard filters, we'll add a save search button to the toolbar.
            $save_search = $this->can_save_search() && empty($this->get_standard_filters());
            $mform = new report_builder_toolbar_search_form($this->get_current_url(),
                array('toolbarsearchtext' => $toolbarsearchtext, 'cansave' => $save_search),
                'post', '', null, true, null, 'toolbarsearch');
        ...
        foreach ($columns as $column) {
            if ($column->display_column()) {
                ...
                if ($column->rowheader) {
                    $table->column_header($ident, true);
                }
        ...
        if (!$this->keep_sessions_open) {
            // Close the session to allow user navigation on other pages whilst report is running.
            \core\session\manager::write_close();
        }
        if ($missing) {
    ...
    function edit_button() {
        ...
        if (has_capability($capability, $this->get_context())) {
    ...
    public function post_config_visibility_where($type, $table, $userid = null, $showhidden = false, $fieldid = 'id',
        ...
        // Check we have the columns for visibility or have the joins to make the visibility check.
        $has_required_cols = true;
        $has_required_joins = true;
        ...
        // Check that the join required for the visibility check is in place.
        if ($table !== 'base' && !in_array($table, $this->get_columnjoins_names()) && !$this->get_requiredjoins_item($table)) {
            $has_required_joins = false;
        }

        if (!$this->get_requiredjoins_item('ctx') && !in_array('ctx', $this->get_columnjoins_names())) {
            $has_required_joins = false;
        }

        // Throw exception if we didn't find the required columns or required joins to make the visibility check.
        if (($has_required_cols || $has_required_joins) !== true) {
            throw new moodle_exception('Report is missing required columns or required joins to do the visibility check');
        }

        // Calculate the correct field names including table alias.
        if ($has_required_cols) {
           // We are dealing with visibility columns.
            ...
            $id = $table . '.' . $fieldid;
            $vis = $table . '.' . $fieldvisible;
            $audvis = $table . '.' . $fieldaudiencevisible;
        }
...
function reportbuilder_send_scheduled_report($sched) {
    ...
    try {
        $count = $report->get_filtered_count(true);
    } catch (moodle_exception $e) {
        mtrace('Error sending scheduled report: ' . $e->getMessage());
    }
    ...
    if ($sched->exporttofilesystem == REPORT_BUILDER_EXPORT_EMAIL_AND_SAVE or $sched->exporttofilesystem == REPORT_BUILDER_EXPORT_SAVE) {
        $reportfilepathname = reportbuilder_get_export_filename($report, $sched->userid, $sched->id) . '.' . $extension;
    ...
    // Check if file size exceeds email limit - get limit in megabytes
    $limit = get_config('totara_reportbuilder', 'filesizelimit');
    $limit_in_bytes = floatval($limit) * 1048576;
    if ((filesize($tempfile) > $limit_in_bytes) && $sched->exporttofilesystem !== REPORT_BUILDER_EXPORT_SAVE) {
        $sched->reportname = format_string($reportrecord->fullname);
        reportbuild_scheduled_report_not_sent($sched, $limit);
        scheduled_report_task_failed::create_from_schedule($sched)->trigger();
        mtrace("Error: Scheduled '{$sched->reportname}' report could not be sent as the attached file exceeds the maximum email attachment limit of {$limit}MB");
        return false;
    }
    ...
    if ($sched->exporttofilesystem == REPORT_BUILDER_EXPORT_EMAIL
        ...
        $attachmentfilename = 'report.' . $extension;
...
function reportbuilder_export_schduled_report(stdClass $sched, reportbuilder $report, $writerclass) {
    ...
    do {
        $tempfilepathname = make_temp_directory('reportbuilderexport') . '/'
            . md5(uniqid($USER->id, true)) . '.' . $extension;
    ...
    if ($sched->compress) {
        ...
        ['dirname' => $dirname, 'filename' => $filename] = pathinfo($tempfilepathname);
        $zipfile = $dirname . '/' . $filename . '.zip';
        $packer->archive_to_pathname([
            // change name of file in zip-archive
            "report.{$extension}" => $tempfilepathname
        ], $zipfile);
        return $zipfile;
    }
...
function reportbuild_scheduled_report_not_sent($sched, $limit): void {
    $userto = \core_user::get_user($sched->userid);
    $fromaddress = core_user::get_noreply_user();
    $subject = get_string('scheduled_report_not_sent_subject', 'totara_reportbuilder');

    $messagedetails = new stdClass();
    $messagedetails->reportname = $sched->reportname;
    $messagedetails->limit = $limit;
    $message = get_string('scheduled_report_not_sent_message', 'totara_reportbuilder', $messagedetails);
    $messagehtml = markdown_to_html($message);
    $messageplain = html_to_text($messagehtml);

    email_to_user($userto, $fromaddress, $subject, $messageplain, $messagehtml);
}
