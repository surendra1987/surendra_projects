...
class totara_reportbuilder_renderer extends plugin_renderer_base {
    ...
    public function cache_pending_notification($report = 0) {
        ...
        if ($report instanceof reportbuilder) {
            ...
            if ($status == RB_CACHE_FLAG_FAIL) {
                ...
                if ($report->_id > 0 && has_capability($capability, $report->get_context())) {
     ...
     */
    public function view_all_reports_link($embedded = false, ?int $contextid = null) {
        $string = $embedded ? 'allembeddedreports' : 'alluserreports';
        $params = [];
        if (!empty($contextid)) {
            $params['contextid'] = $contextid;
        }
        $url = $embedded ? new moodle_url('/totara/reportbuilder/manageembeddedreports.php') : new moodle_url('/totara/reportbuilder/', $params);
    ...
    public function print_description($description, $reportid) {
        ...
        if (!empty($description) &&
            trim(strip_tags($description, '<img>')) !== '') {
    ...
    /**
     * Used internally by result_count_info & result_count_heading to calculate the results text string.
     *
     * @param reportbuilder $report
     * @return string
     */
    protected function calculate_result_count_text(reportbuilder $report): string {
        if ($report->get_missing_filtering()) {
            return '';
        }
        ...
        if ($report->can_display_total_count()) {
            ...
            $resultstr = ((int)$unfilteredcount === 1) ? 'xofyrecord' : 'xofyrecords';
            $a = new stdClass();
            $a->filtered = $filteredcount;
            $a->unfiltered = $unfilteredcount;
            $string = get_string($resultstr, 'totara_reportbuilder', $a);
        } else{
            $resultstr = ((int)$filteredcount === 1) ? 'xrecord_title' : 'xrecords_title';
            $string = get_string($resultstr, 'totara_reportbuilder', $filteredcount);
        }
    ...
    public function result_count_info(reportbuilder $report) {
        ...
        $string = $this->calculate_result_count_text($report);

        return html_writer::span($string, 'rb-record-count', ['role' => 'status']);
    }

    /**
     * Returns the heading for the results table of the provided report.
     *
     * @param reportbuilder $report
     * @param null|string $buttons Buttons to append to the right of the heading.
     * @param int $level
     * @return string
     */
    public function result_count_heading(reportbuilder $report, ?string $buttons = null, int $level = 2): string {
        $string = $this->calculate_result_count_text($report);
        ...
        $buttons_output = '';
        if ($buttons) {
            $buttons_output = html_writer::div($buttons, 'rb-report-results-heading-buttons');
        }
        $heading = html_writer::tag('h' . $level, $string);

        return html_writer::div($heading . $buttons_output, 'rb-report-results-heading');
    }
    ...
    public function edit_report_heading(reportbuilder $report) : string {
        $header_text = get_string('editreportwithurl', 'totara_reportbuilder', array(
            'fullname' => format_string($report->fullname),
            'url' => $this->view_report_link($report->report_url())
        ));

        if($report->tenantid) {
            $header_text .= html_writer::div(get_string('tenant_report', 'totara_reportbuilder'), 'label label-info heading-label');
        }
        return $this->page_main_heading($header_text);
    }
    ...
    public function edit_restriction_header(rb_global_restriction $restriction, $currenttab) {
        ...
        if ($restriction->id) {
            $html .= $this->output->page_main_heading(get_string('editrestriction', 'totara_reportbuilder', $restriction->name));
        } else {
            $html .= $this->output->page_main_heading(get_string('newrestriction', 'totara_reportbuilder'));
        }
