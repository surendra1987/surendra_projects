    ...
    public function definition_after_data() {
        ...
        if ($this->stage == request::STAGE_APPROVAL) {
            ...
            if ($managerjaid) {
                ...
                $managername = empty($manager) ? get_string('errordeletedmanager', 'auth_approved') : totara_job_display_user_job($manager, $managerja);
    ...
    private function hierarchies_in_frameworks($cfgkey, $table, $tablefw) {
        ...
        return array_reduce(
            ...
            function (array $existing, \stdClass $hierarchy) {
                ...
                $entries[$hierarchy->id] = format_text($hierarchy->fullname);
                $existing[$fw] = $entries;
