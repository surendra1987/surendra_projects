...
class idp extends base_admin {
    ...
    public function action(): tui_view {
        ...
        $this->set_url(new moodle_url('/auth/ssosaml/idp.php', $id ? ['id' => $id] : null));

        $query_idp = $this->execute_graphql_operation('auth_ssosaml_idp', ['id' => $id])['data']['idp'];
        ...
        $login_url = new moodle_url(get_login_url(), ['ssosaml_idp' => $idp->id]);

        return static::create_tui_view('auth_ssosaml/pages/IdP', [
            'idp' => $query_idp,
            'metadata' => $metadata,
            'fieldInfo' => user_fields::get_info(),
            'userIdFieldNames' => user_fields::get_user_id_fields(),
            'loginUrl' => $login_url->out(),
        ]);
    }
}
