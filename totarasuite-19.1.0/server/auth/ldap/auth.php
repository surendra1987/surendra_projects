...
class auth_plugin_ldap extends auth_plugin_base {
    ...
    function user_signup($user, $notify=true) {
        ...
        if ($notify) {
            ...
            echo $OUTPUT->header();
            echo $OUTPUT->page_main_heading(get_string('emailconfirm', 'auth_approved'));
    ...
    function update_user_record($username, $updatekeys = false, $triggerevent = false, $suspenduser = false) {
        ...
        if ($newinfo = $this->get_userinfo($username)) {
            ...
            if ($needsupdate) {
                ...
                if (isset($newuser->suspended) && $newuser->suspended == 1) {
                    \totara_core\event\user_suspended::create_from_user($user)->trigger();
                }
                return $user;
            }
        ...
        if ($user) {
            complete_user_login(
                ...
                complete_login_callback::create(
                    [\auth_ldap\complete_user_login_callback::class, 'execute'],
                    [$key, $this->pluginconfig]
                )
            );
        }
