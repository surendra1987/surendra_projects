...
$string['auth_ldap_preventpassindb'] = 'Select yes to prevent passwords from being stored in Totara DB.';
...
$string['auth_ntlmsso_enabled'] = 'Set to yes to attempt Single Sign On with the NTLM domain. <strong>Note:</strong> this requires additional setup on the webserver to work, see <a href="https://help.totaralearning.com/display/latest/LDAP+server">https://help.totaralearning.com/display/latest/LDAP+server</a>';
...
$string['pagedresultsnotsupp'] = 'LDAP paged results not supported (either your PHP version lacks support, you have configured Totara to use LDAP protocol version 2 or Totara cannot contact your LDAP server to see if paged support is available.)';
...
$string['updateremfailfield'] = 'Failed to update LDAP with non-existent field (\'{$a->ldapkey}\'). Key ({$a->key}) - old value: \'{$a->ouvalue}\' new value: \'{$a->nuvalue}\'';
