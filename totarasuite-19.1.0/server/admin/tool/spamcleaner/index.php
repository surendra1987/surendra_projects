...
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(get_string('pluginname', 'tool_spamcleaner'));
...
function print_user_entry($user, $keywords, $count) {
    ...
    if (empty($SESSION->users_result[$user->id])) {
        ...
        $html .= '<td width="30%" align="left"><a href="'.$CFG->wwwroot."/user/profile.php?course=1&amp;id=".$user->id.'" title="'.s($user->username).'">'.fullname($user).'</a>';
