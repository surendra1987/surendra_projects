    ...
    public function issuers_table($issuers) {
        ...
        foreach ($issuers as $issuer) {
            ...
            if ($issuer->is_system_account_connected()) {
                ...
                $username = $systemaccount->get('username');
                $email = $systemaccount->get('email');
                if ($email && $username !== $email) {
                    $username = sprintf('%s (%s)', $email, $username);
                }

                $systemauth = html_writer::link(new moodle_url('/admin/tool/oauth2/system_account.php', ['issuerid' => $issuer->get('id')]), s($username));
            ...
            $icon = $this->pix_icon('endpoints', $str, 'tool_oauth2');
            $editendpointlink = html_writer::link($editendpointsurl, $icon);
