...
if ($action == 'edit') {
    if ($endpoint) {
        $strparams = [ 'issuer' => format_string($issuer->get('name')), 'endpoint' => s($endpoint->get('name')) ];
        $PAGE->navbar->add(get_string('editendpoint', 'tool_oauth2', $strparams));
    } else {
        $PAGE->navbar->add(get_string('createnewendpoint', 'tool_oauth2', format_string($issuer->get('name'))));
    }
...
if ($mform && $mform->is_cancelled()) {
    ...
    if ($data = $mform->get_data()) {
        ...
        if ($endpoint) {
            ...
            echo $OUTPUT->heading(get_string('createnewendpoint', 'tool_oauth2', format_string($issuer->get('name'))));
        }
        $mform->display();
        echo $OUTPUT->footer();
    }
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $cancelurl = new moodle_url('/admin/tool/oauth2/endpoints.php', ['issuerid' => $issuerid]);
        echo $OUTPUT->header();
        $strparams = [ 'issuer' => format_string($issuer->get('name')), 'endpoint' => s($endpoint->get('name')) ];
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('endpointsforissuer', 'tool_oauth2', format_string($issuer->get('name'))));
    ...
    echo $renderer->single_button($addurl, get_string('createnewendpoint', 'tool_oauth2', format_string($issuer->get('name'))));
    echo $OUTPUT->footer();
}
