...
if ($action == 'edit') {
    if ($issuer) {
        $PAGE->navbar->add(get_string('editissuer', 'tool_oauth2', format_string($issuer->get('name'))));
    } else {
...
if ($mform && $mform->is_cancelled()) {
    ...
    if ($data = $mform->get_data()) {
        ...
        echo $OUTPUT->header();
        if ($issuer) {
            echo $OUTPUT->page_main_heading(get_string('editissuer', 'tool_oauth2', format_string($issuer->get('name'))));
        } else {
            echo $OUTPUT->page_main_heading(get_string('createnewissuer', 'tool_oauth2'));
        }
        $mform->display();
        echo $OUTPUT->footer();
    }
    ...
    // Validated by $issuer above
    $label = "createnew{$type}issuer";
    echo $OUTPUT->page_main_heading(get_string($label, 'tool_oauth2'));
    $mform->display();
    echo $OUTPUT->footer();
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(get_string('deleteconfirm', 'tool_oauth2', format_string($issuer->get('name'))), $continueurl, $cancelurl);
        echo $OUTPUT->footer();
    } else {
        require_sesskey();
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $cancelurl = new moodle_url('/admin/tool/oauth2/issuers.php');
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(get_string('authconfirm', 'tool_oauth2', format_string($issuer->get('name'))), $continueurl, $cancelurl);
        echo $OUTPUT->footer();
    } else {
        require_sesskey();
        $params = ['sesskey' => sesskey(), 'id' => $issuerid, 'action' => 'auth', 'confirm' => true, 'response' => true];
        $result = core\oauth2\api::connect_system_account($issuer, new moodle_url('/admin/tool/oauth2/issuers.php', $params));

        switch ($result) {
            case core\oauth2\api::SYSTEM_USER_ERROR:
                redirect($PAGE->url, get_string('authnotconnected', 'tool_oauth2'), null, \core\output\notification::NOTIFY_ERROR);
            case core\oauth2\api::SYSTEM_USER_SUCCESS:
                redirect($PAGE->url, get_string('authconnected', 'tool_oauth2'), null, \core\output\notification::NOTIFY_SUCCESS);
            case core\oauth2\api::SYSTEM_USER_NAMELESS:
                $url = new moodle_url('/admin/tool/oauth2/system_account.php', ['issuerid' => $issuerid]);
                redirect($url, get_string('setsystemaccount', 'tool_oauth2'), null, \core\output\notification::NOTIFY_WARNING);
        }
    }
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->page_main_heading(get_string('oauth2consumerdetails', 'tool_oauth2'));
//    echo $OUTPUT->doc_link('OAuth2_Services', get_string('serviceshelp', 'tool_oauth2'));
    ...
    echo $renderer->single_button($addurl, get_string('createnewgoogleissuer', 'tool_oauth2'), 'post', ['class' => 'mb-1']);
    ...
    $params = ['action' => 'edittemplate', 'type' => 'microsoft', 'sesskey' => sesskey(), 'docslink' => $docs,
        'showrequireconfirm' => true];
    $addurl = new moodle_url('/admin/tool/oauth2/issuers.php', $params);
    echo $renderer->single_button($addurl, get_string('createnewmicrosoftissuer', 'tool_oauth2'), 'post', ['class' => 'mb-1']);

    // Microsoft SMTP template.
    $params = ['action' => 'edittemplate', 'type' => 'microsoftsmtp', 'sesskey' => sesskey(), 'docslink' => $docs,
        'showrequireconfirm' => false];
    $addurl = new moodle_url('/admin/tool/oauth2/issuers.php', $params);
    echo $renderer->single_button($addurl, get_string('createnewmicrosoftsmtpissuer', 'tool_oauth2'), 'post', ['class' => 'mb-1']);
    ...
    $params = ['action' => 'edittemplate', 'type' => 'facebook', 'sesskey' => sesskey(), 'docslink' => $docs,
        'showrequireconfirm' => true];
    $addurl = new moodle_url('/admin/tool/oauth2/issuers.php', $params);
    echo $renderer->single_button($addurl, get_string('createnewfacebookissuer', 'tool_oauth2'), 'post', ['class' => 'mb-1']);

    // Nextcloud template.
    $docs = 'admin/tool/oauth2/issuers/nextcloud';
    $params = ['action' => 'edittemplate', 'type' => 'nextcloud', 'sesskey' => sesskey(), 'docslink' => $docs];
    $addurl = new moodle_url('/admin/tool/oauth2/issuers.php', $params);
    echo $renderer->single_button($addurl, get_string('createnewnextcloudissuer', 'tool_oauth2'), 'post', ['class' => 'mb-1']);

    // Generic issuer.
    $addurl = new moodle_url('/admin/tool/oauth2/issuers.php', ['action' => 'edit']);
    echo $renderer->single_button($addurl, get_string('createnewissuer', 'tool_oauth2'), 'post', ['class' => 'mb-1']);
