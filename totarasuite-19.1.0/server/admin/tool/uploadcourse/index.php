...
if (empty($importid)) {
    ...
    if ($form1data = $mform1->get_data()) {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading_with_help(get_string('uploadcourses', 'tool_uploadcourse'), 'uploadcourses', 'tool_uploadcourse', '', '', 1);
        $mform1->display();
        echo $OUTPUT->footer();
        die();
    }
...
if ($form2data = $mform2->is_cancelled()) {
    ...
    if (isset($form2data->showpreview)) {
        echo $OUTPUT->page_main_heading(get_string('uploadcoursespreview', 'tool_uploadcourse'));
        $processor->preview($previewrows, new tool_uploadcourse_tracker(tool_uploadcourse_tracker::OUTPUT_HTML));
        $mform2->display();
    } else {
        echo $OUTPUT->page_main_heading(get_string('uploadcoursesresult', 'tool_uploadcourse'));
