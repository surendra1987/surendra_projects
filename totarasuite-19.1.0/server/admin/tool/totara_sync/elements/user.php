...
class totara_sync_element_user extends totara_sync_element {
    ...
    function config_form(&$mform) {
        ...
        $mform->addElement('static', 'allow_delete_help', '', get_string('delete_help', 'tool_totara_sync'));
    ...
    public function get_confirmation_message(): ?string {
        if (empty($this->config->sourceallrecords) || $this->config->allow_delete === self::KEEP_USERS) {
            return null;
        }

        $import_count = $this->get_current_import_count();
        if ($import_count === 0) {
            return null;
        }

        $existing_count = $this->get_existing_import_count();
        if ($import_count >= $existing_count) {
            return null;
        }

        $percentage = abs($this->get_current_to_existing_records_percentage($existing_count, $import_count));
        if ($this->config->allow_delete == self::SUSPEND_USERS) {
            return get_string('confirm_import_less_suspend_users', 'tool_totara_sync', $percentage);
        } else {
            return get_string('confirm_import_less_delete_users', 'tool_totara_sync', $percentage);
        }
    }
        ...
        if (!empty($this->config->allow_update)) {
            foreach ($rsupdateaccounts as $suser) {
                ...
                if (isset($suser->suspended)) {
                    ...
                    if ($user->suspended == 1 and $suser->suspended == 0) {
                        $unsuspenduser = true;
                    }
                }
    ...
    public function put_custom_field_data($user, $suser) {
        ...
        // If there are no custom fields to sync, then bail quick and return the user
        if ($suser->customfields === null) {
            return $user;
        }
    ...
    function check_sanity($synctable, $synctable_clone) {
        ...
        if (!empty($this->config->sourceallrecords) && !$this->check_table_has_content($synctable)) {
            throw new totara_sync_exception(
                $this->get_name(),
                'importdata',
                'totara_sync:sync_aborted_due_to_empty_source'
            );
        }
        ...
        // Remove records which has cut exceeded length
        $badids = $this->check_user_truncated_records($synctable);
    ...
    public function validate_custom_fields($synctable) {
        ...
        foreach ($rs as $r) {
            $customfields = $r->customfields !== null ? json_decode($r->customfields, true) : null;
    ...
    /**
     * Get users where their records were cut because of exceeded length.
     *
     * @param string $synctable sync table name
     *
     * @return array with invalid ids from synctable for users that do not have full record
     */
    function check_user_truncated_records($synctable) {
        global $DB;

        $invalidids = array();
        $flag = 'data_exceeded_length';
        $sql = "SELECT s.id, u.idnumber
                FROM {user} u
                INNER JOIN {{$synctable}} s
                  ON (u.idnumber = s.idnumber AND u.idnumber != '')
                WHERE s.username = :flag";

        $rs = $DB->get_recordset_sql($sql, array('flag' => $flag));
        foreach ($rs as $r) {
            $invalidids[] = $r->id;
        }
        $rs->close();
