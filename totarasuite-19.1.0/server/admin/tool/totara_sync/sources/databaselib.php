...
function setup_sync_DB($dbtype, $dbhost, $dbname, $dbuser, $dbpass, array $dboptions = array()) {
    ...
    // Add encrypt & trustservercertificate options for phpunit tests
    if ($dbtype === 'sqlsrv' && defined('PHPUNIT_TEST') && PHPUNIT_TEST) {
        $additional_options = ['encrypt', 'trustservercertificate'];
        foreach ($additional_options as $opt) {
            if (isset($CFG->dboptions[$opt])) {
                $dboptions[$opt] = $CFG->dboptions[$opt];
            }
        }
    }
