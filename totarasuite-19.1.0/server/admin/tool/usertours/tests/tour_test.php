 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_usertours_tour_test extends \core_phpunit\testcase {
     ...
     * @return array
     */
    public static function dirty_value_provider() {
        return [
                'name' => [
                        'name',
                        ['Lorem'],
     ...
     * @return array
     */
    public static function get_config_provider() {
        $allvalues = (object) [
                'some' => 'value',
                'another' => 42,
                'key' => [
    ...
    public function test_reset_step_sortorder() {
        ...
        $DB->expects($matcher)
            ->method('set_field')
            ->willReturnCallback(function ($table, $newfield, $newvalue, $conditions) use ($matcher, $expectations) {
                $invocations = $matcher->numberOfInvocations() - 1;
                [$expected_value, $expected_conditions] = $expectations[$invocations];
                $this->assertEquals('tool_usertours_steps', $table);
                $this->assertEquals('sortorder', $newfield);
                $this->assertEquals($expected_value, $newvalue);
                $this->assertEquals($expected_conditions, $conditions);
            })
            ;
     ...
     * @return array
     */
    public static function should_show_for_user_provider() {
        $time = time();
        return [
     ...
     * @return array
     */
    public static function get_tour_key_provider() {
        $id = rand(1, 100);
        $time = time();
     ...
     * @return array
     */
    public static function sortorder_first_last_provider() {
     ...
     * @return array
     */
    public static function get_filter_values_provider() {
     ...
     * @return  array
     */
    public static function set_filter_values_provider() {
        $cheese = ['cheddar', 'boursin', 'mozzarella'];
        $horses = ['coolie', 'dakota', 'leo', 'twiggy'];
    ...
    public function test_validate_tour_context() {
        ...
        $scenarios = [
            ...
            'workspace uses workspace category context for member' => [
                $workspace_context,
                '/container/type/workspace/workspace.php?id=' . $workspace->id,
                $user1,
            ...
            'engage resource uses the owner context for the owner' => [
                $user_context,
                '/totara/engage/resources/article/index.php?id=101',
                $user1,
                $user_context,
            ],
