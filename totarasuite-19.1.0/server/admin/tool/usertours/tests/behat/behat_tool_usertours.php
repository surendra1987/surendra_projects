...
class behat_tool_usertours extends behat_base {
    ...
    /**
     * Checks that the specified css element is in viewport. Only available in tests using Javascript.
     *
     * @Then /^"(?P<selector_string>(?:[^"]|\\")*)" should be in viewport$/
     * @throws ExpectationException
     * @throws DriverException
     * @param string $cssselector
     * @return void
     */
    public function should_be_in_viewport($cssselector) {
        ...
        JS;
        $isVisibleInViewport = $driver->evaluateScript($js);

        if (!$isVisibleInViewport) {
            throw new ExpectationException('"' . $cssselector . '" is not in viewport', $session);
        }
    }
}
