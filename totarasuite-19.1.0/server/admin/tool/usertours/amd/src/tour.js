...
Tour.prototype.revealStep = function (stepConfig) {
    ...
    this.currentStepNode.fadeIn('', $.proxy(function () {
        // No need to announce if this has been dismissed too quickly
        // (I'm looking at you behat)
...
Tour.prototype.positionStep = function (stepConfig) {
    ...
    var recalculateArrowPosition = function recalculateArrowPosition(data) {
        ...
        // To correct arrow position in RTL
        if (right_to_left()) {
            $(arrowElement).css('right', 'unset');
        }
    ...
    if (stepConfig.backdrop) {
        ...
        if (this.isStepActuallyVisible(stepConfig)) {
            ...
            if (targetPosition === 'fixed') {
                let fixedTop = 0;
                let fixedLeft = 0;
                const parentNode = this.nonStaticParent;
                if(parentNode && parentNode.offset()) {
                    fixedTop = (targetNode.offset().top - buffer) - parentNode.offset().top;
                    fixedLeft = (targetNode.offset().left - buffer) - parentNode.offset().left;
                }
                background.css('left', fixedLeft);
                background.css('top', fixedTop);
    ...
    while (elem.length && elem[0] !== document) {
        ...
        if (position === "absolute" || position === "relative" || position === "fixed") {
            ...
            if (!isNaN(value) && value !== 0) {
                // The z-index should be at least higher than Inspire nav to cover it
                const zIndexNav = window.getComputedStyle(document.body).getPropertyValue('--zindex-navbar');
                return Math.max( value, zIndexNav );
            }
        }
        elem = elem.parent();
    }
 ...
 */
Tour.prototype.nonStaticParent;
Tour.prototype.calculatePosition = function (elem) {
    elem = $(elem);
    while (elem.length && elem[0] !== document) {
        var position = elem.css('position');
        if (position !== 'static') {
            this.nonStaticParent = elem;
            return position;
        }
        elem = elem.parent();
    }
