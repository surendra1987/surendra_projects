...
// Only available if debug_developer is enabled
if (!helper::can_import_profiles()) {
    $heading = get_string('import_profile', 'tool_excimer');

    $title = implode(': ', array_filter([get_string('pluginname', 'tool_excimer'), $heading]));
    ...
    echo $OUTPUT->notification(get_string('import_disabled', 'tool_excimer'));
    echo $OUTPUT->footer();
    exit;
}
