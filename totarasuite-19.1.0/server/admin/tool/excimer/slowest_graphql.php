...
admin_externalpage_setup('tool_excimer_report_slowest_graphql');

$url = new moodle_url("/admin/tool/excimer/slowest_graphql.php");

if ($script || $group) {
    $table = new profile_table('profile_table_slowest_graphql');
    ...
    $table = new grouped_script_profile_table('profile_table_slowest_graphql');
    $table->set_url_path($url);
    $table->sortable(true, 'maxduration', SORT_DESC);
}

$table->set_scripttypes([profile::SCRIPTTYPE_GRAPHQL, profile::SCRIPTTYPE_GRAPHQL_DEV, profile::SCRIPTTYPE_EXTERNALAPI, profile::SCRIPTTYPE_MOBILE]);

profile_table_page::display($table, $url, get_string('report_slowest_graphql', 'tool_excimer'));