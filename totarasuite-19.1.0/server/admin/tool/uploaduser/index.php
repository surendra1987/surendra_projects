...
if (empty($iid)) {
    ...
    if ($formdata = $mform1->get_data()) {
        ...
        echo $OUTPUT->heading_with_help(get_string('uploadusers', 'tool_uploaduser'), 'uploadusers', 'tool_uploaduser', '', '', 1);
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    // Print the header
    echo $OUTPUT->header();
    echo $OUTPUT->page_main_heading(get_string('uploadusersresult', 'tool_uploaduser'));
    ...
    while ($line = $cir->next()) {
        ...
        if (!empty($user->deleted)) {
            if (!$allowdeletes or !has_capability('moodle/user:delete', context_system::instance())) {
                $usersskipped++;
        ...
        if ($existinguser) {
            ...
            $trigger_user_unsuspended_event = false;
            if ($allowsuspends and isset($user->suspended) and $user->suspended !== '') {
                ...
                if ($existinguser->suspended != $user->suspended) {
                    ...
                    if ($existinguser->suspended) {
                        ...
                        $trigger_user_unsuspended_event = true;
                    }
                }
            }
            ...
            if ($doupdate or $existinguser->password !== $oldpw) {
                ...
                // If user suspended then trigger suspended user event.
                if ($trigger_user_suspended_event) {
                    \totara_core\event\user_suspended::create_from_user($user)->trigger();
                }
                // If user unsuspended (activated) then trigger unsuspended user event.
                if ($trigger_user_unsuspended_event) {
...
// Print the header
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(get_string('uploaduserspreview', 'tool_uploaduser'));
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
    ...
    $noerror = uu_check_custom_profile_data($rowcols, $profilefieldvalues) && $noerror;
