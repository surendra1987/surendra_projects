...
if ($ADMIN->fulltree) {
    ...
    $temp->add(new admin_setting_loginpageredirect());
    ...
    if (!empty($CFG->enable_legacy_login)) {
        $temp->add(new admin_setting_confightmleditor('auth_instructions', new lang_string('instructions_deprecated', 'auth'),
...
// Approvalform settings
if ($hassiteconfig) {
    approvalform::init_plugin_settings($ADMIN);
}

// MFA settings
if ($hassiteconfig) {
    $ADMIN->add('modules', new admin_category('mfa', new lang_string('name', 'mfa')));

    $mfa_settings = new admin_settingpage('mfa_settings', new lang_string('manage_multi_factor_authentication', 'mfa'));
    $mfa_settings->add(new \core_mfa\admin\settings\manage_mfa_factors());

    $ADMIN->add('mfa', $mfa_settings);

    foreach (core_plugin_manager::instance()->get_plugins_of_type('mfa') as $plugin) {
        if ($plugin->is_enabled()) {
            $plugin->load_settings($ADMIN, 'mfa', $hassiteconfig);
        }
    }
}
