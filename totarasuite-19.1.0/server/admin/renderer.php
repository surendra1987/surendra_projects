...
class core_admin_renderer extends plugin_renderer_base {
    ...
    public function upgrade_stale_php_files_page() {
        ...
        $docsurl = get_docs_url('Upgrading+from+Totara');
        if (empty($docsurl)) {
            $output .= format_text(get_string('upgradestalefilesinfonohelplink', 'admin'), FORMAT_MARKDOWN);
        } else {
            $output .= format_text(get_string('upgradestalefilesinfo', 'admin', $docsurl), FORMAT_MARKDOWN);
        }
        $output .= html_writer::empty_tag('br');
     ...
     * @param bool $unused_devlibdir Depreacted since Totara 13
    ...
    public function admin_notifications_page($maturity, $insecuredataroot, $errorsdisplayed,
            ...
            $themedesignermode, $unused_devlibdir, $unused_latesterror, $activeusers, $totara_release, $activeusers3mth = 0, $croninfrequent = false) {
        global $CFG, $PAGE;
        $output = '';
        ...
        $output .= $this->header();
        $output .= $this->page_main_heading(get_string('systeminformation', 'core'));
    ...
    public function plugin_management_page(core_plugin_manager $pluginman, \core\update\checker $checker, array $options = array()) {
        ...
        $output .= $this->header();
        $output .= $this->page_main_heading(get_string('pluginsoverview', 'core_admin'));
    ...
    public function environment_check_page($versions, $version, $envstatus, $environment_results) {
        ...
        // Heading.
        $output .= $this->page_main_heading(get_string('environment', 'admin'));
        ...
        foreach ($environment_results as $environment_result) {
            ...
            if ($continue) {
                ...
                if (empty($CFG->docroot) or $environment_result->plugin) {
                    ...
                    $report = $this->doc_link(implode('/', $linkparts), get_string($stringtouse, 'admin', $rec));
                }
    ...
    public function upgradekey_form_page($url) {
        ...
        $output .= $this->page_main_heading(get_string('upgradekeyreq', 'core_admin'));
        ...
        $output .= html_writer::empty_tag('input', array('name' => 'upgradekey', 'type' => 'password', 'class' => 'form-control w-auto d-inline mr-1'));
    ...
    /**
     * Render a warning when users try to upgrade the site without providing the upgradekey.
     *
     * @return string
     */
    public function upgradekey_warning_from_page(): string {
        $output = '';
        $output .= $this->header();
        $output .= $this->warning(get_string('noupgradekeywarning'));
        $output .= $this->footer();
