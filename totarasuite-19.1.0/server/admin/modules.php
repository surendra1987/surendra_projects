...
/// If data submitted, then process and store.
    ...
    if (!empty($show) and confirm_sesskey()) {
        ...
        // Trigger a hook to check if any other components want to prevent enabling of this activity module.
        $hook = new activity_enable($show, true);
        $hook->execute();

        if ($hook->prevent_enable === true) {
            $a = ['mod' => $hook->mod, 'reason' => $hook->prevent_enable_reason];
            print_error('error:cannot_enable_prohibited_activity_module', 'admin', '', $a);
        }
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->page_main_heading($stractivities);
...
/// Print the table of all modules
    ...
    foreach ($modules as $module) {
        ...
        if ($missing) {
            ...
            // Trigger a hook to check if any other components want to prevent enabling of this activity module.
            $hook = new core\hook\activity_enable($module->name);
            $hook->execute();

            $class =   'dimmed_text';
            if ($hook->prevent_enable === true) {
                $a = ['mod' => $hook->mod, 'reason' => $hook->prevent_enable_reason];
                $reason = get_string('error:cannot_enable_prohibited_activity_module', 'admin', $a);
                $visible = $OUTPUT->flex_icon('show', array('classes' => 'ft-state-disabled', 'alt' => $reason, 'title' => $reason)) . "</a>";
            } else {
