 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class webservice_xmlrpc_locallib_test extends \core_phpunit\testcase {
     ...
     */
    public function test_prepare_response(string $returnsdesc, string $returns, string $expected) {
     ...
     * @param array $exception_args
     * @param string|null $code An error code to be provided to generate_error
     * @param string $expected The expected XML-RPC response
     * @throws ReflectionException
     */
    public function test_generate_error(string $exception_class, array $exception_args, ?string $code, string $expected) {
        $server = $this->getMockBuilder('webservice_xmlrpc_server')
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();
        ...
        // Create exception instance when needed
        $exception = new $exception_class(...$exception_args);
    ...
    public static function prepare_response_provider(): array {
        return [
            'Latin script' => [
                ...
                'Mellon!',
                '<?xml version="1.0" encoding="UTF-8"?><methodResponse><params><param><value><string>Mellon!</string></value></param></params></methodResponse>'
            ],
            'Non-Latin glyphs' => [
                'What biscuits do you have?',
                '😂🤵😂 𝒪𝓃𝓁𝓎 𝓉𝒽𝑒 𝒻𝒾𝓃𝑒𝓈𝓉 𝐼𝓉𝒶𝓁𝒾𝒶𝓃 𝒷𝒾𝓈𝒸𝓊𝒾𝓉𝓈 😂🤵😂',
                '<?xml version="1.0" encoding="UTF-8"?><methodResponse><params><param><value><string>😂🤵😂 𝒪𝓃𝓁𝓎 𝓉𝒽𝑒 𝒻𝒾𝓃𝑒𝓈𝓉 𝐼𝓉𝒶𝓁𝒾𝒶𝓃 𝒷𝒾𝓈𝒸𝓊𝒾𝓉𝓈 😂🤵😂</string></value></param></params></methodResponse>'
            ]
        ];
    }
    ...
    public static function generate_error_provider(): array {
        return [
            ...
            'Exception with special chars' => [
                \Exception::class,
                ['P̫̬̳̫̓͊̇r̨͎̜ͧa͚̬̙̺͎̙ͬẏ͎̲̦̲e̶̞͎͙̻͐̉r͙̙ͮ̓̈ͧ̔̃ ̠ͨ́ͭ̎̎̇̿n̗̥̞͗o̼̖͛̂̒̿ͮ͘t̷̞͎̘̘̝̥̲͂̌ͭ ͕̹͚̪͖̖̊̆́̒ͫ̓̀fͤͦͭͥ͊ͩo̼̱̻̹͒̿͒u̡͕̞͕̜̠͕ͥͭ̈̄̈́͐ń̘̼̇͜d̸̰̻͎͉̱̰̥̿͒'],
                null,
