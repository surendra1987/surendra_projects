...
class profile_field_checkbox extends profile_field_base {
     ...
     * @param int $fieldid
     * @param int $userid
     * @param object|null $field Totara: Preloaded field data, if available (saves queries if passed with data)
     * @param object|null $data Totara: Preloaded data for the field, if available (saves queries if passed with field)
     */
    public function __construct($fieldid=0, $userid=0, ?object $field = null, ?object $data = null) {
        global $DB;
        // First call parent constructor.
        parent::__construct($fieldid, $userid, $field, $data);
    ...
    function validate_field_from_inputs(array $params): void {
        ...
        if ($params['data'] !== "0" && $params['data'] !== "1") {
            throw new moodle_exception("{$field}: You must pass 0 or 1 for data for checkbox fields.");
        }

        if (isset($params['data_format'])) {
            throw new moodle_exception("{$field}: data_format should not be passed for checkbox fields.");
        }
