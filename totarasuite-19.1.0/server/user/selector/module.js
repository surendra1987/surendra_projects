...
M.core_user.init_user_selector = function (Y, name, hash, extrafields, lastsearch) {
    ...
    var user_selector = {
        ...
        init : function() {
            ...
            this.searchfield.insert(Y.Node.create('<label for="' + this.get_clean_string(this.name) + '_searchtext">' + this.get_clean_string(searchbutton.get('value')) + '</label>'), this.searchfield);
            searchbutton.remove();
            ...
            this.clearbutton = Y.Node.create('<input type="button" value="' + this.get_clean_string(clearbtn.get('value')) + '" class="btn btn-secondary mx-3"/>');
        ...
        output_options : function(data) {
            ...
            if (!count) {
                var searchstr = M.util.get_string('none', 'moodle');
                if (this.lastsearch != '') {
                    searchstr = M.util.get_string('nomatchingusers', 'moodle');
                    searchstr = this.insert_search_into_str(searchstr, this.get_clean_string(this.lastsearch));
                }
        ...
        /**
         * Provide some string cleaning to pass a security scan on the user selector.
         * @param {string} val
         * @returns a sanitized string.
         */
        get_clean_string : function(val) {
            val = Y.Escape.html(val);
