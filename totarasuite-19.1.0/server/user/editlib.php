 ...
 * @param null|string $section The optional preference page that is set up.
 * @return array containing the user and course records.
 */
function useredit_setup_preference_page($userid, $courseid, ?string $section = null) {
    ...
    if ($user->id == $USER->id) {
        $capability = 'moodle/user:editownprofile';
        if ($section === 'language') {
            $capability = 'moodle/user:editownlanguage';
        }

        // Editing own profile - require_login() MUST NOT be used here, it would result in infinite loop!
        if (!has_capability($capability, $personalcontext)) {
