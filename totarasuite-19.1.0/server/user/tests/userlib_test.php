 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_user_userlib_test extends \core_phpunit\testcase {
     ...
     * @return array
     */
    public static function data_create_user_invalid_username() {
        return [
            'empty_string' => [
                '',
    ...
    /**
     * Test the language menu option for user menu
     */
    public function test_user_menu_lang_menu_option() {
        global $PAGE;
        ...
        // and feature enabled
        set_config('langmenu', true);

        $user_info = user_get_user_navigation_info($user, $PAGE);
        $this->assertEquals(
            get_string('language', 'core'),
            $user_info->navitems[count($user_info->navitems) - 2]->title
        );

        // and feature disabled
        set_config('langmenu', false);
        ...
        // Now test with only one language
        $cache->set($cachekey, [
            'en' => 'English (en)',
        ]);
