@totara @tool @_file_upload @javascript @profile_fields @totara_reportbuilder
Feature: User profile fields handle the unique values correctly.
  ...
  Background:
    ...
    # Create a user decimal input custom field set as unique.
    When I set the following fields to these values:
      | datatype | decimal |
    And I set the following fields to these values:
      | Name                       | Float 1 |
    ...
    And I press "Save changes"
    Then I should see "User profile fields"
    Then I should see "Float 1"

    # Create a user integer input custom field set as unique.
    When I set the following fields to these values:
      | datatype | integer |
    And I set the following fields to these values:
      | Name                       | Number 1 |
  ...
  Scenario: Verify unique user profile fields when creating a user with non-unique values fail uniqueness check in manual user creation.
    ...
    When I set the following fields to these values:
      ...
      | Float 1                          | 5.1                  |
    And I press "Save and go back"
    Then I should not see the form validation error "This value has already been used." for the "text1" user profile field
    And I should see the form validation error "This value has already been used." for the "float1" user profile field

    When I set the following fields to these values:
      | Float 1                          | 5.2                  |
      | Number 1                         | 10                   |
    And I press "Save and go back"
    Then I should not see the form validation error "This value has already been used." for the "float1" user profile field
    And I should see the form validation error "This value has already been used." for the "number1" user profile field
