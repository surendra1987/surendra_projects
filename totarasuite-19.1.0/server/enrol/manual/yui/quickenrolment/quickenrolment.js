        ...
        initializer : function(config) {
            ...
            if (this.get(UEP.DISABLEGRADEHISTORY) != true) {
                recovergrades = create('<div class="'+CSS.ENROLMENTOPTION+' '+CSS.RECOVERGRADES+'"></div>')
                    ...
                    .append(create('<input type="checkbox" class="mx-3" id="'+CSS.RECOVERGRADES+'" name="'+CSS.RECOVERGRADES+'"'+ this.get(UEP.RECOVERGRADESDEFAULT) +' />'))
            }

            this.set(UEP.BASE, create('<div class="'+CSS.PANEL+' '+CSS.HIDDEN+'"></div>')
                .append(create('<div class="modal-backdrop"></div>'))
                .append(create('<div class="'+CSS.WRAP+' modal show modal-dialog modal-content"></div>')
                    .append(create('<div class="'+CSS.HEADER+' header modal-header"></div>')
                        .append(create('<button aria-label="' + M.util.get_string('closebuttontitle', 'core') + '" title="' + M.util.get_string('closebuttontitle', 'core') + '" class="'+CSS.CLOSE+'"></button>'))
                    ...
                    .append(create('<div class="'+CSS.CONTENT+' modal-body"></div>')
                        .append(create('<div class="'+CSS.SEARCHCONTROLS+' form-inline"></div>')
                            ...
                            .append(create('<div class="'+CSS.ENTITYSELECTOR+' my-3"></div>'))
                            ...
                            .append(create('<div class="'+CSS.SEARCH+'"><label for="enrolusersearch" class="accesshide">'+M.util.get_string('usersearch', 'enrol')+'</label></div>')
                                ...
                                .append(create('<input type="button" id="searchbtn" class="'+CSS.SEARCHBTN+' btn btn-secondary ml-3" value="'+M.util.get_string('usersearch', 'enrol')+'" />'))
                            )
                        )
            ...
            if (this.get(UEP.COHORTSAVAILABLE)) {
                this.get(UEP.BASE).one('.'+CSS.ENTITYSELECTOR)
                    .append(create('<input type="radio" id="id_enrol_manual_entity_users" name="enrol_manual_entity" value="users" class="mr-3" checked="checked"/>'))
                    .append(create('<label for="id_enrol_manual_entity_users">'+ M.util.get_string('browseusers', 'enrol_manual')+'</label>'))
                    .append(create('<input type="radio" id="id_enrol_manual_entity_cohorts" name="enrol_manual_entity" class="mx-3" value="cohorts"/>'))
            ...
            for (var i in result.response.users) {
                ...
                users.append(create('<div class="'+CSS.USER+' clearfix list-group-item list-group-item-action" rel="'+user.id+'"></div>')
                    ...
                    .append(create('<div class="'+CSS.OPTIONS+'"></div>')
                        .append(create('<input type="button" class="'+CSS.ENROL+' btn btn-secondary" value="'+M.util.get_string('enrol', 'enrol')+'" aria-label="'+M.util.get_string('enrol_user','enrol', user.fullname)+'" />')))
                );
            }
