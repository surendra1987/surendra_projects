        ...
        if ($course = $DB->get_record('course', array('id' => $instance->courseid))) {
            ...
            if ($result->enroled) {
                ...
                if ($result->notify && !$preventredirect) {
                    $a = new stdClass();
                    $a->course = format_string($course->fullname, true, ['context' => $coursecontext]);
                    $a->program = format_string($result->program, true, ['context' => $result->program_context]);
                    require_once($CFG->dirroot . '/course/lib.php');
                    $courseformat = course_get_format($course);
                    $viewurl = new moodle_url('/course/view.php', array('id' => $course->id));
                    \core\notification::success(get_string('nowenrolled', 'enrol_totara_program', $a));
