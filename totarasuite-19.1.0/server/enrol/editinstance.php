...
if ($instanceid) {
    $instance = $DB->get_record_select(
        'enrol',
        'courseid = :courseid AND enrol = :enrol AND id = :id AND status <> :deleted',
        [
            'courseid' => $course->id,
            'enrol' => $type,
            'id' => $instanceid,
...
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(get_string('pluginname', 'enrol_' . $type));
$mform->display();
echo $OUTPUT->footer();
