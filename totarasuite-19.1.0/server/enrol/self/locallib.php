...
class enrol_self_enrol_form extends moodleform {
    ...
    public function definition($mform = null) {
        ...
        $requests_approval = enrolment_approval_helper::approval_available_for($instance->id, $USER->id);
        ...
        if ($requests_approval) {
            // Hide button, if no job assignment for the learner.
            if (!$job_selector_added) {
                $mform->addElement('submit', 'request_application', get_string('requestapproval', 'enrol_self'));
            }
        } else {
