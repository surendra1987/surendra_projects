...
class enrol_self_plugin extends enrol_plugin {
    ...
    /**
     * Enable approval workflows for self enrolments
     *
     * @return boolean
     */
    ...
    public function enrol_self(stdClass $instance, $data = null) {
        ...
        if (isset($data->job_assignment) && $data->job_assignment) {
            $instance->job_assignment_id = $data->job_assignment;
        }
    ...
    public function do_non_interactive_enrol(stdClass $instance, int $user_id, ?bool $with_notificaton = false): bool {
        if ($this->supports_non_interactive_enrol($instance, $user_id)) {
            ...
            if ($with_notificaton && !$instance->workflow_id) {
                \core\notification::add(
                    get_string('enrol_success_message', 'enrol_self'),
                    \core\notification::SUCCESS
                );
    ...
    public function supports_non_interactive_enrol(stdClass $instance, int $user_id): bool {
        ...
        // Non-interactive enrol may not be available if approval required.
        if (!empty($instance->workflow_id)) {
            // Not available if workflow is not available.
            if (!enrolment_approval_helper::approval_available_for($instance->id, $user_id)) {
                return false;
            }
            // Not available if workflow requires manager selection.
            if (enrolment_approval_helper::workflow_requires_manager($instance->workflow_id)) {
                return false;
            }
        }

        // Check if user is already enrolled.
        if ($DB->record_exists('user_enrolments', array('userid' => $user_id, 'enrolid' => $instance->id)) && !enrolment_approval_helper::user_enrolment_pending($instance->courseid, $user_id)) {
            return false;
        }
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($instance->workflow_id && !enrolment_approval_helper::approval_available_for($instance->id, $USER->id)) {
            return get_string('canntenrolworkflow', 'enrol_self');
        }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        // Totara: Preventing running trim against a null to fix a PHP 8.1 deprecation
        if ($instance->customtext1 !== null && trim($instance->customtext1) !== '') {
