...
class enrol_cohort_plugin extends enrol_plugin {
    ...
    public function update_instance($instance, $data) {
        ...
        // Process enrolments in an adhoc task.
        $adhoctask = new \totara_cohort\task\enrol_audience_in_course_task();
        $adhoctask->set_custom_data(array('cohortid' => $data->customint1, 'courseid' => $instance->courseid));
        $adhoctask->set_component('totara_cohort');
        \core\task\manager::queue_adhoc_task($adhoctask);
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        $params = array(
            ...
            'id' => $data['id'],
            'deleted' => ENROL_INSTANCE_DELETED,
        );
        $sql = "roleid = :roleid AND customint1 = :customint1 AND courseid = :courseid AND enrol = 'cohort' AND id <> :id AND status <> :deleted";
    ...
    /**
     * When deleting a cohort, we're going to run the actual delete in a task as it can
     * potentially take time.
     *
     * If the provided instance has a status that is not ENROL_INSTANCE_DELETED, then the status will
     * be updated and the deletion task will be scheduled.
     *
     * If the status is already ENROL_INSTANCE_DELETED, then the instance will call the parent delete method.
     *
     * @param $instance
     * @return void
     */
    public function schedule_delete_instance($instance): void {
        global $DB;
        ...
        if ($instance->status == ENROL_INSTANCE_DELETED) {
            return;
        }

        $instance->status = ENROL_INSTANCE_DELETED;
        $DB->update_record('enrol', $instance);

        $task = new \enrol_cohort\task\delete_enrol_instance();
        $task->set_custom_data(['instance_id' => $instance->id]);
