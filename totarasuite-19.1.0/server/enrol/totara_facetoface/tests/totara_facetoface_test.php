 ...
 */
class enrol_totara_facetoface_totara_facetoface_test extends \core_phpunit\testcase {
    ...
    public function test_enrol_totara_facetoface_find_best_session() {
        ...
        $sessiondate = new stdClass();
        $sessiondate->timestart = time() + (YEARSECS + DAYSECS);
        $sessiondate->timefinish = time() + (YEARSECS + DAYSECS + MINSECS);
        $sessiondate->sessiontimezone = 'Pacific/Auckland';
        $sessiondata = array(
            'facetoface' => $facetoface->id,
            'capacity' => 3,
