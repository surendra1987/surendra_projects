 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cachestore_redis_compressor_test extends \core_phpunit\testcase {
     ...
     * @return array
     */
    public static function provider_for_tests_setget() {
        if (!cachestore_redis::are_requirements_met()) {
            ...
            // PHPUnit doesn't allow blank providers, so we provide a dummy record
            return [[]];
        }
    ...
    public function test_it_can_use_getset($name, $serializer, $compressor, $rawexpected1, $rawexpected2) {
        if (!cachestore_redis::are_requirements_met()) {
            $this->markTestSkipped('Requirements not met');
        }
