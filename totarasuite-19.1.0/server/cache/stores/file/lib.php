...
class cachestore_file extends cache_store implements cache_is_key_aware, cache_is_configurable, cache_is_searchable  {
    ...
    protected function prep_data_after_read($data, $path) {
        ...
        if ($result === false && $data != serialize(false)) {
            debugging('Failed to unserialise data from cache file: ' . $path, DEBUG_DEVELOPER);
            return false;
        }
    ...
    public function purge() {
        if ($this->isready && is_dir($this->path)) {
            // We want to avoid globbing to find many cache files, as they can number in the millions.
            // Find the desired cache directories first.
            $directories = [];
            if ($this->singledirectory) {
                ...
                foreach (new DirectoryIterator($this->path) as $file_info) {
                    ...
                    if ($file_info->isDir()) {
                        $directories[] = $this->path . DIRECTORY_SEPARATOR . $file_info->getFilename();
                    }
                }
            }

            // Remove each cached file from a cache directory one at a time. We want to avoid big arrays of files.
            foreach ($directories as $current_dir) {
                if ($dh = opendir($current_dir)) {
                    while (($file = readdir($dh)) !== false) {
                        if (pathinfo($file, PATHINFO_EXTENSION) === 'cache') {
                            // Existing core behaviour is to suppress error output here.
                            @unlink($current_dir . DIRECTORY_SEPARATOR . $file);
                        }
                    }
                    closedir($dh);
                }
    ...
    protected function write_file($file, $content) {
        ...
        if (!$result) {
            ...
            // rename() does not follow the *nix rename convention on Windows with PHP 5.
            // If the $newname exists, it will return FALSE and $oldname and $newname will remain in their original state.
