...
class cache_config_writer extends cache_config {
    ...
    protected function config_save() {
        ...
        // Unset confighash when config file has been deleted and we still have a set confighash.
        if (!file_exists($cachefile) && !empty(self::$confighash)) {
            self::$confighash = '';
        }
        ...
        $hash = sha1($content);
        if (($hash === static::$confighash) || (file_exists($cachefile) && $content === file_get_contents($cachefile))) {
            // Config is unchanged so don't bother locking and writing.
            static::$confighash = $hash;
            return;
        }
