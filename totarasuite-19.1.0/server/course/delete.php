...
if ($delete === md5($course->timemodified)) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->page_main_heading($strdeletingcourse);
    // This might take a while. Raise the execution time limit.
    core_php_time_limit::raise();
    // We do this here because it spits out feedback as it goes.
    delete_course($course);
    echo $OUTPUT->heading( get_string("deletedcourse", "", $courseshortname) );
    // Fix sortorder of the cateogry.
    $category = course_category::load_by_id($course->category);
    $category->ensure_sequential_course_sortorder();
    // Update course count in categories.
    $category->update_course_count($category->coursecount-1);
    // Now fix the paths and depths in the context table if needed.
    $fixcontexts = [$coursecontext->id => $coursecontext];
    $category->fix_contexts_after_course_changes($fixcontexts);
...
$message = "{$strdeletecoursecheck}</p><p>{$coursefullname} ({$courseshortname})";
