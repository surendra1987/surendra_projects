...
$percategorypage = optional_param('percategorypage', null, PARAM_INT);
...
if ($categorypage !== 0) {
    $url->param('categorypage', $categorypage);
}
if ($page !== 0) {
    $url->param('page', $page);
}
...
if ($action !== false && confirm_sesskey()) {
    ...
    switch ($action) {
        ...
        case 'deletecategory':
            ...
            echo $renderer->page_main_heading(get_string('deletecategory', 'moodle', $category->get_formatted_name()));
...
if (!is_null($percategorypage)) {
    set_user_preference('coursecat_management_percategorypage', $percategorypage);
} else {
    $percategorypage = get_user_preferences('coursecat_management_percategorypage', 20);
}
if ((int)$percategorypage != $percategorypage || $percategorypage < 2) {
    $percategorypage = 20;
}
if (\totara_core\version_specific\version_specific_feature::is_disabled('totara_19_1_core_course_course_category_pagination')) {
    $percategorypage = 99999;
}
if (!is_null($perpage)) {
...
if ($displaycategorylisting) {
    ...
    echo $renderer->category_listing($category, $categorypage, $percategorypage);
