...
if (!empty($add)) {
    ...
    $hook = new \totara_core\hook\mod_add($course, $add, $section);
    $hook->execute();
...
if ($mform->is_cancelled()) {
    ...
    if (get_string_manager()->string_exists('modulename_help', $module->name)) {
        echo $OUTPUT->heading_with_help($pageheading, 'modulename', $module->name, 'icon', '', 1);
    } else {
        echo $OUTPUT->heading_with_help($pageheading, '', $module->name, 'icon', '', 1);
    }
