...
if (!$cmid && !$courseid) {
    error_or_ajax('invalidarguments', $fromajax);
}
...
if ($courseid) {
    ...
    try {
        ...
        error_or_ajax('mustbeloggedin', $fromajax);
    }
    ...
    if ($user && $rolec) {
        ...
        if ($confirm and confirm_sesskey()) {
            ...
            if (!$completion) {
                error_or_ajax('noselfcompletioncriteria', $fromajax, 'completion');
            }

            // Check if the user has already marked themselves as complete
            if ($completion->is_complete()) {
                error_or_ajax('useralreadymarkedcomplete', $fromajax, 'completion');
            }
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->page_main_heading(get_string('completecoursex', 'core', format_string($course->fullname)));
...
switch($targetstate) {
    ...
    case COMPLETION_INCOMPLETE:
        break;
    default:
        error_or_ajax('unsupportedstate', $fromajax);
}
...
function error_or_ajax($message, $fromajax, $module = 'error') {
    if ($fromajax) {
