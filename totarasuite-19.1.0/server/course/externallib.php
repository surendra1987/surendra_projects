...
class core_course_external extends external_api {
    ...
    public static function update_courses($courses) {
        ...
        foreach ($params['courses'] as $course) {
            ...
            try {
                ...
                // Field 'duedate_op' is not a course record's field but is the course editing form field.
                // Course dutedate-related fields are not allowed to be updated by external API, so if these fields are
                // not null, just set value to 'duedate_op' to avoid dataloss.
                if (!empty($oldcourse->duedate)) {
                    $course['duedate_op'] = course::DUEDATEOPERATOR_FIXED;
                } else if (!empty($oldcourse->duedateoffsetamount) && !empty($oldcourse->duedateoffsetunit)) {
                    $course['duedate_op'] = course::DUEDATEOPERATOR_RELATIVE;
                }
    ...
    public static function delete_courses($courseids) {
        ...
        $category_ids = [];
        // Now fix the paths and depths in the context table if needed.
        $fixcontexts = [];
        ...
        foreach ($params['courseids'] as $courseid) {
            ...
            $fixcontexts[$course->category][] = $coursecontext;
            if (!in_array($course->category, $category_ids)) {
                $category_ids[] = $course->category;
            }
        }

        // If courses were delete without error, then we reorder existing courses in the affected course categories
        if (empty($warnings) && !empty($category_ids)) {
            foreach ($category_ids as $id) {
                $category = \core\model\course_category::load_by_id($id);
                if (empty($category)) {
                    continue;
                }
                $category->ensure_sequential_course_sortorder(true);
                $category->fix_contexts_after_course_changes($fixcontexts[$id]);
            }
