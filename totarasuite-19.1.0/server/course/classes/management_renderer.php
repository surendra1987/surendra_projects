...
class core_course_management_renderer extends plugin_renderer_base {
    ...
    public function management_heading($heading, $viewmode = null, $categoryid = null) {
        $html = '';
        if (!empty($heading)) {
            $html .= $this->page_main_heading($heading);
        }
        $html .= html_writer::start_div('coursecat-management-header clearfix');
        if ($viewmode !== null) {
            $html .= html_writer::start_div('coursecat_view');
     ...
     *
     * @param coursecat|null $category The currently selected category. Also the category to highlight in the listing.
     * @param int $page The current page of the category listing.
     * @param int $perpage The number of categories per page.
     * @return string
     */
    public function category_listing(?coursecat $category = null, int $page = 0, int $perpage = 5) {
        if ($category === null) {
        ...
        $totalcategories = $this->total_visible_categories();
        ...
        $listing = coursecat::get(0)->get_children($options);
        ...
        $html .= $this->category_listing_actions($category, $perpage);
        $html .= $this->category_listing_pagination($page, $perpage);
        $html .= html_writer::start_tag('ul', $attributes);

        $listing_index_current = $offset;
        ...
        foreach ($listing as $listitem) {
            ...
            $html .= $this->category_listitem(
                ...
                $selectedcategory,
                $selectedparents,
                $listing_index_current === 0,
                $listing_index_current === $totalcategories - 1
            );

            $listing_index_current++;
        }
        $html .= html_writer::end_tag('ul');
        $html .= $this->category_listing_pagination($page, $perpage, true);
    ...
    /**
     * Finds the approximate number of visible categories in the category listing.
     * @return int
     */
    protected function total_visible_categories(): int {
        return \core\entity\course_categories::repository()
            ->where('issystem', '=', 0)
            ->where('parent', '=', 0)
            ->count();
    }
     ...
     *
     * @param int $categorypage The current page.
     * @param int $percategorypage The number of categories to display per page.
     * @param bool $showtotals Set to true to show the total number of categories and what is being displayed.
     * @return string
     */
    protected function category_listing_pagination($categorypage, $percategorypage, $showtotals = false) {
        $html = '';
        $totalcategories = $this->total_visible_categories();
        $totalpages = ceil($totalcategories / $percategorypage);
        if ($showtotals) {
            if ($totalpages == 0) {
                $str = get_string('nocategoriesyet');
            } elseif ($totalpages == 1) {
                $str = get_string('showingacategories', 'moodle', $totalcategories);
            } else {
                $a = new stdClass;
                $a->start = ($categorypage * $percategorypage) + 1;
                $a->end = min((($categorypage + 1) * $percategorypage), $totalcategories);
                $a->total = $totalcategories;
                $str = get_string('showingxofycategories', 'moodle', $a);
            }
            $html .= html_writer::div($str, 'listing-pagination-totals dimmed');
        }

        if ($totalcategories <= $percategorypage) {
            return $html;
        }
        $aside = 2;
        $span = $aside * 2 + 1;
        $start = max($categorypage - $aside, 0);
        $end = min($categorypage + $aside, $totalpages - 1);
        if (($end - $start) < $span) {
            if ($start == 0) {
        ...
        $ellipsis = html_writer::tag('span', '...', ['class' => 'page-numbers']);
        $items = array();
        $baseurl = new moodle_url('/course/management.php');

        if ($categorypage > 0) {
            $items[] = $this->action_button(new moodle_url($baseurl, array('categorypage' => 0)), get_string('first'));
            $items[] = $this->action_button(new moodle_url($baseurl, array('categorypage' => $categorypage - 1)), get_string('prev'));
            $items[] = $ellipsis;
        }
        for ($i = $start; $i <= $end; $i++) {
            $class = '';
            ...
            $pageurl = new moodle_url($baseurl, array('categorypage' => $i));
            $items[] = $this->action_button($pageurl, $i + 1, null, $class, get_string('pagea', 'moodle', $i+1));
        }
        if ($categorypage < ($totalpages - 1)) {
            $items[] = $ellipsis;
            $items[] = $this->action_button(new moodle_url($baseurl, array('categorypage' => $categorypage + 1)), get_string('next'));
            $items[] = $this->action_button(new moodle_url($baseurl, array('categorypage' => $totalpages - 1)), get_string('last'));
        }

        $html .= html_writer::div(implode('', $items), 'listing-pagination category-listing-pagination');
        return $html;
    }
    ...
    public function category_listitem(coursecat $category, array $subcategories, $totalsubcategories,
                                      ...
                                      $is_first_item = false, $is_last_item = false
    ) {
        ...
        if ($this->page->url->get_path() == '/course/management.php') {
            $mgmturl = $this->page->url;
        } else {
            $mgmturl = '/course/management.php';
        }
        $viewcaturl = new moodle_url($mgmturl, array('categoryid' => $category->id));
        ...
        if ($isexpanded) {
            ...
            $listing_index_current = 0;
            $listing_index_last = count($subcategories) - 1;
            ...
            foreach ($subcategories as $listitem) {
                ...
                $html .= $this->category_listitem(
                    ...
                    $selectedcategories,
                    $listing_index_current === 0,
                    $listing_index_current === $listing_index_last
                );
     ...
     *
     * @param coursecat|null $category The currently selected category if there is one.
     * @param int $percategorypage The number of pages per category.
     * @return string
     */
    public function category_listing_actions(?coursecat $category = null, $percategorypage = 20) {
        global $CFG;
        ...
        if ($cancreatecategory) {
            $url = new moodle_url('/course/editcategory.php', array('parent' => $category->parent));
        ...
        if (\totara_core\version_specific\version_specific_feature::is_enabled('totara_19_1_core_course_course_category_pagination')) {
            $strall = get_string('all');
            $menu = new action_menu(array(
                new action_menu_link_secondary(new moodle_url($this->page->url, array('percategorypage' => 5)), null, 5),
                new action_menu_link_secondary(new moodle_url($this->page->url, array('percategorypage' => 10)), null, 10),
                new action_menu_link_secondary(new moodle_url($this->page->url, array('percategorypage' => 20)), null, 20),
                new action_menu_link_secondary(new moodle_url($this->page->url, array('percategorypage' => 50)), null, 50),
                new action_menu_link_secondary(new moodle_url($this->page->url, array('percategorypage' => 100)), null, 100),
                new action_menu_link_secondary(new moodle_url($this->page->url, array('percategorypage' => 999)), null, $strall),
            ));
            if ((int)$percategorypage === 999) {
                $percategorypage = $strall;
            }
            $menu->attributes['class'] .= ' courses-per-page';
            $menu->set_menu_trigger(get_string('perpagea', 'moodle', $percategorypage));
     ...
     * @param array|null $actions The posisble actions for the selected category.
     * @param bool $delayload Whether the reload should be delayed.
     * @return string
     */
    ...
    public function category_bulk_actions(?coursecat $category = null) {
        ...
        if (coursecat::can_resort_any()) {
            ...
            $form .= html_writer::div(
                html_writer::select(
                    ...
                    'selectedcategories',
                    false,
                    array('aria-label' => get_string('selectcategorysort'), 'class' => 'form-control')
                )
            );
            $form .= html_writer::div(
                html_writer::select(
                    ...
                    'name',
                    false,
                    array('aria-label' => get_string('selectcategorysortby'), 'class' => 'form-control mt-3')
                )
            );
            $form .= html_writer::div(
                html_writer::select(
                    ...
                    'fullname',
                    false,
                    array('aria-label' => get_string('selectcoursesortby'), 'class' => 'form-control mt-3')
                )
            );
            $form .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'bulksort',
                'value' => get_string('sort'), 'class' => 'btn btn-secondary my-3'));
            $form .= html_writer::end_div();

            $html .= html_writer::start_div('detail-pair row yui3-g my-3');
        ...
        if (coursecat::can_change_parent_any()) {
            ...
            $select = html_writer::select(
                ...
                '',
                array('' => 'choosedots'),
                array('aria-labelledby' => 'moveselectedcategoriesto', 'class' => 'form-control mr-3')
            );
            ...
            $html .= $this->detail_pair(
                ...
                html_writer::div($select . html_writer::empty_tag('input', $submit), 'form-inline')
            );
        }
        $html .= html_writer::end_div();
        return $html;
     ...
     *
     * @param coursecat|null $category The currently selected category. This is what the listing is focused on.
     * @param course_in_list|null $course The currently selected course.
    ...
    protected function listing_pagination(coursecat $category, $page, $perpage, $showtotals = false) {
        ...
        $html .= html_writer::div(implode('', $items), 'listing-pagination course-listing-pagination');
        return $html;
    }
    ...
    public function course_bulk_actions(coursecat $category) {
        $html  = html_writer::start_div('course-bulk-actions bulk-actions form-inline');
        if ($category->can_move_courses_out_of()) {
            ...
            $select = html_writer::select(
                ...
                '',
                array('' => 'choosedots'),
                array('aria-labelledby' => 'moveselectedcoursesto', 'class' => 'form-control mr-3')
            );
    ...
    public function course_search_bulk_actions() {
        ...
        $select = html_writer::select(
            ...
            '',
            array('' => 'choosedots'),
            array('aria-labelledby' => 'moveselectedcoursesto', 'class' => 'form-control')
        );
        $submit = array('type' => 'submit', 'name' => 'bulkmovecourses', 'value' => get_string('move'),
            'class' => 'btn btn-secondary');
        $html .= $this->detail_pair(
    ...
    protected function detail_pair($key, $value, $class ='') {
        ...
        $html .= html_writer::div(html_writer::span($key), 'pair-key');
        $html .= html_writer::div(html_writer::span($value), 'pair-value');
        $html .= html_writer::end_div();
        return $html;
    }
     ...
     * @param moodle_url|null $url The URL to use when changing actions. Defaults to the page URL.
     ...
     * @param course_in_list|null $course The currently selected course if there is one.
    ...
    public function search_listitem_actions(course_in_list $course) {
        ...
        $actions = array();
        // We no longer check $course->can_access(). Now search results page icons are displayed according to the same
        // rules as on the main course_management page.
        // Edit.
        if ($course->can_edit()) {
            $actions[] = $this->output->action_icon(
                new moodle_url('/course/edit.php', array('id' => $course->id)),
                new pix_icon('t/edit', get_string('editcourse', 'core', $course_name)),
                null,
        ...
        if ($course->can_delete()) {
            $actions[] = $this->output->action_icon(
                ...
                new pix_icon('t/delete', get_string('deletecoursea', 'core', $course_name)),
                null,
        ...
        if ($course->can_change_visibility()) {
            $hidetooltip = get_string('hidecourse', 'core', $course_name);
            $showtooltip = get_string('showcourse', 'core', $course_name);
            if (!empty($CFG->audiencevisibility)) {
                $hidetooltip = $showtooltip = get_string('manageaudincevisibilitya', 'totara_cohort', $course_name);
            }
            $actions[] = $this->output->action_icon(
    ...
    public function course_search_form($value = '', $format = 'plain') {
        ...
        $output .= html_writer::start_tag('div', array('class' => 'coursesearchbox invisiblefieldset my-3', 'style' => 'display: inline-block'));
        $output .= html_writer::tag('label', $strsearchcourses, array('for' => $inputid));
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => $inputid, 'size' => $inputsize,
            'name' => 'search', 'value' => s($value), 'class' => 'form-control mx-3'));
