...
/**
 * Page output handler for archiving course progress of a single user
 */
class single_user extends page_output {
    ...
    protected function set_page_url(moodle_page $page): void {
        $page->set_url($this->get_report_completion_url());
    ...
    protected function get_confirmation_page_context(): confirmation_page {
        $confirmation_message = markdown_to_html(
            get_string(
                'confirm_archive_single_user_progress',
                'completion',
                [
        ...
        return new confirmation_page(
            ...
            $this->get_report_completion_url(),
        );
    }
    ...
    protected function get_error_page_context(): error_page {
        $message = markdown_to_html(
            get_string(
                'archive_single_user_no_completions',
                'completion',
                [
                    'user' => fullname($this->user),
                ]
    ...
    public function get_success_page_context(): success_page {
        return new success_page(
            $this->get_report_completion_url(),
            get_string(
                'successfully_archived_single_user_progress',
                'completion',
                [
                    'user' => fullname($this->user),
                ]
    ...
    /**
     * Get the report completion url.
     *
     * @return moodle_url
     */
    private function get_report_completion_url(): moodle_url {
