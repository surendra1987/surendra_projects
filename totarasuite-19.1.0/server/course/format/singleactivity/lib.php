...
class format_singleactivity extends format_base {
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($fetchtypes) {
            $availabletypes = $this->get_supported_activities(empty($this->courseid));
            if ($this->course) {
                ...
                $is_course_creator = has_capability('format/singleactivity:addanyactivity', context_coursecat::instance($this->categoryid));
            } else {
            ...
            foreach (array_keys($availabletypes) as $activity) {
                ...
                if (!$is_course_creator && !has_capability($capability, $testcontext)) {
