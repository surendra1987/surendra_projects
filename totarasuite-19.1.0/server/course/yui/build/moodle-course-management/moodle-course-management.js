...
DragDrop.prototype = {
    ...
    dragStart: function(e) {
        ...
        if (!node.ancestor('#coursecat-management')) {
            // not dragging a course or category
            return;
        }
    ...
    dropEnter: function(e) {
        var drop = e.drop.get('node'),
            tag = drop.get('tagName').toLowerCase();
        if (drop.ancestor('#coursecat-management') && tag === 'div') {
...
Item.prototype = {
    ...
    moveup: function(transactionid, response, args) {
        ...
        if (previous) {
            ...
            if (previous.hasClass('firstcategory')) {
                previous.removeClass('firstcategory');
                node.addClass('firstcategory');
            } else if (node.hasClass('lastcategory')) {
                previous.addClass('lastcategory');
                node.removeClass('lastcategory');
            }
    ...
    movedown: function(transactionid, response, args) {
        ...
        if (next) {
            ...
            nodeup = node.one(' > div a.action-moveup');
            if (node.hasClass('firstcategory')) {
                next.addClass('firstcategory');
                node.removeClass('firstcategory');
            } else if (next.hasClass('lastcategory')) {
                next.removeClass('lastcategory');
                node.addClass('lastcategory');
            }
            if (!nextdown || !nodeup) {
