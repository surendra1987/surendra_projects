...
class core_course_courselib_test extends \core_phpunit\testcase {
     ...
     */
    public static function provider_course_delete_module() {
        $data = array();
    ...
    public function test_course_created_event() {
        ...
        $imstestcase = new enrol_imsenterprise_imsenterprise_test($this->name() . '_imsenterprise');
    ...
    public function test_duplicate_module() {
        ...
        $this->setAdminUser();
        $course = self::getDataGenerator()->create_course();
        $f2f = self::getDataGenerator()->create_module('facetoface', ['course' => $course->id]);
        $cm = get_coursemodule_from_id('facetoface', $f2f->cmid, 0, false, MUST_EXIST);
        ...
        $notification1->courseid = $course->id;
        $notification1->facetofaceid = $f2f->id;
        ...
        foreach ($cm as $prop => $value) {
            if ($prop == 'id' || $prop == 'url' || $prop == 'instance' || $prop == 'added' || $prop == 'idnumber') {
        ...
        // Make sure custom notification was fully copied
        $new_notification = $DB->get_record("facetoface_notification", ["facetofaceid" => (int)$newcm->instance, "type" => MDL_F2F_NOTIFICATION_SCHEDULED]);
        $this->assertEquals($notification1->title, $new_notification->title);
        $this->assertEquals($notification1->body, $new_notification->body);
        $this->assertEquals($notification1->type, $new_notification->type);
        $this->assertEquals($notification1->conditiontype, $new_notification->conditiontype);
        $this->assertEquals($notification1->scheduleunit, $new_notification->scheduleunit);
        $this->assertEquals($notification1->scheduleamount, $new_notification->scheduleamount);
        $this->assertEquals($notification1->recipients, $new_notification->recipients);
    }
     ...
     * @return array
     */
    public static function course_enddate_provider() {
     ...
     * @return array
     */
    public static function course_dates_reset_provider() {
    ...
    public static function course_get_return_url_data_provider() {
        return [
