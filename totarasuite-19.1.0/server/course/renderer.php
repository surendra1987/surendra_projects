...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function course_section_cm_edit_actions($actions, cm_info $mod = null, $displayoptions = array()) {
        ...
        $menu->set_action_label(get_string('edita', 'moodle', $mod->get_formatted_name()));

        foreach ($actions as $action) {
            if ($action instanceof action_menu_link) {
                $action->add_class('cm-edit-action');

                // Ensure sesskey is sent when this is defined as a pure link
                if (isset($displayoptions['donotenhance'])) {
                    $action->attributes['data-addsesskey'] = "true";
                    $action->url->params(['sesskey' => sesskey()]);
                }
            }
            $menu->add($action);
        }
    ...
    function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        if (!has_capability('moodle/course:manageactivities', context_course::instance($course->id))
                || !$this->page->user_is_editing()
                || !($modnames = \container_course\course_helper::get_all_modules(false, false, true, null, $course))
                || empty($modnames)) {
            return '';
        }
    ...
    /**
     * Renders html to display a category search form
     *
     * @param string $url    The url for the category search
     * @param string $value  The default value to populate the search field
     * @param string $format The display format - 'plain' (default), 'short' or 'navbar'
     * @return string        The form output html
     */
    public function coursecat_search_form(string $url = '', string $value = '', string $format = 'plain'): string {
        static $count = 0;
        $formid = 'coursesearch';
        if ((++$count) > 1) {
            $formid .= $count;
        ...
        $strsearch = get_string("searchcategories");
        $searchurl = new moodle_url($url);

        $form = array('id' => $formid, 'action' => $searchurl, 'method' => 'get', 'class' => "form-inline", 'role' => 'form');
        $output = html_writer::start_tag('form', $form);
        $output .= html_writer::start_div('input-group');
        $output .= html_writer::tag('label', $strsearch, array('for' => $inputid, 'class' => 'sr-only'));
        $search = array('type' => 'text', 'id' => $inputid, 'size' => $inputsize, 'name' => 'search',
            'class' => 'form-control', 'value' => $value, 'placeholder' => $strsearch);
        $output .= html_writer::empty_tag('input', $search);
        $button = array('type' => 'submit', 'class' => 'btn btn-default');
        $output .= html_writer::start_span('input-group-btn');
        $output .= html_writer::tag('button', get_string('go'), $button);
    ...
    public function course_section_cm_name_title(cm_info $mod, $displayoptions = array()) {
        ...
        $hook = new \core\hook\course_module_available_info($mod->get_course()->format, $mod->modname);
        $hook->execute();

        $attributes = ['class' => 'instancename', 'data-movetext' => 'true'];
        if (!empty($hook->get_custom_style())) {
            $attributes = array_merge($attributes, $hook->get_custom_style());
        }

        $activitylink .= html_writer::tag('span', $instancename, $attributes);
    ...
    public function course_section_cm_availability(cm_info $mod, $displayoptions = array()) {
        ...
        if ($canviewhidden && !empty($CFG->enableavailability)) {
            ...
            if ($mod->visible || $this->page->user_is_editing()) {
                ...
                if ($hook->get_available_info()) {
                    $formattedinfo = \core_availability\info::format_info($hook->get_available_info() , $mod->get_course());
                    $warning = $OUTPUT->flex_icon('warning', ['alt' => get_string('warning')]);
                    return html_writer::div($warning . ' ' . $formattedinfo, 'text-warning availabilityinfo ' . $hidinfoclass);
                }
        ...
        if (!empty($searchcriteria)) {
            ...
            if (!$totalcount) {
                if (!empty($searchcriteria['search'])) {
                    $content .= $this->page_main_heading(get_string('nocoursesfound', '', $searchcriteria['search']));
                } else {
                    $content .= $this->page_main_heading(get_string('novalidcourses'));
                }
            } else {
                $content .= $this->page_main_heading(get_string('searchresults'). ": $totalcount");
                $content .= $courseslist;
            }
