...
use core_completion\helper as completion_helper;
...
function get_module_metadata($course, $modnames, $sectionreturn = null) {
    ...
    $return = array();
    $urlbase = new moodle_url('/course/mod.php', array('id' => $course->id));
    ...
    foreach($modnames as $modname => $modnamestr) {
        ...
        if ($items !== null) {
            foreach ($items as $item) {
                ...
                if (isset($item->help) && isset($item->helplink) && !empty($CFG->docroot)) {
...
function course_update_section($course, $section, $data) {
    ...
    if (array_key_exists('name', $data) && \core_text::strlen($data['name']) > 1024) {
        throw new moodle_exception('maximumchars', 'moodle', '', 1024);
    }
 ...
 */
function move_courses(array $courseids, int $categoryid): bool {
    // Return false if courseids was empty array
    if (empty($courseids)) {
        return false;
    }
    $target_category = course_category::load_by_id($categoryid);
    // Return false if target category was not exist
    if (!$target_category) {
        return false;
    }
    $newparent = context_coursecat::instance($categoryid);
    $target_category_course_count = $target_category->get_course_count();
    $course_entities = [];
    $source_categories_entities = [];
    // Load course modals and their categories by given course ids
    foreach ($courseids as $courseid) {
        try {
            $course = new \core\entity\course($courseid);
        } catch (moodle_exception $e) {
            continue;
        }
        $course_entities[] = $course;
        if (empty($source_categories_entities[$course->category])) {
            $source_categories_entities[$course->category] = $course->course_category;
        }
    }
    // Check if the new course count was over the max count
    if (count($course_entities) + $target_category_course_count > MAX_COURSES_IN_CATEGORY) {
        debugging("The number of courses (category id: $categoryid) has reached MAX_COURSES_IN_CATEGORY (" . MAX_COURSES_IN_CATEGORY . "), it will cause a sorting performance issue, please increase the value of MAX_COURSES_IN_CATEGORY in lib/datalib.php file. See tracker issue: MDL-25669", DEBUG_DEVELOPER);
    }
    // Clean up course sort orders for target category
    $target_category->ensure_sequential_course_sortorder();
    // Add courses to new category
    foreach ($course_entities as $index => $course) {
        $course->category = $target_category->id;
        $course->sortorder = $target_category->sortorder + $target_category_course_count + $index + 1;
        if ($target_category->visible == 0) {
        ...
        // Save course data in a transaction
        $trans = \core\orm\query\builder::get_db()->start_delegated_transaction();
        $course->save();
    ...
    // Update target category course count.
    $target_category->update_course_count(count($course_entities) + $target_category_course_count);
    // Reorder courses in source categories after the courses was moved
    // Update the source categories' course count
    foreach ($source_categories_entities as $category_entity) {
        $category = course_category::load_by_entity($category_entity);
        $category->ensure_sequential_course_sortorder(true);
    }
    // Regenerate cache
 ...
 */
function archive_course_activities(int $userid, int $courseid, int $windowopens = null) {
    $userids = ($userid == null) ? [] : [$userid];
    return archive_course_activities_bulk($userids, $courseid, $windowopens);
}

/**
 * Archives module activities for a given list of users, course, module and completion
 *
 * @global moodle_database $DB
 * @global object $CFG
 * @param int[] $userids
 ...
 * @param int $windowopens  The time the window opens, so we can act differently for historic uploads
 */
function archive_module_activities_bulk(array $userids, object $course, object $module, object $completion, int $windowopens) {
    global $DB, $CFG;
    $modfile = $CFG->dirroot . '/mod/' . $module->name . '/lib.php';

    // Check if a module instance is attached to the course and the lib file exists
    if (!$DB->record_exists($module->name, array('course' => $course->id)) || !file_exists($modfile)) {
        return;
    }
    ...
    if (plugin_supports('mod', $module->name, FEATURE_ARCHIVE_COMPLETION, 0)) {
        $modfunction = $module->name.'_archive_completion';
        if (!function_exists($modfunction)) {
            ...
            foreach ($userids as $userid) {
                $modfunction($userid, $course->id, $windowopens);
            }
        }
    }

    // Delete grades if activity supports grading.
    if (plugin_supports('mod', $module->name, FEATURE_GRADE_HAS_GRADE, 0)) {
        ...
        $sql = "SELECT gg.*
                ...
                WHERE gi.courseid = :courseid AND gg.userid {$usersql} AND gi.itemtype = :itemtype AND gi.itemmodule = :itemmodule";
    ...
    foreach ($userids as $userid) {
        $cms = get_all_instances_in_course($module->name, $course, $userid, true);
        foreach ($cms as $cm) {
            // Get all instances doesn't return the completion columns.
            $cm = get_coursemodule_from_id($module->name, $cm->coursemodule, $course->id);

            // Reset 'viewed' if activity supports tracking views.
            if (plugin_supports('mod', $module->name, FEATURE_COMPLETION_TRACKS_VIEWS, 0)) {
            ...
            if (!empty($cmcid)) {
                ...
                completion_helper::save_completion_log($course->id, $userid, "Deleted module completion in archive_course_activities<br><ul><li>CMCID: {$cmcid}</li></ul>");
                $transaction->allow_commit();
            }
        }
    }
...
/**
 * Archives activities, with the archive feature, for a list of users and course id
 *
 * @global moodle_database $DB
 * @global object $CFG
 * @param int[] $userids
 ...
 * @return bool true (always)
 */
function archive_course_activities_bulk(array $userids, int $courseid, int $windowopens = null) {
    global $DB, $CFG;
    ...
    $sql = "SELECT DISTINCT m.name
            ...
            ORDER BY m.name";
    $modules = $DB->get_records_sql($sql, array('courseid' => $courseid));

    if (empty($modules) || empty($userids)) {
        // no users or modules, nothing to archive
        return true;
    }
    ...
    foreach ($modules as $mod) {
        archive_module_activities_bulk($userids, $course, $mod, $completion, $windowopens);
    }

    if (count($userids) == 1) {
        $completion->invalidatecache($courseid, $userids[0], true);
    } else {
        // Invalidate cache for whole course if more than one user given - most likely it's all enrolled users
        $completion->invalidatecache($courseid, null, true);
    }
    return true;
}
...
function archive_course_purge_gradebook(int $userid, int $courseid) {
    $userids = ($userid == null) ? [] : [$userid];
    return archive_course_purge_gradebook_bulk($userids, $courseid);
}

/**
 * Purge all grades and outcomes of given users from course
 * and if users are enrolled then pre-create empty grade records.
 *
 * @param int[] $userids
 * @param int $courseid
 * @return bool success
 */
function archive_course_purge_gradebook_bulk(array $userids, int $courseid) {
    if (empty($userids)) {
        // no userids, nothing to archive
        return true;
    }
    ...
    $sql = "SELECT gg.*
              FROM {grade_grades} gg
              JOIN {grade_items} gi ON gi.id = gg.itemid
             WHERE gi.courseid = :courseid AND gg.userid {$usersql}";
 ...
 */
function archive_course_completion(int $userid, int $courseid, bool $inprogress = false) {
    $userids = ($userid == null) ? [] : [$userid];
    return archive_course_completion_bulk($userids, $courseid, $inprogress);
}

/**
 * Archives course completion for a list of users and course id
 *
 * @global moodle_database $DB
 * @global object $CFG
 * @param int[] $userids
 ...
 * @return bool true if course completions existed, else false
 */
function archive_course_completion_bulk(array $userids, int $courseid, bool $inprogress = false) {
    ...
    $params = array_merge($statusparams, $userparams, array('course' => $courseid));
    $where = "course = :course AND userid {$usersql} AND status {$statussql}";
    $course_completions = $DB->get_records_select('course_completions', $where, $params);
    ...
    // Completion history items to store
    $history_items = [];
    // Delete course completions for these users (allows skipping the ones with wrong completion status)
    $delete_for_userids = [];

    foreach ($course_completions as $course_completion) {
        $delete_for_userids[] = $course_completion->userid;

        $completionstatus = completion_completion::get_status($course_completion);
        if ($completionstatus !== $COMPLETION_STATUS[COMPLETION_STATUS_INPROGRESS]) {
            // Only create history items for completed course completions
            $history = new stdClass();
            $history->courseid = $courseid;
            $history->userid = $course_completion->userid;
            ...
            // Set max/min grade.
            if ($grade_item) {
                $history->grademax = $grade_item->grademax;
                $history->grademin = $grade_item->grademin;
            }

            if ($completionstatus == $COMPLETION_STATUS[COMPLETION_STATUS_COMPLETEVIARPL]) {
                if ($grade_item) {
                    // RPL grade is always in percent, so convert it to points.
                    $history->grade = $grade_item->grademin + (($course_completion->rplgrade / 100) * ($grade_item->grademax - $grade_item->grademin));
                } else {
                    // We don't know max/min grade, so we can't convert anything.
                    // Let's set max to 100 because we know that RPL grade is in percent.
                    $history->grade = $course_completion->rplgrade;
                    $history->grademax = 100;
                    $history->grademin = 0;
                }
            } elseif ($grade_item) {
                $grade = new grade_grade(array('itemid' => $grade_item->id, 'userid' => $course_completion->userid));
    ...
    foreach ($history_items as $history_item) {
        $transaction = $DB->start_delegated_transaction();
        $chid = $DB->insert_record('course_completion_history', $history_item);
        completion_helper::log_course_completion_history($chid, 'History created in archive_course_completion');
        $transaction->allow_commit();
    }
    ...
    $course = $DB->get_record('course', array('id' => $courseid));
    $completion = new completion_info($course);
    $completion->delete_course_completion_data_bulk($delete_for_userids);

    if (count($delete_for_userids) == 1) {
        $completion->invalidatecache($courseid, $delete_for_userids[0]);
    } else {
        // Invalidate cache for whole course if more than one user given - most likely it's all enrolled users
        $completion->invalidatecache($courseid);
    }
    return true;
}
 ...
 * @return bool
 */
function course_change_sortorder_after_course($courseid, $moveaftercourseid) {
    $courseid = is_object($courseid) ? $courseid->id : $courseid;

    // They're the same - moronic.
    if ((int)$courseid === (int)$moveaftercourseid) {
    ...
    if ((int)$moveaftercourseid === 0) {
        ...
        // Set the course sort order to the beginning of the order
        $category = $course->course_category;
        $course->sortorder = $category->sortorder;
        $course->save();
        // Reorder courses
        $category_model = course_category::load_by_entity($category);
        $category_model->ensure_sequential_course_sortorder();
    } else {
        // Moving this course after the given course. It could be before it could be after.
        $target_course = new \core\entity\course($moveaftercourseid);
        // Show error if target course category was different to moving course category
        if ($course->category !== $target_course->category) {
        ...
        $params = array(
            'categoryid' => $target_course->category,
            'sortorder' => $target_course->sortorder
        );
        \core\orm\query\builder::get_db()->execute($sql, $params);
        // Insert the course into the order list which after the target course
        $course->sortorder = $target_course->sortorder + 1;
        $course->save();
    }
...
function course_get_user_administration_options($course, $context) {
    ...
    if (!$isfrontpage) {
        ...
        $options->publish = false; // Totara: we don't publish courses on Moodle community.
