...
/**
 * Delete records associated with lesson completion.
 *
 * @param int $userid
 * @param int $courseid
 ...
 *
 */
function lesson_archive_completion(int $userid, int $courseid, $windowopens = null): bool {
    global $DB, $CFG;
    ...
    if ($lessons = $DB->get_records_sql($sql, ['courseid' => $courseid, 'userid' => $userid])) {
        // Loop through and clear the appropriate data for the lesson.
        foreach ($lessons as $lesson) {
            ...
            // Delete overrides.
            if ($overrides = $DB->get_records('lesson_overrides', array('lessonid' => $lesson->id, 'userid' => $userid), 'id')) {
                $les = lesson::load($lesson->id);
                foreach ($overrides as $override) {
                    $les->delete_override($override->id);
                }
            }

            // NOTE: grade_grades and grade_items are deleted automatically during archiving, no need to do it here.
            //       however the lesson_grade table needs to be wiped, since it is what is used to determine the
            //       number of attempts a user has made at the lesson.
