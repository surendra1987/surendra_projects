...
class mod_lti_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        if ($mform->elementExists('showdescription') &&
            course_get_format($this->get_course())->supports_showdescription()) {
    ...
    public function data_preprocessing(&$default_values): void {
        ...
        if (empty($default_values['completionusegrade'])) {
            // Forced unchecked, even if $CFG->revert_TL_43887_until_t20 is enabled.
            $default_values['completionpass'] = 0;
        }
    }
    ...
    public function validation($data, $files): array {
        ...
        if (empty($CFG->revert_TL_43887_until_t20) && empty($data['gradepass']) || $data['gradepass'] <= 0) {
            if (!empty($data['completionunlocked'])) {
                if ($data['completion'] == COMPLETION_TRACKING_AUTOMATIC && $data['completionpass']) {
                    $errors['gradepass'] = get_string('gradepassrequiredforcompletion', 'mod_lti');
                }
            } elseif ($data['instance']) {
                $lti = $DB->get_record('lti', array('id' => $data['instance']), '*', MUST_EXIST);
                if ($lti->completionpass) {
                    $errors['gradepass'] = get_string('gradepassrequiredforcompletion', 'mod_lti');
                }
            }
        }

        if (empty($CFG->revert_TL_43887_until_t20) && array_key_exists('completion', $data) && $data['completion'] == COMPLETION_TRACKING_AUTOMATIC) {
            ...
            if ($completionpass && (empty($data['gradepass']) || grade_floatval((float)$data['gradepass']) == 0)) {
                if (isset($data['completionpass'])) {
                    $errors['completionpassgroup'] = get_string('gradetopassnotset', 'mod_lti');
                } else {
                    $errors['gradepass'] = get_string('gradetopassmustbeset', 'mod_lti');
                }
            }
        }
    ...
    public function add_completion_rules(): array {
        $mform = $this->_form;
        $items = parent::add_completion_rules();

        if (empty($CFG->revert_TL_43887_until_t20)) {
            $group = array();
            $group[] = $mform->createElement(
                'advcheckbox',
                'completionpass',
                null,
                get_string('completionpass', 'lti'),
                array('group' => 'cpass')
            );
            $mform->disabledIf('completionpass', 'completionusegrade', 'notchecked');
            $mform->addGroup($group, 'completionpassgroup', get_string('completionpass', 'lti'), ' &nbsp; ', false);
            $mform->addHelpButton('completionpassgroup', 'completionpass', 'lti');
    ...
    public function completion_rule_enabled($data): bool {
        return empty($CFG->revert_TL_43887_until_t20) && !empty($data['completionpass']);
    }
    ...
    public function get_data(): ?object {
        ...
        // Complementary code to add_completion_rules, otherwise completionpass is not reset when completionusegrade is unchecked.
        if (empty($CFG->revert_TL_43887_until_t20) && isset($data) && empty($data->completionusegrade)) {
