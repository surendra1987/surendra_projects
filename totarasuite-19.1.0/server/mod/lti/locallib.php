    ...
    if ($typeid && !$islti2) {
        ...
        foreach ($services as $service) {
            $serviceparameters = $service->get_launch_parameters('basic-lti-launch-request',
                    $course->id, (string) $USER->id , $typeid, $instance->id);
...
function lti_build_request($instance, $typeconfig, $course, $typeid = null, $islti2 = false) {
    ...
    if (!empty($instance->id) && !empty($instance->servicesalt) && ($islti2 ||
        ...
        $sourcedid = json_encode(lti_build_sourcedid($instance->id, (string) $USER->id, $placementsecret, $typeid));
...
function lti_get_domain_from_url($url) {
    ...
    // Totara: If $url is null, return null. The function implicitly returns a null if there's a non-match.
    if ($url === null) {
        return null;
    }
...
function lti_initiate_login($courseid, $id, $instance, $config, $messagetype = 'basic-lti-launch-request', $title = '',
    ...
    $params['login_hint'] = (string) $USER->id;
...
function serialise_tool_proxy(stdClass $proxy) {
    debugging('serialise_tool_proxy() function has been deprecated', DEBUG_DEVELOPER);
    return array(
        'id' => $proxy->id,
        'name' => $proxy->name,
...
function lti_load_cartridge($url, $map, $propertiesmap = array()) {
    ...
    $curl = new curl();
    // Just to be safe, disabled redirection
    $curl->setopt(array('CURLOPT_FOLLOWLOCATION' => 0));
    $curl->setopt(array('CURLOPT_MAXREDIRS' => 0));
