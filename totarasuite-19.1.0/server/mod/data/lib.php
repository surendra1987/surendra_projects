...
class data_field_base {     // Base class for Database Field Types (see field/*/field.class.php)
    ...
    function display_edit_field() {
        ...
        $this->type = clean_param($this->type, PARAM_ALPHA);
        if (!file_exists($CFG->dirroot.'/mod/data/field/'.$this->type.'/mod.html')) {
            throw new file_exception('Invalid field type specified: ' . $this->type);
        }
        require_once($CFG->dirroot.'/mod/data/field/'.$this->type.'/mod.html');

        echo '<div class="mt-3">';
        echo '<button type="submit" class="btn btn-primary">'.$savebutton.'</button>'."\n";
        echo '<button type="submit" class="btn btn-secondary" name="cancel">'.get_string('cancel').'</button>'."\n";
        echo '</div>';
    ...
    if ($fields = $DB->get_records('data_fields', array('dataid'=>$data->id), 'id')) {
        ...
        foreach ($fields as $field) {
            if ($form) {   // Print forms instead of data
                ...
                $token = $fieldobj->display_add_field($recordid, null, $field->name);
...
function data_get_field_new($type, $data) {
    ...
    if (!file_exists("$CFG->dirroot/mod/data/field/$type/field.class.php")) {
        throw new file_exception('Invalid field type specified: ' . $type);
    }
...
function data_get_field($field, $data, $cm=null) {
    ...
    if ($field) {
        ...
        if (!file_exists("$CFG->dirroot/mod/data/field/$field->type/field.class.php")) {
            throw new file_exception('Invalid field type specified: ' . $field->type);
        }
...
function data_print_template($template, $records, $data, $search='', $page=0, $return=false, moodle_url $jumpurl=null) {
    ...
    $is_phpunit_test = (defined('PHPUNIT_TEST') && PHPUNIT_TEST);

    $cm = get_coursemodule_from_instance('data', $data->id);
    $context = context_module::instance($cm->id);

    if (!$is_phpunit_test) {
        // Cache the fields and data id so that we can save refetching the fields when
        ...
        // No static caching during PHPUnit tests.
        $fields = [];
    }
...
function data_print_preference_form($data, $perpage, $search, $sort='', $order='ASC', $search_array = '', $advanced = 0, $mode= ''){
    ...
    echo html_writer::select($pagesizes, 'perpage', $perpage, false, array('id' => 'pref_perpage', 'class' => 'w-auto'));
    ...
    echo '<select name="sort" id="pref_sortby" class="custom-select w-auto mr-3">';
    ...
    echo '<select id="pref_order" name="order" class="custom-select w-auto mr-3">';
    if ($order == 'ASC') {
    ...
    echo '&nbsp;<input type="checkbox" id="advancedcheckbox" name="advanced" value="1" ' . $checked . ' ' .
         'onchange="showHideAdvSearch(this.checked);" class="mr-2" />' .
         '<label for="advancedcheckbox" class="mr-3">' . get_string('advancedsearch', 'data') . '</label>';
    ...
    foreach ($fields as $field) {
        ...
        if (!empty($search_array[$field->field->id]->data)) {
            $replacement[] = $searchfield->display_search_field($search_array[$field->field->id]->data, $field->field->name);
        } else {
            $replacement[] = $searchfield->display_search_field(null, $field->field->name);
        }
    }
    ...
    echo '<tr><td colspan="4"><br/>' .
         '<input type="submit" class="btn btn-primary mr-3" value="' . get_string('savesettings', 'data') . '" />' .
...
function data_print_header($course, $cm, $data, $currenttab='') {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->page_main_heading(format_string($data->name));
        ...
        if (!empty($newfields)) {
            ...
            foreach ($newfields as $nid => $newfield) {
                ...
                if ($cid != -1 and isset($currentfields[$cid])) {
                    ...
                    /* Make a new field */
                    $newfield->type = clean_param($newfield->type, PARAM_ALPHA);
                    if (!file_exists("$CFG->dirroot/mod/data/field/$newfield->type/field.class.php")) {
                        throw new file_exception('Invalid field type specified: ' . $newfield->type);
                    }
                    include_once("$CFG->dirroot/mod/data/field/$newfield->type/field.class.php");
