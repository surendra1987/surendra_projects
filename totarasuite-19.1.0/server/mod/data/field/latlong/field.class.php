...
class data_field_latlong extends data_field_base {
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($this->field->required) {
            $str .= $OUTPUT->flex_icon('required', array('alt' => get_string('requiredelement', 'form')));
        }
        $classes = 'form-control mx-3';
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($content = $DB->get_record('data_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
            ...
            if(sizeof($servicesshown)==1 && $servicesshown[0]) {
                ...
                $str .= '\n</select><input type="button" class="btn ml-3 btn-secondary" value="' . get_string('go');
