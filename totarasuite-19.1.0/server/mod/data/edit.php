...
if ($DB->count_records('data_fields', ['dataid' => $data->id, 'required' => '1']) > 0) {
    $image = $OUTPUT->flex_icon('required', array('classes' => 'form-required', 'alt' => get_string('requiredelement', 'form')));
    echo html_writer::div(get_string('somefieldsrequired', 'core_form', $image), 'fdescription required');
}
...
if ($data->addtemplate){
    ...
    foreach ($possiblefields as $eachfield){
        ...
        if (strpos($data->addtemplate, "[[".$field->field->name."]]") !== false) {
            ...
            $replacements[] = $errors . $field->display_add_field($rid, $datarecord, $field->field->name);
        }
...
echo '<div class="mdl-align mt-3"><input type="submit" class="btn btn-primary" name="saveandview" ' .
