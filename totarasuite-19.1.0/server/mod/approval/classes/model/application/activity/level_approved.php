...
use mod_approval\notification_helper;
use mod_approval\totara_notification\resolver\level_approved as level_approved_resolver;
...
final class level_approved extends activity {
    ...
    public static function trigger_event(application $application, ?int $actor_id, array $activity_info): void {
        $event = level_approved_event::create_from_application($application, $actor_id);
        $event->trigger();

        // Trigger the notification.
        notification_helper::trigger_notification(
            level_approved_resolver::class,
            [
                ...
                'time_approved' => $event->get_data()['timecreated'],
            ],
            $application
        );
    }
