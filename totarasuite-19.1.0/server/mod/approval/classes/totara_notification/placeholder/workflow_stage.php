...
class workflow_stage extends single_emptiable_placeholder {

    use placeholder_instance_cache;

    /**
     * @var ?workflow_stage_model
     */
    private $model;

    /**
     * workflow_stage constructor.
     * @param workflow_stage_model|null $model
     */
    public function __construct(?workflow_stage_model $model) {
        $this->model = $model;
    }
    ...
    public static function from_id(int $id): self {
        ...
        if (!$instance) {
            $model = workflow_stage_model::load_by_id($id);
    ...
    /**
     * @param workflow_stage_model $model
     *
     * @return self
     */
    public static function from_model(workflow_stage_model $model): self {
    ...
    public static function get_options(): array {
        return [
            option::create(
                'name',
                get_string('notification_placeholder:workflow_stage_name', 'mod_approval')
            ),
        ];
    }
    ...
    public function do_get(string $key): ?string {
        if (null === $this->model) {
            throw new coding_exception("The workflow stage model is empty");
        }
