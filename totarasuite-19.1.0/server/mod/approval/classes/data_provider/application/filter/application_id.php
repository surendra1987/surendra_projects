 ...
 */
class application_id extends filter {
    ...
    public function apply(): void {
        if (!is_array($this->value)) {
            throw new coding_exception('application instance id filter must have an array for value');
        }

        if (count($this->value) > 0) {
            $this->builder->where_in("{$this->application_table_alias}.id", $this->value);
        }
    }
}