 ...
 * @coversDefaultClass mod_approval\model\workflow\interaction\transition\previous
 */
class mod_approval_transition_previous_test extends testcase {
    ...
    public function test_enum(): void {
        $this->assertEquals('PREVIOUS', previous::get_enum());
    }
    ...
    public function test_resolve(): void {
        ...
        $this->assertEquals(null, $transition->resolve($stage1->state_manager->get_initial_state()));
        $this->assertEquals($stage1->id, $transition->resolve($stage2->state_manager->get_initial_state())->get_stage_id());
    ...
    public function test_to_field_method(): void {
        $transition = new previous();
        $this->assertEquals('PREVIOUS', $transition->transition_field());
    }
    ...
    public function test_get_sort_order(): void {
        $transition = new previous();
        $this->assertEquals(10, $transition->get_sort_order());
    }
