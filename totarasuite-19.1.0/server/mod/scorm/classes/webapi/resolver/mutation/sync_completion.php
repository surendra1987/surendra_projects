...
class sync_completion extends mutation_resolver {
    ...
    public static function resolve(array $args, execution_context $ec): mobile_basic_response {
        try {
            $input = new sync_activity_completion_input($args);
            if ($input->get_completion_tracking() !== COMPLETION_TRACKING_MANUAL) {
                throw new activity_completion_exception('Only supported manual completion tracking');
            }
            activity_completion_handler::sync_activity_completion('scorm', $input);
