...
function scorm_get_attempt_status($user, $scorm, $cm='') {
    ...
    // If force new attempt is set to 'When previous attempt completed, passed or failed', then
    // count only the attempts with status completed/passed/failed.
    if ($scorm->forcenewattempt == SCORM_FORCEATTEMPT_ONCOMPLETE) {
        $attempts = scorm_get_completed_attempt_count($user->id, $scorm, true);
    }
    if (empty($attempts)) {
        $attemptcount = 0;
    } else {
 ...
 * @param stdClass $scorm Scorm object
 * @param bool $returnobjects If true, it returns an object with attempts, if false it returns count of attempts
 * @return int|array - Count of the user's completed attempts.
 */
function scorm_get_completed_attempt_count($userid, $scorm, $returnobjects = false) {
    global $DB;
    ...
    $element = $completionelements[$scormversion];
    ...
    if ($returnobjects) {
        $sql = "SELECT DISTINCT attempt AS attemptnumber
                  FROM {scorm_scoes_track}
                 WHERE userid = ?
                   AND scormid = ?
                   AND element = ?
            ...
            $DB->sql_compare_text('value') . " = " . $DB->sql_compare_text('?') . ") ORDER BY attemptnumber";
