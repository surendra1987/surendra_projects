...
function quiz_get_completion_state($course, $cm, $userid, $type) {
    ...
    if (!$quiz->completionattemptsexhausted && !$quiz->completionpass) {
        ...
        // If 'Require grade setting' is enabled and 'Require passing grade setting' is disabled, as long as learners
        // get the score from quiz, mark it as completed.
        if ($completionusegrade && $DB->record_exists('quiz_grades', ['userid' => $userid, 'quiz' => $quiz->id])) {
            return true;
        }
