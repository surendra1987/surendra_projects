 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quiz_attempt_walkthrough_from_csv_test extends \core_phpunit\testcase {

    protected const FILES = array('questions', 'steps', 'results');
     ...
     */
    protected static function get_full_path_of_csv_file($setname, $test) {
     ...
     */
    protected static function load_csv_data_file($setname, $test='') {
        $files = array($setname => static::get_full_path_of_csv_file($setname, $test));
        return self::createCsvDataSet($files)->getTable($setname);
    }
     ...
     */
    public static function get_data_for_walkthrough() {
        $quizzes = self::load_csv_data_file('quizzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $quizzes->getRowCount(); $rowno++) {
            ...
            foreach (static::FILES as $file) {
                if (file_exists(static::get_full_path_of_csv_file($file, $quizsettings['testnumber']))) {
                    $dataset[$file] = static::load_csv_data_file($file, $quizsettings['testnumber']);
                }
            }
