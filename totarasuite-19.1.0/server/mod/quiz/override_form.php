...
class quiz_override_form extends moodleform {
    ...
    protected function definition() {
        ...
        if ($this->groupmode) {
            ...
            if ($this->userid) {
                ...
                $fields = 'u.id,';
                $fields .= $extrafields ? 'u.' . implode(', u.', $extrafields) . ',' : '';
                $fields .= get_all_user_name_fields(true, 'u');

                $groups = $accessallgroups ? [] : groups_get_activity_allowed_groups($cm);
                $users = get_users_by_capability(
                    $this->context, 'mod/quiz:attempt', $fields, $sort, '', '', array_keys($groups)
                );
                ...
                foreach ($users as $id => $user) {
                    if (
                        empty($invalidusers[$id])
                        || (!empty($override)
                        && $id == $override->userid)
                    ) {
                        $userchoices[$id] = self::output_user_for_selection($user, $extrafields);
                    }
                }
                unset($users);
    ...
    /**
     * Returns a users name for selection in a quiz.
     *
     ...
     *
     * @param object $user
     * @param array $extrafields Extra fields to display next to the users name, if null the user identity fields are used.
     * @return string
     */
    private static function output_user_for_selection($user, array $extrafields = []): string {
        $out = fullname($user);
        if ($extrafields) {
            $displayfields = [];
