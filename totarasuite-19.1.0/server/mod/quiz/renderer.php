...
class mod_quiz_renderer extends plugin_renderer_base {
     ...
     *
     * @param array $activities array of stdClasses from {@link quiz_get_recent_mod_activity()}
    ...
    public function render_recent_activities(array $activities, bool $viewfullnames=true) :string {
        ...
        $output = html_writer::tag('h2', get_string('newresponse', 'quiz') . ':', ['class' => 'sectionname']);
    ...
    public function review_page(quiz_attempt $attemptobj, $slots, $page, $showall,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->page_main_heading(get_string('review_title', 'quiz', $attemptobj->get_quiz_name()));

        // Quiz requires a block for navigation so include it on the page if it's not there
        if (count($this->page->blocks->get_regions()) === 0) {
            $block = $attemptobj->get_navigation_panel($this, 'quiz_review_nav_panel', $page, $showall);
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->page_main_heading(($attemptobj->get_quiz()->name));

        // Quiz requires a block for navigation so include it on the page if it's not there
        if (count($this->page->blocks->get_regions()) === 0) {
            $block = $attemptobj->get_navigation_panel($this, 'quiz_attempt_nav_panel', $page);
            $output .= $block->content;
        }
    ...
    public function quiz_notices($messages) {
        ...
        return $this->box(
            $this->heading(get_string('accessnoticesheader', 'quiz'), 2) .
    ...
    public function summary_page($attemptobj, $displayoptions) {
        ...
        // Quiz requires a block for navigation so include it on the page if it's not there
        if (count($this->page->blocks->get_regions()) === 0) {
            $block = $attemptobj->get_navigation_panel($this, 'quiz_attempt_nav_panel', -1);
            $output .= $block->content;
        }
        $output .= $this->page_main_heading(format_string($attemptobj->get_quiz_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'quiz'), 2);
    ...
    public function view_page_buttons(mod_quiz_view_object $viewobj) {
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    ...
                    array('class' => 'continuebutton mt-1'));
        }
    ...
    public function view_information($quiz, $cm, $context, $messages) {
        ...
        $output .= $this->page_main_heading(format_string($quiz->name));
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'quiz'), 2);
    }
    ...
    public function view_result_info($quiz, $context, $cm, $viewobj) {
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                ...
                $resultinfo .= $this->heading(get_string('gradesofar', 'quiz', $a), 2);
            } else {
                $a = new stdClass();
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'quiz'), 2);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'quizteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'quiz'), 2);
    ...
    public function chart(\core\chart_base $chart, $title) {
        return $this->heading($title, 2) . html_writer::tag('div', $this->render($chart), array('class' => 'graph'));
    }
    ...
    public function graph(moodle_url $url, $title) {
        ...
        return $this->heading($title, 2) . html_writer::tag('div', $graph, array('class' => 'graph'));
    }
