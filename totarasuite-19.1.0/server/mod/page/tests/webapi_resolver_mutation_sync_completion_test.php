...
class mod_page_webapi_resolver_mutation_sync_completion_test extends testcase {
    private const MUTATION = 'mod_page_sync_completion';
    private const MOD_NAME = 'page';
    use webapi_phpunit_helper;

    public function test_completion_not_tracked(): void {
        global $DB;
        self::setAdminUser();
        ...
        // The module does not enable completion.
    ...
    public function test_completion_changed(): void {
        global $DB;
        self::setAdminUser();
        $gen = self::getDataGenerator();
        $course = $gen->create_course(['enablecompletion' => 1]);
        ...
        $DB->insert_record('totara_mobile_compatible_courses', $todb);
        $gen->enrol_user(get_admin()->id, $course->id);
        $cm = $gen->create_module('page', ['course' => $course->id, 'completion' => COMPLETION_TRACKING_MANUAL]);
    ...
    public function test_success_manual(): void {
        global $DB;
        self::setAdminUser();
        $gen = self::getDataGenerator();
        $course = $gen->create_course(['enablecompletion' => 1]);
    ...
    public function test_success_must_view(): void {
        global $DB;
        self::setAdminUser();
        $gen = self::getDataGenerator();
        $course = $gen->create_course(['enablecompletion' => 1]);
        ...
        $todb->courseid = $course->id;
        $DB->insert_record('totara_mobile_compatible_courses', $todb);
        $cm = $gen->create_module(self::MOD_NAME, ['course' => $course->id, 'completion' => COMPLETION_TRACKING_AUTOMATIC]);
        $gen->enrol_user(get_admin()->id, $course->id);
        $result = $this->resolve_graphql_mutation(self::MUTATION, [
            'input' => [
                'cm_id' => $cm->cmid,
    ...
    public function test_set_incomplete_where_completion_tracking_must_view(): void {
        global $DB;
        self::setAdminUser();
        $gen = self::getDataGenerator();
        $course = $gen->create_course(['enablecompletion' => 1]);
