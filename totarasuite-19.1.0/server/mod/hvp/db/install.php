...
function xmldb_hvp_install(): void {
    ...
    if (!defined('CLI_SCRIPT') || !CLI_SCRIPT) {
        echo '<h3>' . get_string('welcomeheader', 'hvp') . '</h3>' .
            ...
            get_string('welcomegettingstarted', 'hvp', array(
                'totara_tutorial' => 'href="https://h5p.org/moodle" target="_blank"',
    ...
    if (!PHPUNIT_TEST && !defined('BEHAT_SITE_RUNNING')) {
        // Queue task to notify admins that they need to set up some H5P content types.
        $task = new \mod_hvp\task\installation_notification_task();
