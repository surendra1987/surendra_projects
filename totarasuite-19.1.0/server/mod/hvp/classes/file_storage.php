...
class file_storage implements \H5PFileStorage {
    ...
    public function saveExport($source, $filename) {
        ...
        // Store new export.
        $fs = get_file_storage();
        manager::scan_file($source, $filename, true);
    ...
    public function saveFile($file, $contentid, $contextid = null) {
        ...
        $fs = get_file_storage();
        manager::scan_file($_FILES['file']['tmp_name'], $file->getName(), true);
        ...
        while (false !== ($file = readdir($dir))) {
            if (($file != '.') && ($file != '..') && $file != '.git' && $file != '.gitignore') {
                if (is_dir($source . DIRECTORY_SEPARATOR . $file)) {
                    $suboptions = $options;
                    $suboptions['filepath'] .= $file . DIRECTORY_SEPARATOR;
                    self::readFileTree($source . DIRECTORY_SEPARATOR . $file, $suboptions);
                } else {
                    $record = $options;
                    $record['filename'] = $file;
                    manager::scan_file($source.DIRECTORY_SEPARATOR.$file, $file, true);
                    $fs = get_file_storage();
                    $fs->create_file_from_pathname($record, $source . DIRECTORY_SEPARATOR . $file);
                }
            }
        }
        closedir($dir);
    ...
    public function hasWriteAccess() {
        ...
        if (!is_dir($CFG->dataroot)) {
            // Totara: Convert trigger_error into a debugging call
            debugging('Path is not a directory ' . $CFG->dataroot);
            return false;
        }

        if (!is_writable($CFG->dataroot)) {
            // Totara: Convert trigger_error into a debugging call
            debugging('Unable to write to ' . $CFG->dataroot . ' – check directory permissions –');
            return false;
        }
    ...
    public function saveFileFromZip($path, $file, $stream) {
        ...
        $created = file_put_contents($filepath, $stream);
        manager::scan_file($filepath, $file, true);
