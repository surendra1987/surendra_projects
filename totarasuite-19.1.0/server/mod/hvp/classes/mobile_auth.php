...
class mobile_auth {
    ...
    public static function validate_embed_auth_token($token, $secret) {
        // Get time factor for current time
        // If validation is made within a minute [of the first token being generated] the
        // time factor will be the same
    ...
    public static function has_valid_token($userid, $secret) {
        ...
        // Retrieve user's hvp_auth token record
        $auth = $DB->get_record('hvp_auth', array(
            'user_id' => $userid,
        ));

        // No hvp_auth token for this user, so not a valid request
        if (!$auth) {
            return false;
        }

        // Checks known secret against given secret
