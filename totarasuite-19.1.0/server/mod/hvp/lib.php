...
function hvp_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options = array()) {
    switch ($filearea) {
        ...
        case 'libraries':
            ...
            if ($user->tenantid && $context->contextlevel == CONTEXT_SYSTEM) {
                // For libraries, we check the capability in the user's tenant context, rather than the context of the library.
                $context_to_check = context_tenant::instance($user->tenantid);
            } else {
                $context_to_check = $context;
            }
            if (!has_capability('mod/hvp:getcachedassets', $context_to_check)) {
                return false;
            }
        ...
        case 'cachedassets':
            ...
            $user = \core\entity\user::logged_in();
            if ($user->tenantid && $context->contextlevel == CONTEXT_SYSTEM) {
                // For cachedassets, we check the capability in the user's tenant context, rather than the context of the cachedasset.
                $context_to_check = context_tenant::instance($user->tenantid);
            } else {
                $context_to_check = $context;
            }
        ...
        case 'exports':
            ...
            $ishub = false;
            $hub = optional_param('hub', null, PARAM_TEXT);
            if ($hub) {
 ...
 * @return boolean
 */
function hvp_archive_completion(int $userid, int $courseid, ?int $windowopens = NULL): bool {
    $hvps = builder::table('hvp')
        ->select('id')
