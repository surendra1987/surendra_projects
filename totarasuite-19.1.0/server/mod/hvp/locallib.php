...
function hvp_get_core_settings($context) {
    ...
    $ajaxpaths = array();
    $savefreq = false;
    $ajaxpath = "{$basepath}mod/hvp/ajax.php?contextId={$context->instanceid}&sesskey=" . sesskey() . "&token=";
...
function hvp_get_core_assets($context) {
    ...
    foreach (\H5PCore::$scripts as $script) {
        ...
        $PAGE->requires->js(new moodle_url($liburl . $script . $cachebuster), $in_head);
    }
...
function hvp_add_editor_assets($id = null, $mformid = null) {
    ...
    $PAGE->requires->js(new moodle_url('/mod/hvp/editor.js' . $cachebuster), false);
    ...
    $enablecontenthub = $interface->getOption('hub_is_enabled', null) === "1";
    ...
    $settings['editor'] = array(
      ...
      'ajaxPath' => "{$url}ajax.php?contextId={$context->id}&token={$editorajaxtoken}&sesskey=" . sesskey() . "&action=",
...
function hvp_content_upgrade_progress($libraryid) {
    ...
    if (!\H5PCore::validToken('contentupgrade', required_param('token', PARAM_ALPHANUM))) {
