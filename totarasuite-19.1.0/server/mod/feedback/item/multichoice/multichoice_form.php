...
class feedback_multichoice_form extends feedback_item_form {
    ...
    public function definition() {
        ...
        $completed_count = $this->_customdata['completed_count'];

        $mform =& $this->_form;

        $type_string = get_string($this->type, 'mod_feedback');
        $mform->addElement('header', 'general', $type_string);
        ...
        $attr = array('wrap' => 'virtual', 'rows' => '10', 'cols' => '65');
        $mform->addElement('textarea', 'values', get_string('multichoice_values', 'mod_feedback'), $attr);
        if ($completed_count > 0) {
            $mform->addElement(
                ...
                $OUTPUT->notification(
                    get_string(
                        'multichoice_edit_warning',
                        'mod_feedback',
                        \core_text::strtolower($type_string)
                    ),
