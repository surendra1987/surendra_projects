...
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(format_string($feedback->name));
...
if (empty($students)) {
    ...
    foreach ($students as $student) {
        // User picture and link to the profile page.
        $user_fullname = fullname($student);
        $profileurl = $CFG->wwwroot.'/user/profile.php?id='.$student->id.'&amp;course='.$course->id;
        $profilelink = '<strong><a href="'.$profileurl.'">'.$user_fullname.'</a></strong>';
        ...
        $aria = get_string('select', 'mod_feedback', $user_fullname);
        if ($canbulkmessaging) {
            $data[] = '<input aria-label="'.$aria.'" type="checkbox" class="usercheckbox" name="messageuser[]" value="'.$student->id.'" />';
        }
        $table->add_data($data);
    }
    $table->print_html();
    ...
    if ($canbulkmessaging) {
        ...
        echo '</div>';
        echo '<label for="feedback_message" class="sr-only">'.get_string('message', 'feedback').'</label>';
        print_textarea(true, 15, 25, 30, 10, "message", $message, 0, false, 'feedback_message');
