 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assign_locallib_test extends \core_phpunit\testcase {
    ...
    public function test_gradingtable_status_rendering() {
        ...
        $this->assertEquals('View submission', $xpath->evaluate('string(//td[@id="mod_assign_grading_r0_c8"])')); // Totara: extra flex icon text
    }
     ...
     */
    public static function provider_new_submission_empty_testcases() {
        return [
    ...
    public function test_count_submissions_for_team() {
        $this->markTestIncomplete('This test from 2018 is still incomplete.');
        global $DB;
        ...
        for ($i = 0; $i < 5; $i++) {
            $extra_student = $this->getDataGenerator()->create_user();
            $this->getDataGenerator()->enrol_user($extra_student->id,
                $course->id,
                $studentrole->id);
            groups_add_member($group, $extra_student);
            $extrastudents[$i] = $extra_student;
        }
        ...
        $this->add_submission($extrastudents[0], $assign);

        // Simulate adding a grade.
        $this->setUser($teacher);
        $data = new stdClass();
        $data->grade = '50.0';
        $assign->testable_apply_grade_to_user($data, $extrastudents[0]->id, 0);

        // Simulate a submission.
        $this->setUser($extrastudents[1]);
        $submission = $assign->get_group_submission($extrastudents[1]->id, $group->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $assign->testable_update_submission($submission, $extrastudents[1]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        ...
        // Simulate a submission.
        $this->setUser($extrastudents[2]);
        $submission = $assign->get_group_submission($extrastudents[2]->id, $group->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $assign->testable_update_submission($submission, $extrastudents[2]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        ...
        // Simulate a submission.
        $this->setUser($extrastudents[3]);
        $submission = $assign->get_group_submission($extrastudents[3]->id, $group->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $assign->testable_update_submission($submission, $extrastudents[3]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        ...
        $data = new stdClass();
        $data->grade = '50.0';
        $assign->testable_apply_grade_to_user($data, $extrastudents[3]->id, 0);

        // Create a new submission with status NEW.
        $this->setUser($extrastudents[4]);
        $submission = $assign->get_group_submission($extrastudents[4]->id, $group->id, true);
