...
class assign {
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update = $this->get_instance_from_form($formdata);
    ...
    /**
     * Build the instance record from the form data
     *
     * @param $formdata
     * @return stdClass
     */
    private function get_instance_from_form($formdata) {
        $adminconfig = $this->get_admin_config();
        ...
        $instance->course = $formdata->course;
        $instance->courseid = $formdata->course;
        $instance->intro = $formdata->intro;
        $instance->introformat = $formdata->introformat;
        $instance->alwaysshowdescription = !empty($formdata->alwaysshowdescription);
        $instance->submissiondrafts = $formdata->submissiondrafts;
        $instance->requiresubmissionstatement = $formdata->requiresubmissionstatement;
        $instance->sendnotifications = $formdata->sendnotifications;
        $instance->sendlatenotifications = $formdata->sendlatenotifications;
        $instance->sendstudentnotifications = $adminconfig->sendstudentnotifications;
        if (isset($formdata->sendstudentnotifications)) {
            $instance->sendstudentnotifications = $formdata->sendstudentnotifications;
        }
        $instance->duedate = $formdata->duedate;
        $instance->cutoffdate = $formdata->cutoffdate;
        $instance->allowsubmissionsfromdate = $formdata->allowsubmissionsfromdate;
        $instance->grade = $formdata->grade;
        $instance->teamsubmission = $formdata->teamsubmission;
        $instance->requireallteammemberssubmit = $formdata->requireallteammemberssubmit;
        if (isset($formdata->teamsubmissiongroupingid)) {
            $instance->teamsubmissiongroupingid = $formdata->teamsubmissiongroupingid;
        }
        $instance->blindmarking = $formdata->blindmarking;
        $instance->attemptreopenmethod = ASSIGN_ATTEMPT_REOPEN_METHOD_NONE;
        if (!empty($formdata->attemptreopenmethod)) {
            $instance->attemptreopenmethod = $formdata->attemptreopenmethod;
        }
        if (!empty($formdata->maxattempts)) {
            $instance->maxattempts = $formdata->maxattempts;
        }
        if (isset($formdata->preventsubmissionnotingroup)) {
            $instance->preventsubmissionnotingroup = $formdata->preventsubmissionnotingroup;
        }
        $instance->markingworkflow = $formdata->markingworkflow;
        $instance->markingallocation = $formdata->markingallocation;
        if (empty($instance->markingworkflow)) { // If marking workflow is disabled, make sure allocation is disabled.
            $instance->markingallocation = 0;
        }

        if (!empty($formdata->completionunlocked)) {
            $instance->completionsubmit = !empty($formdata->completionsubmit);
            $instance->completionpass = !empty($formdata->completionpass);
        }
