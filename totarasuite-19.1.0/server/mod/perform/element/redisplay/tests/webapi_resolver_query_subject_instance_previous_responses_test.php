 ...
 */
class performelement_redisplay_webapi_resolver_query_subject_instance_previous_responses_test extends \core_phpunit\testcase {
    ...
    */
    ...
    public function test_trying_to_access_redisplay_element_with_non_associated_participant_section() {
        ...
        $this->expectExceptionMessage('Invalid access to redisplay');
    ...
    public function test_it_returns_previous_responses_with_named_relationships() {
        ...
        $this->assert_result_responses($result);
    }
    ...
    public function test_it_should_check_login_user_capability_without_participant_section_id() {
        ...
        $this->expectException(coding_exception::class);
        $this->expectExceptionMessage('Invalid report access to redisplay');
        $this->resolve_graphql_query(self::QUERY, [
            'input' => [
                'section_element_id' => $redisplay['respondable_section_element']->id,
                'subject_instance_id' => $subject_instance_id,
    ...
    public function test_it_returns_response_from_same_subject_instance(): void {
        [
            ...
            $redisplay_subject_participant_section,
            $subject_instance_id
    ...
    private function create_activity_with_redisplay_on_same_subject_instance(bool $generate_response = true): array {
        ...
        /** @var participant_section $redisplay_subject_participant_section */
        $redisplay_subject_participant_section = $this->get_participant_section_of_user_from_activity(
            $activity->id, $this->users['subject']->id, $section2->id
        );
        $subject_instance_id = $this->get_subject_instances_belonging_to_activity($activity->id)->first()->id;
     ...
     * @return void
     */
    private function assert_result_responses($result): void {
        $this->assertInstanceOf(section_element_response::class, $result['your_response']);
        $this->assertEquals("my previous answer {$this->users['subject']->id}", $result['your_response']->response_data);
        $this->assertFalse($result['is_anonymous']);
     ...
     */
    private function get_participant_section_of_user_from_activity(int $activity_id, int $user_id, ?int $section_id = null): ?participant_section {
