...
class participant_selector_subject implements recipient {
    public static function get_name(): string {
        return get_string('notification_recipient_participant_selector_subject', 'mod_perform');
    }
    ...
    public static function get_user_ids(array $data): array {
        ...
        $subject_selector_relationship_id = core_relationship::load_by_idnumber(constants::RELATIONSHIP_SUBJECT)->id;
        $subjects = [];
        ...
        foreach($subject_instance->manual_relationship_selection_progress as $progress) {
            // Only subject
            if ($progress->manual_relationship_selection->selector_relationship_id != $subject_selector_relationship_id) {
                continue;
            }
            ...
            foreach ($progress->manual_relationship_selectors as $selector) {
                $subjects[] = $selector->user_id;
            }
        }

        //Subject can be a selector for multiple. But only one notification send per subject instance.
