...
class participant_selector_appraiser implements recipient {
    public static function get_name(): string {
        return get_string('notification_recipient_participant_selector_appraiser', 'mod_perform');
    }
    ...
    public static function get_user_ids(array $data): array {
        ...
        $appraiser_selector_relationship_id = core_relationship::load_by_idnumber(constants::RELATIONSHIP_APPRAISER)->id;
        $appraisers = [];

        $current_appraisers = job_assignment::repository()
            ->where('userid', $data['subject_user_id'])
            ->where_not_null('appraiserid')
            ->get()
            ->pluck('appraiserid');

        if (empty($current_appraisers)) {
            return [];
        }
        ...
        foreach($subject_instance->manual_relationship_selection_progress as $progress) {
            // Only for appraiser
            if ($progress->manual_relationship_selection->selector_relationship_id != $appraiser_selector_relationship_id) {
                continue;
            }
            ...
            foreach ($progress->manual_relationship_selectors as $selector) {
                ...
                if (in_array($selector->user_id, $current_appraisers)) {
                    $appraisers[] = $selector->user_id;
                }
            }
        }

        // An appraiser can be a selector for multiple. But only one notification send per subject instance.
        return array_unique($appraisers);
    }
}