 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_url_lib_test extends \core_phpunit\testcase {
    ...
    /**
     * Assert the correct displaytype is returned.
     *
     * @return void
     */
    public function test_url_display_type() {
        global $CFG;
        ...
        // Assert is local URL
        $url->externalurl = $CFG->wwwroot . '/mod/url/tests/index.php';
        $this->assertEquals(RESOURCELIB_DISPLAY_OPEN, url_get_final_display_type($url));

        // Assert external URL
        $url->externalurl = 'https://external.example.com/another/path/';
        $this->assertEquals(RESOURCELIB_DISPLAY_DOWNLOAD, url_get_final_display_type($url));

        // Assert external URL
        $url->externalurl = 'https://external.example.com/another/path/image.png';
        $this->assertEquals(RESOURCELIB_DISPLAY_EMBED, url_get_final_display_type($url));
    }
}
