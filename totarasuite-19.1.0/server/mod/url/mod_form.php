...
class mod_url_mod_form extends moodleform_mod {
    function definition() {
        ...
        $available_options = explode(',', $config->displayoptions);

        $hook = new module_available_display_options($available_options, 'url', $this->current->course);
        $hook->execute();

        if ($this->current->instance) {
            $options = resourcelib_get_displayoptions($hook->available_options, $this->current->display);
        } else {
            $options = resourcelib_get_displayoptions($hook->available_options);
        }
        if (count($options) == 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) ||
            ...
            array_key_exists(RESOURCELIB_DISPLAY_LINK, $options)) {
