  ...
  <ResponseBox class="tui-commentReplyForm">
    <Form class="tui-commentReplyForm__form">
      ...
      <Weka
        ...
        :aria-label="$str('enterreply', 'totara_comment')"
...
import UnsavedChangesWarning from 'totara_comment/components/form/UnsavedChangesWarning';
...
export default {
  ...
  emits: ['form-ready', 'cancel', 'submit'],
  ...
  watch: {
    ...
    replyTo: {
      ...
      immediate: true,
      handler(value) {
        if (!value || !this.editor.extraExtensions.mention) {
