...
export default {
    ...
    subjectInstance() {
      this.jobAssignments =
        this.subjectInstance && !this.isExternalParticipant
          ? this.removeRedundantSubjectStaticInstances(
              this.subjectInstance.subject_static_instances
            )
          : [];
    },
  },
  ...
  methods: {
    /**
     * Remove subject static instance records that have 'former_manager' flagged but don't have a manager.
     * These don't add any useful information.
     *
     * @param {Object} subjectStaticInstances job assignment info related to subject user
     * @return {Object}
     */
    removeRedundantSubjectStaticInstances(subjectStaticInstances) {
      return subjectStaticInstances.filter(
        item =>
          !item.former_manager ||
          (item.job_assignment && item.job_assignment.managerja)
      );
    },
  },
};
