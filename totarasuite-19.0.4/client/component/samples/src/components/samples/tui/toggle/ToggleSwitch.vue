...
<template>
  <div>
    ...
    <p>
      On <code>click</code> we change only the button's
      (<code>aria-pressed</code>) state, not its label.
    </p>

    <SamplesExample>
      <ToggleSwitch
        id="toggle"
        v-model:value="isToggled"
        :disabled="disabled"
        :text="text"
    ...
    <SamplesPropCtl>
      <FormRow label="toggleFirst">
        <RadioGroup v-model:value="toggleFirst" :horizontal="true">
      ...
      <FormRow label="ariaLabel">
        <InputText v-model:value="ariaLabel" />
      </FormRow>
    </SamplesPropCtl>
  </div>
</template>
