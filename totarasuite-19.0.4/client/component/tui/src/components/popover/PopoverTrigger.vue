...
<script>
import { cloneVNode, toHandlers } from 'vue';
    ...
    if (value) {
      this._handlers.click.docClick = e => {
        if (
          ...
          !this.getExcludedElements().some(x => x && x.contains(e.target))
        ) {
...
export default {
  ...
  emits: ['open-changed', 'trigger-type-changed'],
  ...
  render() {
    const trigger = extractSingleChild(
      this.$slots.default && this.$slots.default()
    );
    return cloneVNode(trigger, toHandlers(this.helper.getHandlers()));
  },
};
</script>
