 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_manual_lib_test extends \core_phpunit\testcase {
    /**
     ...
     * @return array
     */
    public static function expiry_manual_enrolment_data_provider() {
        return [ [ 0 ], [ 1 ], [ 2 ] ];
    }
    ...
    public function test_send_expiry_notifications_manual_enrolment(int $expirynotify) {
        ...
        if ($expirynotify >= 1) {
            ...
            $this->assertStringContainsString($coursename, $message->fullmessagehtml);
        }
        if ($expirynotify >= 2) {
            $message = array_pop($messages);
