...
class course_enrolment_manager {
    ...
    public function get_users($sort, $direction='ASC', $page=0, $perpage=25) {
        ...
        if (!array_key_exists($key, $this->users)) {
            ...
            $sql = "SELECT $ufields, COALESCE(ul.timeaccess, 0) AS lastcourseaccess
                      FROM {user} u
                      JOIN {user_enrolments} ue ON (ue.userid = u.id  AND ue.enrolid $instancessql)
                      JOIN {enrol} e ON (e.id = ue.enrolid)
            ...
            $sql .= "WHERE $filtersql
                GROUP BY $ufields, COALESCE(ul.timeaccess, 0)
    ...
    /**
     * Get roles for a list of users.
     *
     * @param array $user_ids
     * @return array
     */
    private function get_users_roles(array $user_ids): array {
        $roles = [];

        if (empty($user_ids)) {
            return $roles;
        }
        $users_role_assignments = get_users_roles($this->context, $user_ids, true, 'c.contextlevel DESC, r.sortorder ASC');
        $plugins = $this->get_enrolment_plugins(false);

        foreach ($user_ids as $user_id) {
            // Ensure all users are represented in the returned roles array.
            $roles[$user_id] = [];
        }

        foreach ($users_role_assignments as $user_role_assignments) {
            foreach ($user_role_assignments as $ra) {
                if ($ra->contextid != $this->context->id) {
                    if (!array_key_exists($ra->roleid, $roles)) {
                        $roles[$ra->userid][$ra->roleid] = null;
                    }
                    // higher ras, course always takes precedence
                    continue;
                }
                if (array_key_exists($ra->userid, $roles)
                    && array_key_exists($ra->roleid, $roles[$ra->userid])
                    && $roles[$ra->userid][$ra->roleid] === false
                ) {
                    continue;
                }
                $changeable = true;
                ...
                $roles[$ra->userid][$ra->roleid] = $changeable;
            }
        }
        return $roles;
    }
    ...
    /**
     * Gets the enrolments for the users in the course - including all suspended plugins and instances.
     *
     * @global moodle_database $DB
     * @param array $user_ids
     * @return array
     */
    private function get_users_enrolments_by_user_id(array $user_ids): array {
        global $DB;
        if (empty($user_ids)) {
            return [];
        }
        list($instancessql, $params, $filter) = $this->get_instance_sql();
        [$userids_in_sql, $userids_params] = $DB->get_in_or_equal($user_ids, SQL_PARAMS_NAMED);

        // Totara: Make sorting order consistent between DBMS.
        $userenrolments = $DB->get_records_select(
            'user_enrolments',
            "enrolid $instancessql AND userid $userids_in_sql",
            $params + $userids_params,
            'id'
        );
        $instances = $this->get_enrolment_instances();
        $plugins = $this->get_enrolment_plugins(false);
        $inames = $this->get_enrolment_instance_names();
        $enrolments_by_user_id = [];
        foreach ($userenrolments as &$ue) {
            ...
            $enrolments_by_user_id[$ue->userid][] = $ue;
        }

        return $enrolments_by_user_id;
    }
    ...
    /**
     * Get course groups the provided user ids belong to.
     *
     * @param array $user_ids
     * @return array
     */
    private function get_users_groups(array $user_ids): array {
        global $DB;
        ...
        [$users_in_sql, $users_in_params] = $DB->get_in_or_equal($user_ids, SQL_PARAMS_NAMED);

        $sql = "select gm.userid, g.id
                from {groups_members} gm
                         inner join {groups} g on g.id = gm.groupid
                where g.courseid = :course_id
                  and gm.userid $users_in_sql order by g.name ASC";
        $result = $DB->get_records_sql_unkeyed($sql, $params + $users_in_params);
        $groups_by_userids = [];

        foreach ($result as $item) {
            $groups_by_userids[$item->userid][] = $item->id;
        }

        return $groups_by_userids;
    }

    /**
     * Return an array of params that would go into the URL to return to this
     * exact page.
     *
     * @return array
     */
    ...
    public function get_users_for_display(course_enrolment_manager $manager, $sort, $direction, $page, $perpage) {
        ...
        // User roles
        $roles_by_user_ids = $this->get_users_roles($user_ids);

        // User groups
        $groups_by_user_ids = $this->get_users_groups($user_ids);

        // User enrolments
        $enrolments_by_user_ids = $this->get_users_enrolments_by_user_id($user_ids);
        ...
        foreach ($users as $user) {
            ...
            // Roles
            $details['roles'] = array();
            foreach ($roles_by_user_ids[$user->id] as $rid => $rassignable) {
                $unchangeable = !$rassignable;
                if (!is_siteadmin() and !isset($assignable[$rid])) {
                    $unchangeable = true;
                }
                $details['roles'][$rid] = array(
                    'text' => $allroles[$rid]->localname,
            ...
            // Groups
            if (!empty($groups_by_user_ids[$user->id])) {
                foreach ($groups_by_user_ids[$user->id] as $group_id) {
                    $details['groups'][$group_id] = $allgroups[$group_id]->name;
                }
            }

            // Enrolments
            $details['enrolments'] = array();
            if (!empty($enrolments_by_user_ids[$user->id])) {
                foreach ($enrolments_by_user_ids[$user->id] as $ue) {
