...
class enrol_cohort_sync_test extends \core_phpunit\testcase {
    ...
    public function test_handler_sync_with_TL_40775_enabled() {
        global $DB, $CFG;

        $CFG->enable_TL_40775_until_T20 = true;
        ...
        $updatedrole = $DB->get_record('role_assignments', array('component' => 'enrol_cohort', 'userid' => $user1->id));
        $updatedrole->roleid = $teacherrole->id;
        $DB->insert_record('role_assignments', $updatedrole);
        $updatedrole->contextid = $cohort2->id;
        $DB->insert_record('role_assignments', $updatedrole);
        $updatedrole->itemid = 0;
        $DB->insert_record('role_assignments', $updatedrole);
        $this->assertEquals(7, $DB->count_records('role_assignments', array()));

        // After cohort sync is run we would expect only the record with a change in roleid to be removed
        enrol_cohort_sync($trace, $course1->id);
        ...
        $this->assertEquals(6, $DB->count_records('user_enrolments', array()));
        $this->assertEquals(7, $DB->count_records('role_assignments', array()));
        $this->assertEquals(4, $DB->count_records('role_assignments', array('component'=>'enrol_cohort', 'itemid'=>$cohortinstance1->id)));
        $this->assertEquals(5, $DB->count_records('groups_members', array()));
        $this->assertEquals(3, $DB->count_records('groups_members', array('component'=>'enrol_cohort', 'itemid'=>$cohortinstance1->id)));
