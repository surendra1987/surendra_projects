...
function print_grade_page_head($courseid, $active_type, $active_plugin=null,
    ...
    if ($shownavigation) {
        ...
        if (isset($headerhelpidentifier)) {
            $output = $OUTPUT->heading_with_help($heading, $headerhelpidentifier, $headerhelpcomponent, '', '', 1);
        } else {
            if (isset($user)) {
                $output = $OUTPUT->context_header(
                        array(
                            'heading' => html_writer::link(new moodle_url('/user/profile.php', array('id' => $user->id,
                ...
                $output = $OUTPUT->page_main_heading($heading);
            }
        }
