...
function get_grade_tree(&$gtree, $element, $current_itemid=null, $errors=null) {
    ...
    if ($type != 'category') {
        if (is_null($current_itemid) OR $grade_item->id != $current_itemid) {
            if ($idnumber) {
                ...
                $name .= '<input class="idnumber ml-2" id="id_idnumber_'.$grade_item->id.'" type="text" name="idnumbers['.$grade_item->id.']" />' . "\n";
                $name .= $closingdiv;
            }
        } else {
    ...
    if ($type != 'category') {
        $return_string .= '<li class="'.$type.' mb-2">'.$icon.$name.'</li>' . "\n";
    } else {
        $return_string .= '<li class="'.$type.' mb-2">'.$icon.$name . "\n";
