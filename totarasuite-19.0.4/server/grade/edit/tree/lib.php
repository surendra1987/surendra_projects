...
class grade_edit_tree {
    ...
    public function build_html_tree($element, $totals, $parents, $level, &$row_count) {
        ...
        $eid    = $element['eid'];
        $name = $this->gtree->get_element_header($element, true, true, true, true, true);
        ...
        if ($element['type'] == 'category') {
            ...
            $headercell->attributes['title'] = $strippedname;
            ...
            foreach ($this->columns as $column) {
                if (!($this->moving && $column->hide_when_moving)) {
                    $gradeitemrow->cells[] = $column->get_item_cell($item, array('id' => $id, 'name' => $name,
