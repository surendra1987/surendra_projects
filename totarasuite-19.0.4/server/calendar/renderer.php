...
class core_calendar_renderer extends plugin_renderer_base {
    ...
    public function event(calendar_event $event, $showactions=true) {
        ...
        if (!empty($event->time)) {
            $output .= html_writer::tag('span', $event->time, array('class' => 'date pull-xs-right mr-3'));
        } else {
            $attrs = array('class' => 'date pull-xs-right mr-3');
        ...
        for ($day = 1; $day <= $display->maxdays; ++$day, ++$dayweek) {
            ...
            if (isset($eventsbyday[$day])) {
                ...
                foreach($eventsbyday[$day] as $eventindex) {
                    ...
                    $link = html_writer::link($dayhref, format_string($events[$eventindex]->name, true), ['title' => format_string($events[$eventindex]->name)]);
            ...
            if (isset($durationbyday[$day])) {
                ...
                foreach($durationbyday[$day] as $eventindex) {
                    $link = html_writer::link($dayhref, format_string($events[$eventindex]->name, true) . ' (' . get_string('continue'). ')', ['title' => format_string($events[$eventindex]->name, true) . ' (' . get_string('continue'). ')']);
