...
if ($canmanage) {
    $buttons = html_writer::link(new moodle_url('/badges/index.php', array('type' => $type, 'id' => $courseid)),
        get_string('managebadges', 'badges'), array('class' => 'btn mr-1 btn-default'));
}
...
if (has_capability('moodle/badges:createbadge', $PAGE->context)) {
    $buttons .= html_writer::link(new moodle_url('newbadge.php', array('type' => $type, 'id' => $courseid)),
        get_string('newbadge', 'badges'), array('class' => 'btn mr-1 btn-default'));
}

echo $OUTPUT->page_main_heading($title, $buttons);
