...
class core_tag_renderer extends plugin_renderer_base {
    ...
    public function tag_search_page($query = '', $tagcollid = 0) {
        $rv = $this->output->page_main_heading(get_string('searchtags', 'tag'));
    ...
    public function tag_index_page($tag, $entities, $tagareaid, $exclusivemode, $fromctx, $ctx, $rec, $page) {
        ...
        $rv = '';
        $rv .= $this->output->page_main_heading($tagname);
    ...
    protected function search_form($query = '', $tagcollid = 0) {
        ...
        $output = '<form action="' . $searchurl . '" class="form-inline">';
        $output .= '<label class="accesshide" for="searchform_query">' . get_string('searchtags', 'tag') . '</label>';
        $output .= '<input id="searchform_query" name="query" type="text" size="40" value="' . s($query) . '" class="form-control mr-2" />';
        ...
        if (count($tagcolls) > 1) {
            ...
            $output .= html_writer::select($tagcolls, 'tc', $tagcollid, null, array('id' => 'searchform_tc', 'class' => 'mr-2'));
        }
