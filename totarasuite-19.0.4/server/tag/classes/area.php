...
class core_tag_area {
    ...
    public static function display_name($component, $itemtype) {
        if (\totara_topic\topic_helper::supports_topic_resolver($component)) {
            return new lang_string('engage', 'totara_engage');
        }
    ...
    /**
     * Find the appropriate tag collection ID for the given component
     *
     * @param $component
     * @return int
     */
    public static function get_default_collection_for($component): int {
        // If the component supports topics then make sure that it has the same tag collection ID as the existing
        // engage tag areas (they should all be the same), if there are any.
        if (topic_helper::supports_topic_resolver($component)) {
            $engage_collection = builder::table('tag_area')
                ->select('tagcollid')
                ->where('component', '=', 'engage_article')
                ->order_by('id')
                ->first();
            if ($engage_collection) {
    ...
    protected static function create($record) {
        ...
        if (empty($record->tagcollid)) {
            $record->tagcollid = self::get_default_collection_for($record->component);
        }
