...
class core_tag_collection {
    ...
    public static function get_areas_names($tagcollid, $enabledonly = true) {
        ...
        $itemtypes = array();
        $engage_was_already_done = false;
        foreach ($allitemtypes as $itemtype => $it) {
            foreach ($it as $component => $v) {
                if (topic_helper::supports_topic_resolver($component)) {
                    // We only use the first component which supports topics
                    if ($engage_was_already_done) {
                        continue;
                    }
                    $itemtypes[$v->id] = get_string('engage', 'totara_engage');
                    $engage_was_already_done = true;
                    continue;
                }
    ...
    public static function is_deleteable(int $tagcollectionid): bool {
        ...
        // Other than 'hashtag_collection_id' and the engage tag collection all the
