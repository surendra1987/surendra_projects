...
class core_message_externallib_test extends externallib_advanced_testcase {
    ...
    public function test_delete_message() {
        ...
        try {
            ...
            $this->assertStringContainsString('You do not have permission to delete this message', $e->getMessage());
        }
        ...
        $this->assertTrue($DB->record_exists(
            ...
            array('messageid' => $m2to3, 'userid' => $user3->id, 'action' => \core_message\api::MESSAGE_ACTION_DELETED)
        ));
        ...
        $this->assertTrue($DB->record_exists(
            'message_user_actions',
            array('messageid' => $m3to2, 'userid' => $user3->id, 'action' => \core_message\api::MESSAGE_ACTION_DELETED)
        ));
    ...
    public static function validate_messagearea_search_limitnum_data_provider(): array {
        return [
            // Default config
            [null, -1, false],
            [null, 0, true],
    ...
    /**
     * @dataProvider validate_messagearea_search_limitnum_data_provider
     * @param $configured_value
     * @param $limitnum_value
     * @param bool $expected_valid
     * @return void
     */
    public function test_validate_messagearea_search_limitnum($configured_value, $limitnum_value, bool $expected_valid): void {
        global $CFG;

        $max_value = 20; // default
        if ($configured_value) {
            $CFG->message_area_search_max_limitnum = $configured_value;
            $max_value = $configured_value;
        }
        ...
        if (!$expected_valid) {
            ...
            $this->expectExceptionMessage("The limitnum parameter must be between 1 and {$max_value}");
        }
        core_message_external::data_for_messagearea_search_users($user->id, 'User', $limitnum_value);
    }

    public function test_messagearea_search_limitnum_cannot_be_null() {
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);

        $this->expectException('invalid_parameter_exception');
        $this->expectExceptionMessage('the server was expecting "int" type');
        core_message_external::data_for_messagearea_search_users($user->id, 'User', null);
    }

    public function test_messagearea_search_limitnum_returns_correct_amount_of_results() {
        global $CFG;
        ...
        // Leave out 'limitnum' parameter: Should return up to default max limit (20).
        $result = core_message_external::data_for_messagearea_search_users($user->id, '');
        $this->assertCount(20, $result->noncontacts);

        // Same for zero value
        $result = core_message_external::data_for_messagearea_search_users($user->id, '', 0);
        $this->assertCount(20, $result->noncontacts);

        // Increase max limit config.
        $CFG->message_area_search_max_limitnum = 22;
        $result = core_message_external::data_for_messagearea_search_users($user->id, '');
        $this->assertCount(22, $result->noncontacts);

        $result = core_message_external::data_for_messagearea_search_users($user->id, '', 0);
        $this->assertCount(22, $result->noncontacts);

        // Increase max limit config above existing amount of users - all other users should be returned (23 created above + admin).
        $CFG->message_area_search_max_limitnum = 100;
        $result = core_message_external::data_for_messagearea_search_users($user->id, '');
        $this->assertCount(24, $result->noncontacts);

        $result = core_message_external::data_for_messagearea_search_users($user->id, '', 0);
        $this->assertCount(24, $result->noncontacts);

        // Try passing in 'limitnum' parameter.
        $result = core_message_external::data_for_messagearea_search_users($user->id, '', 2);
        $this->assertCount(2, $result->noncontacts);

        $result = core_message_external::data_for_messagearea_search_users($user->id, '', 50);
        $this->assertCount(24, $result->noncontacts);
    }
    ...
    public function test_messagearea_get_most_recent_message() {
        ...
        $this->assertStringContainsString('Word.', $result['text']);
    }
