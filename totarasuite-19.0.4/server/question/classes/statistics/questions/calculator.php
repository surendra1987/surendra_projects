    ...
    public function calculate($qubaids) {
        ...
        if ($lateststeps) {
            ...
            foreach ($this->stats->get_all_slots() as $slot) {
                ...
                $slot_covariance = $this->stats->for_slot($slot)->covariancewithoverallmark;
                if ($slot_covariance !== null && $slot_covariance >= 0) {
                    $sumofcovariancewithoverallmark += sqrt($slot_covariance);
                }
            }
            $this->progress->end_progress();
