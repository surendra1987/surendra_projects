...
abstract class question_bank {
    ...
    /**
     * Returns true if the current user is allowed to add a question of a particular type in a given context.
     *
     * This is simpler than question_has_capability_on(), you should only use it if you don't have question data, just a type name.
     *
     * @param string $qtypename
     * @param context $context
     * @return bool
     */
    public static function user_can_add_qtype(string $qtypename, context $context): bool {
        $hook = new question_can_manage_type($qtypename, $context);
        $hook->execute();
        if (!$hook->is_allowed()) {
            return false;
        }
     ...
     */
    public static function get_creatable_qtypes(context $context = null) {
        $config = self::get_config();
        ...
        foreach ($allqtypes as $name => $qtype) {
            if (static::qtype_enabled($name)
                && static::user_can_add_qtype($name, $context)
            ) {
