...
class qtype_calculated_qe2_attempt_updater extends question_qtype_attempt_updater {
    ...
    protected function substitute_values_for_eval($expression) {
        if (is_null($this->search) || is_null($this->safevalue)) {
            return $expression;
        }
...
/**
 * Adds the new 'moodle/question:managecalculated' capability to all roles that
 * have the 'moodle/question:add' capability.
 */
function qtype_calculated_assign_manage_calculated_capability() {
    global $DB;

    // Unfortunately, this script runs *before* the new capability is loaded
    // from access.php.So need to force the capability to be registered at this
    // point in time.
    update_capabilities('qtype_calculated');

    $roles = $DB->get_records(
        'role_capabilities',
        ['capability' => 'moodle/question:add']
    );

    foreach ($roles as $role) {
        assign_capability(
            'moodle/question:managecalculated',
            $role->permission,
            $role->roleid,
            $role->contextid,
            true
