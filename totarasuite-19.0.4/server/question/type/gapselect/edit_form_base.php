...
class qtype_gapselect_edit_form_base extends question_edit_form {
    ...
    public function validation($data, $files) {
        ...
        // Check that choices are not removed after answers are recorded.
        if ($data['id']) {
            $new_count = 0;
            ...
            $old_count = $DB->count_records('question_answers', ['question' => $data['id']]);
            if ($new_count != $old_count) {
                ...
                if ($submissions_exist) {
                    ...
                    $errors['choices[' . min($new_count, $old_count) . ']'] = get_string('cannoteditafterattempts', 'mod_quiz', $old_count);
                }
            }
        }
