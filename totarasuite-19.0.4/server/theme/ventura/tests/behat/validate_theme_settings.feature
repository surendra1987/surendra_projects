...
Feature: Theme settings basic validations
  ...
  Scenario: Confirm Images tab has all the required elements
    ...
    And the URL for image nested in "[data-testid=login-bg-image]" should match "/theme\/image.php\/ventura\/totara_core\/[0-9]+\/default_login_background/"
    And the URL for image nested in "[data-testid=course-default-image]" should match "/theme\/image.php\/ventura\/core\/[0-9]+\/course_defaultimage/"
    And the URL for image nested in "[data-testid=program-default-image]" should match "/theme\/image.php\/ventura\/totara_program\/[0-9]+\/defaultimage/"
    And the URL for image nested in "[data-testid=certification-default-image]" should match "/theme\/image.php\/ventura\/totara_certification\/[0-9]+\/defaultimage/"
    And the URL for image nested in "[data-testid=engage-resource-default-image]" should match "/theme\/image.php\/ventura\/engage_article\/[0-9]+\/default/"
    And the URL for image nested in "[data-testid=workspace-default-image]" should match "/theme\/image.php\/ventura\/container_workspace\/[0-9]+\/default_space/"
