...
function core_login_process_password_reset_request() {
    ...
    if ($mform->is_cancelled()) {
        ...
        echo $OUTPUT->page_main_heading($strforgotten);
    ...
    echo $OUTPUT->render_from_template('core/forgot_password', ['form' => $form]);

    echo $OUTPUT->footer();
}

/**
 * Redirect user to MFA verify page on password reset if required.
 *
 * @param string $token
 * @return void
 */
function core_login_process_password_require_mfa(string $token): void {
    global $CFG, $OUTPUT;
    $user = core_login_process_password_get_user_by_token($token);

    // If no user is found, redirect to forgot password page.
    ...
    $framework = new \core_mfa\framework($user->id);

    if ($framework->is_mfa_required()) {
        (new \core_mfa\escalation\forgot_password($user->id))->trigger(['token' => $token]);
 ...
 * @return stdClass|false user
 */
function core_login_process_password_get_user_by_token(string $token) {
    global $DB;
    ...
    return $DB->get_record_sql($sql, array($token));
}
...
function core_login_process_password_set($token) {
    ...
    if (empty($data)) {
        ...
        complete_user_login(
            $user,
            \core\login\complete_login_callback::create('password_set_complete_login_callback')
        );
    }
}
...
/**
 * Persist the referrer in the $SESSION->wantsurl property if it's not already set
 * and the referrer isn't the login page itself.
 *
 * If $SESSION->wantsurl is not empty then this will make no change.
 *
 * @param string|null $referer What URL to preseve. Defaults to get_local_referrer().
 * @return void
 */
function core_login_set_wants_url($referer = null): void {
    global $SESSION, $CFG;
