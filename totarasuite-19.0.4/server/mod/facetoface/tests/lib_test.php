...
class mod_facetoface_lib_test extends mod_facetoface_facetoface_testcase {
    ...
    function test_facetoface_update_instance() {
        ...
        $this->markTestIncomplete('Will be fixed in TL-21048.');
    ...
    function test_facetoface_update_attendees_with_signupwaitlist_permission() {
        // Create a new session with no date, waitlist enabled
        // Create a user with signupwaitlist permission
        // signup_helper::update_attendees
        // User's status should not change.
        global $DB;
        $this->setAdminUser(); // Need to set this user to assign the user. Could be any user with signupwaitlist permission!

        $course = $this->getDataGenerator()->create_course();
        // Use the Editing Trainer, who has signupwaitlist permission
        $user = $this->getDataGenerator()->create_and_enrol($course, 'editingteacher');

        $seminar_data = new \stdClass();
        $seminar_data->course = $course->id;
        $seminar_generator = $this->facetoface_generator;
        $seminar = $seminar_generator->create_instance($seminar_data);

        $session_data = new stdClass();
        $session_data->facetoface = $seminar->id;
        $session_data->allowoverbook = 1; // Enable waitlist
        $session_id = $seminar_generator->add_session($session_data);

        $seminar_event = new seminar_event($session_id);
        $signup_id = $seminar_generator->create_signup($user, $seminar_event)->id;
        $expected_row = $DB->get_record(
            'facetoface_signups_status',
            ['signupid' => $signup_id, 'superceded' => 0]
        );
        $expected_status = $expected_row->statuscode;

        // Remove the session date
        $session = $seminar_event->get_sessions();
        $session->delete();
        $this->assertFalse($seminar_event->is_sessions());

        signup_helper::update_attendees($seminar_event);
     ...
     */
    public static function facetoface_messaging_settings() {
        $data = array(
     ...
     */
    public static function data_provider_facetoface_get_completion_state(): array {
    ...
    /**
     * Tests for mod_facetoface_refresh_events.
     */
    public function test_facetoface_refresh_events() {
        global $DB;
        $now = time();
        $f2fgen = $this->getDataGenerator()->get_plugin_generator('mod_facetoface');

        // Create a seminar for course 1 and add a session.
        $course1 = $this->getDataGenerator()->create_course();
        $facetoface1 = $this->getDataGenerator()->create_module('facetoface', array('course' => $course1->id, 'shortname' => 's1'));
        ...
        $sid = $f2fgen->add_session(array('facetoface' => $facetoface1->id, 'sessiondates' => array($sessiondate)));
        $seminar = new seminar($facetoface1->id);

        // Create a seminar for course 2 and add a session.
        $course2 = $this->getDataGenerator()->create_course();
        $facetoface2 = $this->getDataGenerator()->create_module('facetoface', array('course' => $course2->id, 'shortname' => 's2'));

        $sessiondate->timefinish = $now + 5 * HOURSECS;
        $sid = $f2fgen->add_session(array('facetoface' => $facetoface2->id, 'sessiondates' => array($sessiondate)));
        $seminar2 = new seminar($facetoface2->id);

        // Change Seminar1 name.
        $seminar = new seminar($facetoface1->id);
        $seminar->set_shortname('seminar1_name');
        $seminar->save();

        // Change Seminar2 name.
        $seminar2 = new seminar($facetoface2->id);
        $seminar2->set_shortname('seminar2_name');
        $seminar2->save();

        // Trigger refresh events for course1. It should not affect seminars for course2.
        $this->assertTrue(facetoface_refresh_events($course1->id));
        $eventparams = array('modulename' => 'facetoface', 'eventtype' => 'facetofacesession', 'instance' => $facetoface1->id);
        $events = $DB->get_records('event', $eventparams);
        foreach ($events as $event) {
            $this->assertEquals('seminar1_name', $event->name);
        }

        // Checking events are not updated for seminars in course 2.
        $eventparams = array('modulename' => 'facetoface', 'eventtype' => 'facetofacesession', 'instance' => $facetoface2->id);
        $events = $DB->get_records('event', $eventparams);
        foreach ($events as $event) {
            $this->assertEquals('s2', $event->name);
        }
        ...
        $this->assertTrue(facetoface_refresh_events());

        // Check all events were updated after we've updated seminars and after calling the refresh events without params.
        $events = $DB->get_records('event');
        foreach ($events as $event) {
            if ($event->modulename === 'facetoface' && $event->instance === $facetoface1->id && $event->eventtype === 'facetofacesession') {
                $this->assertEquals('seminar1_name', $seminar->get_shortname());
            }
            if ($event->modulename === 'facetoface' && $event->instance === $facetoface2->id && $event->eventtype === 'facetofacesession') {
                $this->assertEquals('seminar2_name', $seminar2->get_shortname());
            }
        }
    }
     ...
     * @return void
     */
    public function test_facetoface_cm_info_dynamic(): void {
        $generator = self::getDataGenerator();
        self::setAdminUser();

        $course = $generator->create_course();
        $facetoface = $this->facetoface_generator->create_instance([
            'name' => 'facetoface1',
            'course' => $course->id,
        ]);
        ...
        $signup = signup::create(get_admin()->id, $seminarevent);
        signup_helper::signup($signup);
        $signupstatus = $signup->get_signup_status();

        self::assertEquals($signupstatus->get_statuscode(), booked::get_code());
        $modinfo = get_fast_modinfo($course);

        /** @var cm_info $cminfo */
        $cminfo = current($modinfo->get_cms());
        self::assertEquals(2, $cminfo->get_url()->get_param('booking'));
    }
}