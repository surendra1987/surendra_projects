...
class behat_facetoface extends behat_base {
    ...
    public function i_click_on_the_seminar_event_action_in_row(string $action, string $row) {
        ...
        // Move the mouse over the button so that the menu is visible
        $action = $this->getSession()->getDriver()->mouseover($ddxpath);

        $this->click_node($ddnode);
        $this->wait_for_pending_js();
        $menunode = $this->find(
            'xpath',
            $menuxpath,
        ...
        $script = 'document.querySelector(".mod_facetoface__sessionlist__action__dropdown[aria-expanded=\"true\"] + .mod_facetoface__sessionlist__action__dropdown__menu") && window.scrollTo(0, document.querySelector(".mod_facetoface__sessionlist__action__dropdown[aria-expanded=\"true\"] + .mod_facetoface__sessionlist__action__dropdown__menu").getClientRects()[0].y)';
        $driver->executeScript($script);
        $this->click_node($menunode);
    }
    ...
    public function i_set_the_seminar_signup_job_assignment_to_for($jobassignment, $username) {
        $escaper = new Escaper();
        $username_escaped = $escaper->escapeLiteral($username);
        $edit_escaped = $escaper->escapeLiteral(get_string('edit'));
        $xpath = "//table[@id='facetoface_sessions']//th[contains(@class, 'user_namelink') and contains(., {$username_escaped})]/ancestor::tr/td[contains(@class, 'session_positionnameedit')]//a[contains(., {$edit_escaped})]";
        $nodes = $this->find_all('xpath', $xpath);
        if (empty($nodes) || count($nodes) > 1) {
    ...
    /**
     * We are using legacy seminar notifications
     *
     * @Given /^I (am|am not) using legacy seminar notifications$/
     */
    public function i_am_using_legacy_seminar_notifications(string $using_legacy) {
        \behat_hooks::set_step_readonly(false);

        // Set seminar notification defaults
        $am_using = ($using_legacy === 'am');
        set_config('facetoface_allow_legacy_notifications', $am_using);
        set_config('legacy_notifications', $am_using, 'facetoface');
    }
