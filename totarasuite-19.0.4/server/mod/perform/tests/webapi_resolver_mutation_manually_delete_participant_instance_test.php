...
/**
 * @coversDefaultClass \mod_perform\webapi\resolver\mutation\manually_delete_participant_instance
 *
 * @group perform
 */
class mod_perform_webapi_resolver_mutation_manually_delete_participant_instance_test extends testcase {
    private const MUTATION = 'mod_perform_manually_delete_participant_instance';

    use webapi_phpunit_helper;

    public function test_manually_delete_participant_instance_opened(): void {
        [$participant_instance, $args] = $this->generate_data();

        $result = $this->resolve_graphql_mutation(self::MUTATION, $args);
        self::assertTrue($result);
        self::assertNull(participant_instance::repository()->find($participant_instance->id));
    }

    public function test_manually_delete_participant_instance_closed(): void {
        ...
        $result = $this->parsed_graphql_operation('mod_perform_manually_change_participant_instance', $input);
    ...
    public function test_failed_ajax_query_id_null(): void {
        ...
        $this->assert_webapi_operation_failed(
            $result,
            'Variable "$input" of required type "mod_perform_manually_delete_participant_input!'
        );
    }
    ...
    public function test_failed_ajax_query_id_zero(): void {
        ...
        $args['input']['participant_instance_id'] = 0;
        $result = $this->parsed_graphql_operation(self::MUTATION, $args);
        $this->assert_webapi_operation_failed(
            $result,
            'Invalid parameter value detected (invalid participant instance id)'
        );
    }
    ...
    public function test_failed_ajax_query_id_999(): void {
        ...
        $args['input']['participant_instance_id'] = 999;
        $result = $this->parsed_graphql_operation(self::MUTATION, $args);
        $this->assert_webapi_operation_failed(
            $result,
            'Invalid activity'
    ...
    public function test_failed_ajax_query_guestuser(): void {
        ...
        self::setGuestUser();
        $args['input']['participant_instance_id'] = $participant_instance->id;
        $result = $this->parsed_graphql_operation(self::MUTATION, $args);
        $this->assert_webapi_operation_failed(
            $result,
    ...
    private function generate_data() {
        ...
        return [$participant_instance, $args];
    }
}