...
class participant_selector_managers_manager implements recipient {
    public static function get_name(): string {
        return get_string('notification_recipient_participant_selector_managers_manager', 'mod_perform');
    }
    ...
    public static function get_user_ids(array $data): array {
        ...
        $managers_manager_relationship_id = core_relationship::load_by_idnumber(constants::RELATIONSHIP_MANAGERS_MANAGER)->id;
        ...
        foreach($subject_instance->manual_relationship_selection_progress as $progress) {
            // Only manager's manager
            if ($progress->manual_relationship_selection->selector_relationship_id != $managers_manager_relationship_id) {
                continue;
            }
            ...
            foreach ($progress->manual_relationship_selectors as $selector) {
                // Check selector is the current manager's manager
                foreach ($current_managers as $manager) {
                    $current_managers_manager = job_assignment::get_all_manager_userids($manager);
                    // No managers for selected manager
                    if (empty($current_managers_manager)) {
                        continue;
                    }

                    if (in_array($selector->user_id, $current_managers_manager)) {
                        $managers_manager[] = $selector->user_id;
                    }
                }
            }
        }

        // A manager's manager can be a selector for multiple. But only one notification send per subject instance.
        return array_unique($managers_manager);
    }
}