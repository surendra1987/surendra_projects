...
function resource_print_heading($resource, $cm, $course, $notused = false) {
    global $OUTPUT;
    echo $OUTPUT->page_main_heading(format_string($resource->name));
}
 ...
 * @param object $resource must contain at least 'id' from table 'resource'
 * @return int display type constant
 */
function resource_get_final_display_type($resource) {
    global $DB;

    if (empty($resource->course)) {
        $course_id = $DB->get_field('resource', 'course',['id' => $resource->id] ,MUST_EXIST);
    } else {
        $course_id = $resource->course;
    }

    $hook = new \core\hook\module_get_final_display_type('resource', $course_id, $resource);
    $hook->execute();

    if (!is_null($hook->override_display_type)) {
        return $hook->override_display_type;
    }
