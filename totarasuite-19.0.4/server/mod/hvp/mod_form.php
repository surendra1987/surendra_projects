...
class mod_hvp_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $count = $DB->count_records_sql("SELECT COUNT('x') FROM {hvp_libraries} WHERE runnable = 1 AND semantics IS NOT NULL");
        $h5paction[] = $mform->createElement('radio', 'h5paction', '', get_string('upload', 'hvp'), 'upload');

        if ($count || get_config('mod_hvp', 'hub_is_enabled')) {
        ...
        $show_warning = false;
        if (!$count) {
            if (!get_config('mod_hvp', 'hub_is_enabled')) {
                // Check users can upload or not.
                if (has_capability('mod/hvp:updatelibraries', $this->get_context())) {
                    $show_warning_key = 'h5p_disabled_warning';
                } else {
                    $actions_disabled = true;
                    $show_warning_key = 'h5p_disabled_warning_no_permission';
                }
                $show_warning = true;
            } else {
                if (!has_any_capability(['mod/hvp:installrecommendedh5plibraries', 'mod/hvp:updatelibraries'], $this->get_context())) {
                    $show_warning = true;
                    $show_warning_key = 'h5p_disabled_warning_no_permission';
                }
            }
        ...
        // If true, we include this radion option, as we need to disable both.
        if ($actions_disabled) {
            $h5paction[] = $mform->createElement('radio', 'h5paction', '', get_string('create', 'hvp'), 'create');
        }
        ...
        // Position warning message bellow action option.
        if ($show_warning) {
            $mform->addElement('static', 'warning', null, '<span class="error">' . get_string($show_warning_key, 'hvp') . '</span>');
        }

        $mform->setDefault('h5paction', 'create');

        // Upload and shows if upload option is enabled.
        if (!$actions_disabled) {
            $mform->addElement('filepicker', 'h5pfile', get_string('h5pfile', 'hvp'), null, array('maxbytes' => $COURSE->maxbytes, 'accepted_types' => '*'));
        } else {
            // Disabled both of radios.
            $mform->disabledIf('h5paction', 'maximumgrade');
        }
        ...
        if ($CFG->theme == 'boost' || in_array('boost', $PAGE->theme->parents)) {
            ...
            $mform->addElement('static', 'h5peditor', get_string('editor', 'hvp'),
                               '<div class="h5p-editor">' . get_string('javascriptloading', 'hvp') . '</div>');
            $mform->addHelpButton('h5peditor', 'editor', 'hvp');
        }

        // Hidden fields.
        $mform->addElement('hidden', 'h5plibrary', '');
        $mform->setType('h5plibrary', PARAM_TEXT);
    ...
    public function validation($data, $files) {
        ...
        if (array_key_exists('completion', $data) && $data['completion'] == COMPLETION_TRACKING_AUTOMATIC) {
            $completionpass = $data['completionpass'] ?? $this->current->completionpass;
    ...
    public function get_data() {
        ...
        if (isset($data) && empty($data->completionusegrade)) {
