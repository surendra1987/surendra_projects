...
switch($action) {
    ...
    case 'contentsuserdata':
        $cm = get_coursemodule_from_instance('hvp', required_param('content_id', PARAM_INT));
        require_login($cm->course, false, $cm);
        $token = required_param('token', PARAM_ALPHANUM);

        // Query String Parameters.
        $contentid = required_param('content_id', PARAM_INT);
        $dataid = required_param('data_type', PARAM_TEXT);
        ...
        \mod_hvp\content_user_data::handle_ajax(
            $contentid, $subcontentid, $dataid, $token, $data, $preload, $invalidate, $contextid
        );
        break;
    ...
    case 'restrictlibrary':
        ...
        if (!\H5PCore::validToken('library_' . $libraryid, required_param('token', PARAM_ALPHANUM))) {
    ...
    case 'setfinished':
        ...
        \mod_hvp\user_grades::handle_ajax($token, $contextId, $score, $maxScore);
        break;
    ...
    case 'updatesubcontentscore':
        ...
        \mod_hvp\user_grades::handle_dynamic_grading($token, $contextId, $subContentId, $score);
        break;
    ...
    case 'getsubcontentscore':
        $subContentId = required_param('subcontent_id', PARAM_INT);
        $xapi_results = $DB->get_record('hvp_xapi_results', array('id' => required_param('subcontent_id', PARAM_INT)));
        $cm = get_coursemodule_from_instance('hvp', $xapi_results->content_id);
        require_login($cm->course, false, $cm);

        \mod_hvp\user_grades::return_subcontent_grade($subContentId);
        break;
    ...
    case 'libraries':
        require_login(0, false);
        $contextId = required_param('contextId', PARAM_INT);
        if (!framework::has_editor_access('nopermissiontoviewcontenttypes', $contextId)) {
            break;
        }
        ...
        $language = optional_param('default-language', null, PARAM_LANG);
     ...
     */
    case 'files':
        $cm = get_coursemodule_from_instance('hvp', required_param('contentId', PARAM_INT));
        require_login($cm->course, false, $cm);

        $token = required_param('token', PARAM_ALPHANUM);
        $contentid = required_param('contentId', PARAM_INT);
        $contextId = required_param('contextId', PARAM_INT);
        if (!framework::has_editor_access('nopermissiontouploadfiles', $contextId)) {
            break;
        }
    ...
    case 'libraryupload':
        ...
        $token = required_param('token', PARAM_ALPHANUM);
        $contextId = required_param('contextId', PARAM_INT);
        if (!framework::has_editor_access('nopermissiontouploadcontent', $contextId)) {
            break;
        }
    ...
    case 'xapiresult':
        ...
        \mod_hvp\xapi_result::handle_ajax($token, $contextId, $xapiResult);
        break;

    case 'translations':
        require_login(0, false);
        $contextId = required_param('contextId', PARAM_INT);
        if (!framework::has_editor_access('nopermissiontogettranslations', $contextId)) {
            break;
        }
    ...
    case 'contenthubmetadatacache':
        require_login(0, false);
        $contextId = required_param('contextId', PARAM_INT);
        if (!framework::has_editor_access('nopermissiontoviewcontenthubcache', $contextId)) {
            break;
        }
        $editor = framework::instance('editor');
    ...
    case 'contenthubregistration':
        ...
        $token = required_param('_token', PARAM_ALPHANUM);
