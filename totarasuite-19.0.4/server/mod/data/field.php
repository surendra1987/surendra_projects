...
if (($mode == 'new') && (!empty($newtype)) && confirm_sesskey()) {          ///  Adding a new field
    ...
    echo '<select id="defaultsort" name="defaultsort" class="custom-select w-auto">';
...
/*        $options[DATA_TIMEMODIFIED] = get_string('timemodified', 'data');
    ...
    echo html_writer::select($options, 'defaultsortdir', $data->defaultsortdir, false, array('class' => 'custom-select w-auto'));
    echo '<input type="submit" class="btn btn-secondary ml-3" value="'.get_string('save', 'data').'" />';
    echo '</div>';
    echo '</form>';
    echo '</div>';
