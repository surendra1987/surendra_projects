...
function url_print_heading($url, $cm, $course, $notused = false) {
    global $OUTPUT;
    echo $OUTPUT->page_main_heading(format_string($url->name));
}
 ...
 * @param object $url must contain at least 'id' from table 'url'
...
function url_get_final_display_type($url) {
    ...
    if (empty($url->course)) {
        $course_id = $DB->get_field('url', 'course',['id' => $url->id] ,MUST_EXIST);
    } else {
        $course_id = $url->course;
    }
    $hook = new \core\hook\module_get_final_display_type('url', $course_id, $url);
    $hook->execute();
