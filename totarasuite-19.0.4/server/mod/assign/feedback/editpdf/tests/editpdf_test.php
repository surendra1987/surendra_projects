...
use assignfeedback_editpdf\task\convert_submissions as convert_submissions_task;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignfeedback_editpdf_editpdf_test extends \core_phpunit\testcase {
    ...
    public function test_ensure_pdf_compatible() {
        ...
        $sourcefile = $CFG->dirroot.'/mod/assign/feedback/editpdf/tests/fixtures/invalid.pdf';
        $fs->create_file_from_pathname($pdfsubmission, $sourcefile);
        ...
        // Expecting Throwable error, not Exception error.
        self::assertNotFalse(strpos(
            $output,
            'PDF "invalid.pdf" file uploaded by  user is not valid, trying it through the ghostscript to clean it up.'
        ));
    }

    private function run_task() {
        $sink = $this->redirectEvents();
        $task = new convert_submissions_task();
        ob_start();
        $task->execute();
