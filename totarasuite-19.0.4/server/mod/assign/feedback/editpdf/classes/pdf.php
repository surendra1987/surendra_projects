...
class pdf extends Fpdi {
    ...
    protected function create_page_from_source($pageno) {
        ...
        $this->AddPage($orientation, array($size['width'], $size['height']));
    ...
    public static function ensure_pdf_compatible(\stored_file $file) {
        ...
        try {
            ...
            if (CLI_SCRIPT) {
                mtrace('PDF "' . $file->get_filename() . '" file uploaded by ' . $file->get_author() .
                    ' user is not valid, trying it through the ghostscript to clean it up.');
            }
            $pagecount = 0;
        }
