...
echo $OUTPUT->header();
echo $OUTPUT->page_main_heading(format_string($assign->name, true, array('context' => $context)));
...
if ($groupmode) {
    ...
    $params['assignid'] = $assign->id;
    if ($accessallgroups) {
        $sql = 'SELECT o.*, ' . get_all_user_name_fields(true, 'u') . '                  FROM {assign_overrides} o                  JOIN {user} u ON o.userid = u.id                 WHERE o.assignid = :assignid              ORDER BY ' . $sort;
        $overrides = $DB->get_records_sql($sql, $params);
    } else if ($groups) {
        list($insql, $inparams) = $DB->get_in_or_equal(array_keys($groups), SQL_PARAMS_NAMED);
        $params += $inparams;
