...
class feedback_item_multichoicerated extends feedback_item_base {
    ...
    public function build_editform($item, $feedback, $cm) {
        ...
        $feedback_structure = new mod_feedback_structure($feedback, $cm);
        $groupid = groups_get_activity_group($cm, true);
        $completed_count = $feedback_structure->count_completed_responses($groupid);
        //build the form
        $customdata = array(
            'item' => $item,
            'common' => $commonparams,
    ...
    public function get_printval($item, $value) {
        ...
        foreach ($presentation as $pres) {
            if ($value->value == $index) {
                ...
                $printval = format_string($item_label[1], false);
                break;
            }
            $index++;
        }
    ...
    protected function get_options($item) {
        ...
        foreach ($lines as $idx => $line) {
            ...
            $options[$idx + 1] = "<span class=\"weight\">($weight) </span>" . format_string($optiontext, false);
        }
