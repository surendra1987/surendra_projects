 ...
 */
final class form_report_actions extends template {

    /**
     *
     * @return form_report_actions
     */
    public static function create(int $id, reportbuilder $report, \stdClass $extrafields): form_report_actions {
        global $OUTPUT;
        ...
        $data = [
            ...
            'hasusage' => $hasusage,
            'is_active' => $is_active,
            'confirm_message' => get_string('delete_form_warning', 'mod_approval', format_string($extrafields->title)),
            'flex_icon_delete_disabled' => $flex_icon_delete_disabled,
            'sesskey' => sesskey()
        ];

        $widget = new form_report_actions($data);
        ...
        $refreshurl = new moodle_url(refresh::URL, ['id' => $id]);
        if (!$report->embedded) {
            $refreshurl->param('rid', $report->get_id());
        }
        $widget->set_refresh_url($refreshurl);
    ...
    /**
     * @param moodle_url|null $refresh_url
     * @return void
     */
    public function set_refresh_url(?moodle_url $refresh_url): void {
        if (null === $refresh_url) {
            $this->data['refreshurl'] = null;
            return;
        }
        $this->data['refreshurl'] = $refresh_url->out(false);
    }
