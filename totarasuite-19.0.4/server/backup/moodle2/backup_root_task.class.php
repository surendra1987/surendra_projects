...
// Protected API starts here
    ...
    protected function define_settings() {
        ...
        if (
            !(isset($CFG->mod_hvp_backup_libraries) && $CFG->mod_hvp_backup_libraries === '0')
            && $this->plan->get_type() == backup::TYPE_1COURSE
            && $this->plan->get_mode() == backup::MODE_GENERAL
        ) {
            $course_id = $this->plan->get_courseid();
            $course_modules = get_course_mods($course_id);
            $hvp_module_found = false;
            foreach ($course_modules as $course_module) {
                if (isset($course_module->modname) && $course_module->modname == 'hvp') {
                    $hvp_module_found = true;
                    break;
                }
            }
            if ($hvp_module_found) {
                $hvp_include_libraries = new backup_generic_setting('hvp_libraries', base_setting::IS_BOOLEAN, false);
                $hvp_include_libraries->set_ui(
                    new backup_setting_ui_checkbox(
                        $hvp_include_libraries,
                        get_string('rootsettinghvplibraries', 'backup'),
                    )
                );
                $this->add_setting($hvp_include_libraries);
                $activities->add_dependency($hvp_include_libraries);
            }
        }
