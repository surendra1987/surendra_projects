        ...
        if ($info->is_tracked_user($USER->id)) {
            ...
            foreach ($completions as $completion) {
                ...
                if ($criteria_title) {
                    ...
                    $desc = $criteria_title;
                    if ($completion->get_status() !== get_string('yes') && $completion->get_status() !== get_string('no')) {
                        $desc .= html_writer::empty_tag('br');
                        $desc .= $completion->get_status();
                    }

                    // Condition met?
                    $ismet = $completion->is_complete() ? strtoupper(get_string('yes')) : strtoupper(get_string('no'));

                    $row = new html_table_row();
                    $row->cells[0] = new html_table_cell($desc);
                    $row->cells[1] = new html_table_cell($ismet);
            ...
            if (!empty($activities)) {
                ...
                // Title, description and is met.
                $ismet = strtoupper(get_string('no'));
                ...
                if ($aggregation->method == COMPLETION_AGGREGATION_ALL) {
                    $desc = get_string('completeallactivities', 'block_completionstatus', $a);
                    if ($a->first == $a->second) {
                        $ismet = strtoupper(get_string('yes'));
                    }
                } else {
                    $desc = get_string('completeanyactivities', 'block_completionstatus', $a);
                    if (!empty($a->first)) {
                        $ismet = strtoupper(get_string('yes'));
                    }
                }
            ...
            $row = new html_table_row();
            $row->cells[0] = new html_table_cell(html_writer::tag('b', get_string('criteria', 'block_completionstatus')));
            $row->cells[1] = new html_table_cell(html_writer::tag('b', get_string('conditionmet', 'block_completionstatus')));
            $row->cells[1]->style = 'text-align: right;';
            $rows[] = $row;
