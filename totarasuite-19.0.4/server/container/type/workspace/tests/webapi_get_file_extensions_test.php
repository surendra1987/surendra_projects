...
class container_workspace_webapi_get_file_extensions_test extends \core_phpunit\testcase {
    use webapi_phpunit_helper;

    /**
     * @return void
     */
    public function test_get_file_extensions_for_public_workspaces(): void {
        $workspace = $this->create_workspace_with_files();

        // Verify the owner can access the files
        $this->assert_can_see_file_extensions($workspace);

        // Verify a member can access files
        $member = $this->getDataGenerator()->create_user();
        member::added_to_workspace($workspace, $member->id, false, $workspace->owners()->first()?->id);
        $this->setUser($member);
        $this->assert_can_see_file_extensions($workspace);
     ...
     * @return void
     */
    public function test_get_file_extensions_for_private_workspaces(): void {
        ...
        $non_member = $this->getDataGenerator()->create_user();
        $this->setUser($non_member);
        $this->assert_cannot_see_file_extensions($workspace);
    }

    /**
     * @return void
     */
    public function test_get_file_extensions_for_hidden_workspaces(): void {
     ...
     */
    private function assert_can_see_file_extensions(workspace $workspace): void {
        ...
        $this->assertArrayHasKey('png', $results);
    }
     ...
     * @param workspace $workspace
     */
    private function assert_cannot_see_file_extensions(workspace $workspace): void {
        $exception = null;
        $results = null;
        try {
            $results = $this->resolve_graphql_query(
