 ...
 * vendor/bin/phpunit --verbose totara_certification_certification_test totara/certification/tests/certification_test.php
 */
class totara_certification_certification_test extends reportcache_advanced_testcase {
            ...
            for ($i = 1; $i <= CERTIFICATION_USERS; $i++) {
                if ($i % 2) { // Half of the users (with odd $i) have an assignment due date.
                    ...
                    $program_generator->assign_to_program($certprogram->id,
                        assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[$i]->id, $record, true);
                    $this->userswithassignmentduedate[] = $this->users[$i];
                } else {
                    $program_generator->assign_to_program($certprogram->id,
                        assignments::ASSIGNTYPE_INDIVIDUAL, $this->users[$i]->id, null, true);
                }
            }
        }
    ...
    private function actions_stage_3() {
        ...
        $certcron->execute();
        $assignmentscron = new user_assignments_task();
    ...
    private function check_stage_1() {
        global $DB;
        ...
        foreach ($this->certprograms as $certprogram) {
            for ($i = 1; $i <= CERTIFICATION_USERS; $i++) {
                ...
                $this->assertEquals(assignments::ASSIGNTYPE_INDIVIDUAL, $progassignment->assignmenttype);
                ...
                $this->assertEquals(program::PROGRAM_EXCEPTION_NONE, $progassignment->exceptionstatus); // No exceptions.
                ...
                $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status); // Status is incomplete.
                // When it was started.
    ...
    private function check_stage_2() {
        global $DB;
        ...
        foreach ($this->certprograms as $certprogram) {
            for ($i = 1; $i <= CERTIFICATION_USERS; $i++) {
                ...
                $progcompletion = reset($progcompletions);
                if ($didfirstcompletion) {
                    $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion->status); // Status complete!
    ...
    private function check_stage_3() {
        global $DB;
        ...
        foreach ($this->certprograms as $certprogram) {
            for ($i = 1; $i <= CERTIFICATION_USERS; $i++) {
                ...
                $this->assertLessThan($this->setuptimemaximum, $progcompletion->timecreated);
                if ($didfirstcompletion) {
                    $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status); // Set during window opening.
                    $this->assertEquals(userdate($timeexpires), userdate($progcompletion->timedue)); // Set when certified.
                    $this->assertGreaterThanOrEqual($this->firstcompletiontimeminimum, $progcompletion->timestarted);
                    $this->assertLessThanOrEqual($this->firstcompletiontimemaximum, $progcompletion->timestarted);
                } else {
    ...
    private function check_stage_4() {
        global $DB;
        ...
        foreach ($this->certprograms as $certprogram) {
            for ($i = 1; $i <= CERTIFICATION_USERS; $i++) {
                ...
                $this->assertLessThan($this->setuptimemaximum, $progcompletion->timecreated);
                if ($didsecondcompletion) {
                    $this->assertEquals(program::STATUS_PROGRAM_COMPLETE, $progcompletion->status); // Set when completed.
                    $this->assertEquals(userdate($timeexpires), userdate($progcompletion->timedue)); // Set when certified.
    ...
    private function check_stage_5() {
        global $DB;
    ...
    private function check_stage_6() {
        global $DB;
        ...
        foreach ($this->certprograms as $certprogram) {
            for ($i = 1; $i <= CERTIFICATION_USERS; $i++) {
                ...
                if ($didthirdcompletion) {
                    ...
                    $this->assertEquals(program::STATUS_PROGRAM_INCOMPLETE, $progcompletion->status); // Set during setup.
                    if ($hasassignmentduedate) {
                        $this->assertEquals(userdate($this->assignmentduedate), userdate($progcompletion->timedue)); // Has timedue.
                    } else {
                        $this->assertEquals(-1, $progcompletion->timedue); // No timedue.
