...
class rb_source_cohort_associations extends rb_base_source {
    ...
    public function __construct() {
        $this->base = "(SELECT e.id, e.customint1 AS cohortid, e.courseid AS instanceid,
            ...
            FROM {enrol} e
            JOIN {course} c ON e.courseid = c.id AND c.containertype = 'container_course'
            ...
            WHERE pa.assignmenttype = " . assignments::ASSIGNTYPE_COHORT . " AND p.certifid IS NULL
            UNION ALL
            SELECT pa.id, pa.assignmenttypeid AS cohortid, p.id AS instanceid,
            ...
            FROM {prog_assignment} pa
            JOIN {prog} p ON pa.programid = p.id
            WHERE pa.assignmenttype = " . assignments::ASSIGNTYPE_COHORT . " AND p.certifid IS NOT NULL)";
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
            ...
            array(
                ...
                'extrafields' => array('cohortid' => 'base.cohortid', 'type' => 'base.instancetype', 'instanceid' => 'base.instanceid'),
                'nosort' => true
            )
        );
        $columnoptions[] = new rb_column_option(
    ...
    public function phpunit_column_test_add_data(totara_reportbuilder_column_test $testcase) {
        ...
        $course = generator::instance()->create_course(['name' => 'cohort course']);
        $cohort = generator::instance()->create_cohort(['name' => 'cohort']);

        /** @var enrol_cohort_plugin $enrol */
    ...
    public function limit_to_tenant(reportbuilder $report, bool $tenantsisolated): array {
        $context_alias = uniqid('ct_');
        $tenant_id = $report->tenantid;
        $joins[] = $this->get_join('cohort');
        $joins[] = new \rb_join(
            $context_alias,
            'INNER',
            '{context}',
            "cohort.contextid = $context_alias.id AND $context_alias.tenantid = $tenant_id",
