...
/**
 * Class dataformatter_sort_time_test
 *
 ...
 */
class totara_catalog_dataformatter_sort_time_test extends \totara_catalog\dataformatter\dataformatter_test_base {

    public function test_sort_time() {
        $context = context_system::instance();

        $df = new sort_time('datefieldname');
        $this->assertCount(1, $df->get_required_fields());
        $this->assertSame('datefieldname', $df->get_required_fields()['date']);

        $this->assertSame([formatter::TYPE_SORT_TIME], $df->get_suitable_types());
        $this->assertTrue($df->is_suitable_for_type(formatter::TYPE_SORT_TIME));

        $test_params = ['date' => '12345'];
