...
/**
 * @coversDefaultClass virtualmeeting_msteams\providers\auth
 */
class virtualmeeting_msteams_auth_test extends \core_phpunit\testcase {
    /** @var user_entity */
    private $user;
     ...
     * @return array
     */
    public static function data_profile_returns(): array {
        return [
            'all' => [
                [
                    'displayName' => 'Bob Bobby',
                    'mail' => 'bob.bobby@example.com',
     ...
     * @covers ::get_profile
     * @dataProvider data_profile_returns
     */
    public function test_get_profile_returns(array $response, array $expected): void {
        $this->create_user_auth();
        $client = new matching_mock_client();
        $client->add_response_callable(
            ...
            new response(json_encode($response), 200, []));
        $auth = new auth($client);
        $this->assertEquals($expected, $auth->get_profile($this->user, false));
    }

    /**
     * @return array
     */
    public static function data_invalid_get_requests(): array {
        return [
            'empty' => [[]],
    ...
    public function test_authorise_success(): void {
        ...
        $entity = virtual_meeting_auth_entity::repository()->find_by_plugin_and_user('msteams', $this->user->id, true);
    ...
    public function test_create_authoriser(): void {
        set_config('client_id', 'klient.eyedee', 'virtualmeeting_msteams');
        set_config('client_secret', 's33krit', 'virtualmeeting_msteams');
        $client = new matching_mock_client();
        ...
        $this->assertEquals('client_id=klient.eyedee&client_secret=s33krit&scope='.urlencode(constants::SCOPE_MEETING), $formdata->as_string());
    }
}
