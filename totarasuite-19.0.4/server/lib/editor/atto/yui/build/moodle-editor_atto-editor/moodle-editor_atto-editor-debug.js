...
Y.extend(Editor, Y.Base, {
    ...
    initializer: function() {
        ...
        // If the textarea is visible, set the editor height to it's height
        if (this.textarea.getDOMNode().offsetParent) {
          this.editor.setStyle(
            'height',
            this.textarea.getDOMNode().getBoundingClientRect().height + 'px'
          );
        } else {
          this.editor.setStyle(
            'height',
            20 * this.textarea.getAttribute('rows') + 8 + 'px'
          );
        }

        var toolbar = this._wrapper.getDOMNode().querySelector('.editor_atto_toolbar');
        var lastToolbarHeight = toolbar.getBoundingClientRect().height;

        var resizeTextArea = function() {
            var minheight = parseInt(this.editor.getStyle('min-height'));
            var newToolbarHeight = toolbar.getBoundingClientRect().height;
            if (lastToolbarHeight === newToolbarHeight) {
                return;
            }

            var height = parseInt(this.editor.getStyle('height'));
            height = height + lastToolbarHeight - newToolbarHeight;
            if (height > minheight) {
                this.editor.setStyle('height', height + 'px');
            } else {
                this.editor.setStyle('height', null);
            }
        ...
        if (typeof ResizeObserver === "function") {
            // For decent browsers
            var observer = new ResizeObserver(resizeTextArea);
            observer.observe(toolbar);
        } else if (typeof MutationObserver === "function") {
            // Less recent supported browsers
            var mo = new MutationObserver(function(mutationList) {
                mutationList.forEach(function(mutation) {
                    mutation.addedNodes.forEach(function(child) {
                        if (child.tagName === 'IMG' && !child.complete) {
                            // image dimensions aren't always added prior to load being complete
                            child.addEventListener('load', function() {
                                resizeTextArea();
                            });
                        }

                        // other manipulations could cause a change in height
                        resizeTextArea();
                    });
                });
            });
        ...
        M.util.js_complete('totara_form_element_editor_atto');
    },
...
EditorTextArea.prototype = {
    ...
    updateFromTextArea: function() {
        var html = this.textarea.get('value');
        // Clear it first.
        this.editor.setHTML('');

        // When Atto updates the HTML, the browser re-creates the node causing autoplay to be triggered
        // Using regex on the HTML prevents this
        html = html.replace(/<(audio|video) ([^>]*)autoplay([^>]*)>/g, '<$1 $2data-autoplay$3>');

        // Copy cleaned HTML to editable div.
        this.editor.append(this._cleanHTML(html));
...
EditorClean.prototype = {
    ...
    getCleanHTML: function() {
        ...
        // Why regex & not DOM methods? because even if the element is not attached to the DOM
        // the browser still autoplays it
        html = html.replace(/<(audio|video) ([^>]*)data-autoplay([^>]*)>/g, '<$1 $2autoplay$3>');
