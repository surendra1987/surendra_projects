...
class moodle_exception extends Exception {
    ...
    function __construct($errorcode, $module='', $link='', $a=NULL, $debuginfo=null) {
        ...
        $heading_string = '';
        $heading = $errorcode.'_heading'; // Append the heading to get lang string for dialog title.
        if (get_string_manager()->string_exists($heading, $module)) {
            $heading_string = get_string($heading, $module);
        }
 ...
 */
function default_error_handler($errno, $errstr, $errfile, $errline, $errcontext = null) {
    if ($errno == 4096) {
 ...
 *
 * @deprecated since Totara 17.0 - Please use \core\uuid::generate() instead.
...
function get_docs_url($path = null) {
    ...
    // Totara: $path is expected to be a string
    $path = $path ?? '';
    ...
    if (strpos($totaraversion, 'dev') || str_starts_with($totaraversion, 'evergreen')) {
        $totaraversion = 'latest';
    }
...
function is_major_upgrade_required() {
    ...
    $totaracoremajordbchanges = 2025012200;
...
function make_writable_directory($dir, $exceptiononerror = true) {
    ...
    if (!mkdir_recursive($dir, $CFG->directorypermissions)) {
        if ($exceptiononerror) {
 ...
 * Recursive mkdir.
 *
 * Unlike mkdir($dir, $perms, true), this is resistant to race conditions.
 *
 * @param string $dir
 * @param int $permissions
 * @return bool
 */
function mkdir_recursive($dir, $perms = 0777) {
    // Avoid triggering a PHP warning if directory already exists.
    if (file_exists($dir)) {
        return is_dir($dir);
    }

    // Try and create the directory. If it succeds, we're good.
    ...
    // Failure! Try and recurse. Stop when dirname() is no longer giving us something
    // new to try, or one of the parent directories cannot be created.
    $parent = dirname($dir);
    $bad_parent = !$dir || $parent === $dir || $dir === '.';
    if ($bad_parent || !mkdir_recursive($parent, $perms)) {
        return false;
    }

    // The parent directory has been created, now try and create the one we were asked to.
    if (@mkdir($dir, $perms)) {
        return true;
    }

    // Check again, if it still doesn't exist, we failed.
...
class renderer_base {
    ...
    protected function get_mustache() {
        ...
        if ($this->mustache === null) {
            ...
            $cache_handler = new \core\output\mustache_file_cache_handler($cachedir);
     ...
     * @param \totara_tui\output\component $component
     * @return string
     */
    public function render_tui_component(\totara_tui\output\component $component): string {
        return $this->render_component($component);
    }
     ...
     * also call $component->register($moodle_page_instance) before the header is sent
     * to ensure required bundles are sent with the page.
     ...
     */
    protected function render_component(\totara_tui\output\component $component) {
        // Delegate rendering of this to the totara_tui renderer.
        $output = $this->page->get_renderer('totara_tui');
