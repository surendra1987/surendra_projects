...
class core_useragent {
    ...
    /**
     * Checks the user agent is Safari based and that the WebKit version is equal to or greater than that specified.
     *
     * @param string|int $version A version to check for, returns true if its equal to or greater than that specified.
     * @return bool
     */
    ...
    /**
     * Checks the user agent is Safari based and that the browser (not WebKit, unlike check_safari_version) version is equal to or
     * greater than that specified.
     *
     * @param string|int $version A version to check for, returns true if its equal to or greater than that specified.
     * @return bool
     */
    public static function check_safari_browser_version($version = null) {
        // Desktop version of Apple Safari browser - no mobile or touch devices.
        $useragent = self::get_user_agent_string();
        if ($useragent === false) {
            return false;
    ...
    /**
     * Checks the user agent is built-in WebKit on iOS
     *
     * @return bool true if WebKit on iOS
     */
    public static function is_webkit_ios() {
        return self::check_webkit_ios_platform_version();
    }

    /**
     * Checks the user agent is the built-in WebKit on iOS, and the iOS version is greater than or equal than that specified.
     *
     * It is no longer possible to detect the WebKit version directly as Apple has frozen it in the UA string.
     *
     * @param string|int $version A version to check for, returns true if its equal to or greater than that specified.
     * @return bool
     */
    public static function check_webkit_ios_platform_version($version = null) {
        // Safari on iPhone, iPad and iPod touch.
        $useragent = self::get_user_agent_string();
        if ($useragent === false) {
            return false;
        ...
        if (!strpos($useragent, 'iPhone OS') && !strpos($useragent, 'iPad')) {
            return false;
        }
        if (empty($version)) {
            return true; // No version specified.
        }
        // iPhone, iPod, also iPad in mobile mode (iPad in desktop mode uses a desktop Safari UA string)
        if (preg_match("/(?:CPU iPhone|iPad;.* CPU) OS ([0-9_]+)/i", $useragent, $match)) {
            $match_version = str_replace('_', '.', $match[1]);
            if (version_compare($match_version, $version) >= 0) {
                return true;
            }
        }
        return false;
    ...
    public static function get_browser_version_classes() {
        ...
        if (self::is_ie()) {
            ...
            $classes[] = 'safari';
            if (self::is_safari_ios() || self::is_webkit_ios()) {
                $classes[] = 'ios';
            } else if (self::is_webkit_android()) {
    ...
    /**
     * Returns true if a mobile device is used
     * @return bool
     */
    public static function is_mobile(): bool {
        $instance = self::instance();
        $device_type = $instance->guess_device_type();

        return in_array($device_type, [self::DEVICETYPE_MOBILE, self::DEVICETYPE_TABLET]);
    }

    /**
     * Returns true if the mobile app is used
     * @return bool
     */
    public static function is_mobile_app(): bool {
        $instance = self::instance();
        return preg_match('/TotaraMobileApp/i', $instance->useragent);
    }
    ...
    /**
     * Check if the current browser is marked as a legacy browser.
     *
     * @deprecated since Totara 18.0
     * @return bool
     */
    public static function is_totara_legacy_browser(): bool {
        return false;
    }
}
