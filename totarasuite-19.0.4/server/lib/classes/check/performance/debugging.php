...
class debugging extends check {
    ...
    public function get_result(): result {
        ...
        if (!$CFG->debugdeveloper) {
            $status = result::OK;
            $summary = get_string('check_debugmsg_comment_nodeveloper', 'admin');
        } else {
            $status = result::WARNING;
            $summary = get_string('check_debugmsg_comment_developer', 'admin');
        }

        $details = get_string('check_debugmsg_details', 'admin');
