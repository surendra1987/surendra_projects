...
class cookiesecure extends check {
    ...
    public function get_name(): string {
        return get_string('check_cookiesecure_name', 'admin');
    }
    ...
    public function get_result(): result {
        global $CFG;
        $details = get_string('check_cookiesecure_details', 'admin');
        if (!is_https()) {
            $status = result::WARNING;
            $summary = get_string('check_cookiesecure_http', 'admin');
            return new result($status, $summary, $details);
        }

        if (!is_moodle_cookie_secure()) {
            $status = result::ERROR;
            $summary = get_string('check_cookiesecure_error', 'admin');
        } else {
            $status = result::OK;
            $summary = get_string('check_cookiesecure_ok', 'admin');
        }
        return new result($status, $summary, $details);
    }
}
