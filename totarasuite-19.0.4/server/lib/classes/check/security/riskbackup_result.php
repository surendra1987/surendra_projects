 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class riskbackup_result extends result {
    ...
    public function __construct() {
        ...
        // List of users that are able to back up personal info note:
        ...
        $this->summary = get_string('check_riskbackup_warning', 'admin', $a);
    }
    ...
    public function get_details(): string {
        ...
        if ($this->systemroles) {
            ...
            foreach ($this->systemroles as $role) {
                ...
                $links[] = \html_writer::tag('li', get_string('check_riskbackup_editrole', 'admin', $role));
            }
            $links = \html_writer::tag('ul', implode('', $links));
            $details .= get_string('check_riskbackup_details_systemroles', 'admin', $links);
        }
        ...
        if ($this->overriddenroles) {
            ...
            foreach ($this->overriddenroles as $role) {
                ...
                $links[] = \html_writer::tag('li', get_string('check_riskbackup_editoverride', 'admin', $role));
            }
            $links = \html_writer::tag('ul', implode('', $links));
            $details .= get_string('check_riskbackup_details_overriddenroles', 'admin', $links);
        }
        ...
        foreach ($rs as $user) {
            ...
            $users[] = \html_writer::tag('li', get_string('check_riskbackup_unassign', 'admin', $a));
        }
        $rs->close();
        if (!empty($users)) {
            $users = \html_writer::tag('ul', implode('', $users));
            $details .= get_string('check_riskbackup_details_users', 'admin', $users);
        }
