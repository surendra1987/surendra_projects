...
class client extends \oauth2_client {
    ...
    public function get_userinfo() {
        ...
        return $this->map_user_info_to_fields($userinfo);
    }
     ...
     * @return array
     */
    protected function map_user_info_to_fields(stdClass $userinfo): array {
        ...
        foreach ($map as $openidproperty => $moodleproperty) {
            ...
            $getfunc = function($obj, $prop) use (&$getfunc) {
                $proplist = [];
                // Allow '-' character to be escaped with a backslash
                if (preg_match('/^(?<prop1>.*?[^\\\])(-|$)(?<prop2>.*)$/', $prop, $matches)) {
                    $matches['prop1'] = str_replace('\\-', '-', $matches['prop1']);
                    $proplist[] = $matches['prop1'];
                    if (!empty($matches['prop2'])) {
                        $proplist[] = $matches['prop2'];
                    }
                }
