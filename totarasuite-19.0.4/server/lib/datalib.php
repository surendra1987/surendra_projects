 ...
 *
 * @deprecated since Totara 18.0, 17.3, 16.9, 15.15
 * @global object
 * @uses SITEID
...
function search_users($courseid, $groupid, $searchtext, $sort='', array $exceptions=null) {
    ...
    // There is no alternative/replacement funntion to convert to.
    debugging('The function search_users has been deprecated and will be removed in a future version', DEBUG_DEVELOPER);
    ...
    // is supposed to see.
    $includedfields = ['id', 'username', 'email', 'firstname', 'lastname', 'city', 'country', 'lastaccess', 'confirmed', 'suspended'];
    $extrafields = get_extra_user_fields_sql(true, '', '', $includedfields);
    ...
    if ($sort) {
        ...
        $order = get_safe_orderby($neworderbymap, $sort);
    }
...
function get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
    ...
    // This function has been deprecated.
    // There is no alternative/replacement funntion to convert to.
    debugging('The function get_users_listing has been deprecated and will be removed in a future version', DEBUG_DEVELOPER);
    ...
    return $DB->get_records_sql("SELECT id, username, email, city, country, lastaccess, confirmed, deleted, suspended $extrafields
                                   FROM {user}
                                  WHERE $select
 ...
 * @param bool $completion_tracked - An optional condition to limit results to courses with completion enabled
 * @return object {@link $COURSE} records
 */
function get_courses_search($searchterms, $sort, $page, $recordsperpage, &$totalcount,
                            $requiredcapabilities = array(), $completion_tracked = null) {
    global $CFG, $DB, $USER;
    ...
    $completion_cond = '';
    if (isset($completion_tracked)) {
        $completion_cond = ' AND c.enablecompletion = :encomp';
        $params['encomp'] = $completion_tracked ? COMPLETION_ENABLED : COMPLETION_DISABLED;
    }
 ...
 * @return void
 */
function fix_course_sortorder(): void{
    global $DB;
