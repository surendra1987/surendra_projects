...
class lang_installer {
    ...
    /** @var string the language pack version to use */
    protected $version;
    ...
    public function __construct($langcode = '') {
        ...
        $this->version = $this->get_language_version();

        $langotherroot = $CFG->langotherroot ?? '';
        $langotherroot = rtrim($langotherroot, '/');
        if (!empty($langotherroot)) {
            $lang_path = $CFG->dataroot . '/lang';
            $lang_path_exists = false;

            if ($langotherroot !== $lang_path) {
                if (file_exists($lang_path) && is_link($lang_path)) {
                    $target = readlink($lang_path);
                    $target = rtrim($target, '/');
                    if ($target == $langotherroot) {
                        $lang_path_exists = true;
                    }
                }
            } else {
                $lang_path_exists = true;
    ...
    /**
     * Returns the language version based on the maturity level and major version number.
     *
     * @global object $CFG The global configuration object.
     * @return string The language version, e.g. ‘T18’.
     */
    private function get_language_version(): string {
        global $CFG;
