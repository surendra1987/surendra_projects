...
function check_totara_environment($version = null) {
    ...
    if (!$version) {
        ...
        // Normalise versions like 19.0-alpha1 to 19, or 18.7 to 18
        if (!str_contains($version, 'evergreen')) {
            $version = (int)$version;
        }
    }
...
function normalize_version($version) {
    ...
    if (str_starts_with($version, 'evergreen')) {
        return 'evergreen';
    }
...
/// Now search the version we are using
    ...
    $current_version = normalize_version($release);
    if (strpos($release, 'evergreen') === false && strpos($release, 'dev') !== false or strpos($release, 'rc') !== false) {
        // When final version is required, dev and rc is NOT enough!
        // When using the older versioning approach, deduct by 0.001 to ensure linear path is maintained
        if (substr_count($current_version, '.') <= 1) {
            $current_version = $current_version - 0.001;
        }
    }

    /// And finally compare them, saving results
    if ($current_version === 'evergreen' || version_compare($current_version, $needed_version, '>=')) {
        $result->setStatus(true);
    } else {
        $result->setStatus(false);
    }
