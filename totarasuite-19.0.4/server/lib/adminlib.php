...
use core\entity\encrypted_config;
use core\entity\encrypted_config_plugin;
use core\model\encrypted_config as encrypted_config_model;
use core\model\encrypted_config_plugin as encrypted_config_plugin_model;
...
abstract class admin_setting {
    ...
    /** @var bool Whether this setting is required - Will display an asterisk on the field's label */
...
class admin_setting_encryptedconfig extends admin_setting_configpasswordunmask {
    ...
    public function config_write($name, $value) {
        ...
        // store change
        empty($this->plugin)
            ? $this->save_config($name, $value)
            : $this->save_config_plugin($name, $value);
    ...
    private function save_config(string $name, $value): bool {
        $entity = encrypted_config::repository()
            ->where('name', $name)
            ->get()->first();

        // Do nothing when value is null and entity does not exist.
        if ($value === null && empty($entity)) {
            return true;
        }

        if (empty($entity)) {
            $entity = new encrypted_config([
                'name' => $name,
                'value' => '',
            ]);
            $entity->save();
        }
        $model = encrypted_config_model::load_by_entity($entity);

        // delete if value is null.
        $value === null
            ? $entity->delete()
            : $model->set_encrypted_attribute('value', $value);

        cache_helper::invalidate_by_definition('core', 'config', array(), 'core');
        return true;
    }

    private function save_config_plugin(string $name, $value): bool {
        $entity = encrypted_config_plugin::repository()
            ->where('name', $name)
        ...
        if (empty($entity)) {
            $entity = new encrypted_config_plugin([
                'name' => $name,
                'plugin' => $this->plugin,
                'value' => '',
            ]);
        ...
        $model = encrypted_config_plugin_model::load_by_entity($entity);
        ...
        cache_helper::invalidate_by_definition('core', 'config', array(), $this->plugin);
        return true;
    }
    ...
    private function get_config_plugin(string $name) {
        $entity = encrypted_config_plugin::repository()
            ->where('name', $name)
            ->where('plugin', $this->plugin)
            ->get()
...
/**
 * TOTARA:
 * Special class for autossoserver
 */
class admin_setting_loginpageredirect extends admin_setting_configselect {
    /**
     * Calls parent::__construct with specific arguments
     */
    public function __construct() {
        parent::__construct('loginpageredirect', get_string('loginpageredirect', 'auth'), get_string('loginpageredirect_help', 'auth'), 'fasdff', null);
    }
    ...
    public function load_choices(): bool {
        ...
        foreach ($authsenabled as $authname) {
            $authplugin = get_auth_plugin($authname);
            foreach ($authplugin->loginpage_idp_list(true) as $provider) {
                $key = implode(':::', [$authname, $provider['name']]);
                $this->choices[$key] = $provider['name'];
            }
        }
        return true;
    }
...
class admin_setting_manageauths extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        // Show mfa compatible notification on auth plugins list
        if (\core_mfa\mfa::has_incompatible_auth_plugins()) {
        ...
        // Check if manual plugin is supporting mfa
        $auth_plugin = get_auth_plugin('manual');
        if (!$auth_plugin::supports_mfa()) {
            $displayname = $displayname .' '.$OUTPUT->flex_icon('warning', array('alt' => strip_tags(get_string('error:auth_plugin_compatibility', 'core_mfa'))));
        }
        ...
        // Check if nologin plugin is supporting mfa
        $auth_plugin = get_auth_plugin('nologin');
        if (!$auth_plugin::supports_mfa()) {
            $displayname = $displayname .' '.$OUTPUT->flex_icon('warning', array('alt' => strip_tags(get_string('error:auth_plugin_compatibility', 'core_mfa'))));
        }
        ...
        foreach ($displayauths as $auth => $name) {
            ...
            if (in_array($auth, $authsenabled)) {
                ...
                // Trigger a hook to check if any other components want to prevent enabling of this auth plugin.
                $hook = new core_auth\hook\auth_enable($auth, $authsenabled);
                $hook->execute();

                if ($hook->prevent_enable === true) {
                    $a = ['auth' => $hook->auth, 'reason' => $hook->prevent_enable_reason];
                    $reason = get_string('error:cannot_enable_prohibited_auth_plugin', 'auth', $a);
                    $hideshow = $OUTPUT->flex_icon('show', array('classes' => 'ft-state-disabled', 'alt' => $reason, 'title' => $reason));
                } else {
            ...
            // Check if auth plugin is supporting mfa
            ...
            // settings link
            $settings = '';
            $settings_url = core_plugin_manager::instance()->get_plugin_info('auth_' . $auth)->get_settings_url();
            if ($settings_url) {
                $settings = html_writer::link($settings_url, $txt->settings);
            }
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($formats as $format) {
            ...
            if ($format->is_enabled()) {
                ...
                $hook = new core_course\hook\format\format_enable($format->name);
                $hook->execute();
                ...
                if ($hook->prevent_enable === true) {
                    ...
                    $reason = get_string('error:cannot_enable_prohibited_course_format_plugin', 'admin', $a);
                    $hideshow = $OUTPUT->flex_icon('show', array('classes' => 'ft-state-disabled', 'alt' => $reason, 'title' => $reason));
                } else {
...
function admin_externalpage_setup($section, $extrabutton = '', array $extraurlparams = null, $actualurl = '', array $options = array()) {
    ...
    if ($PAGE->user_allowed_editing()) {
        ...
        $PAGE->set_button($OUTPUT->single_button($url, $caption, 'post'));
    }
 ...
 *
 * @param boolean $reload will reuse previously-created tree if available, unless true
 * @param boolean $requirefulltree true means all settings required, false only pages required
 * @param boolean $reset_only if true, the static variable representing the tree will be nulled,
 *                              with no other action. Reload and requirefulltree are ignored.
 * @return admin_root|null object
 */
function admin_get_root($reload = false, $requirefulltree = true, $reset_only = false) {
    global $CFG, $DB, $OUTPUT;

    /** @var admin_root $ADMIN */
    static $ADMIN = NULL;

    // This is required to ensure that the static variable used by this function can be reset
    // at the end of every unit test.
    if ($reset_only) {
        $ADMIN = null;
        return null;
    }
...
/**
 * Checks if an admin page has required fields
 *
 * @param $node - Instance of admin_category or admin_settingpage
 * @return bool - True if page contains required fields, false if not
 */
function admin_any_settings_required($node) {
    if ($node instanceof admin_category) {
        foreach ($node->children as $child) {
            if (admin_any_settings_required($child) !== true) {
                continue;
            }
...
/**
 * Given a table and optionally a column name should replacement be done?
 *
...
function db_should_replace($table, $column = ''): bool {
    ...
    // Prevent replacements in some important tables. Any table with 'log' in the name is blacklisted later.
