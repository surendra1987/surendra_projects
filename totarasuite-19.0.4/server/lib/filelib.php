...
function file_rewrite_pluginfile_urls($text, $file, $contextid, $component, $filearea, $itemid, array $options=null) {
    ...
    if (!empty($options['reverse'])) {
        // Totara: Don't pass a null to a string function
...
/**
 * List of file mimetypes that should always be downloaded when served
 * via the {@see totara_tweak_file_sending()} function.
 *
 * @return array
 */
function totara_tweak_always_download_files(): array {
    global $CFG;
    ...
    if (empty($CFG->allow_inline_uploaded_html)) {
        $always_download[] = 'text/html';
    }
...
/**
 * Decide if/what content security policies (or headers in general) should apply
 * when serving files. {@see totara_tweak_file_sending()}.
 *
 * Returns an array to be fed into the PHP header() function.
 *
 * @param array $options The $options passed to totara_tweak_file_sending
 * @param string $mimetype
 * @return array|string[]
 */
function totara_tweak_content_security_policy(array $options, string $mimetype): array {
    global $CFG;

    // If the file is HTML and we allow inline HTML, don't apply the policy
    if ($mimetype === 'text/html' && !empty($CFG->allow_inline_uploaded_html)) {
        return [];
    }

    // If we allow XSS, don't apply the policies either
    if (!empty($options['allowxss'])) {
        return [];
    }

    // Otherwise return the security policies to apply
    return ['Content-Security-Policy: script-src none;'];
}
...
function totara_tweak_file_sending(&$mimetype, &$forcedownload, array &$options, int &$lifetime) {
    ...
    // Very dangerous file types -- can result in XSS, so we always force them to be
    // downloaded instead of rendered, even with consistent cleaning off.
    $always_download = totara_tweak_always_download_files();
    if (in_array($mimetype, $always_download) && empty($options['allowxss'])) {
        $forcedownload = true;
    }

    // Apply any headers needed for content security policies
    $content_security_policies = totara_tweak_content_security_policy($options, $mimetype);
    foreach ($content_security_policies as $content_security_policy) {
        header($content_security_policy);
    }

    $hook = new \core\hook\send_file_options((string)$mimetype, (bool)$forcedownload, $options, $lifetime);

    if (!$forcedownload) {
        if (!empty($CFG->disableconsistentcleaning) or !empty($options['allowxss'])) {
            $hook->execute();
            [$mimetype, $forcedownload, $options, $lifetime] = $hook->get_all();
    ...
    if (empty($CFG->disableconsistentcleaning) and empty($options['allowxss'])) {
        ...
        if (empty($CFG->pdf_is_not_safe)) { 
            $hopefullysafetypes[] = 'application/pdf';
        }
    ...
    $hook->set_all($mimetype, $forcedownload, $options, $lifetime);
    $hook->execute();
    [$mimetype, $forcedownload, $options, $lifetime] = $hook->get_all();
    return $mimetype; // BC only
}
...
function mimeinfo($element, $filename) {
    ...
    // Totara: Filename is expected to be a string
    $filename = $filename ?? '';
...
function get_mimetype_description($obj, $capitalise=false) {
    ...
    // Totara: Make sure $mimetype is a string (don't pass a null to a string function)
...
function make_content_disposition($dispositiontype, $filename = null) {
    ...
    $filename_header = 'filename*=utf-8\'\''.$filename_enc;

    // Check for device type and if it is mobile then the disposition is set to attachment.
    // Necessary for android devices since it won't open in browser
    if (core_useragent::is_mobile()) {
        $filename_header = 'filename="'.$filename.'"';
    }

    return 'Content-Disposition: '.$dispositiontype.'; '.$filename_header;
}
 ...
 *  (string|null) theme - This option will go with the preview. In order to let the preview load the sizes overrided
 *      by theme if there are any.
 *
 * @category files
...
function send_stored_file($stored_file, $lifetime=null, $filter=0, $forcedownload=false, array $options=array()) {
    ...
    if (!empty($options['preview']) && $stored_file->get_mimetype() !== 'image/gif') {
        ...
        $helper = \core\image\preview_helper::instance($theme);
        $preview_file = $helper->get_file_preview($stored_file, $options['preview']);
        ...
        if ($preview_file === null) {
            ...
            send_file($CFG->dirroot.'/pix/'.$fileicon.'.png', basename($fileicon).'.png', null, 0, false, $forcedownload, '', $dontdie);
        } else {
            // preview images have fixed cache lifetime
...
class curl {
    ...
    protected function multi($requests, $options = array()) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            foreach($requests[$i] as $n=>$v) {
                if ($n === 'url') {
                    // TOTARA: Skip the URL opt, we don't allow it. The URL has already been extracted.
                    continue;
                }
                $options[$n] = $v;
            }
    ...
    /**
     * For use only in unit tests - reset any hanging mock curl responses.
     * This is useful for cleanup between tests.
     * @return void
     */
    public static function reset_mock_responses(): void {
        if ((defined('PHPUNIT_TEST') && PHPUNIT_TEST)) {
            self::$mockresponses = [];
        } else {
            throw new coding_exception('reset_mock_responses function is only available for unit tests.');
        }
    }
    ...
    protected function request($url, $options = array()) {
        ...
        if ($this->options['CURLOPT_FOLLOWLOCATION'] and $this->info['http_code'] != 200) {
            $redirects = 0;
 ...
 * @param null|string $theme   Theme to override the preview mode's size. Default will be used.
...
function file_pluginfile($relativepath, $forcedownload, $preview = null, $theme = null) {
    ...
    if ($component === 'blog') {
        ...
        send_stored_file($file, 10*60, 0, true, array('preview' => $preview, 'theme' => $theme)); // download MUST be forced - security!
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'description' and $context->contextlevel == CONTEXT_SYSTEM) {
            ...
            \core\session\manager::write_close(); // Unlock session during file serving.
            send_stored_file($file, 60*60, 0, true, array('preview' => $preview, 'theme' => $theme));
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'event_description'  and $context->contextlevel == CONTEXT_SYSTEM) {
            ...
            \core\session\manager::write_close(); // Unlock session during file serving.
            send_stored_file($file, 0, 0, true, array('preview' => $preview, 'theme' => $theme));
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'icon' and $context->contextlevel == CONTEXT_USER) {
            ...
            $options = array('preview' => $preview, 'theme' => $theme);
            ...
            \core\session\manager::write_close(); // Unlock session during file serving.
            send_stored_file($file, 0, 0, true, array('preview' => $preview, 'theme' => $theme)); // must force download - security!
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'description') {
            ...
            \core\session\manager::write_close(); // Unlock session during file serving.
            send_stored_file($file, 60*60, 0, false, array('preview' => $preview, 'theme' => $theme));
    ...
    // ========================================================================================================================
        ...
        if (function_exists($filefunction)) {
            // if the function exists, it must send the file and terminate. Whatever it returns leads to "not found"
            $filefunction($course, $birecord, $context, $filearea, $args, $forcedownload, array('preview' => $preview, 'theme' => $theme));
        }
