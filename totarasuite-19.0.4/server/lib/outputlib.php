...
class theme_config {
    ...
    public function css_urls(moodle_page $page) {
        ...
        // Totara: Legacy CSS
        $legacy = core_useragent::is_totara_legacy_browser();
    ...
    public function post_process($css) {
        ...
        if (!empty($csspostprocess) && function_exists($csspostprocess)) {
    ...
    private function get_overridden_image_url(string $component, string $image_name): ?moodle_url {
        ...
        if (!empty($theme_file)) {
            ...
            $tenantid = 0;
            if (isloggedin()) {
                $tenantid = !empty($USER->tenantid) ? $USER->tenantid : 0;
            } else if (isset($SESSION->themetenantid) && is_number($SESSION->themetenantid)) {
                $tenantid = (int)$SESSION->themetenantid;
            }

            $theme_file->set_tenant_id($tenantid);
    ...
    /**
     * Discover whether the current theme is, or inherits from, a particular named theme.
     *
     * @param string $name
     * @return bool
     */
    public function is_or_inherits_from(string $name): bool {
        if ($this->name === $name) {
            return true;
        }
        return in_array($name, $this->parents);
    }
}
