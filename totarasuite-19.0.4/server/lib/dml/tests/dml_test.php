...
class core_dml_dml_test extends \core_phpunit\database_driver_testcase {
    ...
    public function test_fix_sql_params() {
        ...
        $sql = "SELECT * FROM {{$tablename}} WHERE name = :long_long_long_placeholder_with_more_than_sixty_characters_in_it";
        $params = array('long_long_long_placeholder_with_more_than_sixty_characters_in_it' => 'record1');
        $sqlarray = $DB->fix_sql_params($sql, $params);
        $this->assertIsArray($sqlarray);
        $this->assertCount(1, $sqlarray[1]);
    ...
    /**
     * In MySQL the converstion to float can create problems as it's locale dependent.
     * Let's make sure float work as expected
     */
    public function test_float_values_are_correctly_used_in_emulated_bound_params() {
        $DB = $this->tdb;
        $dbman = $this->tdb->get_manager();
        ...
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('boolcol', XMLDB_TYPE_INTEGER, '1');
        $table->add_field('intcol', XMLDB_TYPE_INTEGER, '10');
        $table->add_field('deccol', XMLDB_TYPE_NUMBER, '10,2');
        $table->add_field('floatcol', XMLDB_TYPE_FLOAT, '10,2');
        $table->add_field('floatcol2', XMLDB_TYPE_FLOAT, '12,6');
        $table->add_field('stringcol', XMLDB_TYPE_CHAR, '255');
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $dbman->create_table($table);
        ...
        $record->boolcol = true;
        $record->intcol = 123;
        $record->deccol = 123.45;
        $record->floatcol = 123.45;
        $record->floatcol2 = 12345.06789;
        $record->stringcol = '123';
        $DB->insert_record($tablename, $record);

        $sql = "SELECT * FROM {{$tablename}} WHERE boolcol = ?";
        $result = $DB->get_records_sql($sql, [1]);
        $this->assertNotEmpty($result, 'Conversion of boolean parameter unsuccessful');

        $sql = "SELECT * FROM {{$tablename}} WHERE intcol = ?";
        $result = $DB->get_records_sql($sql, [123]);
        $this->assertNotEmpty($result, 'Conversion of integer parameter unsuccessful');

        $sql = "SELECT * FROM {{$tablename}} WHERE deccol = ?";
        $result = $DB->get_records_sql($sql, [123.45]);
        $this->assertNotEmpty($result, 'Conversion of decimal parameter unsuccessful');

        $sql = "SELECT * FROM {{$tablename}} WHERE floatcol = ?";
        $result = $DB->get_records_sql($sql, [123.45]);
        $this->assertNotEmpty($result, 'Conversion of float parameter unsuccessful');

        $sql = "SELECT * FROM {{$tablename}} WHERE floatcol2 = ?";
        $result = $DB->get_records_sql($sql, [12345.06789]);
        $this->assertNotEmpty($result, 'Conversion of float parameter unsuccessful');

        $sql = "SELECT * FROM {{$tablename}} WHERE floatcol2 = ?";
        $result = $DB->get_records_sql($sql, [12345.067]);
        $this->assertEmpty($result, 'Conversion of float parameter unsuccessful');

        $sql = "SELECT * FROM {{$tablename}} WHERE stringcol = ?";
        $result = $DB->get_records_sql($sql, [123]);
        $this->assertNotEmpty($result, 'Conversion of string parameter unsuccessful');
    ...
    public function test_get_record_select() {
        ...
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('course', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('shortname', XMLDB_TYPE_CHAR, '255', null, null, null, '0');
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $dbman->create_table($table);

        $DB->insert_record($tablename, array('course' => 3));
        $DB->insert_record($tablename, array('course' => 2));
        $DB->insert_record($tablename, array('course' => 46, 'shortname' => 'not_really_shortname_for_testing_purpose_but_long_long_name'));
        ...
        $record = $DB->get_record_select(
            $tablename,
            "shortname = :long_long_long_placeholder_with_more_than_sixty_characters_in_it",
            ['long_long_long_placeholder_with_more_than_sixty_characters_in_it' => 'not_really_shortname_for_testing_purpose_but_long_long_name']);
        $this->assertInstanceOf('stdClass', $record);

        $this->assertEquals(46, $record->course);
        $this->assertEquals('not_really_shortname_for_testing_purpose_but_long_long_name', $record->shortname);
    ...
    public static function validate_db_name_for_mysql_data_provider(): array {
        return [
            ['test', true],
            ['', false],
            ...
            ['no-hyphen', false],
            ['no\backslash', false],
            ['no"doublequote', false],
            ['no\'singlequote', false],
            ['no`backtick', false],
            ['yes_underscore', true],
            ['yes_dollar_$ign', true],
            ['yes_CAPITALS', true],
            ['yes_basic_multilingual_plane_大あ', true],
        ];
    }

    /**
     * @dataProvider validate_db_name_for_mysql_data_provider
     * @param string $identifier
     * @param bool $valid
     * @return void
     */
    public function test_validate_db_name_for_mysql(string $identifier, bool $valid): void {
        global $CFG;
        require_once "{$CFG->dirroot}/lib/dml/mysqli_native_moodle_database.php";

        $db = new mysqli_native_moodle_database();
        $this->assertEquals($valid, $db->is_valid_db_name($identifier));
    }

    /**
     * Make sure that insert_records_via_batch closes the transaction it opens if there is an
     * expcetion encoutnered, and that the exception is still thrown.
     *
     * @return void
     */
    public function test_insert_records_via_batch_closes_transaction_on_exception(): void {
        global $DB;

        // Make sure there is no transaction running before we call the function.
        self::assertFalse($DB->is_transaction_started());

        try {
            $DB->insert_records_via_batch('user', [['invalid_field' => 123]]);
        } catch (Throwable $e) {
            // This exception is fired by the code which is within the transaction.
            self::assertEquals('The item provided by the iterator is not an object', $e->getMessage());
        }

        // Make sure there is no transaction running after we call the function.
        self::assertFalse($DB->is_transaction_started());
    }
}
