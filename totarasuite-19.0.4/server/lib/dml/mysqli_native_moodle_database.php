...
class mysqli_native_moodle_database extends moodle_database {
    ...
    protected function build_fts_subquery(string $table, array $searchfields, string $searchtext): array {
        ...
        foreach ($searchfields as $field => $weight) {
            ...
            // A wildcard character in the first 2 positions in a search string will not work well with MySQL binary search.
            if (stripos(substr($searchtext, 0, 2), '*') !== false) {
    ...
    protected function get_fts_mode(string $searchtext): int {
        ...
        // We want to exclude the dash character for MySQL binary mode searches.
        if (preg_match('/^[\p{L}0-9_]+\*$/u', $searchtext)) {
