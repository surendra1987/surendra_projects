...
class core_event_test extends \core_phpunit\testcase {
    ...
    public function test_entity_snapshots() {
        global $DB;

        $event = \core_tests\event\unittest_executed::create(array('context'=>\context_system::instance(), 'other'=>array('sample'=>1, 'xx'=>10)));
        $course1 = new \core\entity\course(1);
        $this->assertNotEmpty($course1);

        $event->add_entity_snapshot($course1);

        $result = $event->get_entity_snapshot(\core\entity\course::class, $course1->id);
        // Convert to arrays because record snapshot returns a clone of the object.
        $this->assertSame($course1->to_array(), $result->to_array());

        $user = $event->get_entity_snapshot(\core\entity\user::class, 1);
        $this->assertEquals(1, $user->id);
        $this->assertSame('guest', $user->username);
        ...
        $event2 = \core_tests\event\unittest_executed::restore($event->get_data(), array());
        try {
            $event2->get_entity_snapshot(\core\entity\course::class, $course1->id);
            $this->fail('Reading of snapshots from restored events is not ok');;
        } catch (\moodle_exception $e) {
            $this->assertInstanceOf('\coding_exception', $e);
            $this->assertEquals('Coding error detected, it must be fixed by a programmer: It is not possible to get snapshots from restored events', $e->getMessage());
        ...
        $this->expectExceptionMessage('core\orm\entity\model is not an instance of core\orm\entity\entity');

        $event->get_entity_snapshot(\core\orm\entity\model::class, 1);
    }
...
and nothing else.";
    ...
    public function test_set_event_data() {
        $this->setAdminUser();
        // success
        ...
        $data = $event->get_data();
        $data['other']['test_string'] = 'what is going on?';
        $status = $event->set_data($data);
        $this->assertTrue($status);
        $this->assertEquals($data, $event->get_data());
        // fail
        $event->trigger();
        $data['other']['test_second_string'] = 'This will be fail';
        $status = $event->set_data($data);
        $this->assertFalse($status);
        $this->assertNotEquals($data, $event->get_data());
    }
}
