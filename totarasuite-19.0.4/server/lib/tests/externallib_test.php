...
class core_externallib_test extends \core_phpunit\testcase {
    protected $DB;
...
', FORMAT_HTML);
    ...
    public static function all_external_info_provider() {
        global $DB;
    ...
    public function test_call_external_function_when_not_logged_in() {
        $category = $this->getDataGenerator()->create_category();
        $params = array(
            'contextid' => context_coursecat::instance($category->id)->id,
            'name' => 'aaagrrryyy',
        ...
        $params = array('cohortids' => array($cohort1->id, $cohort2->id));
        $result = external_api::call_external_function('core_cohort_get_cohorts', $params);
        $this->assertEquals('sessionerroruser', $result['exception']->errorcode);
    }
