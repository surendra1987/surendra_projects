 ...
 */
class core_component_test extends \core_phpunit\testcase {

    private $oldpsr0namespaces;
    private $oldpsr4namespaces;
    ...
    public function setUp(): void {
        $psr0namespaces = new ReflectionClass('core_component');
        $this->oldpsr0namespaces = $psr0namespaces->getStaticPropertyValue('psr0namespaces');

        $psr4namespaces = new ReflectionClass('core_component');
        $this->oldpsr4namespaces = $psr4namespaces->getStaticPropertyValue('psr4namespaces');
    }
    protected function tearDown(): void {
        $psr0namespaces = new ReflectionClass('core_component');
        $psr0namespaces->setStaticPropertyValue('psr0namespaces', $this->oldpsr0namespaces);

        $psr4namespaces = new ReflectionClass('core_component');
        $psr4namespaces->setStaticPropertyValue('psr4namespaces', $this->oldpsr4namespaces);

        $this->oldpsr0namespaces = null;
        $this->oldpsr4namespaces = null;
        parent::tearDown();
    }
    ...
    public function test_get_subplugins() {
        ...
        $this->assertFileDoesNotExist("$CFG->dirroot/mod/choice/db/subplugins.php");
    ...
    public static function component_classes_provider(): array {
        return [
            ['wellington', 'stupid', 'empty'],
            ['blackboard_core', 'stupid', 'empty'],
            ['totara_core', 'stupid', 'empty'],
            ['totara_core', 'stupid\monkey', 'empty'],
            ['totara_core', '\stupid\monkey', 'empty'],
            ['totara_core', 'stupid\monkey\\', 'empty'],
            ['totara_core', '\stupid\monkey\\', 'empty'],
            ['_', 'stupid', 'empty'],
            ['totara_reportbuilder', 'rb\display', 'display_classes'],
            ['core', 'log', 'log'],
            ['core', '', 'all'],
            [null, '', 'empty'],
        ];
    ...
    /**
     * Assertions from component_class_provider
     *
     * @param string $action
     * @param array|null $result
     * @return void
     */
    private function assert_expected_component_class(string $action, ?array $result): void {
        if ($action == 'empty') {
            $this->assertIsArray($result);
            $this->assertEquals([], $result);
        } else if ($action == 'display_classes') {
            $this->assertGreaterThan(26, $result);
            foreach ($result as $class) {
                $this->assertStringContainsString('totara_reportbuilder\rb\display\\', $class);
            }
            $this->assertContains('totara_reportbuilder\rb\display\base', $result);
        } else if ($action == 'log') {
            $this->assertCount(1, $result);
            foreach ($result as $class) {
                $this->assertSame('core\log\dummy_manager', $class);
            }
        } else if ($action == 'all') {
            $this->assertGreaterThan(0, $result);
            foreach ($result as $class) {
                $this->assertStringStartsWith('core\\', $class);
            }
        }
    }

    /**
     * @dataProvider component_classes_provider
     * @return void
     */
    public function test_get_component_classes_in_namespace(?string $component, ?string $namespace, string $action) {
        $deprecated_result = core_component::get_component_classes_in_namespace($component, $namespace);
        $this->assertDebuggingCalled();
        // Convert to array of classes
        $deprecated_result = array_keys($deprecated_result);
        $this->assert_expected_component_class($action, $deprecated_result);

        foreach ($deprecated_result as $key => $class) {
            $reflection = new ReflectionClass($class);
            // The deprecated function also returns abstract classes, the new one doesn't
            if ($reflection->isAbstract()) {
                unset($deprecated_result[$key]);
            }
        }

        // Now do the same with the replacement function
        $result = core_component::get_namespace_classes($namespace, null, $component);
        $this->assert_expected_component_class($action, $result);

        sort($deprecated_result);
        sort($result);
        $this->assertEquals($result, $deprecated_result);
     ...
     */
    public static function classloader_provider() {
        global $CFG;
     ...
     */
    public static function psr_classloader_provider() {
        global $CFG;
     ...
     */
    public static function get_class_file_provider() {
        global $CFG;
    ...
    /**
     * Tests that we can get namespaced classes grouped by frankenstyle name.
     */
    public function test_get_namespace_classes_grouped_by_component() {
        $target_component = 'mod_forum';
        $target_namespace = 'output\\email';

        $components = core_component::get_namespace_classes_grouped_by_component($target_namespace);

        $this->assertIsArray($components);
        $this->assertArrayHasKey($target_component, $components);
        $this->assertIsArray($components[$target_component]);
        $this->assertNotEmpty($components[$target_component]);

        foreach ($components[$target_component] as $class) {
            // Check the classes is of the expected pattern, we don't care what the name actually is.
            $this->assertStringStartsWith($target_component . '\\' . $target_namespace, $class);
        }

    }

    /**
     * Tests that we can get namespaced classes grouped by frankenstyle name while restricting by instanceof
     */
    public function test_get_namespace_classes_grouped_by_component_instanceof() {
        $target_component = 'totara_core';
        $target_namespace = 'output';

        $components = core_component::get_namespace_classes_grouped_by_component($target_namespace);
        $this->assertIsArray($components);
        // Check at least core exists also. It must exist.
        $this->assertArrayHasKey('core', $components);
        // Check that there are also more, although we won't know how many exactly.
        $this->assertGreaterThan(10, count($components));
        // Check that we have the target component, if not this test is destined to fail.
        $this->assertArrayHasKey($target_component, $components);
        $without_restriction = count($components[$target_component]);

        // Ensure that there are at least some classes without a restriction, otherwise this test is useless.
        $this->assertGreaterThan(0, $without_restriction);

        // Sorry if you get here because this class changed or is no longer present!
        $components = core_component::get_namespace_classes_grouped_by_component($target_namespace, \totara_core\output\select_region::class);
        $this->assertIsArray($components);
        $this->assertArrayHasKey($target_component, $components);
        $with_restriction = count($components[$target_component]);

        // Ensure that there are more classes without a restriction than with a restriction, otherwise this test is useless.
        $this->assertGreaterThan($with_restriction, $without_restriction);
    }
}