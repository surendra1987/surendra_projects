 ...
 */
class core_plugin_manager_test extends \core_phpunit\testcase {
    ...
    public function test_plugin_states() {
        ...
        // Deleted present.
        set_config('version', 2013091300, 'tool_premigration');
        ...
        foreach ($plugininfos as $type => $infos) {
            foreach ($infos as $name => $info) {
                ...
                } else if ($info->component === 'tool_premigration') {
    ...
    public function test_plugin_versioning() {
        global $CFG;

        // Get system version information.
        require($CFG->dirroot . '/version.php');
        if ($maturity != MATURITY_STABLE) {
            $this->markTestSkipped('Test only applies to stable branches.');
        }
        $major_version = substr($version, 0, 8);

        // Prepare to look through all standard plugins.
        $pluginman = core_plugin_manager::instance();
        $allplugins = $pluginman->get_plugins();
        $outliers = [];

        // Do not add any *new* entries without permission from a release manager.
        // It is necessary and ok to update existing entries when version changes.
        $expected = [];

        // Check each standard plugin's version against the system major version, they should be the same.
        foreach ($allplugins as $type => $plugins) {
            foreach ($plugins as $plugin) {
                /** @var \core\plugininfo\base $plugin */
                if ($plugin->source == core_plugin_manager::PLUGIN_SOURCE_STANDARD) {
                    if (substr($plugin->versiondisk, 0, 8) != $major_version) {
                        // Plugin major version is different from system major version.
                        $to_check = $plugin->type . '_' . $plugin->name . ' (' . $plugin->versiondisk . ')';
                        if (!in_array($to_check, $expected)) {
                            // Plugin is not in the whitelist of major version differences.
                            $outliers[] = $to_check;
                        }
                    }
                }
        ...
        $this->assertEqualsCanonicalizing([], $outliers, 'Plugins detected with major version bump in stable release, should be ' . $major_version . 'XX');
    }
}
