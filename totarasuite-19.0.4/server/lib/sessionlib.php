...
function set_moodle_cookie($username) {
    ...
    if ($username !== '') {
        ...
        $ciphertext = rc4encrypt($username);
        $mac = substr(hash_hmac('sha256', $ciphertext, get_site_identifier()), 0, 16);
        setcookie($cookiename, $mac . $ciphertext, time() + (DAYSECS * 60), $CFG->sessioncookiepath, $CFG->sessioncookiedomain, $cookiesecure, $CFG->cookiehttponly);
    }
}
...
function get_moodle_cookie() {
    ...
    if (empty($_COOKIE[$cookiename])) {
        ...
        $mac = substr($cookie, 0, 16);
        $ciphertext = substr($cookie, 16);
        if ($mac != substr(hash_hmac('sha256', $ciphertext, get_site_identifier()), 0, 16)) {
            // invalid MAC -- cookie tampered with or site identifier changed
            return '';
        }
        $username = rc4decrypt($ciphertext);
        return $username;
    }
}
 ...
 * @param bool     $force_reset_page always resets the page object,
*                  regardless whether course set on it has changed or not
 * @return void
 */
function cron_setup_user($user = null, $course = null, bool $force_reset_page = true) {
    ...
    $course = $course ?? $SITE;
    if ($force_reset_page || $PAGE->course != $course) {
