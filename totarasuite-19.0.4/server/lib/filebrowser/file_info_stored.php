...
class file_info_stored extends file_info {
    ...
    public function get_imageinfo() {
        if ($this->lf->is_content_available()) {
    ...
    public function count_non_empty_children($extensions = '*', $limit = 1) {
        ...
        // This method is repeatedly invoked and the number of invocations rises
        // linearly with the number of courses, categories and any other module
        // that uses the files table. Which is why the results have to be cached.
        $context_filepaths = self::get_cached_context_file_counts(
            $this->context->id
        );

        $reqd_filepath = self::make_context_file_counts_cache_filepath_key(
            $this->lf->get_component(),
            $this->lf->get_filearea(),
            $this->lf->get_itemid(),
            $this->lf->get_filepath()
        );

        $file_count = 0;
        foreach ($context_filepaths as $filepath => $ext_counts) {
            if (strpos($filepath, $reqd_filepath) !== 0) {
                continue;
            }
    ...
    /**
     * Creates a context/file count cache.
     *
     * The cache contains records from the mdl_file table and is organized in
     * this way:
     * [
     *      ctx1 => [
     *          comp1:area1:id1:/dir1/       => [.png => 1, .jpg => 4, ...],
     *          comp1:area1:id1:/dir1/dir2/  => [...],
     *          ...
     *          comp1:area1:id1:/dir3/       => [...],
     *          ...
     *          comp1:area1:id1:/dir3/dir4/ => [...],
     *          ...
     *          comp1:area1:id2:/dir5/ => [...],
     *          ...
     *          comp1:area2:id3:/dir6/ => [...],
     *          ...
     *          compX:areaY:idZ:/dirA/ => [...],
     *          ...
     *      ],
     *      ctx2 => [...],
     *      ...
     ...
     * ]
     *
     * It would be nice if the cache could get all the 'relevant' records from
     * the database at one go. However, it is not easy to define what 'relevant'
     * is since the this method cannot know which context, component, filearea,
     * itemid, filepath and file extension combination is going to be needed in
     * the immediate future. This is why the cache caches by context ids only;
     * hopefully that is what is 'relevant' for this request; it also restricts
     * the amount of memory needed for the cache - the mdl_files table could be
     * very large.
     *
     * It also does not help that PHP recreates the entire multiverse to service
     * every request. That means unless you use a proper, _persistent_ cache
     * definition, this cache is going to be thrown away and rebuilt many, many
     * times for the same set of unchanging mdl_file records.
     *
     * @param int $context_id context id to look up.
     *
     * @return array<string,array<string,int>> the sub mapping of file paths to
     *         extension counts for the given context id.
     */
    private static function get_cached_context_file_counts(int $context_id): array {
        global $DB;
        $cache = cache::make('core', 'files_context_filepath_count');

        if (!$cache->has($context_id)) {
            $sql = "
                SELECT
                    f.id,
                    f.component,
            ...
            $filepaths = [];
            $records = $DB->get_records_sql($sql, ['contextid' => $context_id]);

            foreach ($records as $record) {
                $filepath_key = self::make_context_file_counts_cache_filepath_key(
                    $record->component,
                    $record->filearea,
                    $record->itemid,
                    $record->filepath
                ...
                $ext_key = '.' . core_text::strtolower($ext);

                $count = $filepaths[$filepath_key][$ext_key] ?? null;
                if ($count === null) {
                    $filepaths[$filepath_key] = [$ext_key => 0];
                }

                $filepaths[$filepath_key] = [$ext_key => ++$count];
            }

            $cache->set($context_id, $filepaths);
        }
    ...
    /**
     * Creates a key for the filepaths data structure used in the cache.
     *
     ...
     */
    private static function make_context_file_counts_cache_filepath_key(
        string $component,
        string $filearea,
        int $item_id,
        string $filepath
    ): string {
        return implode(':', [$component, $filearea, $item_id, $filepath]);
    }
