...
function grade_get_grades($courseid, $itemtype, $itemmodule, $iteminstance, $userid_or_ids=null) {
    ...
    if ($course_item->needsupdate) {
        ...
        if (\core_course\local\grade_helper::use_async_course_regrade($courseid)) {
            ...
            // If async, then everything needs update.
            $needsupdate = array_keys(
                ...
                if ($userids) {
                    ...
                    foreach ($userids as $userid) {
                        ...
                        if ($grade_item->gradetype == GRADE_TYPE_TEXT or $grade_item->gradetype == GRADE_TYPE_NONE) {
                            ...
                            $grade->grade          = false;
                            $grade->str_grade      = get_string('needs_regrading', 'grades');
                            $grade->str_long_grade = $grade->str_grade;
...
function grade_update_mod_grades($modinstance, $userid = 0) {
    ...
    $mod_lib_file = $CFG->dirroot . '/mod/' . $modinstance->modname . '/lib.php';
    if (!file_exists($mod_lib_file)) {
        debugging('missing lib.php file in module ' . $modinstance->modname);
        return false;
    }
    require_once($mod_lib_file);
    ...
    if (function_exists($updategradesfunc) and function_exists($updateitemfunc)) {
        ...
        // Do not call $modinstance->modname_grade_item_update($modinstance) function here,
        // it will be called inside $modinstance->modname.'_update_grades' function,
