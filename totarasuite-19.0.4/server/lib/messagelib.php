...
function message_send($eventdata) {
    ...
    // TOTARA Unless it is a message, in which case we still need to add it to the messages, but without sending to any processors.
    if ($eventdata->notification && !empty($defaultpreferences->{$preferencebase.'_disable'})) {
        return $messageid;
    }
    ...
    if ($eventdata->notification) {
        ...
        if (isset($eventdata->userfrom->realid) && $eventdata->userfrom->id == \mod_facetoface\facetoface_user::FACETOFACE_USER) {
            $tabledata->useridfrom = $eventdata->userfrom->realid;
        } else {
        ...
        if (!$conversationid = \core_message\api::get_conversation_between_users(
            [$eventdata->userfrom->id, $eventdata->userto->id]
        )) {
    ...
    if (!$eventdata->notification) {
        // TOTARA: If the preference was disabled, we still needed to do all the work above in order to record the message
        // record in the insert_record statement just above, but we don't want to fire off notifications via any processors.
        if (!empty($defaultpreferences->{$preferencebase.'_disable'})) {
            return $tabledata->id;
        }

        // Sending conversation message instead of notification message.
        return core\message\manager::send_message_to_conversation($eventdata, $tabledata, $processorlist);
    }
