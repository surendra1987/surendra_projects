...
class auth_plugin_base {
    ...
    /**
     * Get locked/unlocked_if_empty profile fields map configured by auth plugin.
     * $locked_fields_map = [
     *      'locked' => ['address'],
     *      'unlockedifempty' => ['phone1']
     * ]
     *
     * @return array
     */
    public function get_locked_fields_map(): array {
        $custom_fields = $this->get_custom_user_profile_fields();
        ...
        foreach ($fields as $field) {
            $config_variable = $this->get_config_lock_variable_name($field);
            if (isset($this->config->{$config_variable})) {
                if ($this->config->{$config_variable} === 'locked') {
                    $locked_fields_map['locked'][] = $field;
                } else if ($this->config->{$config_variable} === 'unlockedifempty') {
                    $locked_fields_map['unlockedifempty'][] = $field;
                }
            }
        }

        return $locked_fields_map;
    }

    /**
     * Get auth config lock variable name by field.
     *
     * @param string $field
     * @return string
     */
    public function get_config_lock_variable_name(string $field): string {
        return 'field_lock_' . $field;
    }

    /**
     * Each auth plugin whether to support custom fields for auth lock config, more info
     * check display_auth_lock_options() usage for each auth plugin.
     *
     * @return bool
     */
    public function can_support_custom_fields_for_auth_lock(): bool {
        return false;
    }
}
...
function login_attempt_failed($user) {
     ...
     * Per Moodle: "This is perhaps overzealous but is the documented way of reloading the cache, as per
     * the test method 'test_check_user_preferences_loaded'."
     */
    unset($user->preferences);

    /*
     * Each thread must wait for the lock and must fetch the current count before incrementing it. This
     * ensures that login_failed_count is correct across threads and that the lockout threshold is
     * correctly honoured.
     */
    $resource = 'user:' . $user->id;
    $lockfactory = lock_config::get_lock_factory('core_failed_login_count_lock');
    ...
    try {
        ...
        // No threshold means no lockout, and we're done here.
        if (empty($CFG->lockoutthreshold)) {
            // Release our core_failed_login_count_lock.
            $lock->release();
        ...
        // If lockout window has passed, reset the count.
        ...
        // Always release the core_failed_login_count_lock on a failure.
        $lock->release();
        throw $e;
    }
}
 ...
 * @param admin_settingpage $settings admin settings page instance
 ...
 * @param array $customfields list of custom profile fields, if custom fields are passed in, please override can_support_custom_fields_for_auth_lock()
 *                            from auth_plugin_base class for auth plugin to set true, it's false by default.
 * @since Totara 11 (Moodle 3.3)
 */
function display_auth_lock_options(admin_settingpage $settings, $auth, $userfields, $helptext, $mapremotefields, $updateremotefields, $customfields = array()) {
    global $DB;
