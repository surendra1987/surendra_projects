...
class util {
    ...
    public static function update_local_user(\stdClass $server, array $serveruser, $sso = false) {
        ...
        if ($userinfo) {
            ...
            if ($record) {
                if (isset($serveruser->suspended) and $serveruser->suspended != 0 and $olduser->suspended == 0) {
                    $usersuspended = true;
                } else if ((isset($serveruser->suspended) and $serveruser->suspended = 0 and $olduser->suspended != 0)) {
                    $userunsuspended = true;
                }
        ...
        if ($userundeleted) {
            ...
            if ($userunsuspended) {
                \totara_core\event\user_unsuspended::create_from_user($userrecord)->trigger();
            }
        }
    ...
    public static function finish_sso(\stdClass $server, $ssotoken) {
        ...
        complete_user_login(
            $user,
            complete_login_callback::create([util::class, 'post_login_callback'], [$server->id, $serveruser['id'], $ssotoken])
        );
    }

    /**
     * Callback after completing user login.
     *
     * @param $server_id
     ...
     */
    public static function post_login_callback($server_id, $server_user_id, $ssotoken) {
        global $DB, $CFG, $USER, $SESSION;

        $ssosession = new \stdClass();
        $ssosession->sid          = session_id();
        $ssosession->ssotoken     = $ssotoken;
        $ssosession->serverid     = $server_id;
        $ssosession->serveruserid = $server_user_id;
