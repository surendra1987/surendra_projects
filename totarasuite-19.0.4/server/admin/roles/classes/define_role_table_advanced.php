...
class core_role_define_role_table_advanced extends core_role_capability_table_with_risks {
    ...
    protected function get_description_field($id) {
        return '<textarea class="form-textarea form-control" id="'. s($id) .'" name="description" rows="10" cols="50">' .
            htmlentities($this->role->description) .
            '</textarea>';
    }
    ...
    public function display() {
        ...
        // Totara: Show a banner if there are roles assigned in the context level
        $this->print_assignable_level_warnings();
    ...
    /**
     * Get the number of roles in each context level already assigned
     *
     * @param int $role_id
     * @return array
     */
    protected function get_assigned_context_counts(int $role_id): array {
        global $DB;

        if ($this->contextcounts !== null) {
            return $this->contextcounts;
        }

        $sql = "SELECT c.contextlevel, COUNT(*) AS row_count
                  FROM {role_assignments} ra
                  LEFT JOIN {context} c ON ra.contextid = c.id
                  WHERE ra.roleid = :roleid
                  GROUP BY c.contextlevel";
        $this->contextcounts = $DB->get_records_sql($sql, array('roleid' => $role_id));
        return $this->contextcounts;
    }

    /**
     * Prints the warning banners for the context level option
     */
    protected function print_assignable_level_warnings(): void {
        global $OUTPUT;
        ...
        $banners = [];
        $context_counts = $this->roleid ? $this->get_assigned_context_counts($this->roleid) : array();
        foreach ($this->allcontextlevels as $context_level => $context_level_name) {
            if (isset($context_counts[$context_level]) && $context_counts[$context_level]->row_count > 0) {
                $banners[] = $OUTPUT->notification(
                    get_string('context:assignederror', 'role', $context_level_name),
