...
class issuer extends persistent {
    ...
    public function definition() {
        ...
        // Totara: For SMTP, prevent confirmation or login fields
        $show_login_actions = $this->type !== 'microsoftsmtp';
        ...
        if ($default_branding_allowed && $show_login_actions) {
        ...
        // Totara: Only show the login ability for certain providers
        if ($show_login_actions) {
            // Image.
            ...
            $mform->addElement('hidden', 'image', '');
            $mform->addElement('hidden', 'showonloginpage', false);
            $mform->setType('showonloginpage', PARAM_BOOL);
        }

        if ($this->showrequireconfirm && $show_login_actions) {
