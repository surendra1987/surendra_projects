...
if ($action == 'edit') {
    if ($userfieldmapping) {
        $PAGE->navbar->add(get_string('edituserfieldmapping', 'tool_oauth2', format_string($issuer->get('name'))));
    } else {
        $PAGE->navbar->add(get_string('createnewuserfieldmapping', 'tool_oauth2', format_string($issuer->get('name'))));
    }
...
if ($mform && $mform->is_cancelled()) {
    ...
    if ($data = $mform->get_data()) {
        ...
        echo $OUTPUT->header();
        if ($issuer) {
            echo $OUTPUT->heading(get_string('edituserfieldmapping', 'tool_oauth2', format_string($issuer->get('name'))));
        } else {
            echo $OUTPUT->heading(get_string('createnewuserfieldmapping', 'tool_oauth2', format_string($issuer->get('name'))));
        }
        $mform->display();
        echo $OUTPUT->footer();
    }
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $cancelurl = new moodle_url('/admin/tool/oauth2/userfieldmappings.php', ['issuerid' => $issuerid]);
        echo $OUTPUT->header();
        $str = get_string('deleteuserfieldmappingconfirm', 'tool_oauth2', format_string($issuer->get('name')));
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('userfieldmappingsforissuer', 'tool_oauth2', format_string($issuer->get('name'))));
    $userfieldmappings = core\oauth2\api::get_user_field_mappings($issuer);

    $collection = \core\collection::new($userfieldmappings);

    if (!$collection->has(fn ($x) => $x->get('internalfield') === 'external_id')) {
        echo $OUTPUT->render(
            new \core\output\notification(
                get_string('no_mapping_for_internal_field_identification_warning', 'tool_oauth2', 'external_id'),
                'warning'
            )
        );
    }
    ...
    echo $renderer->single_button($addurl, get_string('createnewuserfieldmapping', 'tool_oauth2', format_string($issuer->get('name'))));
    echo $OUTPUT->footer();
}
