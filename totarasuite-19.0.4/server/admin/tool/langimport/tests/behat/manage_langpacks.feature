...
Feature: Manage language packs
  ...
  Scenario: Install language pack
    ...
    # Check the language pack is visible in user preferences and that the language link is available in the usermenu
    When I reload the page
    And I follow "Language" in the user menu
    Then I should see "en" in the "Preferred language" "select"
    And I should see "en_ar" in the "Preferred language" "select"
    # Check the language pack is not visible when the setting is disabled.
    And I navigate to "Language settings" node in "Site administration > Localisation"
