 ...
 */
class tool_totara_sync_comp_db_field_mapping_test extends totara_sync_database_testcase {
    ...
    public function test_field_mapping_for_types() {
        ...
        $compframework = $hierarchygenerator->create_framework('competency', ['idnumber' => 'CF1']);

        // Create a type
        $comptypeid = $hierarchygenerator->create_comp_type(['idnumber' => 'T1', 'fullname' => 'Type 1']);

        $type_cf1_data = ['hierarchy' => 'competency', 'typeidnumber' => 'T1', 'value' => '', 'shortname' => 'field1'];
        $hierarchygenerator->create_hierarchy_type_text($type_cf1_data);
        $type_cf2_data = ['hierarchy' => 'competency', 'typeidnumber' => 'T1', 'value' => '2345', 'shortname' => 'field2'];
        ...
        $source->set_config('import_customfield_' .$comptypeid . '_field1', '1');
        $source->set_config('import_customfield_' .$comptypeid . '_field2', '1');
        $source->set_config('fieldmapping_customfield_' .$comptypeid . '_field1', 'textcf1');
        $source->set_config('fieldmapping_customfield_' .$comptypeid . '_field2', 'menucf1');
        ...
        $this->assertEquals('Competency 2', current($records)->fullname);
        $this->assertEquals('comp2', current($records)->shortname);
        $comptype = $DB->get_record('comp_type', ['idnumber' => 'T1']);
        $this->assertEquals($comptype->id, current($records)->typeid);
    }
}
