 ...
 */
class tool_totara_sync_pos_db_field_mapping_test extends totara_sync_database_testcase {
    ...
    public function test_field_mapping_for_types() {
        ...
        $posframework = $hierarchygenerator->create_framework('position', ['idnumber' => 'PF1']);

        // Create a type
        $postypeid = $hierarchygenerator->create_pos_type(['idnumber' => 'T1', 'fullname' => 'Type 1']);

        $type_cf1_data = ['hierarchy' => 'position', 'typeidnumber' => 'T1', 'value' => '', 'shortname' => 'field1'];
        $hierarchygenerator->create_hierarchy_type_text($type_cf1_data);
        $type_cf2_data = ['hierarchy' => 'position', 'typeidnumber' => 'T1', 'value' => '2345', 'shortname' => 'field2'];
        ...
        $source->set_config('import_customfield_' .$postypeid . '_field1', '1');
        $source->set_config('import_customfield_' .$postypeid . '_field2', '1');
        $source->set_config('fieldmapping_customfield_' .$postypeid . '_field1', 'textcf1');
        $source->set_config('fieldmapping_customfield_' .$postypeid . '_field2', 'menucf1');
        ...
        $this->assertEquals('Position 2', current($records)->fullname);
        $this->assertEquals('pos2', current($records)->shortname);
        $postype = $DB->get_record('pos_type', ['idnumber' => 'T1']);
        $this->assertEquals($postype->id, current($records)->typeid);
    }
}
