    ...
    function definition () {
        ...
        if ($CFG->tenantsenabled) {
            ...
            if ($tenants) {
                ...
                $mform->addElement('select', 'tenantid', get_string('choosetenant', 'totara_tenant'), $tenants);
                $mform->setType('tenantid', PARAM_INT);
                $mform->addHelpButton('tenantid', 'choosetenant', 'totara_tenant');
                $mform->disabledIf('tenantid', 'uutype', 'eq', UU_USER_UPDATE);
            }
        }
